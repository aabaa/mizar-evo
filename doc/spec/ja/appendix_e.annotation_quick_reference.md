...
|---|---|---|---|---|
|ステートメント/項目の注釈 | `@name` または `@name(...)` |項目またはsource の場所の直前に注釈が付けられます。証明のヒント、レンダリングのヒント、診断、評価 |第2章、第21章、第22章 |
...
|型を表示 | `@show_type(expr)` |開発ポジション | `expr` の推論された型をバインドせずに出力します。 |
|式を評価する | `@eval(expr)` |トップレベル、証明ブロック、またはアルゴリズム本体 |検証時の評価を実行し、可能な場合は計算結果を出力します。 |
