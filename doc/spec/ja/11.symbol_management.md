...
Mizar では、述語、functor、モード、属性の **同義語** と **反意語** を定義できます。これらは、意味上の同等性を維持しながら、代替表記を提供します。
...
| **module** |moduleレベルで定義 | `pred`、`func`、`mode`、`attr` |
...
1. **ローカル定義**はmoduleレベルの定義より優先されます
2. **moduleレベルの定義**はインポートされたシンボルより優先されます
3. **インポートの競合** によりコンパイル時エラーが発生します (修飾またはエイリアスによって解決する必要があります)
4. **再定義**には `redefine` キーワードが必要です
