...
parameter list で、suffix が use site で active attribute name として解決できるか、
現在の `attr_pattern` (A.6) が導入する declaration-site `attribute_name` である場合に
`param_prefix` 分割を試みます。この判定に失敗した場合、spelling 全体を
1 つの attribute name として active lexicon に照合します。両方が有効な場合は
`param_prefix` 分割が優先されます。共有された parser-facing lexer は、parser が
最終的な attribute production に入る前にこの分割候補を露出してもよいですが、非
attribute grammar 位置では parser がその token sequence を拒否しなければなりません。
ハイフンは引き続きグローバルな予約 special symbol ではありません。
