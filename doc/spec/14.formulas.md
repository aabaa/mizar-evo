...
> [!NOTE]
> The `&` operator is a **formula-level** logical conjunction. It differs from the `and` keyword used in **statement-level** condition chaining (e.g., `assume that ... and ...`, `such that ... and ...`). See [Chapter 15 (Statements), §15.3.2](./15.statements.md#1532-collective-assumption-assume-that) for the `and` keyword.
...
> [!NOTE]
> For the `qua` operator syntax and semantics, see [Chapter 13 (Term Expressions), §13.6](./13.term_expression.md#136-type-qualification-qua). For the FOL validity conditions discharged by the verifier, see [§13.8.7](./13.term_expression.md#1387-qua-operator-encoding).
