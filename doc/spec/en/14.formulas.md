...
> Canonical language: English. Japanese companion: [../ja/14.formulas.md](../ja/14.formulas.md).
...
Atomic formulas are the simplest formulas, formed by predicate applications and
`is` assertions. The parser-facing grammar preserves a generic `is_assertion`;
name and type resolution later classifies it as a type assertion or an
attribute assertion.
...
Predicate applications may chain user-defined predicate symbols. After overload
resolution, a chain such as `a < b < c` is interpreted as adjacent predicate
applications conjoined together, conceptually `a < b & b < c`. Chains that
cannot be resolved as adjacent predicate applications are rejected during
name/type resolution.

Template predicate applications put template arguments immediately after the
predicate symbol, as in `a, b are_congruent[H]`. The `template_args` production
is shared with theorem references and template functor applications.

The chain split is deterministic. For every segment after the first, the
implicit left boundary is exactly the final term of the previous resolved
segment. The segment's written `term_list` supplies the remaining arguments.
No other regrouping is attempted. A segment-level `does not` / `do not`
modifier negates only that adjacent predicate application before the chain
conjunction is formed.

Built-in predicates (`=`, `<>`, and `in`) are single atomic formulas and do not
participate in user predicate chains. Mixed chains such as `a < b = c` are
syntax errors unless rewritten as complete formulas joined by `&`, `or`, or
another formula connective.
...
After resolution, type assertions check whether a term belongs to a given type.
They use the `is` keyword.

The subject term must be statically admissible for the asserted type or
attribute chain. For a type assertion, the asserted type must be reachable from
the subject's inferred type by ordinary type checking, widening, or an explicit
view such as `qua`. For an attribute assertion, the subject must be admissible
for the attribute's declared radix and parameters. Otherwise the formula is a
type error; the verifier does not create an unconstrained attribute atom outside
the attribute's guarded domain.
...
After resolution, attribute assertions check whether a term has one or more
attributes. Multiple adjectives form an attribute chain interpreted
**conjunctively**. The `non` modifier negates individual attributes; `not`
negates the entire assertion.
...
Quantified formulas may appear as the rightmost operand of a binary connective.
This reflects their lowest binding power and rightward extent: `P implies for x
being T holds Q(x)` parses as `P implies (for x being T holds Q(x))`.
...
> [!NOTE]
> The repetition forms (`& ... &`, `or ... or`) are accepted only when the left
> and right formulas are alpha-equivalent except for exactly one closed integer
> literal occurrence, the two endpoint literals are finite, and the endpoint
> difference determines a unit-step closed range. The repeated formulas are
> expanded by substituting each integer in that inclusive range. Multiple
> varying positions, symbolic endpoints, non-integer endpoints, or zero-length
> ambiguous patterns are rejected.
...
Here `[a, b]` is the bracket-functor term form from Chapter 13. Because the
`[` appears where a term primary is expected, it is not parsed as template
arguments.
...
| `G is Group.associative` | `is_Group_associative(G)` after checking that `G` is admissible as a `Group` |
| `x is not finite` | `¬is_finite(x)` |

> [!NOTE]
> `non` in adjective chains produces negation of the attribute predicate (`¬is_empty`), while `not` in `x is not finite` produces formula-level negation of the entire assertion (`¬is_finite(x)`). Both result in logical negation, but `non` targets a single adjective within a chain, whereas `not` negates the whole resolved attribute assertion.
