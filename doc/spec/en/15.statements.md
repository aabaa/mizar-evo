...
> Canonical language: English. Japanese companion: [../ja/15.statements.md](../ja/15.statements.md).
...
- A trailing `by references` clause discharges well-formedness constraints introduced by the declared type at the use site; it does not add arbitrary assumptions. The referenced labels or theorems must prove the generated declaration constraints before the variable enters scope.
- Multiple typed segments are allowed; variables within one segment share that segment's type
...
**Semantics:**
- Creates syntactic abbreviations expanded by capture-avoiding substitution
  during verification
- The type is inferred from the right-hand side expression
- No label is needed (labels are for propositions, not definitions)
- Multiple assignments in one `set` statement are introduced left-to-right
- Free variables in the right-hand side are captured by resolved binding id at
  the definition site; later shadowing of the same display name does not change
  the abbreviation
...
**Semantics:**
- Inline functors are expanded by capture-avoiding substitution at each use site
...
- Free variables in the defining term are captured at the definition site
- Applications must satisfy the formal parameter type guards before expansion
- The `deffunc` name is an identifier-only local abbreviation. It does not
  introduce a lexer user-symbol dictionary entry or an operator spelling.
...
**Semantics:**
- Inline predicates are expanded by capture-avoiding substitution at each use site
- Scoped to the enclosing proof block — not visible outside
- Zero-argument definitions are allowed: `defpred Trivial() means 0 = 0;`
- Free variables in the defining formula are captured at the definition site
- Applications must satisfy the formal parameter type guards before expansion
- The `defpred` name is an identifier-only local abbreviation. It does not
  introduce a lexer user-symbol dictionary entry or symbolic predicate
  spelling.
...
- Each introduced variable binds its occurrences in the statement's `such
  that` conditions. For subsequent statements it remains in scope through the
  end of the innermost enclosing proof or reasoning block. Nested child blocks
  inherit the binding unless they shadow it; the variable does not escape to a
  parent or sibling block.
- Labels attached to `such that` conditions use the ordinary label scope below;
  this variable-scope rule does not add a distinct condition, fact, or proof
  lifetime.
...
> [!NOTE]
> Multiple witnesses are introduced from left to right and must match the
> existential goal's binder order after elaboration.
...
**Semantics:**
- Requires evidence for `is_T(x)` for the target type `T`
- All items in a `reconsider` statement are checked against the same target type
- If both the `simple_justification` and proof block are omitted, proof-free
  widening, inheritance, cluster-closure, or already recorded local type facts
  must discharge the target-type obligation; otherwise the statement is
  rejected with `type.narrowing_requires_proof`
...
> [!WARNING]
> `now ... end` is **deprecated**. It requires the reader to read the entire block before knowing what proposition is being established, making proof intent harder to follow. Prefer an explicit `thus <goal> by ...` or a `proof ... end` block with a clearly stated thesis. `now` is retained for compatibility with existing Mizar proofs.
...
- The label refers to the formula obtained by closing the block skeleton:
  `let` introductions become universal quantifiers, assumptions become
  antecedents, and `thus` conclusions become the consequent conjunction
- Can be referenced later in `by` justifications
- Witnesses introduced by `consider` or `given` inside the block may be used in
  local reasoning, but they cannot occur free in the labeled formula exported by
  the block. Such a leak is a scope error.
...
Every assertion in Mizar must be discharged: it may cite a simple reference
list, provide a full proof block, or omit explicit proof text and use no-hint
verification.
...
- **Bulk references**: All public theorem items from a module (e.g., `mml.foo.*`); see [Chapter 16 (Theorems and Proofs), §16.5](./16.theorems_and_proofs.md#165-proof-justification-by) for trade-offs
...
The empty alternative is intentional. Where another production requires a
`justification`, an immediately following `;` omits explicit proof text but
does not by itself discharge the assertion or correctness obligation. Verification uses
the no-hint policy in Chapter 16 §16.5; failure is semantic, not syntactic.
...
1. **Variable Scope**: Variables introduced by `let`, `given`, or `consider` are scoped to their enclosing block.
   - A `given` variable binds its occurrences in its declaration's `such that` conditions. For subsequent statements it remains visible through the rest of that enclosing block, including nested child blocks unless shadowed. It is not visible after the block ends or in a sibling block.
...
| Mizar Source | FOL Encoding |
| --- | --- |
| `set c = t;` | Capture-avoiding substitution: replace later uses of `c` with the definition-time closure of `t` |

### 15.11.3 Inline Definitions (`deffunc`, `defpred`)

Inline definitions are expanded by capture-avoiding substitution from their
definition-time closure. They do not generate local FOL function or predicate
symbols and do not add local abbreviation axioms to the proof context.

| Mizar Source | FOL Encoding |
| --- | --- |
| `deffunc F(x be T) -> U equals t;` | Expansion rule: `F(a)` lowers to `t[x/a]`, after checking `is_T(a)` |
| `defpred P(x be T) means φ;` | Expansion rule: `P(a)` lowers to `φ[x/a]`, after checking `is_T(a)` |
...
| `consider x, y be T such that ψ by J;` | Justified `∃x, y. (is_T(x) ∧ is_T(y) ∧ ψ)` → Skolem constants |
...
| `take t, u;` | Existential witnesses: for goal `∃x:T, y:U. ψ`, generate witness type obligations `is_T(t)` and `is_U(u)`, then instantiate `ψ[x/t, y/u]` |
| `take k = t;` | Same witness obligation as `take t;`, and introduces local abbreviation `k = t` for the remaining obligation |
...
| `reconsider x as T;` | Accepted only if current proof-free widening, inheritance, cluster-closure, or local type facts already prove `is_T(x)`; otherwise E0102 requests a justification |
| `reconsider y = t as T by J;` | Introduces `y = t` and proves `is_T(t)` using `J` |
| `reconsider x, y = t as T by J;` | Proves every listed item has type `T`, introducing names for equated items |
