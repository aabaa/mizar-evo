...
> Canonical language: English. Japanese companion: [../ja/01.introduction.md](../ja/01.introduction.md).
...
The redesign of Mizar is guided above all by **three core design principles** that together define its direction — **readability**, **AI-readiness**, and **scalability**. These three pillars are the lens through which every design decision in this specification is made.

- **Readability**: Formal text should read as closely as possible to the natural language of working mathematicians. A stable, uniform, and search-friendly syntax keeps proofs legible to humans and minimizes the cognitive distance between informal reasoning and its formal counterpart.
- **AI-readiness**: The system is designed from the ground up to collaborate with automated reasoning tools and Large Language Models (LLMs). Standardized interfaces (such as the Model Context Protocol) and a minimal, machine-checkable trusted kernel let AI propose proofs at scale while logical soundness remains independently guaranteed. Notably, the same readability that serves humans also makes Mizar text high-quality material for training and prompting LLMs.
- **Scalability**: Modern software engineering practices — module systems, namespaces, templates, a richer type system, package management, and incremental verification — allow the library and its community to grow without sacrificing maintainability or reproducibility. Scalability is also a matter of **memory**: rather than keeping the proof bodies or elaboration states of imported libraries resident, the system is designed to hold only the active module, the public interfaces it imports, compact import-filtered registration indexes, and that module's own verification conditions — so memory grows with the work in front of the verifier, not with the size of the library behind it (see [§12.6](./12.modules_and_namespaces.md#126-logic-encoding-and-separate-compilation) and [§23.7](./23.package_management_and_build_system.md#237-cluster-resolution-graph)).

These pillars are realized through several concrete factors in the evolving landscape of formalization of mathematics and interactive theorem proving (ITP):
...
3. **Expanding Capabilities**:  
    By introducing algorithm specification and verification, we aim to broaden Mizar's applicability beyond pure mathematics into areas such as software verification, cryptographic protocol analysis, and quantum computing.
...
The redesign of Mizar introduces several significant enhancements and new features, grouped below under the three design pillars introduced in [§1.1](#11-background-for-redesigning-mizar) — **readability**, **AI-readiness**, and **scalability**.

### Readability

1. **Stable, Mathematics-Faithful Syntax**:
    - A uniform syntax that mirrors traditional mathematical notation, deliberately favoring disciplined expressivity over extensible syntax or meta-programming.
    - Minimizes cognitive overhead so users can focus on the essence of mathematical reasoning.

2. **Source Encoding and Syntax Policy**:
    - **UTF-8 Support**: Source files are fully encoded in UTF-8, allowing the use of Unicode characters in comments and annotations so users can document their work in their native languages.
    - **ASCII-Based Core Syntax**: The core logical syntax remains ASCII-based to guarantee canonical representation and efficient searchability across the library, preventing notation fragmentation and maintaining the "search-friendly" nature of Mizar code.

3. **Automated Documentation Generator**:
    - Recognizing code as knowledge, the system generates comprehensive HTML documentation directly from Mizar source files and annotations, keeping formal proofs accessible and understandable.
...
5. **Substitution-Based Verification Kernel**:
    - **Formula/Substitution Evidence Verification**: For every inference step, evidence records the formulas, explicit substitutions, provenance, and target binding that the kernel must check.
    - **Kernel-Derived SAT Check**: Verification is performed by applying checked substitutions inside the kernel, deriving the SAT problem deterministically, and confirming its unsatisfiability (UNSAT).
    - **Minimal TCB**: The trusted kernel is strictly limited to evidence checking, instantiation, deterministic SAT encoding, and a trusted in-process Rust SAT checker, drastically reducing the Trusted Computing Base and making it safe to reject bad proofs proposed by AI.

6. **Advanced Automation**:
    - Computational automation is provided through verified `algorithm` definitions and `by computation` proof steps (see [§20. Algorithm and Verification](./20.algorithm_and_verification.md)), while proof scripts themselves remain declarative natural deduction.
    - Integration with external theorem provers (e.g., Vampire, E) for enhanced proving capabilities.
    - Incorporation of machine learning techniques to assist in proof development.

7. **Comprehensive Annotation System**:
    - A versatile annotation system that doubles as structured, machine-readable context for automated reasoning, in addition to improving documentation and meta-information.
...
8. **Interactive Development Architecture**:
    - **Incremental Parsing and Verification**: Unlike the traditional batch-processing approach, the new architecture supports incremental verification. This allows for partial checking of documents, enabling immediate feedback on errors and proof validity as the user types. By avoiding full rechecks of large developments, incremental verification also supports scalability.
...
9. **Standardized Tooling Interfaces (LSP & MCP)**:
...
10. **Enhanced Module System and Namespaces**:
...
13. **Algorithm Validation and Program Generation**:
    - New syntax and semantics for algorithm specification and formal verification, broadening Mizar's applicability beyond pure mathematics.
...
14. **Integrated Package Management and Build System**:
    - Introduction of a dedicated package manager to handle dependencies, versioning, and reproducible builds. This facilitates the transition from a monolithic repository to a decentralized ecosystem of modular theories.

15. **Modernized Standard Library**:
...
17. **Memory-Scalable Verification**:
    - Separate compilation loads only the public signature and theorem items of imported modules — never their proof bodies or private lemmas (see [§12.6](./12.modules_and_namespaces.md#126-logic-encoding-and-separate-compilation)).
    - Cluster and registration data live in a single global index from which an import-scoped view is derived by filtering, rather than being materialized per import closure; explanation and trace data are served by bounded, incremental queries and stored as external artifacts rather than held resident (see [§23.7](./23.package_management_and_build_system.md#237-cluster-resolution-graph)).
    - The result is a resident-set model in which memory scales with the active source file, its imported interfaces, the import-filtered indexes, and the active module's own verification conditions and in-flight discharge state — and is independent of imported proof bodies, private lemmas, proof witnesses, and registration data outside the import closure.

These features aim to enhance Mizar's expressiveness, ease of use, and power in formalizing complex mathematical concepts and proofs. By grouping every addition under readability, AI-readiness, and scalability, the redesign keeps each new capability anchored to the same guiding principles — balancing Mizar's traditional strengths in natural-language-like formalization with modern programming-language features and proof automation techniques.
