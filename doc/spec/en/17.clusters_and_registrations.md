...
Several functorial clusters for the same functor application can apply at the same time. Unlike rival overloaded definitions, they do not compete: each cluster simply adds proved attributes to the functor's result, and the verifier collects all the compatible attributes together. A conflict is reported only when the applicable registrations would force incompatible underlying types for the result, or would derive attributes that contradict one another.
