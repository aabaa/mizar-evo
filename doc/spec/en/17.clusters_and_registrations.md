...
Cluster resolution is performed by a **forward traversal of the cluster-db graph** (§23.6.2.1), independently of the ATP backends. Because the cluster syntax permits only conjunctive antecedents and consequents (§17.10), every registration is a Horn clause of the form `A₁ ∧ … ∧ Aₙ → B₁ ∧ … ∧ Bₘ`, which maps directly onto an edge (or hyperedge) in that graph. No external solver is needed.

The resolved attribute sets are passed to ATP backends as ground facts, so proof search never needs to chain through cluster axioms at runtime (§21.4.2). The derivation steps are recorded in `resolution-trace/` (§23.6.2.4) and can be verified by the minimum kernel as a propositional chain.
