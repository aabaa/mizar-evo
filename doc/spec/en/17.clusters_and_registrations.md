...
condition has been accepted. Earlier items in the same module, and earlier
statements in the same registration block, cannot use it retroactively. Imported
registrations are active from the start of the importing module's non-import
body, subject to normal import visibility. A registration whose proof is pending
or failed is not silently used as a fallback; dependent resolution sites must
report that the expected registration is unavailable.
