...
**Deterministic normalization.** Normalization is a deterministic function of the term and the set of in-scope rules. The result does **not** depend on registration order or import order. The fixed strategy is:
...
| **Matching** | Patterns are matched modulo alpha-equivalence and variable binding. Pattern variables are instantiated, and the instantiated terms must satisfy the type and attribute guards introduced by the surrounding `let` declarations. A `such` condition on a surrounding `let` is an applicability side condition: it must already be available as a recorded local fact or cited fact before the rule may apply, but it does not by itself make the rule more specific. |
| **Rule selection** | When several rules match the same redex, the **most specific** matching rule applies. Specificity is defined over the whole matching constraint: an `LHS` pattern may be more specific by pattern subsumption, and pattern-variable guards are compared with the type-specificity ordering of §19.2.3. Remaining mutually incomparable or equally specific matches are disambiguated by the lexicographically smallest **FQN** (module FQN + label), which is independent of import order. |

Specificity-first selection mirrors overload resolution (§19) for the type-guard comparison: among matching rules, narrower patterns and more specific guarded variables are preferred. Unlike overload resolution, reduction normalisation must remain a total deterministic function, so incomparable matches are not ambiguity errors; the FQN tie-break makes those cases deterministic and reproducible across machines and build configurations.

> [!NOTE]
> The strategy-normal form depends on the *set* of in-scope rules, so adding a rule — for example via a new import — can still change the result of automatic normalisation. This is the one residual source of change; it is made auditable by traceability (below) and is always recoverable by explicit citation.
