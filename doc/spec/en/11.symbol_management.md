...
The original pattern may also be an application of an algorithm (§20.1.1). The
synonym then gives the algorithm mathematical notation without a separate `func`
definition. A synonym of an algorithm is resolved to the original call and may
appear only where that call may appear:

```mizar
synonym n ! for factorial(n);
```
...
| `synonym n ! for factorial(n)` | `∀n. factorial_notation(n) = factorial(n)` |

The `factorial` row assumes a promoted algorithm (§20.7.2). An algorithm that is
not promoted has no logical function symbol, so its synonym has no encoding of
its own and is resolved to the original call (§11.1.2).
