...
> Canonical language: English. Japanese companion: [../ja/08.type_inference.md](../ja/08.type_inference.md).
...
**Omitted justification gate.** A `reconsider` statement without `by` or an
attached proof block is accepted only when every generated target-type
obligation for the statement is discharged by proof-free evidence already
available at that source point:

- widening through the mode or structure hierarchy;
- unique inheritance/view evidence;
- active cluster closure and already recorded local type facts.

The omitted form never asks the prover to search for a new proof and never
creates an implicit `by` step. If any item needs a narrowing proof, a new
attribute proof, or facts that are not already accepted, the verifier rejects
the statement with `type.narrowing_requires_proof` and requests an explicit
`by` justification or proof block.
...
4. **Verify**: Discharge proof-free omitted `reconsider` obligations from
   widening, inheritance/view, cluster-closure, or recorded local type
   evidence, and discharge all remaining obligations using explicit
   justifications or proof blocks
...
When the `reconsider` justification is omitted, the generated goal must be
provable from current widening, inheritance/view, cluster-closure, or recorded
local type facts without opening a proof search. Otherwise E0102
(`type.narrowing_requires_proof`) is reported.
