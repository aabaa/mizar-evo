...
> Canonical language: English. Japanese companion: [../ja/12.modules_and_namespaces.md](../ja/12.modules_and_namespaces.md).
...
| --- | --- | --- | --- |
| **`mml`** | Mizar Mathematical Library | Core, trusted standard library maintained by the Mizar team | Mizar Team |
...
The standard library contains foundational theories that all users can rely upon. It carries forward the Mizar Mathematical Library — migrated MML content lives under this root — and is maintained by the Mizar team with strict quality control.
...
* Package names must be lowercase `snake_case`, matching the manifest rule in
  §23.1: `[a-z][a-z0-9]*(?:_[a-z0-9]+)*`
* Hyphenated package names are not permitted in import paths; the package name is
  a namespace component, and no hyphen-to-underscore normalization is applied
...
Import declarations form the module's **import prelude**. Ignoring comments and layout, all `import` statements in a `.miz` file must appear contiguously before the first non-import top-level item. The import prelude ends at the first top-level item that is not an `import` statement, including `export`, `reserve`, `definition`, `registration`, theorem, proof, algorithm, or any other declaration.

Imports are file-scoped: every import in the prelude contributes to one active
import closure for the entire module body. These imports seed the base lexical
environment at the start of the module body. Current-module lexical
declarations and operator metadata then extend that environment only after
their declaration items are complete. `import` statements are not permitted
inside definitions, registrations, proofs, algorithms, or after any non-import
top-level item. A later import-shaped statement is a syntax error.

This restriction allows lexing and disambiguation to build the imported base
symbol set once from the import prelude before applying source-position
updates from declarations in the module body.
...
### 12.3.1 Placement Rules

1. The import prelude starts at the first `import` statement in the file, after optional module-level documentation comments and other trivia.
2. The prelude contains only `import` statements.
3. The prelude ends at the first non-import top-level item.
4. No `import` statement may appear after the prelude.
5. The imported base lexical environment is built from the complete prelude
   and is stable for the rest of the file. Current-module declarations add
   source-position-scoped entries after their declaration points.

### 12.3.2 Resolution Rules
...
`export` statements, when present, appear after the import prelude and before ordinary declarations.
...
* **Usage**: `export mml.algebra.group;`
...
A public **algorithm** (§20.1.1), including one with no visibility modifier, exposes its signature and contracts as part of the module signature $\Sigma$ and permits cross-package computation when computable. Its source body remains opaque; evaluation uses a verified runtime artifact with any required private helpers (§20.9.1). Those helpers do not become directly accessible to importers. `private` prevents direct external access to an algorithm, while `public` does not establish computability or termination. Compatibility is governed by §23.2.
...
2. **Signature Loading**: Ordinary imports load only the logical signature and public theorem items. Computational evaluation additionally loads validated runtime artifacts under §20.9.1; source proof bodies remain opaque.
...
| --- | --- |
| Namespace `mml.algebra.group` | Prefix `mml__algebra__group__` |
| Predicate `is_abelian` | `mml__algebra__group__is_abelian` |

### 12.6.3 Memory Model

The bounds below describe opaque imports. Cross-package MVM evaluation additionally needs the active runtime artifacts and execution state (§20.9.1), loaded only for computational consumers.

Separate compilation is not only a build-time optimization; it also defines the verifier's **resident-set model**. Because importing a module loads only its public signature and theorem items (§12.6.1) — never its proof bodies, private lemmas, or elaboration state — the memory needed to verify a module does not grow with the proof content of the libraries behind it.

Concretely, peak memory while verifying one module is approximately:

```
peak memory
  ≈ size(active source file and its typed AST)
  + size(imported public interfaces)            (* incl. public theorem statements — §12.6.1; architecture §3: ModuleSummary *)
  + size(import-filtered registration indexes)  (* §23.7: cluster-db view by filtering *)
  + size(the active module's verification conditions)  (* canonical VcIr, produced before discharge — architecture §7 *)
  + size(in-flight discharge state)             (* active VC queue and ATP portfolio — §23.4, architecture §14 *)
  + small caches
```

Per-obligation ATP work — the discharge state and prover portfolio — is bounded to the verification conditions currently in flight, not to all obligations in the library. The whole module's canonical `VcIr` is materialized before discharge (architecture §7), so it is the module's own VC set, not a single obligation, that contributes to the bound above.

This resident set is **independent of**:

- the proof bodies and private lemmas of imported modules (not loaded — §12.6.1);
- registration data outside the current import closure (the `cluster-db` is one global index from which an import-scoped view is derived by filtering, not materialized per closure — §23.7.7);
- proof witnesses of imported obligations (stored as external artifacts referenced by hash — §23.5.1).

This is the same principle that the cluster and registration infrastructure follows on its side: the cluster-db is a single global index from which an import-scoped view is derived by filtering, rather than being duplicated per import closure (§23.7.7), and explanation and trace data are served by bounded, incremental queries (§23.7.8) instead of being held resident. Together, separate compilation and import-scoped filtering let the verifier and the language server keep only interfaces and indexes in memory, loading proof bodies, traces, and detailed AI-facing data lazily and only when a specific query requires them.

> [!NOTE]
> This section states a qualitative resident-set model, not a performance guarantee. Concrete memory budgets and benchmark metrics (e.g., peak RSS per file or per import-closure size) are an evaluation-methodology concern and are not normative parts of this specification.
