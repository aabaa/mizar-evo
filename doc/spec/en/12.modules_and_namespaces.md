...
A public **algorithm** (§20.1.1) exposes its *signature and contracts* — name, parameter and return types, `requires`, and `ensures` — as part of the module signature $\Sigma$; its implementation **body** is private detail, never loaded by importers (consistent with the opaque-import model of §12.6.1). Because the body is not loaded, importers cannot evaluate an imported algorithm under `by computation` (§20.9.1); a downstream proof depends only on its contract. This signature-versus-body split is what makes an algorithm-body-only edit a non-breaking change, while a change to a public algorithm's contract is observable to dependents (§23.2).
...
2. **Signature Loading**: Only the logical signature and public theorem items are loaded.
