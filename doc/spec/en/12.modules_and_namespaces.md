...
Imports are file-scoped: every import in the prelude contributes to one active import closure and one active lexical environment for the entire module body. `import` statements are not permitted inside definitions, registrations, proofs, algorithms, or after any non-import top-level item. A later import-shaped statement is a syntax error.

This restriction allows lexing and disambiguation to build the active symbol set once from the import prelude before parsing the module body.
...
5. The active lexical environment for the file is built from the complete prelude and is stable for the rest of the file.
