...
Import declarations form the module's **import prelude**. Ignoring comments and layout, all `import` statements in a `.miz` file must appear contiguously before the first non-import top-level item. The import prelude ends at the first top-level item that is not an `import` statement, including `export`, `definition`, `registration`, theorem, proof, algorithm, or any other declaration.
