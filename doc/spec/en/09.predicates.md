# 9. Predicates

> Canonical language: English. Japanese companion: [../ja/09.predicates.md](../ja/09.predicates.md).

- [9. Predicates](#9-predicates)
  - [9.1 Overview and Syntax](#91-overview-and-syntax)
  - [9.2 Predicates vs Attributes](#92-predicates-vs-attributes)
  - [9.3 Definition Styles: Symbolic vs Phrase](#93-definition-styles-symbolic-vs-phrase)
  - [9.4 Dependent and Typed Parameters](#94-dependent-and-typed-parameters)
  - [9.5 Correctness Conditions](#95-correctness-conditions)
    - [9.5.1 Predicate Properties](#951-predicate-properties)
  - [9.6 Predicate Redefinition](#96-predicate-redefinition)
  - [9.7 Notation Styles](#97-notation-styles)
  - [9.8 Symbol Resolution and Imports](#98-symbol-resolution-and-imports)
  - [9.9 Logic Encoding Details](#99-logic-encoding-details)
  - [9.10 Complete Syntax (EBNF)](#910-complete-syntax-ebnf)

## 9.1 Overview and Syntax

Predicates express properties and relations. Each predicate has a name, typed parameters, and a logical definition. Predicates always yield boolean values (true or false).

**Syntax (EBNF)**:

```ebnf
pred_def         ::= "pred" label ":" pred_pattern "means" formula_definiens ";" ;
label            ::= identifier ;
pred_pattern     ::= [ loci ] def_predicate_symbol [ template_loci ] [ loci ] ;
loci             ::= locus_list | "(" locus_list ")" ;
locus_list       ::= locus { "," locus } ;
locus            ::= identifier ;
template_loci    ::= "[" locus_list "]" ;
def_predicate_symbol ::= identifier | user_symbol ;
predicate_symbol ::= def_predicate_symbol ;
formula_definiens ::= formula
                    | formula_case { "," formula_case } [ "otherwise" formula ] ;
formula_case      ::= formula "if" formula ;
```

**Example**:

```mizar
definition
  let X, Y be set;
  pred SubsetDef: X c= Y means
    for x being object st x in X holds x in Y;
end;
```

Predicates can be:

* **Built-in**: e.g., `x in X`, `x = y`, `x <> y`
* **User-defined**: declared with `pred` and `means`
* **Overloaded**: disambiguated by parameter types

Predicate symbols may use arbitrary user notation symbols (§2.5.2). A
predicate definition makes its symbol available only after the declaration
item is complete; later predicate definitions in the same module are not
available by forward reference. Visibility does not affect lexing inside the
defining module.

> [!NOTE]
> For **inline predicate definitions** within proofs (`defpred`), see [Chapter 15 (Statements), §15.2.4](./15.statements.md#1524-inline-predicate-definition-defpred).

## 9.2 Predicates vs Attributes

Predicates and attributes are both logical assertions, but serve different purposes:

| Aspect | Predicates (`pred`) | Attributes (`attr`) |
| --- | --- | --- |
| **Purpose** | Express general relations between terms | Refine types by adding properties |
| **Arity** | Any (unary, binary, n-ary) | Unary (applied to a single subject) |
| **Syntax** | `x c= y`, `a divides b` | `x is even`, `G is associative` |
| **Type system role** | Used in formulas and proofs | Used in type expressions and clustering |
| **Clustering** | Not clusterable | Participates in cluster registrations |

**When to use predicates**:
- Expressing relations between multiple objects: `x c= y`
- Defining mathematical relations: `a divides b`
- General logical assertions in proofs

**When to use attributes**:
- Refining types: `let n be positive even Integer`
- Enabling type inference through clusters
- Properties that should be inherited via subtyping

**Example comparison**:

```mizar
:: As an attribute (for type refinement)
definition
  let n be Integer;
  attr EvenAttr: n is even means 2 divides n;
end;

:: As a predicate (for general use)
definition
  let a, b be Integer;
  pred DividesDef: a divides b means
    ex k be Integer st b = a * k;
end;

:: Attributes participate in type expressions
let n be positive even Integer;

:: Predicates are used in formulas
assume 2 divides n;
```

> [!NOTE]
> Detailed attribute syntax and semantics are defined in [Chapter 6 (Attributes)](./06.attributes.md).

## 9.3 Definition Styles: Symbolic vs Phrase

Mizar supports two styles of predicate patterns:

### 9.3.1 Symbolic Predicates

Use symbolic operators for mathematical notation:

```mizar
definition
  let x, y be Real;
  pred LessOrEqualDef: x <= y means
    x < y or x = y;
end;
```

### 9.3.2 Phrase Predicates

Use word-based patterns for readability:

```mizar
definition
  let n be Nat;
  pred EvenDef: n is_even means ex k be Nat st n = 2 * k;
end;

definition
  let G be Group, a, b be Element of G;
  pred CommuteDef: a, b are_commute means
    G.binop(a, b) = G.binop(b, a);
end;
```

### 9.3.3 Multi-argument Predicates

Predicates can take multiple arguments on either or both sides of the symbol:

```mizar
definition
  let X, Y, Z be set;
  pred PartitionDef: X, Y partition Z means
    X \/ Y = Z & X /\ Y = {};
end;
```

Predicate applications may chain the defined symbol at use sites, even though a
predicate definition pattern introduces only one predicate symbol:

```mizar
a < b < c;  :: resolved as a < b & b < c
```

The resolver accepts a chain only when the adjacent applications are
well-typed; otherwise the formula is rejected as an ill-formed predicate
application.

The adjacent split uses the final term of the previous resolved segment as the
single shared boundary term for the next segment. For multi-argument predicates,
the next segment's written arguments are appended after that boundary term; if
the resulting arity or types do not match a resolved predicate root, the chain is
rejected rather than regrouped.

## 9.4 Dependent and Typed Parameters

Parameter types can depend on earlier parameters:

```mizar
definition
  let G be Group;
  let H be Subgroup of G;
  let a, b be Element of G;
  pred CongruenceDef: a, b are_congruent_mod H means
    a * b" in H;
end;
```

You may also constrain types using attributes:

```mizar
definition
  let G be non empty associative Magma;
  let a, b be Element of G;
  pred CommuteDef: a, b are_commute means
    G.binop(a, b) = G.binop(b, a);
end;
```

Domain restrictions via `assume`:

```mizar
definition
  let n, m be Integer;
  assume m <> 0;
  pred DivisibleDef: m divides n means
    ex k be Integer st n = m * k;
end;
```

## 9.5 Correctness Conditions

Unlike functors, predicates defined via `means` do **not** require existence or uniqueness proofs because they directly specify a boolean condition.

However, the following conditions apply:

| Condition | Description |
| --- | --- |
| **Well-formedness** | The defining formula must be syntactically valid |
| **Type correctness** | All terms in the formula must be type-correct |
| **Definedness** | If `assume` is used, the predicate is undefined outside the assumed domain |

> [!NOTE]
> Predicates do not have a uniqueness requirement like functors, since they simply evaluate to true or false.

### 9.5.1 Predicate Properties

Predicates can declare algebraic properties that the verifier uses for automatic reasoning. Each property requires a proof obligation.

**Syntax (EBNF):**

```ebnf
pred_property ::= ( "symmetry" | "asymmetry" | "connectedness"
                 | "reflexivity" | "irreflexivity" ) justification ";" ;
```

**Properties and FOL Obligations:**

For a binary predicate `P(a, b)` with arguments of type T:

| Property | FOL Obligation | Verifier Effect |
|---|---|---|
| `symmetry` | `∀a,b:T. P(a,b) → P(b,a)` | Auto-swaps arguments when matching |
| `asymmetry` | `∀a,b:T. P(a,b) → ¬P(b,a)` | Derives negation when arguments are swapped |
| `connectedness` | `∀a,b:T. a ≠ b → (P(a,b) ∨ P(b,a))` | Derives disjunction for distinct elements |
| `reflexivity` | `∀a:T. P(a,a)` | Auto-discharges `P(x,x)` goals |
| `irreflexivity` | `∀a:T. ¬P(a,a)` | Auto-derives contradiction from `P(x,x)` |

**Example:**

```mizar
definition
  let x, y be Real;
  pred LessThanDef: x < y means ...;
  asymmetry
  proof
    let a, b be Real;
    assume a < b;
    thus not b < a by mml.number.real.Lt_asymmetric;
  end;
  irreflexivity
  proof
    let a be Real;
    thus not a < a by mml.number.real.Lt_irreflexive;
  end;
  connectedness
  proof
    let a, b be Real;
    assume a <> b;
    thus a < b or b < a by mml.number.real.Lt_connected;
  end;
end;

definition
  let X, Y be set;
  pred SubsetDef: X c= Y means
    for x being object st x in X holds x in Y;
  reflexivity
  proof
    let A be set;
    thus A c= A;
  end;
end;
```

> [!NOTE]
> Properties are placed inside the `definition ... end;` block, after the defining formula and any correctness conditions.

## 9.6 Predicate Redefinition

Existing predicates can be redefined for more specific types. This improves clarity and removes redundant conditions.

**Syntax**:

```ebnf
redefine_pred  ::= "redefine" "pred" label ":" pred_pattern "means" formula_definiens ";"
                   "coherence" [ "with" label ] justification ";" ;
```

**Example**:

```mizar
definition
  let x be object, f be Function;
  pred fixpoint_general: x is_a_fixpoint_of f means
    x in dom f & x = f.x;
end;

definition
  let A be non empty set, a be Element of A, f be Function of A, A;
  redefine pred fixpoint_restricted: a is_a_fixpoint_of f means
    a = f.a;
  coherence with fixpoint_general
  proof
    :: dom f = A, so membership is implied
    ...;
  end;
end;
```

Redefinition requirements:

* Use `redefine pred`
* Give the redefinition its own label before the pattern (e.g., `fixpoint_restricted`)
* Refer to the original using `coherence with` and the original label (e.g., `fixpoint_general`)
* Provide a `coherence` proof to show logical equivalence under the new parameter types

A redefinition sharpens the *same* predicate as the original; it is not a rival definition. So when several redefinitions of one original predicate apply at the same place, the verifier does not call this ambiguous. Each redefinition has proved, through its coherence condition, that it is logically equivalent to the original wherever it applies — so they all agree on whether the predicate holds, and picking one rather than another can never change a truth value.

This covers only redefinitions of a single original predicate. Two genuinely different predicates that share a name but have incomparable parameter types are still ambiguous and must be disambiguated as described in Chapter 19.

## 9.7 Notation Styles

Supported notations:

| Notation | Pattern | Example |
| --- | --- | --- |
| **Infix** | `x symbol y` | `x c= y`, `a divides b` |
| **Prefix** | `symbol x` | `is_prime n` |
| **Postfix** | `x symbol` | `n is_even` |
| **Multi-infix** | `x, y symbol z` | `a, b partition c` |

**Symbolic examples**:

```mizar
pred SubsetDef: X c= Y means ...;       :: infix symbolic
pred LessThanDef: x < y means ...;      :: infix symbolic
```

Primitive predicates `in`, `=`, and `<>` are built-in predicate applications,
not predicate symbols available in `pred` or `redefine pred` patterns. They are
listed separately in [§9.9.2](#992-built-in-predicates).

**Phrase examples**:

```mizar
pred DividesDef: m divides n means ...;      :: infix phrase
pred PrimeDef: is_prime n means ...;         :: prefix phrase
pred CommuteDef: a, b are_commute means ...; :: multi-infix phrase
```

> [!NOTE]
> Unlike functors, **predicates do not have operator precedence or associativity**. Predicate applications are parsed as atomic formulas and combined using logical connectives (`&`, `or`, `implies`, etc.) which have their own fixed precedence. See [Chapter 14 (Quantification and Logical Connectives)](./14.formulas.md) for logical operator precedence.

## 9.8 Symbol Resolution and Imports

User-defined predicates are made available by importing their defining modules. Conflicts between names are resolved by:

* **Longest-match rule** during lexing
* **Qualified names** when needed:

```mizar
import algebra.group as grp;
let G be set;
assume grp.is_subgroup(H, G);
```

Ambiguous predicate symbols can also be resolved using qualified names:

```mizar
let R be DoubleLoopStr;
let a, b be Element of R.carrier;
assume a, b algebra.additive.are_commute;  :: addition commutativity
assume a, b algebra.multiplicative.are_commute;  :: multiplication commutativity
```

> [!NOTE]
> For attribute disambiguation using the `qua` operator, see [Chapter 6 (Attributes), §6.6](./06.attributes.md#66-disambiguation).

> [!NOTE]
> Predicate symbols must be unambiguous only when they are actually used. For **synonyms** and **antonyms** of predicates, see [Chapter 11 (Symbol Management), §11.1](./11.symbol_management.md#111-synonyms-and-antonyms).

## 9.9 Logic Encoding Details

This section describes how predicates map to First-Order Logic.

### 9.9.1 Predicates as FOL Predicates

Each user-defined predicate is encoded as an FOL predicate symbol.

| Mizar Source | FOL Encoding |
| --- | --- |
| `x c= y` | `subset(x, y)` |
| `m divides n` | `divides(m, n)` |
| `a, b are_commute` | `are_commute(a, b)` |

### 9.9.2 Built-in Predicates

The built-in predicates have standard FOL encodings:

| Mizar Source | FOL Encoding | Description |
| --- | --- | --- |
| `x in X` | `x ∈ X` | Set membership |
| `x = y` | `x = y` | Equality (with standard axioms) |
| `x <> y` | `x ≠ y` (i.e., `¬(x = y)`) | Inequality |

These built-in predicates are primitive and do not have `means` definitions.

### 9.9.3 Predicate Definitions as Bi-conditionals

A predicate definition generates a universally quantified bi-conditional axiom:

```
pred X c= Y means for x st x in X holds x in Y
⇒  forall X, Y (subset(X, Y) ↔ forall x (x ∈ X → x ∈ Y))
```

More generally:

```
pred P(x1, ..., xn) means φ
⇒  forall x1, ..., xn (P(x1, ..., xn) ↔ φ)
```

### 9.9.4 Typed Parameters as Guards

Type constraints on parameters are encoded as antecedents:

| Mizar Source | FOL Encoding |
| --- | --- |
| `let x, y be Real; pred x < y means ...` | `forall x, y (is_Real(x) ∧ is_Real(y) → (less(x, y) ↔ ...))` |
| `let G be Group; let a be Element of G; pred ...` | `forall G, a (is_Group(G) ∧ is_Element(a, G) → ...)` |

### 9.9.5 Domain Restrictions (`assume`)

When a predicate uses `assume`, the condition becomes an additional antecedent:

```
let n, m be Integer;
assume m <> 0;
pred m divides n means ...

⇒  forall m, n (is_Integer(m) ∧ is_Integer(n) ∧ m ≠ 0 → (divides(m, n) ↔ ...))
```

### 9.9.6 Negation Encoding

Predicate negation is encoded as logical negation:

| Mizar Source | FOL Encoding |
| --- | --- |
| `x does not divide y` | `¬divides(x, y)` |
| `a, b do not are_commute` | `¬are_commute(a, b)` |

### 9.9.7 Redefinition Encoding

A redefinition generates an equivalence axiom under restricted types:

```
redefine pred fixpoint_restricted: a is_a_fixpoint_of f means a = f.a;
⇒  forall A, a, f (
     ¬is_empty(A) ∧ is_set(A) ∧ is_Element(a, A) ∧ is_Function_of(f, A, A)
     → (fixpoint(a, f) ↔ a = f(a))
   )
```

Having several redefinitions of the same predicate active at once is safe: each one contributes an equivalence with the *same* predicate symbol, guarded by the condition under which it applies. The verifier keeps track of which redefinitions are active so it can report them, but in the underlying logic the predicate is always the one symbol introduced by the original definition.

## 9.10 Complete Syntax (EBNF)

Summary of predicate-related syntax.

```ebnf
(* Predicate Definition *)
pred_def         ::= "pred" label ":" pred_pattern "means" formula_definiens ";" ;
label            ::= identifier ;
pred_pattern     ::= [ loci ] def_predicate_symbol [ template_loci ] [ loci ] ;
loci             ::= locus_list | "(" locus_list ")" ;
locus_list       ::= locus { "," locus } ;
locus            ::= identifier ;
template_loci    ::= "[" locus_list "]" ;
def_predicate_symbol ::= identifier | user_symbol ;
predicate_symbol ::= def_predicate_symbol ;
formula_definiens ::= formula
                    | formula_case { "," formula_case } [ "otherwise" formula ] ;
formula_case      ::= formula "if" formula ;

(* Predicate Redefinition *)
redefine_pred    ::= "redefine" "pred" label ":" pred_pattern "means" formula_definiens ";"
                     "coherence" [ "with" label ] justification ";" ;

(* Predicate Properties *)
pred_property    ::= ( "symmetry" | "asymmetry" | "connectedness"
                   | "reflexivity" | "irreflexivity" ) justification ";" ;

(* User Predicate Application - see Chapter 14 *)
user_predicate_application ::= predicate_segment { predicate_chain_segment } ;
predicate_segment            ::= [ term_list ] [ negation ] predicate_head [ term_list ] ;
predicate_chain_segment      ::= [ negation ] predicate_head term_list ;
predicate_head               ::= predicate_symbol [ template_args ] ;
negation                     ::= "does" "not" | "do" "not" ;
term_list                    ::= term_expression { "," term_expression } ;

(* Built-in Predicates - primitive applications, not definition symbols *)
builtin_pred     ::= "in" | "=" | "<>" ;
```
