...
The `qua` operator creates a type-restricted view. The term `x` itself is unchanged in the FOL encoding; `qua` affects only type-dispatch.
...
In both cases the validity obligation is discharged automatically by the verifier using the `inherit` chain and cluster registration axioms; no user proof is required. For the complete FOL axiom set (transitive closure, cycle freedom, cluster propagation, consistency), see [§13.8.7](./13.term_expression.md#1387-qua-operator-encoding).
...
> [!NOTE]
> For advanced type conversion scenarios, including implicit conversions, overload resolution, and symbol redefinition, see **Chapter 19 (Overload Resolution)**.
