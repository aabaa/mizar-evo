...
> **注**: ここで挙げた `head_symbols` / `all_symbols` / `statement.surface` / `search_text.doc` などは **現状の `*.mizir.json` の明示フィールドではない**。`*.mizir.json` から直接得られるのは `exports` の FQN・source_range と obligations の `used_axioms` まで（§23.5.1）。残りは source comments / documentation extraction からの抽出、または新規抽出経路の整備を要する。したがって index 化は「既存 artifact をそのまま束ねる」のではなく、**この抽出を含むタスク**として見積もること。最初の薄い版は既存フィールド（FQN・source_range・used_axioms）から始め、FOL/normalized/embedding 系は後段に回す。
...
> **注**: 上の patch / 応答例が依存する `obligation_id`（例: `vc-42`）には、**編集をまたいで同一 obligation を指し続ける安定アンカー**が要る。`VcId` の決定的採番自体は既存（[architecture 07.vc_generation.md:445-448](../design/architecture/en/07.vc_generation.md#L445-L448): sorted seed list ＋ per-seed expansion index）だが、index ベースのため obligation の挿入・削除でずれ、**cross-edit 安定性は未規定**。この安定アンカーの仕様化は patch protocol（特に dry-run）の前提として先に行う必要がある。wire schema の凍結はそれ以降。
...
> **注**: `mizar context obligation` は能力としては完全新規ではなく、**既存の bounded extraction（§23.7.5）を obligation 単位で束ねる表面整備**に近い。見積もりは「新機能」ではなく「既存抽出の CLI/JSON 化＋トークン予算と allowed_patch_kinds の付加」として扱うとよい。
...
## 9. proof witness 仕様（方向は妥当・凍結は下層安定後）

> **注**: proof_witness 形式の**凍結は遅延項目**とする。下記の binder_id ベース per-axiom scoping という**方向は妥当**だが、形式はまだ tentative であり、ここを今規範化すると後で破壊的変更を招く。形式の確定は `*.mizir.json` や kernel certificate が安定した後に行う。先行して固めるべきは「形式」ではなく、「将来 binder_id ベースにする」という**設計方針の合意**まで。
...
> **注**: `recovery_actions` も完全新規ではなく、**既存の machine-readable fix / explanation API（competing candidates・suggested fix）の構造化拡張**に近い。見積もりは「診断基盤の新設」ではなく「既存 suggested fix を patch kind に対応づくスキーマへ整える」作業として扱うとよい。
...
## 16. まとめ方：言語仕様の章ではなく architecture 配下の tooling 設計文書として

> **注**: この内容は **tooling contract であって言語仕様ではない**（本節末尾でも述べるとおり）。したがって言語リファレンスに混ぜず、`doc/design/architecture/`（`12.diagnostics_and_lsp` や `09.atp_interface` の隣）か独立の tooling spec に、まず**非規範の設計文書**として置く。下の目次は言語仕様の章ではなく、その設計文書の構成案として読む。各項目を規範に昇格させる順序は §17 の段階順に従い、wire schema の凍結は下層アーティファクト安定後とする。

非規範の tooling 設計文書としては、次の構成が筋が良いです。

```text
AI Agent Interface and Context Protocol（architecture 配下の tooling 設計文書）

1.  Design Principles
2.  Trust Boundary and Safe Edit Classes      ← 今すぐポリシー化（§8）
3.  Context Budget Model
4.  AI Resource URI Scheme
5.  Theorem and Symbol Index Artifacts         ← 薄い版から（§3 注）
6.  Obligation Context Extraction              ← cross-edit anchor が前提（§4 注）
7.  Patch Proposal Format                      ← wire schema は下層安定後に凍結
8.  Patch Verification API
9.  Proof Repair State Machine
10. MCP Binding                                ← 遅延
11. Authorization Scopes                       ← scope 語彙のみ今、本体は遅延（§14）
12. Dataset Export Format                      ← 遅延
13. Benchmarks and Metrics                     ← 遅延（corpus 整備後）
```
...
> **注**: 下の Phase 1 より前に、規範化や下層実装に依存しない **Phase 0** を置く。また Phase 2 の前提条件を明示する。
>
> - **Phase 0（今すぐ・安価・下層非依存）**: §8 の安全境界 **Green/Yellow/Red** と §14 の **MCP scope 語彙**を明文化する。これらは検証器が無くてもポリシーとして機能し、[architecture/08 reasoning_boundary](../design/architecture/en/08.reasoning_boundary.md) と `require_kernel_certificates` の自然な延長になる。
> - **Phase 1 への補足**: theorem/symbol index は §3 の注のとおり、既存フィールド（FQN・source_range・used_axioms）の薄い版から始め、抽出経路の整備を別タスクとして見積もる。
> - **Phase 2 の前提**: dry-run patch verification の前に、§4 の注で述べた **cross-edit 安定 obligation anchor の仕様化**を完了しておくこと。
> - **遅延の実装的理由**: そもそも現実装は verifier 本体・artifact 生成がまだ未成熟で、検証器が無ければ patch dry-run も index も実体を持てない。wire schema / MCP 束縛 / benchmark の凍結を後段に回すのは、概念的な慎重さだけでなく、**それらが包む下層アーティファクトがまだ動いていない**という実装面の理由による。
