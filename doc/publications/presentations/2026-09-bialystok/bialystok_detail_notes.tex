...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item all imports appear before the first non-import item;
\item imports provide the initial active lexicon. Local declarations extend it after their declaration points; imported items retain their source FQNs;
...
\midrule
\texttt{field} & stored data & constructor arguments; equality of exact instances \\
\texttt{property} & derived value from an implementation & \texttt{means}: existence/uniqueness; \texttt{equals}: direct term \\
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item A property declaration gives no value. Implementations supply values; constructors take fields only. Overlapping implementations need \texttt{coherence}.
\end{itemize}
For the table, read the row labels first, then the design reason.
...
\item non-identical inherited member types need \texttt{coherence} proofs of subtype inclusion. Same-name, same-type mappings need no proof;
...
\item non-identical inherited member types need coherence proofs of subtype inclusion. Same-name, same-type mappings need no proof;
\item the additive/multiplicative naming convention becomes a checked view.
\end{itemize}
After the example, explain which design obligation the syntax shows.
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item The diagram assumes the shown parent mappings from \texttt{AddLoopStr} and \texttt{MulLoopStr} to \texttt{Magma}. The code adds their shared child.
\end{itemize}
}
\end{frame}
...
\smallskip
\textbf{Inheritance diagram}~\badgeSketch
\begin{center}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item same-name, same-type members can join even when their root declarations differ. Other joins need \texttt{coherence} proofs of subtype inclusion;
\item renamed paths remain distinct views. A diamond is allowed; invalid mappings or missing proofs produce diagnostics.
\end{itemize}
\note{
\smallskip
\textbf{Presenter script:}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item same-name, same-type members can join even when their root declarations differ. Other joins need coherence proofs of subtype inclusion;
\item renamed paths remain distinct views. A diamond is allowed; invalid mappings or missing proofs produce diagnostics.
\end{itemize}
}
\end{frame}
...
\item \texttt{explain-attribute} and resolution traces show why an attribute follows or why resolution fails, e.g. empty -> finite -> countable.
\end{itemize}
\note{
\smallskip
\textbf{Presenter script:}
...
\item the verifier uses an import-filtered view of the global cluster graph;
\item explain-attribute and resolution traces show why an attribute follows or why resolution fails, e.g. empty -> finite -> countable.
\end{itemize}
After the example, explain which design obligation the syntax shows.
...
\item cluster applications need no repeated proofs. Reductions may need local guard evidence or an explicit equality citation.
\end{itemize}
\smallskip
\textbf{Questions for Bialystok:}
\begin{itemize}
...
\item the kernel checks supplied formulas and substitutions, then runs its trusted SAT check. It does not select premises or invent substitutions;
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item \texttt{KernelEvidence} contains source formulas, explicit substitutions, provenance, and target/goal bindings;
\item the kernel checks this evidence, derives instantiated formulas and SAT clauses, and requires UNSAT from its trusted in-process Rust SAT checker;
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item KernelEvidence contains source formulas, explicit substitutions, provenance, and target/goal bindings;
\item the kernel checks this evidence, derives instantiated formulas and SAT clauses, and requires UNSAT from its trusted in-process Rust SAT checker;
\item backend resolution traces, SMT proof objects, logs, and exit codes are not trusted acceptance evidence.
\end{itemize}
}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item the de Bruijn discipline: acceptance uses a small trusted core, including its SAT checker;
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item cache keys cover source, dependency slices, package/lockfile, toolchain, schema, policy, registrations, obligations, evidence, and witnesses;
\item reuse requires all relevant keys to match; missing data causes a cache miss;
\item a theorem proof-body edit does not rebuild importers if its exported statement and accepted status stay unchanged;
...
\item predicate and functor parameters cannot be used in \texttt{attr}, \texttt{mode}, \texttt{struct}, \texttt{func}, or \texttt{pred} items;
\item eligible templates get short forms: \texttt{Module over R} for \texttt{Module[R]}, \texttt{Subset of X} for \texttt{Subset[X]}. Constrained templates require brackets.
\end{itemize}
\note{
\smallskip
\textbf{Presenter script:}
...
\item predicate and functor parameters cannot be used in attr, mode, struct, func, or pred items;
\item eligible templates get short forms: Module over R for Module[R], Subset of X for Subset[X]. Constrained templates require brackets.
\end{itemize}
After the example, explain which design obligation the syntax shows.
...
\smallskip
\textbf{Mizar Evo}~\badgeSketch~{\footnotesize(from specification §18.2.2; \texttt{Product} laws omitted)}
\begin{lstlisting}[language={mizar},basicstyle=\ttfamily\footnotesize]
definition
...
\item commutativity alone is not enough for this sketch: \texttt{Product} also needs associativity and a unit for the empty product.
\end{itemize}
\note{
\smallskip
\textbf{Presenter script:}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Mizar Evo (sketch from specification §18.2.2; Product laws omitted):
\item type extends commutative Magma states what the parameter must provide before any proof search begins;
\item commutativity alone is not enough for this sketch: Product also needs associativity and a unit for the empty product.
\end{itemize}
After the example, explain which design obligation the syntax shows.
...
\smallskip
\textbf{Instantiation}~\badgeSketch~{\footnotesize(; assumes the required laws and attribute evidence)}
\begin{lstlisting}[language={mizar},basicstyle=\ttfamily\small]
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Instantiation (sketch; assumes the required laws and attribute evidence):
...
\item first-order discipline: each instantiation is checked; templates add no new logic.
\end{itemize}
\smallskip
\textbf{Questions for Bialystok:}
\begin{itemize}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item the Mizar Virtual Machine (MVM) supports only ground equalities and predicates, using computable algorithms defined in the current package;
\item step, time, and depth limits are optional; each defaults to \texttt{0} (unlimited);
\item algorithms from other packages are opaque. Their \texttt{ensures} contract is usable when termination is known; their body cannot be executed here.
\end{itemize}
\note{
\smallskip
\textbf{Presenter script:}
...
\setlength{\parskip}{0pt}
\item the Mizar Virtual Machine (MVM) supports only ground equalities and predicates, using computable algorithms defined in the current package;
\item step, time, and depth limits are optional; each defaults to 0 (unlimited);
\item algorithms from other packages are opaque. Their ensures contract is usable when termination is known; their body cannot be executed here.
\end{itemize}
After the example, explain which design obligation the syntax shows.
...
\item a terminating algorithm becomes a functor after all contract and termination obligations are proved for inputs satisfying requires;
\item this is the only way to add recursion to the mathematical layer. It requires a proof.
\end{itemize}
}
\end{frame}
...
\setlength{\parskip}{0pt}
\item contracts, invariants, and termination measures generate verification conditions that pass through the same ATP-plus-kernel boundary as ordinary theorems (story 4);
\item by computation uses MVM replay with optional limits;
\item code extraction (to runtime targets) is strictly downstream of verified artifacts and can never affect acceptance.
\item Algorithms let the library express more. They do not change what it means for a theorem to be accepted.
\end{itemize}
}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item algorithms live in \texttt{definition} blocks. Proofs can use verified promotion or \texttt{by computation} for computable ground calls in the current package;
\item a partial algorithm's \texttt{ensures} needs evidence that the call terminates;
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item algorithms live in definition blocks. Proofs can use verified promotion or by computation for computable ground calls in the current package;
\item a partial algorithm's ensures needs evidence that the call terminates;
\item first-order set-theoretic foundations stay intact.
\item Which computational examples would show clear benefits to mathematicians without shifting the culture toward programming?
...
\item proposed origin metadata links migrated items to MML citations; the migration mapping still needs to be defined.
\end{itemize}
\smallskip
\textbf{Questions for Bialystok:}
\begin{itemize}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item The diagram shows the ATP path. Only obligations still open after deterministic discharge go to ATP; earlier discharge also needs evidence.
\end{itemize}
\smallskip
\textbf{Read aloud:}
\begin{itemize}
...
\item 24 draft architecture documents covering the pipeline, kernel, kernel evidence, and AI agent interface (\texttt{doc/design/architecture/});
...
\item 24 draft architecture documents covering the pipeline, kernel, kernel evidence, and AI agent interface (doc/design/architecture/);
\item a Rust workspace of 20 crates - lexer, parser, resolver, checker, VC generator, ATP bridge, kernel, build system - roughly 400k lines including tests;
\item focused audits completed in 2026: kernel soundness, template logic encoding, SAT solver dependency;
\item the roadmap is split into small tasks that can be verified separately.
\end{itemize}
...
\item KernelEvidence and the trusted SAT check (story 4). [done: \texttt{figures/certificate\_replay.pdf}, used in frame 5.3]
\end{enumerate}
\note{
\smallskip
\textbf{Presenter script:}
...
\item Core ATP path with responsibility groups (Part 10). [done: \texttt{figures/pipeline.pdf}, used in frame 10.1]
...
\item Core ATP path with responsibility groups (Part 10). [done: figures/pipeline.pdf, used in frame 10.1]
