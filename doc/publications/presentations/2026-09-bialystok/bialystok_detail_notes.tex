...
\item Explain frames 3.3, 5.2, and 8.2. Cover the other frames briefly and skip deep dives unless needed. The handout keeps the details for later review.
\end{itemize}
}
\end{frame}
...
\item Current Mizar already names the parent: \texttt{(1-sorted)}. Evo adds an explicit mapping to a common \texttt{Magma} view. Both examples describe a carrier and a binary operation; the Evo example also shows this extra view.
\end{itemize}
}
\end{frame}
...
\end{itemize}
The Evo examples follow the specification. Some are not yet supported by the implementation. See frame 11.0 for the current scope.
...
\item Examples assume required imports and earlier declarations. ... marks omitted proof text.
\end{itemize}
...
A property declaration supplies no value. An implementation must provide it.
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Explain: distinguish stored fields, derived properties, and attributes. Constructors take fields only. Overlapping implementations need \texttt{coherence}.
\end{itemize}
\smallskip
\textbf{Read aloud:}
\begin{itemize}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item A property declaration supplies no value. An implementation must provide it.
\end{itemize}
For the table, read the row labels first, then the design reason.
...
\item We want stronger proof search while keeping proof checking small.
\item Evo requires checkable evidence from search. The search result alone cannot establish a kernel-verified proof.
\end{itemize}
\smallskip
\textbf{Key Phrase:}
\begin{center}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Credit MizAR, MPTP, and hammer research. Explain Evo's evidence contract without claiming that earlier tools must trust their search backends.
\end{itemize}
\smallskip
\textbf{Read aloud:}
\begin{itemize}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Explain: search proposes evidence; the kernel checks it.
...
\setlength{\parskip}{0pt}
\item schemes support second-order patterns (induction, separation, replacement). They work, but they use a separate mechanism with separate rules;
\item Classic scheme blocks define theorem schemas. Parameterized definitions use other language forms.
\end{itemize}
}
\end{frame}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Mizar already supports parameterized constructions, such as \texttt{Polynom-Ring L}.
\item Schemes and parameterized definitions use different mechanisms.
\item Evo proposes a common template system for definitions and theorem schemas, with explicit rules for each kind of parameter.
\end{itemize}
\smallskip
\textbf{Message:}
\begin{itemize}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item The aim is to make generic mathematics easier to write and maintain. We need migration examples to test whether these rules help.
\end{itemize}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Mizar already supports parameterized constructions, such as Polynom-Ring L.
\item Schemes and parameterized definitions use different mechanisms.
\item Evo proposes a common template system for definitions and theorem schemas, with explicit rules for each kind of parameter.
\item The aim is to make generic mathematics easier to write and maintain. We need migration examples to test whether these rules help.
\item Source: POLYNOM3, definition 10.
\end{itemize}
}
\end{frame}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Mizar already has arithmetic automation and proofs of program correctness.
\item Evo proposes a language form for algorithms with contracts, linked to verification, execution, and code extraction.
\item The question is how to connect these steps within one language and toolchain.
\end{itemize}
\smallskip
\textbf{Key Phrase:}
\begin{center}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Explain: the contract uses mathematics. The algorithm computes the result, and the library functor \texttt{Gcd} specifies it. The \texttt{decreasing} measure on \texttt{y} proves termination.
\end{itemize}
\smallskip
\textbf{Read aloud:}
\begin{itemize}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Linked records connect journal citations, library locations, verification artifacts, and MML origins.
\end{itemize}
\note{
\smallskip
\textbf{Presenter script:}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Specification: 24 chapters plus appendices; English is canonical, with Japanese companions.
\item Implemented parts: frontend processing, selected semantic cases, ATP candidate generation, and kernel evidence checking.
\item In progress: the full path from source input through ATP search and kernel checking to published artifacts. Tests with real external provers are still needed.
\item Deferred: MVM execution and code extraction.
\item Full MML migration remains future work.
\end{itemize}
\smallskip
\textbf{Message:}
\begin{itemize}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item This is the September 2026 scope. Component tests do not establish that the full pipeline works. See \texttt{doc/design/todo.md}, Completion Gates.
\end{itemize}
\smallskip
\textbf{Read aloud:}
\begin{itemize}
...
\item Choose two or three questions: what must be preserved, which small MML articles to migrate first, and which proposal needs the most change.
\item Invite readers to review the other questions in the handout later.
\end{itemize}
}
\end{frame}
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item ALGSTR\_0: \url{https://mizar.uwb.edu.pl/version/current/mml/algstr\_0.miz}
\item STRUCT\_0: \url{https://mizar.uwb.edu.pl/version/current/mml/struct\_0.miz}
\item NAT\_1: \url{https://mizar.uwb.edu.pl/version/current/mml/nat\_1.miz}
\end{itemize}
For the table, read the row labels first, then the design reason.
...
\setlength{\parsep}{0pt}
\setlength{\parskip}{0pt}
\item Specification used: 10 September 2026. For grammar and semantics, see
\item doc/spec/en/00.index.md in the repository.
\item Online files may change after this talk.
\end{itemize}
For the table, read the row labels first, then the design reason.
