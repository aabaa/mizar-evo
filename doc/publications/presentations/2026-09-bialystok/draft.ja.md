...
45分の概要説明では、`**...**` で囲んだ文を上から順に読む。両PDFで濃い青の太字になる。
冒頭はタイトルページのノートを使い、その後は強調した説明文のないスライドを省略できる。
それ以外の文章、コード、表、「後日の確認用」の節はレジュメに残す。
コードの構文色分けは、発表の優先度を表さない。ノートの「Read aloud」も同じ箇所を強調する。
説明40分・質疑や追加説明5分の仮の配分は、README.md の Two-Tier Pacing を参照する。
...
- `notations` や `definitions` など、環境部のリストでは記事の順序が意味を持つ。どの記事を含めるかだけでなく、どの順序で並べるかまで理解する必要がある。
- Accommodator が環境を解決するが、解決結果はレビュー可能なソーステキストではない。
- 初等群論のような理論全体を使いたい場合でも、利用者は個々の記事を列挙する必要がある。
...
- `export` を使うと、関連するモジュールを一つの窓口にまとめられます。
  これにより、ライブラリは初等群論のようなまとまりを、一つの import で利用できる形で提供できます。
...
- 現行 Mizar では、台集合と一つの二項演算を表す構造が、`addMagma` と `multMagma` として別々に定義されています。
- 共通する定義や証明も、加法用と乗法用に繰り返し記述されています。
- Evo のメンバー改名により、馴染みのある演算名を保ちながら、両方の構造で共通の `Magma` の理論を利用できます。

メッセージ:

- **共通の理論を一度定義・証明し、改名したビューを通じて再利用します。**
...
- **`Magma` に対して一度定義・証明し、加法・乗法の両方のビューを通じて再利用します。**
- 二つのビューは区別されます。定理を使う際には、選んだ演算についてその仮定を満たす必要があります。
...
- **ラベルによって cluster の利用を追跡できます。`by` での明示的な引用は任意です。**
- **検証器は、有効な環境内で適用条件を満たす cluster を自動的に適用します。**
...
### Frame 4.5 - 多重定義: この記号はどの定義を意味するのか

- **同じ記号に複数の定義があり得ます。綴りだけでは、どの定義が適用されるか分かりません。**
- cluster による推論の結果、引数が複数のパラメータ型に適合することがあります。
- **例えば、有限な位相空間は countable でも compact でもあります。
  `f` にそれぞれの型に対する多重定義があれば、両方が適用候補になります。**
- 明確な選択規則と、それだけでは決まらない場合に意図を明示する方法が必要です。

発表者ノート:

- 仕様§19.2.1と§19.4.4の例を、必要な registration がある前提で使っています。
  二つは独立した定義であり、どちらのパラメータ型も他方より特殊ではありません。

### Frame 4.6 - Evo の答え: 定義を選ぶか、曖昧さを示す

必要な registration がある場合の、二つの独立した多重定義(sketch):
...
- **Evo は cluster 推論後の、より特殊なパラメータ型を優先します。戻り値の型では多重定義を選びません。**
- **最適な定義が複数残れば、曖昧性を報告します。この例では `qua` で意図する型を明示します。**
- **こうして、証明探索を始める前に意味を決定します。**

発表者ノート:

- 出典: 仕様§19.3.1と§19.4。呼び出しは説明用です。
- スコープと優先規則も適用します。同じ根に属する整合した `redefine` は統合します (§19.5)。
- インポートの曖昧さは `module_path.symbol_name` で限定できます (§19.6.3)。

### Frame 4.7 - 保存されるもの、問いたいこと
...
- **template の本体は宣言時に一度検査されます。利用時には必要な制約を満たすことを確認します。template は新しい論理を加えません。**
