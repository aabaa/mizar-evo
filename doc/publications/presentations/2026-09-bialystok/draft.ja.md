...
- 右辺は厳密に小さくなければならず、インポートされた規則が循環を作れない。規則選択は決定的(特殊性優先、FQN でタイブレーク)。
...
### Frame 5.5 - 編集クラス: Green / Yellow / Red
...
### Frame 7.6 - scheme は普通のテンプレートになる
...
- パラメータ推論はどこで止まり、明示的な `[T]` や `qua` を要求すべきか。
...
- 通常の `func` 定義は一階述語論理の定義による拡張であり、再帰できない。
- `terminating` アルゴリズムは、停止性義務を果たした後、本物の functor に昇格し、あらゆる証明で使える。
- これが再帰が数学の層に入る唯一の扉であり、その扉は証明の形をしている。

### Frame 8.5 - 計算は真理を再定義しない
...
1. 実証済みの設計にかかる3つの圧力(Part 1)。
2. environ から import への移行(物語1)。
3. 構造体継承とダイアモンドの整合性(物語2)。
4. 推論境界: 意味論 / ATP 探索 / カーネル検査(物語4)。
5. 証明書オブジェクトと再生(物語4)。
6. 差分検証のフィンガープリントグラフ(物語5)。
7. Formalized Mathematics の記事-ライブラリ リンクモデル(物語8)。
8. 物語を重ねた全パイプライン(Part 10)。
9. ロードマップ年表(Part 11)。
