...
proof-reuse validation を再実装してはならない。`mizar-driver` と
`mizar-build` のスケジューリング波との real pipeline wiring は、所有 crate が
実 seam を公開するまで risk tag `external_dependency_gap` とする。phase
