...
| active-case admission と stable failure assembly | tag/phase gate、expected-output matching、deterministic failure diagnostic | phase-specific facade-to-owner transition | Task 259 は parse-only case runner/failure projection を移動済み。残る declaration/type boundary は owning Tasks 260B-263 まで `runner.rs` に保持。 |
...
| fixture import provider | parser fixture lexical summary と adapter | active phase が共有する parser/frontend seam | Task 261 まで `runner.rs` に保持。後段 phase による利用を維持。 |
| declaration-symbol observation | shared resolver result を consume し、deterministic payload、expected-value、failure projection を組み立てる | shared resolver output から declaration-symbol result | Task 260B で private declaration-symbol owner へ移動。既存 integration test は `tests/metadata.rs` に残す。 |
...
| 260B | 既存 declaration-symbol case/observation/payload/expectation/failure helper を移動。test move ではない。 |
