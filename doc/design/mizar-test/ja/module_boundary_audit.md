...
| 255B | four-edge radix source test 2件と専用 Task 208/207 helper を `asserted_head_four_edge_radix.rs` へ移動。 |
| 255C | three-edge object-radix source test と専用 Task 206 helper を移動。 |
| 255D | two-edge object-radix source test と専用 Task 204 helper を移動。 |
| 255E | 最後の non-long-chain source test 16件を移動し、active-fixture boundary を保持して Task 255 を完了。 |
