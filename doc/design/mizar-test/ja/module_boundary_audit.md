...
| source/frontend staging | source package preparation/cleanup、root/path/snapshot identity、frontend execution/result transport | parse、declaration-symbol、type-elaboration が共有 | Task 258 で parent-only visibility の private `shared.rs` へ移動済み。 |
| active-case admission と stable failure assembly | tag/phase gate、expected-output matching、deterministic failure diagnostic | phase-specific facade-to-owner transition | Task 259 は parse-only case runner/failure projection を移動済み。残る declaration/type boundary は owning Tasks 260-263 まで `runner.rs` に保持。 |
