...
| source extraction | exact source-shape recognition と real AST/resolver payload construction | syntax/resolver input から checker input | Task 262A で common source-AST primitive を private `type_elaboration/source_ast.rs` へ移動済み。残る extractor family は後続 Task 262 subtask まで `runner.rs` に保持。 |
...
| 262 | parent: type-elaboration source-extraction leaf を移動。Task 262A 後も open。 |
