...
| source extraction | exact source-shape recognition と real AST/resolver payload construction | syntax/resolver input から checker input | Tasks 262A-262B で common source-AST primitive/projection、Task 262D で shared exact fixture-import projection を private `type_elaboration/source_ast.rs`、Tasks 262C/262E で reserve type-expression/symbol projection、declaration segmentation、local-mode expansion を private `type_elaboration/source_reserve.rs`、Tasks 262F-262I で standalone formula constant、shared exact numeral、builtin binary formula、builtin type-assertion formula を private `type_elaboration/source_formula.rs` へ移動済み。他の formula extraction と保持 caller は後続 Task 262 subtask まで `runner.rs` に保持。 |
...
| 262J1 | 次: shared imported formula symbol resolver/provenance pair だけを private source-formula leaf へ移動。 |
| 262J2 | J1 後: imported predicate/functor transport、exact extractor、exact infix projection、dedicated allowlist だけを private source-formula leaf へ移動。 |
...
| `test_gap` | Tasks 262H0/262I0/262J0 で対応する move-only task 前の bounded preservation-matrix gap を修復する。behavior/coverage credit は不変。 |
