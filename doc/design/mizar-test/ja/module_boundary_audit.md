...
を使う。保持した Task 262D declaration/mode caller がまだ consume する3 helper、
...
262D で caller を移した時点で narrow しなければならない。他の type-head、
...
traversal/expansion は Task 262D と後続 bounded inventory のため `runner.rs` に保持する。
...
| type-elaboration admission/execution | lower-stage fail-closed gate と checker/core handoff dispatch | resolver output から source bridge | Tasks 262-263 の間は `runner.rs` に保持。現在の `type_elaboration.rs` は `source_ast` と `source_reserve` leaf の private facade であり、後続 move により orchestration owner となる。 |
| source extraction | exact source-shape recognition と real AST/resolver payload construction | syntax/resolver input から checker input | Tasks 262A-262B で common source-AST primitive/projection を private `type_elaboration/source_ast.rs`、Task 262C で reserve type-expression/symbol projection を private `type_elaboration/source_reserve.rs` へ移動済み。declaration segmentation、mode expansion、formula extraction、保持 caller は後続 Task 262 subtask まで `runner.rs` に保持。 |
...
public runner facade
  ...
  -> type-elaboration owner
     ...
     -> source extraction
        ...
        -> reserve type-expression and symbol projection
...
| 262 | parent: type-elaboration source-extraction leaf を移動。Tasks 262A-262C 後も open。 |
...
| 262C | 完了: reserve type-expression、visible symbol/admission、source-text projection を private source-reserve leaf へ移動し、declaration/mode caller は Task 262D のため保持。 |
| 262D | 次: bounded reserve declaration-segmentation/local-mode traversal/expansion family を移動し、Task 262C の temporary helper visibility 3個を narrow。 |
