...
| type-elaboration admission/execution | lower-stage fail-closed gate と checker/core handoff dispatch | resolver output から source bridge | Task 263A で generic checker-handoff assembly/validation を private `checker_handoff.rs` へ移動。top-level admission/dispatch、config、output consumer は後続 Task 263 family のため `runner.rs` に残り、phase facade は private leaf 4個を所有する。 |
