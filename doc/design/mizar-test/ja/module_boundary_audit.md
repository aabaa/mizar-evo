...
| 257 | parent: inventory 済みの残る fixture、bridge-gap、corruption、isolation 8 family を移動。Task 257H まで pending。 |
| 257A | 完了: 先頭 binary/parenthesized fixture/route-isolation test 18件を `binary_route_fixtures.rs` へ移動し、Task 257B separator を保持。 |
| 257B | builtin-object reserve active fixture 3件を `reserve_object_fixtures.rs` へ移動。 |
| 257C | Task 180 standalone contradiction fixture だけを `formula_constant_fixture.rs` へ移動。 |
| 257D | distinct/multiple/heterogeneous reserve fixture 11件を `reserve_fixtures.rs` へ移動。 |
| 257E | non-long-chain active mode-chain fixture test 26件を `mode_chain_fixtures.rs` へ移動。 |
