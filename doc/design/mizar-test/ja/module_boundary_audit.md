...
| payload validation と detail-key rendering | exact checker/core output validation、expected/actual matching、deterministic key、diagnostic | source bridge output から runner result | Tasks 263E-263I で shared output transport/builder 3個と type-assertion/binary/shared-parenthesized validator/private helper を private `type_elaboration/output.rs` へ移動。Tasks 263J-263M で type-assertion/binary/shared parenthesized result/detail と payload-detail core も移動し、named wrapper/config は後続 bounded work に残す。key/order は編集しない。 |
