...
order-preserving な6 family に分割する。257A は binary-route test 18件、257B
は reserve fixture test 15件、257C は mode-chain fixture test 26件、257D は
asserted-head/type-assertion fixture test 39件、257E は source-gap/equality
test 3件、257F は root test 12件と既存 Task 216-222 module 内 nested test
28件を含む。6 count の合計は残る root test 113件と nested test 28件に一致
する。parent Task 257 は 257F まで pending。
...
| 257 | parent: inventory 済みの残る fixture、bridge-gap、corruption、isolation 6 family を移動。Task 257F まで pending。 |
| 257A | 完了: 先頭 binary/parenthesized fixture/route-isolation test 18件を `binary_route_fixtures.rs` へ移動し、Task 257B separator を保持。 |
| 257B | reserve active-fixture test 15件を `reserve_fixtures.rs` へ移動。 |
| 257C | non-long-chain active mode-chain fixture test 26件を `mode_chain_fixtures.rs` へ移動。 |
| 257D | active asserted-head/type-assertion fixture test 39件を `asserted_head_fixtures.rs` へ移動。 |
| 257E | source-gap/four-edge-equality test 3件を `source_gap_and_equality.rs` へ移動。 |
| 257F | Task 216-222 module を保持して最後の root test 12件/nested corruption/isolation test 28件を移動し、Task 257 を完了。 |
