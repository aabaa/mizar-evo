...
| type-elaboration admission/execution | lower-stage fail-closed gate と checker/core handoff dispatch | resolver output から source bridge | Tasks 262-263 の間は `runner.rs` に保持。現在の `type_elaboration.rs` は `source_ast` の initial private phase facade のみであり、後続 move により orchestration owner となる。 |
| source extraction | exact source-shape recognition と real AST/resolver payload construction | syntax/resolver input から checker input | Tasks 262A-262B で common source-AST primitive/projection を private `type_elaboration/source_ast.rs` へ移動済み。semantic resolution と残る extractor family は後続 Task 262 subtask まで `runner.rs` に保持。 |
...
| 262 | parent: type-elaboration source-extraction leaf を移動。Tasks 262A-262B 後も open。 |
