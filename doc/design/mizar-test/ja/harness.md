...
| type-elaboration | active な `.miz` type-elaboration case を frontend parsing と resolver declaration/symbol collection で run し、対応済み reserve-only builtin declaration bridge を checker-owned `BindingEnv`、`DeclarationInput`、`TypedAst`、`ResolvedTypedAst` まで実行し、`mizar-core` の `ResolvedTypedAstSummary::from_ast` で summary-readiness を確認し、同じ reserve binding から binder-only `CoreContext` input を準備し、未対応 checker payload family を stable external dependency gap として surface する |
...
| `mizar-checker` task 12 plus task 16-20 source bridge continuation, reserve summary-readiness, and binder-only core context follow-up | `type_elaboration` / `type-elaboration` | prepared/implemented。active sidecars は `active_type_elaboration` を使い、lower stages を先に実行し、reserve-only の builtin `set` / `object` declaration を checker-owned `BindingEnv`、binding ごとの `DeclarationInput`、binding 固有の `TypeExpressionInput` site、`TypedAst`、checker-owned `ResolvedTypedAst`、`mizar-core` `ResolvedTypedAstSummary::from_ast` read、binder-only `CoreContext` preparation へ抽出する。未対応 checker payload family は `type_elaboration.external_dependency.ast_payload_extraction` に残す | より広い type pass/fail semantic assertions は AST-wide source-to-checker payload extraction を待つ。 |
...
lower stages が pass した後、runner は reserve-only source のうち、unrecovered な
...
含まないものだけを checker-owned reserve declaration payload として抽出する。
runner は reserve binding を含む checker-owned module `BindingEnv`、binding ごとの
`DeclarationInput`、binding 固有の `TypeExpressionInput` site を構築するため、
`reserve x, y for set` は source range を共有しつつ binding ごとに distinct typed site を
持つ。その payload は collected `SymbolEnv` に対して `mizar-checker` の
`TypeNormalizer` と `DeclarationChecker` で check され、declaration と type-entry link を
持つ checker-owned `TypedAst` に組み立てられ、empty-but-real な cluster/overload
predecessor output と source-preserved node hint / declaration expression metadata により
checker-owned `ResolvedTypedAst` へ投影される。runner はその real `ResolvedTypedAst`
...
regression evidence が binding-env construction、declaration checking、`TypedAst`
