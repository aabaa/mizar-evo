...
chain まで、この pass slice を拡張する。task 50 は active
...
task 56 の one-edge chain slice のどちらも適用されない場合、checker type normalization は
...
local-mode source では task-55/task-56 expansion を渡さない。task 56 は attributed dependency を持つ
...
として credit しない。diagnostic-free な対応済み output（bare builtin と task-55/56
