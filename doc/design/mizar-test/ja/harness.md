...
Task 80 addendum for `type-elaboration`: task 84 / task 85 より前は、documented
...
`TypeCaseAttr` provenance / `AttributeInput` bridge と task-85 negative
`empty`/builtin-`set` bridge 外の imported attribute は引き続き
...
Task 85 addendum for `type-elaboration`: documented `parser.type_fixtures`
imported attribute `empty` は、既存 `non empty set` fixture についてのみ builtin
`set` 上の checker-owned imported negative `AttributeInput` として渡してよい。
runner は
`type_elaboration.checker.checker.declaration.deferred.evidence_query` を期待し、
summary を imported module AST extraction と扱ったり、attributed-type
...
Task 86 addendum for `type-elaboration`: formula-only theorem source は active
checker boundary case として parser / resolver まで実行してよいが、real
theorem/formula payload extraction、local proof context、recorded fact、theorem
acceptance、`formula_statement` runner support が存在するまでは
`type_elaboration.external_dependency.ast_payload_extraction` に留めなければ
ならない。runner は formula payload、proof skeleton、CoreIr、ControlFlowIr、VC、
proof payload を合成してはならない。

Task 87 addendum for `type-elaboration`: term-bearing theorem formula source は
active checker boundary case として parser / resolver まで実行してよいが、real
term/formula payload extraction、term inference、formula checking、recorded fact、
theorem acceptance、`formula_statement` runner support が存在するまでは
`type_elaboration.external_dependency.ast_payload_extraction` に留めなければ
ならない。runner は term payload、formula payload、proof skeleton、CoreIr、
ControlFlowIr、VC、proof payload を合成してはならない。

Task 98 addendum for `type-elaboration`: `parser.type_fixtures` 由来の imported
predicate/functor surface を使う theorem formula source は active checker boundary
case として parser / resolver まで実行してよいが、imported predicate/functor semantic
...
runner は imported semantic payload、term payload、formula payload、proof skeleton、
CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。

Task 100 addendum for `type-elaboration`: Chapter 14 の builtin membership
predicate と Chapter 13 の numeral operand を使う theorem formula は active
checker boundary case として parser / resolver まで実行してよいが、real
...
support が存在するまでは
`type_elaboration.external_dependency.ast_payload_extraction` に留めなければならない。
runner は term payload、formula payload、membership fact、theorem acceptance、
CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。

Task 101 addendum for `type-elaboration`: Chapter 14 の builtin inequality
predicate と Chapter 13 の numeral operand を使う theorem formula は active
checker boundary case として parser / resolver まで実行してよいが、real
term/formula payload extraction、inequality desugaring または equality semantic
checking、formula checking、recorded fact、theorem acceptance、
`formula_statement` runner support が存在するまでは
`type_elaboration.external_dependency.ast_payload_extraction` に留めなければならない。
runner は term payload、formula payload、inequality fact、theorem acceptance、
CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。

Task 102 addendum for `type-elaboration`: Chapter 14 の builtin type-assertion
form と Chapter 13 の numeral term を使う theorem formula は active checker
boundary case として parser / resolver まで実行してよいが、real term/formula
payload extraction、type-assertion type payload extraction、term inference、
type-assertion semantic checking、formula checking、recorded fact、theorem
acceptance、`formula_statement` runner support が存在するまでは
`type_elaboration.external_dependency.ast_payload_extraction` に留めなければならない。
runner は term payload、formula payload、type-assertion fact、theorem acceptance、
CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。

Task 103 addendum for `type-elaboration`: `parser.type_fixtures` を import し、
documented `empty` attribute を Chapter 14 の attribute-assertion form と Chapter
13 の numeral subject で使う theorem formula は active checker boundary case
として parser / resolver まで実行してよいが、real term/formula payload
...
`formula_statement` runner support が存在するまでは
`type_elaboration.external_dependency.ast_payload_extraction` に留めなければならない。
runner は term payload、formula payload、imported attribute assertion payload、
imported module AST extraction、theorem acceptance、CoreIr、ControlFlowIr、VC、
proof payload を合成してはならない。

Task 104 addendum for `type-elaboration`: `parser.type_fixtures` を import し、
documented `empty` attribute を Chapter 14 の attribute-assertion form と Chapter
13 の numeral subject で attribute-level `non empty` assertion として使う theorem
formula は active checker boundary case として parser / resolver まで実行してよいが、
real term/formula payload extraction、imported attribute-level non-empty assertion
...
`AttributeInput` payload extraction、`formula_statement` runner support が存在する
までは `type_elaboration.external_dependency.ast_payload_extraction` に留めなければ
ならない。runner は term payload、formula payload、imported attribute-level
non-empty assertion payload、imported module AST extraction、theorem acceptance、
CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。

Task 105 addendum for `type-elaboration`: Chapter 13 の set-enumeration term
operand と Chapter 14 の builtin equality を使う theorem formula は active
checker boundary case として parser / resolver まで実行してよいが、real
set-enumeration term payload extraction、term/formula payload extraction、term
inference、equality/formula checking、recorded fact、theorem acceptance、
`formula_statement` runner support が存在するまでは
`type_elaboration.external_dependency.ast_payload_extraction` に留めなければならない。
runner は set-enumeration payload、term payload、formula payload、theorem
acceptance、CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。

Task 99 addendum for `type-elaboration`: Chapter 14 の implication、universal
quantification、negation を使う theorem formula は active checker boundary case
として parser / resolver まで実行してよいが、real formula payload extraction、
quantifier binder/context payload、formula checking、recorded fact、theorem
acceptance、`formula_statement` runner support が存在するまでは
`type_elaboration.external_dependency.ast_payload_extraction` に留めなければならない。
runner は formula payload、binder/context payload、fact、theorem acceptance、
CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。
...
task 85 は上の `type_elaboration` consumer row を refine する。imported attribute
gap list から除外されるのは task 84 の `TypeCaseAttr` fixture に加えて、documented
negative `empty` over builtin `set` fixture だけである。runner は positive
`empty set` と builtin `object` 上の imported `empty` を active external-gap
sidecar として保持し、generic imported attribute、imported module AST
extraction、attribute argument、owner provenance、evidence payload、CoreIr、
ControlFlowIr、VC、proof row を supported slice の外に残す。

task 86 も同じ row を refine する。formula-only theorem source は active
`type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、theorem/formula
payload、fact、proof skeleton、CoreIr、ControlFlowIr、VC、proof payload は credit
しない。

task 87 も同じ row を refine する。term-bearing theorem formula は active
`type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、term/formula
payload、term inference、formula checking、fact、proof skeleton、CoreIr、
ControlFlowIr、VC、proof payload は credit しない。上の row の task-86 theorem
formula boundary entry は task 86 の formula-only case と task 87 の term-bearing
equality case の両方を含む。

task 98 も同じ row を refine する。imported predicate/functor theorem formula は
active `type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、imported semantic
payload、term/formula payload、term inference、formula checking、fact、proof
skeleton、CoreIr、ControlFlowIr、VC、proof payload は credit しない。上の theorem
formula boundary entry は task 98 の imported predicate/functor case も含む。

task 100 も同じ row を refine する。builtin membership theorem formula は
active `type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、term/formula
payload、membership operand type inference/checking、formula checking、fact、
theorem acceptance、CoreIr、ControlFlowIr、VC、proof payload は credit しない。
上の theorem formula boundary entry は task 100 の builtin membership case も含む。

task 101 も同じ row を refine する。builtin inequality theorem formula は
active `type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、term/formula
payload、inequality desugaring または equality semantic checking、formula checking、
fact、theorem acceptance、CoreIr、ControlFlowIr、VC、proof payload は credit しない。
上の theorem formula boundary entry は task 101 の builtin inequality case も含む。

task 102 も同じ row を refine する。builtin type-assertion theorem formula は
active `type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、term/formula
payload、type-assertion type payload extraction、term inference、type-assertion
semantic checking、formula checking、fact、theorem acceptance、CoreIr、ControlFlowIr、
VC、proof payload は credit しない。上の theorem formula boundary entry は task 102 の
builtin type-assertion case も含む。

task 103 も同じ row を refine する。`parser.type_fixtures` の `empty` を使う
imported attribute assertion theorem formula は active `type_elaboration` runner を
通じて extraction-gap boundary としてだけ実行可能である。deferred
`formula_statement` runner obligation を満たさず、term/formula payload、imported
attribute assertion attribute-chain/provenance payload extraction、term inference、
attribute admissibility/semantic checking、formula checking、fact、theorem
acceptance、imported module AST extraction、theorem formula 向け checker
`AttributeInput` payload extraction、CoreIr、ControlFlowIr、VC、proof payload は
credit しない。上の theorem formula boundary entry は task 103 の imported
attribute assertion case も含む。

task 104 も同じ row を refine する。`parser.type_fixtures` の `empty` を使う
attribute-level `non empty` imported attribute assertion theorem formula は active
`type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、term/formula
payload、imported attribute-level non-empty assertion attribute-chain/provenance
payload extraction、term inference、negated attribute admissibility/semantic
checking、formula checking、fact、theorem acceptance、imported module AST
extraction、theorem formula 向け checker `AttributeInput` payload extraction、
CoreIr、ControlFlowIr、VC、proof payload は credit しない。上の theorem formula
boundary entry は task 104 の attribute-level non-empty imported attribute
assertion case も含む。

task 105 も同じ row を refine する。Chapter 13 の set-enumeration term operand
と Chapter 14 の builtin equality を使う theorem formula は active
`type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、
set-enumeration term payload extraction、term/formula payload、term inference、
equality/formula checking、fact、theorem acceptance、CoreIr、ControlFlowIr、VC、
proof payload は credit しない。上の theorem formula boundary entry は task
105 の set-enumeration case も含む。

task 99 も同じ row を refine する。connective/quantifier theorem formula は
active `type_elaboration` runner を通じて extraction-gap boundary としてだけ実行可能で
ある。deferred `formula_statement` runner obligation を満たさず、formula
payload、quantifier binder/context payload、formula checking、fact、theorem
acceptance、CoreIr、ControlFlowIr、VC、proof payload は credit しない。上の theorem
formula boundary entry は task 99 の connective/quantifier case も含む。
...
`empty`/builtin-`set` bridge 外の broader imported attribute / mode / structure、
...
`empty`/builtin-`set` bridge、task-80 boundary を超える imported attribute、
...
provenance、bracket `type_arg_list` / `qua`-argument provenance、structure base-shape evidence、term、formula、coercion site、overload
evidence、recorded fact、CoreIr、ControlFlowIr、VC payload、proof evidence は
