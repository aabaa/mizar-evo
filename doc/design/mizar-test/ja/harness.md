...
| type-elaboration | active な `.miz` type-elaboration case を frontend parsing と resolver declaration/symbol collection で run し、対応済み reserve-only builtin declaration bridge を checker-owned `BindingEnv`、`DeclarationInput`、`TypedAst`、`ResolvedTypedAst` まで実行し、未対応 checker payload family を stable external dependency gap として surface する |
...
| `mizar-checker` task 12 plus task 16/17 source bridge continuation | `type_elaboration` / `type-elaboration` | prepared/implemented。active sidecars は `active_type_elaboration` を使い、lower stages を先に実行し、reserve-only の builtin `set` / `object` declaration を checker-owned `BindingEnv`、binding ごとの `DeclarationInput`、binding 固有の `TypeExpressionInput` site、`TypedAst`、checker-owned `ResolvedTypedAst` へ抽出する。未対応 checker payload family は `type_elaboration.external_dependency.ast_payload_extraction` に残す | より広い type pass/fail semantic assertions は AST-wide source-to-checker payload extraction を待つ。 |
...
checker-owned `ResolvedTypedAst` へ投影される。active pass case は、source が少なくとも
1 個の対応済み reserve binding を持ち、runner regression evidence が binding-env construction、
declaration checking、`TypedAst` assembly、`ResolvedTypedAst` assembly の実行を確認する場合だけ、
...
coercion site、overload evidence、recorded fact、Core / VC payload、proof evidence は
...
この runner は `CoreIr`、`ControlFlowIr`、VC seed、public checker diagnostic code を
publish しない。
