...
current production runner layoutは正確に17 path、19,803行である。これはbehaviorを
保存したTasks 249-263 split後のcanonical crate-relative source-path tableである。

| Production path | Lines | Ownership |
|---|---:|---|
| `src/runner.rs` | 2,266 | snapshot failureを含むpublic report/status、corpus orchestration、public active iterator、parse/declaration admission、type-case execution、verify-only baseline comparison、top-level detail dispatch。 |
...
| `src/runner/type_elaboration.rs` | 578 | 正確に11個のprivate leafを持つprivate type-elaboration facade。 |
...
| `src/runner/type_elaboration/checker_handoff.rs` | 1,259 | checker-owned binding/declarationとexact Task-180 statement/proof/terminal handoff assembly/validation、test-only real-bundle near-miss construction。 |
| `src/runner/type_elaboration/long_chain_config.rs` | 82 | shared exact long-chain definition table。 |
| `src/runner/type_elaboration/output.rs` | 1,566 | checker output、validation、result/detail projection、diagnostic、exact Task-180 CoreIr construction/deterministic rerun。 |
...
11 `mod` declaration、17-path/hash pair、documented public API、exact discovered-
