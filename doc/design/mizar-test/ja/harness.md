...
current production runner layoutは正確に19 path、20,651行である。Checker Task 248は
bounded source-context leafを1件追加し、behavior-preserving Tasks 249-263 splitと
`runner.rs` facade/top-level orchestration-only boundaryを維持する。

| Production path | Lines | Ownership |
|---|---:|---|
| `src/runner.rs` | 2,382 | snapshot failureを含むpublic report/status、corpus orchestration、public active iterator、proof-verification orchestration、parse/declaration admission、type-case execution、verify-only baseline comparison、top-level detail dispatch。 |
...
| `src/runner/type_elaboration.rs` | 585 | 正確に12個のprivate leafを持つprivate type-elaboration facade。 |
...
| `src/runner/type_elaboration/checker_handoff.rs` | 1,295 | checker-owned binding/declaration、exact Task-180 statement/proof/terminal handoff assembly/validation、Task-248 empty-later-payload assembly、test-only real-bundle near-miss construction。 |
...
| `src/runner/type_elaboration/source_context.rs` | 509 | exact Task-248 resolver-shell/source-context projection、route isolation、checker producer invocation、immutable handoff assembly。 |
...
12 `mod` declaration、19-path/hash pair、documented public API、exact discovered-
...
future `type_elaboration` incrementはprivate `source_type` leaf 1件がownする。
...
existing expectation/trace rowはrebaselineしない。このfrozen boundaryはChecker
Task 249 implementationまでdocumentation-onlyである。
