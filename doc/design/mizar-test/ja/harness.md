...
Task 75/76 addendum for `type-elaboration`: later declaration を名前参照する
forward same-module local-mode / local-structure reserve head は active
lower-stage boundary case である。runner は checker handoff 前の
...
base-shape、constructor-witness payload を合成してはならない。
