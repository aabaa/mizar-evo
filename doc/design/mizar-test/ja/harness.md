...
current production runner layoutは正確に17 path、19,262行である。これはbehaviorを
保存したTasks 249-263 split後のcanonical crate-relative source-path tableである。

| Production path | Lines | Ownership |
|---|---:|---|
| `src/runner.rs` | 2,188 | public report/status、corpus orchestration、public active iterator、parse/declaration admission、type-case execution、top-level detail dispatch。 |
...
| `src/runner/type_elaboration.rs` | 577 | 正確に11個のprivate leafを持つprivate type-elaboration facade。 |
...
| `src/runner/type_elaboration/checker_handoff.rs` | 856 | checker-owned binding/declarationとexact Task-180 final handoff assembly/validation。 |
| `src/runner/type_elaboration/long_chain_config.rs` | 82 | shared exact long-chain definition table。 |
| `src/runner/type_elaboration/output.rs` | 1,538 | checker output、validation、result/detail projection、diagnostic。 |
| `src/runner/type_elaboration/parenthesized_routes.rs` | 745 | parenthesized reserved-variable route ownership。 |
| `src/runner/type_elaboration/result.rs` | 29 | expected-keyとstable failure projection。 |
| `src/runner/type_elaboration/source_ast.rs` | 147 | common exact AST/import projection。 |
| `src/runner/type_elaboration/source_formula.rs` | 2,629 | common formula/source payload extractionとexact theorem/formula site/range。 |
...
11 `mod` declaration、17-path/hash pair、変更しないpublic API、exact discovered-
