...
Task 75 addendum for `type-elaboration`: later declaration を名前参照する
forward same-module local-mode reserve head は active lower-stage boundary
case である。runner は checker handoff 前の
`type_elaboration.lower_stage.frontend:malformed_type_expression` を期待し、
future declaration から checker `ModeExpansion` を合成してはならない。
