...
Task 78 addendum for `type-elaboration`: documented `parser.type_fixtures`
import summary 由来の imported structure reserve head は active source-to-checker
extraction-gap boundary case である。runner は
`type_elaboration.external_dependency.ast_payload_extraction` を期待し、この
summary を real imported module AST extraction と扱ったり、imported structure
provenance、structure type-head payload、base-shape / constructor-witness
evidence、positive structure elaboration、CoreIr、ControlFlowIr、VC、proof
payload を合成してはならない。

Task 79 addendum for `type-elaboration`: documented `parser.type_fixtures`
import summary 由来の imported mode reserve head は active source-to-checker
extraction-gap boundary case である。runner は
`type_elaboration.external_dependency.ast_payload_extraction` を期待し、この
summary を real imported module AST extraction と扱ったり、imported mode
provenance、mode type-head payload、`ModeExpansion` payload、positive mode
elaboration、CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。

Task 80 addendum for `type-elaboration`: documented `parser.type_fixtures`
import summary 由来の imported attribute reserve type は active
source-to-checker extraction-gap boundary case である。runner は
`type_elaboration.external_dependency.ast_payload_extraction` を期待し、この
summary を real imported module AST extraction と扱ったり、imported attribute
provenance、`AttributeInput` payload、attributed-type evidence、positive
attributed type elaboration、CoreIr、ControlFlowIr、VC、proof payload を合成してはならない。
...
non-builtin declaration、task-80 boundary を超える imported attribute、task-78/task-79 boundary を超える imported mode / structure、attribute argument、
