...
Step 5C.11 の parse 経路は、対応する制限付き形容詞の拒否ケースのみを受け入れる。
登録ヘッダー内のパーサー診断と位置を照合し、診断の欠落、修復済み構文、無関係な
エラー、不正なメタデータを拒否する。登録の正当性、受理状態、閉包、簡約は対象外。
...
ことを確認する。さらに同じcomplete checker `BindingEnv`をstandalone Core-33
local-binder producerへ渡す。Coreは`BindingId`をreinterpretせずbindingごとにfresh
variableを1個allocateし、runnerはexplicit association、source/module identity、binder
source range、checker provenance、empty item registry、empty core diagnostics、
empty core worklistを確認する。これは summary/context readiness の確認だけであり、
...
Private Profile-A consumerはretained `BindingEnv`をstandalone Core-33 local-binder
producerへ2回渡し、exact 2-row reserve/definition-parameter association、checker binding
orderのfresh Core identity、source/context provenance、deterministic equality、empty semantic
Core stateもverifyする。Route/coverage creditは追加しない。

このrouteはtype result、expression、fact、obligation、formula、statement、proof、`CoreIr`、
...
Capture/resolver-semantic/Task277B successorはselectしない。
...
Freeze済みの [Core Task 33I261 contract](../../task_contracts/ja/CORE-SOURCE-ATTRIBUTE-ITEM-CONTEXT-33I261.md) は existing private Task261 real-source test leafをsole probeとして指定する。Authenticated definitionからCore `Attribute` item seed 1件をderiveし、Core contextをprepareし、retained complete `BindingEnv`へ33LBをapplyしてstandalone producerをinvokeできる。Two probesはretained input、exact one-row association、complete item/source-map/boundary/worklist state、deterministic replay、default-deny mutation matrixをassertする。Implementation/required verification/exact task-only commit `4c6ecafc2a9bee7a4eb6e3f27336733fc672bd57` はcomplete。Production runner branch、installer、`.miz`、expectation、trace、active metadata、diagnostic、snapshot、coverage creditは変更しない。Core34--36、`GeneratedOrigin`、`MT10-CIR-TE`、Task277Bはdeferred/not-readyのままとする。

### Core Task 33I262 private item-context probe

Freeze済みの [Core Task 33I262 contract](../../task_contracts/ja/CORE-SOURCE-MODE-ITEM-CONTEXT-33I262.md) は existing private Task262 real-source test leafをsole probeとして指定する。Authenticated definitionからCore `Mode` item seed 1件をderiveし、Core contextをprepareし、retained complete `BindingEnv`へ33LBをapplyしてstandalone producerをinvokeできる。Exactly two testsはretained input、exact one-row association、complete item/source-map/boundary/worklist state、deterministic replay、ten Core mutation、four foreign-environment combinationを検証する。Implementation/required verification complete、documentation repair後final review no findings。Production runner branch、installer、`.miz`、expectation、trace、active metadata、diagnostic、snapshot、coverage creditは変更しない。Core34--36、`GeneratedOrigin`、`MT10-CIR-TE`、Task277Bはdeferred/not-readyのままとする。
...
durable derived private-runner ABIであり、language-specification authority
ではない。

dormant runnerはprivate SDC outputをby valueでconsumeし、unchanged
Task-269SDP lowerをauthenticated
`given_type_range() == 95..98`、`given_type_head_range() == 95..98`、
`given_type_spelling() == "set"` のためだけに呼ぶ。reserve binding 0が
`14..17` を供給し、syntax由来semantic rowを構築しない。
...
pub(in crate::runner) fn source_proof_local_given_descendant_type_output(
    ast: &SurfaceAst, module: ModuleId, shells: &DeclarationShellSet,
    symbols: &SymbolEnv, source_text: &str,
...
各dead-code allowanceにはinline Task-269SDT dormant-route rationaleを付ける。
production wrapperは常に `None` mutationを選択しdispatchしない。source
mismatchは `None`、selected validation failureは
`Some(Err(display_string))`、exact successは `Some(Ok(output))`。
outputはimmutable Typed/Resolved viewだけを公開する。exact runner tests:
...
`source_proof_local_given_descendant_use_term_output` routeを実装する。cfg-test
...
`task257c4c3_real_imported_fixture_builds_checker_identity_handoff` 1件だけを追加した。C4C1
frontend/import helperとexisting test-only typed profileをreuseし、public C4C3 producerをdirect callして
immutable resolver-use→typed-binder row 1件、source/module/summary、dense absence、dependency non-mutationだけを
assertする。Deterministic replayはchecker-owner C4C3 testがcoverする。
...
label・本体・生の locus 名を判定根拠にしない。追加 resolver error は拒否し、残る G1/G2/G9 の5行は
...
dependent-return の exact 1行だけを type_elaboration/type_check に追加し、pass・functors.dependent_return、
唯一の spec.en.10.functors.dependent_return.semantic ref と active_type_elaboration、厳密な両 id/source/sidecar path、
空の diagnostic/failure/payload/snapshot を照合する。いずれかの id または filename で他 stage を予約し、inventory の欠落・重複を拒否する。
runner は[直接 checker](../../mizar-checker/ja/type_checker.md#source-dependent-functor-return-typing) を呼び、
実 declaration binding、引数・本体の Known 型、検査済等式、source 所有の両 Pending clause を照合する。
...
既存 module 4行と synonym 負例を維持する。厳密な functor-synonym / predicate-antonym 正例を完全な metadata/別 stage 認証の下で type_check に通し、failure key/category はなく、それぞれ notation.synonym/notation.antonym domain と唯一の canonical ref を照合する。
harness は実 resolved source・symbols・中立 typed node と明示的 Synonym/Antonym profile を既存 checker に渡す。認証済み本体/呼出し型検査と両 original-root application の適合を要求し、自分で locus mapping/逆 fact を作らず、resolver mismatch 不在を成功にしない。不正 relation・無関係な診断・下位段階失敗に実績を与えない。
...
narrower-loci の正確な source/sidecar identity、変更しない pass/type_elaboration/type_check oracle、唯一の semantic ref と active tag を予約する。一つの immutable source orchestration 内で、記述された登録を既存 checker/Core/VC/kernel/policy 経路で新たに証明し、その局所出力で [declaration checker](../../mizar-checker/ja/type_checker.md#narrower-loci-predicate-declaration-checking) を呼ぶ。共有の記述 type gate と両 formal binding、実際に狭い root 対応、型検査済みの逆順等式本体、pending coherence request 一つを要求する。source・registration/choice/citation・policy・binding・正規化 signature・root 順序/曖昧性・coherence owner の変異は拒否、または正しく変更された pending goal の保持を確認する。caller acceptance・偽造 database・証明成功・新規 VC snapshot は認めない。
