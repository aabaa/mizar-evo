...
Task 85 / task 116 addendum for `type-elaboration`: documented
`parser.type_fixtures` imported attribute `empty` は、既存 `non empty set` と
`empty set` fixture についてのみ builtin `set` 上の checker-owned imported
negative または positive `AttributeInput` として渡してよい。
runner は
`type_elaboration.checker.checker.declaration.deferred.evidence_query` を期待し、
summary を imported module AST extraction と扱ったり、attributed-type
existential/evidence payload、non-`set` head 上の imported `empty`、positive imported
attributed type elaboration、structure-qualified
...
task 85 と task 116 は上の `type_elaboration` consumer row を refine する。
imported attribute gap list から除外されるのは task 84 の `TypeCaseAttr` fixture
に加えて、documented negative / positive `empty` over builtin `set` fixtures である。
runner は builtin `object` 上の imported `empty` を active external-gap sidecar
として保持し、generic imported attribute、imported module AST extraction、
attribute argument、owner provenance、evidence payload、CoreIr、ControlFlowIr、VC、
proof row を supported slice の外に残す。
