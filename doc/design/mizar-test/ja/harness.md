...
| type-elaboration | active な `.miz` type-elaboration case を frontend parsing と resolver declaration/symbol collection で run し、対応済み source-derived builtin type-expression bridge を checker-owned `ResolvedTypedAst` まで実行し、未対応 checker payload family を stable external dependency gap として surface する |
...
| `mizar-checker` task 12 plus task 16 source bridge and Campaign II task 17 | `type_elaboration` / `type-elaboration` | prepared/implemented。active sidecars は `active_type_elaboration` を使い、lower stages を先に実行し、reserve-only の builtin `set` / `object` type expression を `mizar-checker` で normalize し、`TypedAst` に組み立て、checker-owned `ResolvedTypedAst` へ投影し、未対応 checker payload family は `type_elaboration.external_dependency.ast_payload_extraction` に残す | より広い type pass/fail semantic assertions は AST-wide source-to-checker payload extraction を待つ。 |
...
lower stages が pass した後、runner は reserve-only source 内の unrecovered な source
`TypeExpression` node のうち、head が builtin `set` または `object` で、attributes、
arguments、parameter prefix、non-builtin symbol head を持たないものだけを
checker-owned `TypeExpressionInput` payload として抽出する。その payload は collected
`SymbolEnv` に対して `mizar-checker` の `TypeNormalizer` で normalize され、type-entry
links を持つ最小の checker-owned `TypedAst` shell に組み立てられ、empty-but-real な
cluster/overload predecessor output と source-preserved node hint / expression metadata により
checker-owned `ResolvedTypedAst` へ投影される。active pass case は、source が少なくとも
1 個の対応済み builtin `TypeExpression` site を持ち、runner regression evidence が
normalization、`TypedAst` assembly、`ResolvedTypedAst` assembly の実行を確認する場合だけ、
この対応済み source-derived slice を empty detail key で assert してよい。

runner は不足している AST-wide source-to-checker bridge を引き続き捏造しない。
parsed/resolved `.miz` の declaration、term、formula、coercion site、
attribute / mode / structure payload、overload evidence、recorded fact、proof
evidence は対応済み extraction slice の外に残る。active case がこれら未対応
payload family のいずれかを必要とする場合、runner は stable detail key
...
deferred のままにする。この runner は `CoreIr`、`ControlFlowIr`、VC seed、public
checker diagnostic code を publish しない。
