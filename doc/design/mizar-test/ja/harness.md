...
`R` 部分を supersede する。task-83 `R` provenance/type-head bridge 外の broader
imported structure は、non-`R` の source-derived fixture が存在するまで deferred
のままとし、将来の case は
...
Task 83 addendum for `type-elaboration`: documented `parser.type_fixtures`
imported structure `R` は checker-owned imported structure type head として渡して
よい。runner は
...
task-83 `R` bridge と task-78 boundary を超える imported structure、task 82 の provenance/type-head
