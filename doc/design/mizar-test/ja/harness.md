...
Task 86 / task 115 addendum for `type-elaboration`: formula-only theorem
...
`thesis` formula constant site/range を checker recovery `FormulaInput` として渡し、
`type_elaboration.checker.checker.formula.external.formula_payload` と
`type_elaboration.checker.checker.formula.unsupported_payload` を期待する。
...
`type_elaboration` runner を通じて実行可能であることを示す。task 115 は exact
`FormulaPayloadBoundary: thesis` source だけを supersede し、source-derived
`thesis` formula constant site/range を checker recovery `FormulaInput` として渡してから、
missing formula payload と unsupported formula semantics で fail closed する。
