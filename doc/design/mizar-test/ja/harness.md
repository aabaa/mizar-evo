...
Task 80 addendum for `type-elaboration`: task 84 より前は、documented
...
`TypeCaseAttr` provenance / `AttributeInput` bridge 外の imported attribute は
引き続き `type_elaboration.external_dependency.ast_payload_extraction` を期待する。
...
non-builtin declaration、task-84 `TypeCaseAttr` bridge と task-80 boundary を超える imported attribute、
