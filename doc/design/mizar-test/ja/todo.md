...
10. **消費側ランナーの支援。** [ ] — 消費側 crate が律速。
    ...
    - 現在の task-10 ledger は、`mizar-parser` task 3（`parse-only`）、
      ...
      task 60 の same-module attributed local-mode structure-RHS evidence-query active fail slice、checker task 61 の same-module attributed local-mode attributed-builtin-RHS evidence-query active fail slice、checker task 62 の same-module local-mode structure-RHS chain evidence-query active fail slice を
