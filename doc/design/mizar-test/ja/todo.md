...
| harness | [harness.md](./harness.md) | `src/harness.rs`、`src/main.rs`、`src/runner.rs` | [~] metadata plan、validation-mode CLI、profile filtering、active parse/declaration/type runner |
...
| minimal_crate | [minimal_crate.md](./minimal_crate.md) | crate 境界＋CLI | [~] metadata plan、validation mode、CLI fixture は実装済み。coverage gate は未完 |
...
- `miz_corpus`: generated/fuzz/stress metadata、corpus-policy profile
  constraints、pass/fail ratio reporting、stress exclusion checks を
  enforceable にする。
