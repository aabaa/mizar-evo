...
10. **消費側ランナーの支援。** [ ] — 消費側 crate が律速。
    ...
    - 現在の task-10 ledger は、`mizar-parser` task 3（`parse-only`）、
      ...
      checker task 93 の proof-local declaration extraction-gap boundary も
...
18. **Source-derived reserve declaration semantic bridge。** [x]
    ...
    - 未対応の non-builtin declaration（task 93 の proof-local declaration boundary と task 92 の mode/structure definition boundary を超えるもの）、task-84 `TypeCaseAttr` bridge、task-85
      ...
      structure base-shape payload、task-92 extraction-gap boundary を超える definition payload、task-93 extraction-gap boundary を超える proof-local declaration payload、task-86/task-87/task-88/task-89/task-93 extraction-gap boundary を超える
