...
| モジュール | 仕様 | ソース | 状態 |
|---|---|---|---|
| layout | [layout.md](./layout.md) | `src/layout.rs`、`src/path_rules.rs` | [~] discovery/pairing 実装済み。Public API 同期と unknown-root policy は未完 |
| expectation_schema | [expectation_schema.md](./expectation_schema.md) | `src/expectation.rs` | [~] core schema 実装済み。provenance/profile/general snapshot 強化は未完 |
| staged_model | [staged_model.md](./staged_model.md) | `src/staged_model.rs` | [~] stage id 実装済み。prerequisite validation は未完 |
| traceability | [traceability.md](./traceability.md) | `src/traceability.rs` | [~] syntax/backref 実装済み。coverage mode/status/prerequisite は未完 |
| harness | [harness.md](./harness.md) | `src/harness.rs`、`src/main.rs`、`src/runner.rs` | [~] metadata plan + active parse/declaration/type runner |
| miz_corpus | [miz_corpus.md](./miz_corpus.md) | `tests/` 配下のコーパスツリー | [~] root discovery 実装済み。provenance/profile/reporting rules は未完 |
| snapshot | [snapshot.md](./snapshot.md) | `src/expectation.rs`、`src/runner.rs`、将来の `src/snapshot.rs` | [~] transitional parse-only `SurfaceAst`。general API/hash/update/determinism は未完 |
| fail_soundness | [fail_soundness.md](./fail_soundness.md) | 将来の `src/fail_soundness.rs`、ハーネス規則＋コーパスケース | [ ] |
| minimal_crate | [minimal_crate.md](./minimal_crate.md) | crate 境界＋CLI | [~] metadata plan 実装済み。validation-mode/CLI fixture は未完 |
...
`897d549` で修復済みである。残る task 2 の traceability gap は、
manifest-order validator と regression test が未実装であることによる
`source_drift` / `test_gap` である。

監査からの follow-up ownership:

- `layout`: documented Public API を `DiscoveredLayout` と harness-owned
  `TestCase` に同期し、strict/permissive unknown-root behavior を定義する。
- `expectation_schema`: metadata の retention/profile filtering、generated
  origin table、certificate/kernel `rejection_reason`、diagnostic ordering、
  将来の general `[[snapshots]]` hash registry を検証する。
- `traceability`: manifest order validation、mode-aware coverage/status
  computation、stage/`depends_on`/obsolete validation、既存 link-validator
  error の regression tests を追加する。
- `harness`: generic な documented outcome/reporting API と exported
  runner-specific report を整合させる。`parser.type_fixtures`
  import-summary exception を文書化するか削除する。
- `miz_corpus`: generated/fuzz/stress metadata、profile filtering、pass/fail
  ratio reporting、stress exclusion checks を enforceable にする。
...
9. **コーパスサイズとレビュー規則の検証。** [ ]
   ...
   - task 2 gap として、generated/fuzz/property origin metadata、
     ...
     retention、profile filtering、stress exclusion、fuzz-category
     preservation を閉じる。
