...
symbol、attribute / mode / structure argument、attributed mode / structure head、term、formula、
...
base-shape evidence diagnostic、または same-module local mode reserve head の missing
mode-expansion payload diagnostic の checker detail key を代わりに assert してよい。
