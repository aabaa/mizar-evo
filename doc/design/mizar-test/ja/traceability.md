...
`mizar-core` の `ResolvedTypedAstSummary::from_ast` と binder-only `CoreContext`
preparation で readiness として読む。
...
最小 `TypedAst`、`ResolvedTypedAst`、summary-readiness、binder-only core context path の実行を確認する場合だけ、
