...
task 92 の mode/structure definition external-gap case、および task 93 の
proof-local declaration external-gap case を除き、task 85 の
...
case が task 93 の proof-local declaration boundary と task 92 の
mode/structure definition boundary を超える未対応の non-builtin declaration、
imported symbol、unresolved / ambiguous symbol、
attribute / mode / structure argument、structure-qualified attribute provenance、
type-argument / term-argument provenance、proof-local declaration payload、term、
formula、coercion、overload payload、fact、CoreIr、ControlFlowIr、VC payload、proof payload
extraction を必要とする場合、
