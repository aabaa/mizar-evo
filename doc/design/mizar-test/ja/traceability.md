...
task 228 は shared Chapter 4/13/14/16 と Task 74 structural-producer reference 5 件、dedicated checker reference 1 件を持つ active seven-expansion set-terminal four-hop asserted-head row 1 件を追加する。credit は real set expansion 7 個、distinct `ChainMode6`/`ChainMode2` provenance、直接検証する bare link 4 本、terminal-only tail normalization、ordinal 1 / `BindingId(0)`、BaseModeDef-RHS `BuiltinSet` 1 個、inferred variable 1 個、constraint/fact/candidate/diagnostic/deferred 0 個の checked assertion 1 個に限定する。finite matrix、先行 owner 52 件、immutable output、focused sibling、real sidecar は active count 176 を保護する。active corpus は既存 expectation を rebaseline せず 391 cases / 355 requirements、type-elaboration 223/211、pass/fail 207/184 を持ち、focused、relevant-crate、workspace verification は成功した。broader semantics は credit しない。

## task 229 active traceability

task 229 は shared Chapter 4/13/14/16 と Task 74 structural-producer reference 5 件、dedicated checker reference 1 件を持つ active seven-expansion object-terminal four-hop asserted-head row 1 件を追加する。credit は real object expansion 7 個、distinct `ChainObjectMode6`/`ChainObjectMode2` provenance、直接検証する bare link 4 本、terminal-only tail normalization、ordinal 1 / `BindingId(0)`、BaseObjectModeDef-RHS `BuiltinObject` 1 個、inferred variable 1 個、object/set coercion なしの constraint/fact/candidate/diagnostic/deferred 0 個の checked assertion 1 個に限定する。finite matrix、先行 owner 53 件、immutable output、focused sibling、real sidecar は active count 177 を保護する。active corpus は既存 expectation を rebaseline せず 392 cases / 356 requirements、type-elaboration 224/212、pass/fail 208/184 を持ち、focused、relevant-crate、workspace verification は成功した。broader semantics は credit しない。

## task 230 active traceability

task 230 は shared Chapter 4/13/14/16 と Task 74 structural-producer reference 5 件、dedicated checker reference 1 件を持つ active seven-expansion set-terminal five-hop asserted-head row 1 件を追加する。credit は real set expansion 7 個、distinct `ChainMode6`/`ChainMode1` provenance、直接検証する bare link 5 本、terminal-only `ChainMode1 -> BaseMode -> set` normalization、ordinal 1 / `BindingId(0)`、BaseModeDef-RHS `BuiltinSet` 1 個、inferred variable 1 個、constraint/fact/candidate/diagnostic/deferred 0 個の checked assertion 1 個に限定する。finite matrix、先行 owner 54 件、immutable output、focused sibling、real sidecar は active count 178 を保護する。active corpus は既存 expectation を rebaseline せず 393 cases / 357 requirements、type-elaboration 225/213、pass/fail 209/184 を持つ。focused、relevant-crate、workspace verification は成功し、broader semantics は credit しない。

## task 231 active traceability

task 231 は shared Chapter 4/13/14/16 と Task 74 structural-producer reference 5 件、dedicated checker reference 1 件を持つ active seven-expansion object-terminal five-hop asserted-head row 1 件を追加する。credit は real object expansion 7 個、distinct `ChainObjectMode6`/`ChainObjectMode1` provenance、直接検証する bare link 5 本、terminal-only `ChainObjectMode1 -> BaseObjectMode -> object` normalization、ordinal 1 / `BindingId(0)`、BaseObjectModeDef-RHS `BuiltinObject` 1 個、inferred variable 1 個、object/set coercion なしの constraint/fact/candidate/diagnostic/deferred 0 個の checked assertion 1 個に限定する。finite matrix、先行 owner 55 件、immutable output、focused sibling、real sidecar は active count 179 を保護する。active corpus は既存 expectation を rebaseline せず 394 cases / 358 requirements、type-elaboration 226/214、pass/fail 210/184 を持ち、focused、relevant-crate、workspace verification は成功した。broader semantics は credit しない。

## task 233 active traceability

task 233 は shared Chapter 4/13/14/16 と builtin-type-expression reference 5 件、dedicated checker reference 1 件を持つ active exact single-left-parenthesized builtin-object reserved-variable equality row 1 件を追加する。credit は real unrecovered `ParenthesizedTerm` 1 個、inner/direct-right `x` 各1個、独立 wrapper/inner/right source metadata、ordinal 1/2 の `BindingId(0)` lookup、object/set coercion と独立 wrapper payload なしで既存 equality consumer へ渡す canonical reserve-derived `BuiltinObject` identity 1 個の透明な再利用に限定する。backlink 6 件、finite exact/near-miss/provenance/corruption matrix、先行 binary-formula owner 53 件、immutable output、real sidecar は active count 180 を保護する。active corpus は既存 expectation を rebaseline せず 395 cases / 359 requirements、type-elaboration 227/215、pass/fail 211/184 を持つ。arbitrary parenthesis/precedence、formula grouping、closure/order、truth/fact、acceptance、proof/IR/VC、child graph、broader semantics は credit しない。

## task 234 active traceability

task 234 は shared Chapter 4/13/14/16 と Task 74 structural-producer reference 5 件、dedicated checker reference 1 件を持つ active test-first seven-expansion set-terminal full-distance six-hop asserted-head row 1 件を追加する。credit は real expansion 7 個、distinct `ChainMode6`/`BaseMode` provenance、直接検証する bare link 6 本、terminal-only `BaseMode -> set` normalization、ordinal 1 / `BindingId(0)`、BaseModeDef-RHS `BuiltinSet` 1 個、inferred variable 1 個、constraint/fact/candidate/diagnostic/deferred 0 個の checked assertion 1 個に限定する。backlink 6 件、全5,039 order finite matrix、先行 owner 56 件、immutable output、focused sibling、real sidecar は active count 181 を保護する。active corpus は既存 expectation を rebaseline せず 396 cases / 360 requirements、type-elaboration 228/216、pass/fail 212/184 を持ち、broader semantics は credit しない。
...
Task 242 は Chapter-03-derived builtin-type bridge 1 件、shared Chapter 04/13/
14/16 reference、dedicated checker reference 1 件を持つ exact single-left-
parenthesized builtin-object reserved-variable inequality row 1 件を追加する。
credit は real unrecovered `ParenthesizedTerm` 1 個、inner `x` と direct-right
`x` 各 1 個、
独立 wrapper/inner/right metadata、ordinal 1/2 の `BindingId(0)` lookup、
written `object` anchor の canonical `BuiltinObject` 1 個の透過的再利用、
inferred term 2 個、type entry 6 個、ordered expected constraint 2 個、object/
set coercion と独立 wrapper payload のない checked inequality 1 個に限定する。
shared 5 + dedicated 1 backlink、finite matrix、先行 binary-formula owner 55
件、immutable output、focused Tasks 190/223/233/241、real sidecar は active
count 184 を保護する。active corpus は既存 expectation を rebaseline せず
399 cases / 363 requirements、type-elaboration 231/219、pass/fail 215/184 を
持つ。parenthesized membership と active imported provenance は Task 242 の
credit 外。未成立 imported expansion/evidence/signature payload と proof/
CoreIr/ControlFlowIr/VC は deferred。

## Task 243 Active Traceability

Task 243 は shared Chapter 04/13/14/16 reference と dedicated checker reference
1 件を持つ exact single-left-parenthesized builtin-set reserved-variable
membership row 1 件を追加する。credit は real unrecovered `ParenthesizedTerm`
1 個、inner `x` と direct-right `x` 各 1 個、独立 wrapper/inner/right metadata、
ordinal 1/2 の `BindingId(0)` lookup、written `set` anchor の canonical
`BuiltinSet` 1 個の透過的再利用、inferred term 2 個、type entry 5 個、left
expected input 0 個、変更しない Task 120 direct-right producer が供給する
right-owned expected-set constraint 1 個、独立 wrapper payload のない checked
membership 1 個に限定する。shared 4 + dedicated 1 backlink、finite matrix、
先行 binary-formula owner 56 件、immutable output、focused Tasks 120/223/233/
241/242、real sidecar は active count 185 を保護する。active corpus は既存
expectation を rebaseline せず 400 cases / 364 requirements、type-elaboration
232/220、pass/fail 216/184 を持つ。extraction gap の解除はこの exact source
...
implemented `advanced_semantics`/covered/required/pass requirement
`spec.en.13.set_expressions.nested_capture.semantic` 1件をfreezeする。Dependencyは
`spec.en.13.set_expressions.parser`だけ、testはnested-capture pass sidecar
だけ。Sidecarはpass/`type_check`、empty diagnostics、tags/failure fieldsなし、inactive。

Rowとrequired Chapter-13 `spec_coverage_audit.md` mappingはpresent。Final
trace/audit counts/hashes、metadata、全5 CLI measurementはcentral contractに
...
Resolver/capture/semantic/route/Task277B creditは追加しない。
...
implementationはrow/backlink/status/fixture/expectation/runner route/executable coverageを
追加しない。trace manifestはSHA-256
...
docs prerequisite verificationはcase/requirement `428/395`、pass/fail `235/193`、
