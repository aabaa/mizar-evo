...
|---|---|---|
| `TestKind` | `Expectation` から公開し harness-owned `TestCase` が consume する `expectation` corpus role | `#[non_exhaustive]` downstream forward-compatible surface。 |
...
| `snapshots` | no | transitional parse-only `SurfaceAst` baseline path、exact Core Task-31 `CoreIr` baseline 1件、exact VC Task-31 VcIr baseline 1件、または [限定 algorithm return/state baseline](harness.md#algorithm-return-contract-admission) または [void-claim baseline](harness.md#void-claim-admission)、[computation request baseline](harness.md#computation-request-admission)。該当する場合のみ。 |
...
task 61 は real direct attributed-builtin RHS expansion を持つ attributed local-mode reserve head が、
...
task 66 は one-edge attributed-builtin-RHS chain を持つ attributed local-mode reserve head が、
...
term-argument provenance 欠落のため external extraction-gap key で停止することを許可する。
task 70 は bracket-form local-mode reserve head が、real bracket type-argument と
`qua`-argument provenance 欠落のため、bracket type-argument payload extraction や
mode-head resolution の前に external extraction-gap key で停止することを許可する。
...
local-structure reserve head fail sidecar、task 77 は forward local-attribute
reserve type fail sidecar について、reserve head が later local declaration を
active 前に名前参照する場合に
...
task 78 は task-83 `R` bridge と task-97 `TypeCaseStruct` bridge 外の
imported structure reserve-head fail sidecar
について、source が documented `parser.type_fixtures` import summary を通じて
active runner に到達する場合に `failure_category = "external_dependency_gap"`、
...
task 83 と task 97 は documented imported structure `R` と `TypeCaseStruct`
sidecar について、runner が real imported structure type head を checker に渡した後に
`failure_category = "external_dependency_gap"`、
...
を許可する。これらの sidecar は imported provenance/type-head extraction だけを
...
task 79 は task-82 `TypeCaseMode` bridge 外の imported mode reserve-head fail
sidecar について、source が documented `parser.type_fixtures` import summary を
通じて active runner に到達する場合に `failure_category = "external_dependency_gap"`、
...
task 82 は同じ imported mode reserve-head sidecar について、runner が
`SymbolEnv` 由来の real imported `SymbolKind::Mode` symbol head を渡すように
なった場合に
`stable_detail_key = "type_elaboration.checker.checker.type.external.mode_expansion_payload"`
へ移ることを許可する。これは imported mode provenance と type-head payload
...
引き続き credit しない。
task 80 は task-84 `TypeCaseAttr` bridge、task-85 negative
`empty`/builtin-`set` bridge、task-116 positive `empty`/builtin-`set` bridge 外の broader imported attribute reserve fail sidecar
について、source が documented `parser.type_fixtures` import summary を通じて
...
`failure_category = "external_dependency_gap"`、
`rejection_reason = "formula_statement_recovery_formula_payload_gap"`、
`stable_detail_key = "type_elaboration.checker.checker.formula.external.formula_payload"`
を使う。diagnostic payload は同じ missing formula payload key を含む。non-exact
formula-only theorem shape は
`type_elaboration.external_dependency.ast_payload_extraction` に残す。これらの
sidecar は `formula_statement` runner、formula constant semantics、
child-formula graph payload、checker theorem/formula semantic checking、recorded
fact、proof skeleton、CoreIr、ControlFlowIr、VC、proof payload を credit しない。
...
proof payload は credit しない。
task 113 は exact
`ImportedAttributeAssertionPayloadBoundary: 1 is empty` source について task 103 を
supersede する。これは `rejection_reason =
"imported_attribute_assertion_formula_payload_gap"` と
`stable_detail_key =
...
は credit しない。
task 112 / task 117 は exact formula connective / quantifier theorem fail sidecar
について task 99 を supersede し、`failure_category = "external_dependency_gap"`、
`rejection_reason = "formula_connective_quantifier_shell_payload_gap"`、
`stable_detail_key =
"type_elaboration.checker.checker.formula.external.quantifier_payload"` を使う。
runner は implication、universal quantification、negation の source-derived
checker `FormulaInput` shell と、2 つの source constant に対応する exact
`FormulaKind::Contradiction` payload を渡し、diagnostic payload list は formula
payload と quantifier payload の detail key を含まなければならない。この sidecar
は formula constant semantic truth value、child-formula graph payload、quantifier
binder/context payload、formula checking、recorded fact、theorem acceptance、
`formula_statement` runner、CoreIr、ControlFlowIr、VC、proof payload を credit しない。
task 180 は別の exact pass sidecar
`SourceDerivedContradictionConstantBoundary: contradiction` を追加する。
standalone real leaf は deferred reason なしで checked になるため diagnostic
payload を持たない。`spec_refs` は Chapter 14、16 と dedicated checker bridge
row を含む。pass status は formula type/well-formedness だけを credit し、
falsehood/fact publication、theorem acceptance、proof-goal closure、implicit
closure/child graph、`formula_statement`、proof、CoreIr、ControlFlowIr、VC を
credit しない。
task 88 は proof-block theorem fail sidecar について
`failure_category = "external_dependency_gap"`、
`rejection_reason = "proof_skeleton_payload_extraction_gap"`、
`stable_detail_key = "type_elaboration.external_dependency.ast_payload_extraction"`
を許可する。これらの sidecar は `thus thesis;` conclusion を持つ theorem proof block が
...
`formula_statement` runner、CoreIr、ControlFlowIr、VC、proof payload は credit しない。
task 92 は mode/structure definition fail sidecar について同じ
`failure_category`、`rejection_reason`、`stable_detail_key` を許可する。これらの
sidecar は mode definition と structure definition が parser / resolver 実行へ
到達したことを記録するが、checker mode/structure definition declaration payload
extraction、mode expansion、structure base-shape / constructor / selector evidence、
definition-local context、recorded fact、`formula_statement` runner、CoreIr、
...
bracket `type_arg_list` / `qua`-argument provenance、
