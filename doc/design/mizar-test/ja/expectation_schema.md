...
ことを許可する。task 115 は exact unrecovered
...
を使う。diagnostic payload には
`type_elaboration.checker.checker.formula.unsupported_payload` も含める。non-exact
...
は credit しない。
task 112 は exact formula connective / quantifier theorem fail sidecar について
task 99 を supersede し、`failure_category = "external_dependency_gap"`、
...
checker `FormulaInput` shell を渡し、diagnostic payload list は formula payload と
quantifier payload の detail key を含まなければならない。この sidecar は formula
constant、child-formula graph payload、quantifier binder/context payload、formula
checking、recorded fact、theorem acceptance、`formula_statement` runner、CoreIr、
ControlFlowIr、VC、proof payload を credit しない。
