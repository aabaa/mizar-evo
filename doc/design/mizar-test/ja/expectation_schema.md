...
`mizar-core` `ResolvedTypedAstSummary::from_ast` readiness read へ通す。同じ source
type-expression range を共有する複数 identifier でも distinct typed site を使わなければならない。
この summary read を `CoreIr`、`ControlFlowIr`、VC、proof execution として扱ってはならない。
...
expansion、term、formula、coercion、fact、overload evidence、CoreIr、ControlFlowIr、
VC payload、proof evidence を捏造せず構築できるまで deferred のままにする。
