...
task 86 は formula-only theorem fail sidecar について
`failure_category = "external_dependency_gap"`、
`rejection_reason = "formula_statement_payload_extraction_gap"`、
`stable_detail_key = "type_elaboration.external_dependency.ast_payload_extraction"`
を許可する。これらの sidecar は source が parser / resolver 実行へ到達したことを
