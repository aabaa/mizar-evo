...
`checker.type.external.mode_expansion_payload`; task 58 does not make
attributed reserve heads evidence-query ready, and the runner withholds
task-55/task-56 expansions from mixed attributed/bare local-mode sources. Task 56 also adds an active fail
...
RHSs, attributed-RHS chains, forward-reference or cyclic local-mode chains, and non-reserve
declarations remain outside this source bridge.
...
terminal local-structure RHS diagnostic slice and task-58 terminal
attributed-builtin RHS diagnostic slice, to `mizar-checker`'s source
