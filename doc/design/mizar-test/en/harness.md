...
`checker.type.external.mode_expansion_payload` unless the task-55/task-56
supported expansion slice applies; the runner does not claim an evidence query
for the fully expanded attributed mode type and withholds task-55/task-56
expansions from mixed attributed/bare local-mode sources. Task 56 also adds an active fail
...
arguments, parameterized or contextual mode definitions, attributed mode or
structure RHSs, forward-reference or cyclic local-mode chains, and non-reserve
...
terminal local-structure RHS diagnostic slice, to `mizar-checker`'s source
