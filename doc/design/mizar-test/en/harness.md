...
Task 75/76 addendum for `type-elaboration`: forward same-module local-mode and
local-structure reserve heads that name later declarations are active
lower-stage boundary cases. The runner expects
...
type-head, base-shape, or constructor-witness payloads from future
declarations.
