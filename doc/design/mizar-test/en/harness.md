...
`thesis` formula constant site/range as a checker recovery `FormulaInput` and
expecting `type_elaboration.checker.checker.formula.external.formula_payload`
plus `type_elaboration.checker.checker.formula.unsupported_payload`. Non-exact
...
recovery `FormulaInput`, then failing closed on missing formula payload and
unsupported formula semantics. It does not satisfy the deferred
