...
Task 112 addendum for `type-elaboration`: the exact theorem formula using
Chapter 14 implication, universal quantification, and negation may run through
parser and resolver, then the active runner passes source-derived checker
`FormulaInput` shells for the implication, quantified formula, and negation. It
must fail closed on missing formula payloads and missing quantifier payloads
until formula constants, child-formula graph payloads, binder/context payloads,
formula checking, recorded facts, theorem acceptance, and `formula_statement`
runner support exist. The runner must not synthesize formula constants,
child links, binder/context payloads, facts, theorem acceptance, CoreIr,
ControlFlowIr, VC, or proof payloads.
...
payloads. It does not satisfy the deferred `formula_statement` runner
obligation and does not credit formula constants, child-formula graph payloads,
quantifier binder/context payloads, formula checking, facts, theorem
acceptance, CoreIr, ControlFlowIr, VC, or proof payloads.
