...
Task 75 addendum for `type-elaboration`: a forward same-module local-mode
reserve head that names a later declaration is an active lower-stage boundary
case. The runner expects
`type_elaboration.lower_stage.frontend:malformed_type_expression` before
checker handoff and must not synthesize a checker `ModeExpansion` from the
future declaration.
