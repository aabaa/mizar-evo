...
Task 100 addendum for `type-elaboration`: a theorem formula using the Chapter
14 builtin membership predicate with Chapter 13 numeral operands may run through
parser and resolver as an active checker boundary case, but it must remain on
...
formula checking, recorded facts, theorem acceptance, and `formula_statement`
runner support exist. The runner must not synthesize term payloads, formula
payloads, membership facts, theorem acceptance, CoreIr, ControlFlowIr, VC, or
proof payloads.

Task 101 addendum for `type-elaboration`: a theorem formula using the Chapter
14 builtin inequality predicate with Chapter 13 numeral operands may run
through parser and resolver as an active checker boundary case, but it must
remain on `type_elaboration.external_dependency.ast_payload_extraction` until
real term/formula payload extraction, inequality desugaring or equality
semantic checking, formula checking, recorded facts, theorem acceptance, and
`formula_statement` runner support exist. The runner must not synthesize term
payloads, formula payloads, inequality facts, theorem acceptance, CoreIr,
ControlFlowIr, VC, or proof payloads.

Task 102 addendum for `type-elaboration`: a theorem formula using the Chapter
14 builtin type-assertion form with a Chapter 13 numeral term may run through
parser and resolver as an active checker boundary case, but it must remain on
`type_elaboration.external_dependency.ast_payload_extraction` until real
...
theorem acceptance, and `formula_statement` runner support exist. The runner
must not synthesize term payloads, formula payloads, type-assertion facts,
theorem acceptance, CoreIr, ControlFlowIr, VC, or proof payloads.

Task 103 addendum for `type-elaboration`: a theorem formula importing
`parser.type_fixtures` and using its documented `empty` attribute in the
Chapter 14 attribute-assertion form with a Chapter 13 numeral subject may run
through parser and resolver as an active checker boundary case, but it must
remain on `type_elaboration.external_dependency.ast_payload_extraction` until
real term/formula payload extraction, imported attribute assertion
...
extraction for theorem formulas, and `formula_statement` runner support exist.
The runner must not synthesize term payloads, formula payloads, imported
attribute assertion payloads, imported module AST extraction, theorem
acceptance, CoreIr, ControlFlowIr, VC, or proof payloads.

Task 104 addendum for `type-elaboration`: a theorem formula importing
...
form with a Chapter 13 numeral subject may run through parser and resolver as
an active checker boundary case, but it must remain on
`type_elaboration.external_dependency.ast_payload_extraction` until real
term/formula payload extraction, imported attribute-level non-empty assertion
attribute-chain/provenance payload extraction, term inference, negated
attribute admissibility/semantic checking, formula checking, recorded facts,
theorem acceptance, imported module AST extraction, checker `AttributeInput`
payload extraction for theorem formulas, and `formula_statement` runner support
exist. The runner must not synthesize term payloads, formula payloads, imported
attribute-level non-empty assertion payloads, imported module AST extraction,
theorem acceptance, CoreIr, ControlFlowIr, VC, or proof payloads.

Task 105 addendum for `type-elaboration`: a theorem formula using Chapter 13
set-enumeration term operands and Chapter 14 builtin equality may run through
parser and resolver as an active checker boundary case, but it must remain on
`type_elaboration.external_dependency.ast_payload_extraction` until real
set-enumeration term payload extraction, term/formula payload extraction, term
inference, equality/formula checking, recorded facts, theorem acceptance, and
`formula_statement` runner support exist. The runner must not synthesize
set-enumeration payloads, term payloads, formula payloads, theorem acceptance,
CoreIr, ControlFlowIr, VC, or proof payloads.

Task 99 addendum for `type-elaboration`: a theorem formula using Chapter 14
implication, universal quantification, and negation may run through parser and
resolver as an active checker boundary case, but it must remain on
`type_elaboration.external_dependency.ast_payload_extraction` until real formula
payload extraction, quantifier binder/context payloads, formula checking,
recorded facts, theorem acceptance, and `formula_statement` runner support
exist. The runner must not synthesize formula payloads, binder/context payloads,
...
Task 98 refines the same row: imported predicate/functor theorem formulas are
...
obligation and do not credit imported semantic payloads, term/formula payloads,
term inference, formula checking, facts, proof skeletons, CoreIr, ControlFlowIr,
VC, or proof payloads. In the row above, the theorem formula boundary entry now
also covers task 98's imported predicate/functor case.

Task 100 refines the same row: builtin membership theorem formulas are
executable through the active `type_elaboration` runner only as extraction-gap
boundaries. They do not satisfy the deferred `formula_statement` runner
obligation and do not credit term/formula payloads, membership operand type
inference/checking, formula checking, facts, theorem acceptance, CoreIr,
ControlFlowIr, VC, or proof payloads. In the row above, the theorem formula
boundary entry now also covers task 100's builtin membership case.

Task 101 refines the same row: builtin inequality theorem formulas are
executable through the active `type_elaboration` runner only as extraction-gap
boundaries. They do not satisfy the deferred `formula_statement` runner
obligation and do not credit term/formula payloads, inequality desugaring or
equality semantic checking, formula checking, facts, theorem acceptance, CoreIr,
ControlFlowIr, VC, or proof payloads. In the row above, the theorem formula
boundary entry now also covers task 101's builtin inequality case.

Task 102 refines the same row: builtin type-assertion theorem formulas are
executable through the active `type_elaboration` runner only as extraction-gap
boundaries. They do not satisfy the deferred `formula_statement` runner
obligation and do not credit term/formula payloads, type-assertion type payload
...
row above, the theorem formula boundary entry now also covers task 102's builtin
type-assertion case.

Task 103 refines the same row: imported attribute assertion theorem formulas
using `parser.type_fixtures` `empty` are executable through the active
`type_elaboration` runner only as extraction-gap boundaries. They do not satisfy
the deferred `formula_statement` runner obligation and do not credit
term/formula payloads, imported attribute assertion attribute-chain/provenance
payload extraction, term inference, attribute admissibility/semantic checking,
formula checking, facts, theorem acceptance, imported module AST extraction,
checker `AttributeInput` payload extraction for theorem formulas, CoreIr,
ControlFlowIr, VC, or proof payloads. In the row above, the theorem formula
boundary entry now also covers task 103's imported attribute assertion case.

Task 104 refines the same row: attribute-level `non empty` imported attribute
assertion theorem formulas using `parser.type_fixtures` `empty` are executable
through the active `type_elaboration` runner only as extraction-gap boundaries.
They do not satisfy the deferred `formula_statement` runner obligation and do
not credit term/formula payloads, imported attribute-level non-empty assertion
attribute-chain/provenance payload extraction, term inference, negated
attribute admissibility/semantic checking, formula checking, facts, theorem
acceptance, imported module AST extraction, checker `AttributeInput` payload
extraction for theorem formulas, CoreIr, ControlFlowIr, VC, or proof payloads.
In the row above, the theorem formula boundary entry now also covers task
104's attribute-level non-empty imported attribute assertion case.

Task 105 refines the same row: set-enumeration theorem formulas using Chapter
13 set-enumeration term operands and Chapter 14 builtin equality are executable
through the active `type_elaboration` runner only as extraction-gap boundaries.
They do not satisfy the deferred `formula_statement` runner obligation and do
not credit set-enumeration term payload extraction, term/formula payloads, term
inference, equality/formula checking, facts, theorem acceptance, CoreIr,
ControlFlowIr, VC, or proof payloads. In the row above, the theorem formula
boundary entry now also covers task 105's set-enumeration case.

Task 99 refines the same row: connective/quantifier theorem formulas are
executable through the active `type_elaboration` runner only as extraction-gap
boundaries. They do not satisfy the deferred `formula_statement` runner
obligation and do not credit formula payloads, quantifier binder/context
payloads, formula checking, facts, theorem acceptance, CoreIr, ControlFlowIr,
VC, or proof payloads. In the row above, the theorem formula boundary entry now
also covers task 99's connective/quantifier case.
