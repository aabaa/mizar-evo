...
documented `R` portion. Broader imported structures outside the task-83 `R`
provenance/type-head bridge remain deferred until a non-`R` source-derived
fixture exists; such future cases should expect
...
Task 83 addendum for `type-elaboration`: the documented `parser.type_fixtures`
imported structure `R` may be passed as a checker-owned imported structure type
head. The runner expects
...
Task 80 addendum for `type-elaboration`: before task 84, an imported attribute
reserve type from the documented `parser.type_fixtures` import summary was an
active source-to-checker extraction-gap boundary case. Imported attributes
outside the task-84 `TypeCaseAttr` provenance/`AttributeInput` bridge still
expect `type_elaboration.external_dependency.ast_payload_extraction`. The
...
`TypeCaseAttr` bridge and task-80 boundary, imported structures beyond the task-83 `R` bridge and task-78
