...
Task 78 addendum for `type-elaboration`: an imported structure reserve head
from the documented `parser.type_fixtures` import summary is an active
source-to-checker extraction-gap boundary case. The runner expects
`type_elaboration.external_dependency.ast_payload_extraction` and must not
treat the summary as real imported module AST extraction or synthesize imported
structure provenance, structure type-head payloads, base-shape or
constructor-witness evidence, positive structure elaboration, CoreIr,
ControlFlowIr, VC, or proof payloads.

Task 79 addendum for `type-elaboration`: an imported mode reserve head from the
documented `parser.type_fixtures` import summary is an active source-to-checker
extraction-gap boundary case. The runner expects
`type_elaboration.external_dependency.ast_payload_extraction` and must not
treat the summary as real imported module AST extraction or synthesize imported
mode provenance, mode type-head payloads, `ModeExpansion` payloads, positive
mode elaboration, CoreIr, ControlFlowIr, VC, or proof payloads.

Task 80 addendum for `type-elaboration`: an imported attribute reserve type from
the documented `parser.type_fixtures` import summary is an active
source-to-checker extraction-gap boundary case. The runner expects
`type_elaboration.external_dependency.ast_payload_extraction` and must not
treat the summary as real imported module AST extraction or synthesize imported
...
bridge. Non-builtin declarations, imported attributes beyond the task-80
boundary, imported modes or structures beyond the task-78/task-79 boundaries, attribute arguments, mode/structure
