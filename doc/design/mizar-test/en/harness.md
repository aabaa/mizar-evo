...
The Step 5C.9 route retains its seven initial mapped rows and adds only then/hence
at formula_statement / statement_check, authenticating its exact paths, kind, domain,
spec ref, tag and absent auxiliary payloads. Existing resolver/checker inputs carry
actual predecessor linking. Only checker case-completeness failure after statement
checking earns the verification key; formula success grants no proof acceptance.
...
The later SDP harness path is test-private and exact-source selected. It
authenticates parser/resolver evidence and constructs only the immutable lower
projection. It must return `None` for missing final LF, theorem/Given/now/Set
shape changes, neighboring 269 sources, broad fixtures, and every public
active route. It does not dispatch a corpus case or synthesize binding, term,
capture, proof, or semantic payload.
...
This is the durable derived private-runner ABI for the
[central Task-269SDT contract](../../task_contracts/en/269SDT.md); it is not
language-specification authority.
...
The completed [277C contract](../../task_contracts/en/CHECKER-FRAENKEL-TEMPLATE-STRUCTURAL-277C.md)
adds one private leaf, `runner/tests/type_elaboration/template_fraenkel_structural_composition.rs`,
and one `tests.rs` registration. The leaf uses existing helpers and unchanged inactive
F5 only to prove the exact composed row/debug profile; it adds no fixture, sidecar,
expectation, trace, metadata, coverage credit, runner route, dispatcher, or semantic
verdict. Its one registration changes the mizar-test raw list only from 611 to 612,
while checker owns the four direct producer tests. Exact measurements and remaining
...
valid uncapped `100/100` (`20/20/15/15/10/10/5/5`). The historical pre-commit
staging/cached review passed over 28 paths (4 Rust and 24 documentation paths), including one
new private leaf, with no unstaged paths at review time; cached diff check
passed at 2435 insertions / 101 deletions. The task-only commit, immediate
post-commit proof, and accepted fresh-inventory disposition are closed in the
...
It reuses the C4C1 frontend/import helper and the existing test-only typed
profile, calls the public C4C3 producer directly, and asserts only the one
immutable resolver-use-to-typed-binder row, source/module/summary, dense
absence, and dependency non-mutation. Deterministic replay remains covered by
the checker-owner C4C3 test.

This is the sole current API consumer and remains library-test-only. It adds no
...
extends the same existing private leaf with exactly
`task257c4c4_real_imported_fixture_builds_mapper_primary_handoff`. The test
reuses the C4C1 frontend helper, completed C4C2 collector, C4C3 typed
profile, and public C4C3/C4C4 producers. It asserts the retained C4C3 row,
outer-x-only `3/1/0` binding projection, one-node Task-252 arena, and exact
`1/1/0` primary/reference/request output.

This is the sole current C4C4 API consumer and remains library-test-
...
the existing private nested-capture leaf. The probe runs the
unchanged C4C7 source through the existing frontend and resolver, calls the
standalone graph producer, and asserts only diagnostics-free and unrecovered
...
The passing probe remains library-test-only and unregistered. No fixture, sidecar,
...
five G1/G2/G9 rows remain ineligible even if tagged. Reserved identities cannot fall through other stages; no predicate/functor proof credit follows.
...
The dependent-return row alone enters type_elaboration/type_check with pass, functors.dependent_return,
sole spec.en.10.functors.dependent_return.semantic ref and active_type_elaboration, exact ids/source/sidecar paths
and empty diagnostic/failure/payload/snapshot fields. Either id or filename reserves every other stage; missing/duplicate inventory rows fail.
The runner invokes the [direct checker](../../mizar-checker/en/type_checker.md#source-dependent-functor-return-typing)
and compares actual declaration bindings, argument/body Known types, checked equality and both source-owned Pending clauses.
...
The exact ensures-return and var-const-assert algorithm rows are admitted only at proof_verification/vc_generation with their unchanged outcome/domain, sole active tag and original semantic spec ref plus the matching narrow snapshot ref. The state row requires `spec.en.mizar_vc.vc_ir.algorithm_var_const_assert_snapshot` and `snapshots/vc/pass_proof_verification_algorithm_var_const_assert_001.vc_ir.snap`; the return row retains its existing exact ref/path. IDs, source/sidecar paths and contradictory payloads are authenticated before sealed checker/Core/VC processing. Whole VcSets and complete debug baselines are compared deterministically. In-memory no-contract controls own `snapshots/vc/step5c14_algorithm_no_ensures.vc_ir.snap` and `snapshots/vc/step5c14_algorithm_state_no_contract.vc_ir.snap` without extra active rows. Static observations retain their separate zero-VC route; wrong stages, malformed input and unsupported source receive no pass credit.

## Void-claim admission

Only `pass_proof_verification_claim_block_theorem_001` additionally enters proof_verification/vc_generation with its unchanged algorithms.claim domain/pass outcome, sole active tag, original `spec.en.20.algorithms.claim.block` ref followed by `spec.en.mizar_vc.vc_ir.algorithm_void_claim_snapshot`, and exact `snapshots/vc/pass_proof_verification_claim_block_theorem_001.vc_ir.snap`. Admission and snapshot-scope validation authenticate IDs, source/sidecar paths, ordered refs and empty contradictory payloads; all aliases and other stages fail closed. Actual checker seals, unified Core theorem/algorithm dependency and VC generation must succeed; full Core/CFG/VcSet reruns and the whole VC debug baseline retain wrapper, target, quantified proposition, proof-local goal and both accounting rows. No ordinary-theorem fallback, source-name shortcut, accepted proof or extra active row is allowed; existing static/return/state routes are preserved.

## Contradictory assertion admission

Only `fail_proof_verification_algorithm_assert_unprovable_001` enters proof_verification/verification/fail with domain `algorithms.assertions`, category `proof_failure`, key `algorithms.assert.unprovable`, empty public codes and sole active tag. Preserve its original `spec.en.20.algorithms.state.var_const_assert` ref followed by `spec.en.mizar_vc.vc_ir.algorithm_assert_failure_snapshot` and exact `snapshots/vc/fail_proof_verification_algorithm_assert_unprovable_001.vc_ir.snap`; validate all IDs, paths, stages, ordered refs and absent contradictory payloads in both admission and snapshot scope. Compare the complete raw Open VcSet before mapping only the replay-authenticated failing VcId; unsupported source or missing evidence is not the key. The same source with positive equality owns `snapshots/vc/step5c14_algorithm_assert_reflexive.vc_ir.snap` as a full one-open-VC control without an active row. Both baselines retain the real object guard, generated formulas and Deferred termination accounting; source bytes, other C14 profiles and acceptance boundaries stay unchanged.

## Three-registration source-proof prerequisite

Rust integration tests run the unchanged whole C13 ambiguity source through the fresh proof facade; the existing sidecar, trace/map and advanced-semantics admission remain inactive for this prerequisite.
...
## Bounded while admission

Activate only `pass_proof_verification_algorithm_while_invariant_001` at unchanged pass/proof_verification/vc_generation, algorithms.loops and original while_invariant semantic reference. Append `spec.en.mizar_vc.vc_ir.algorithm_while_invariant` and `snapshots/vc/pass_proof_verification_algorithm_while_invariant_001.vc_ir.snap`; its trace section is `VC Task 47; bounded while invariant VcIr snapshot`. Exact source/sidecar identity, sole active tag, absent contradictory payloads and complete deterministic VcSet comparison remain mandatory. Controls inspect all three goals, cutpoint/write identities, conservative unresolved dependencies and seed accounting; renaming and altered assignment/invariant operands must affect the real graph. No proof-verification success or termination credit follows.

## Bounded means-prefix functor admission

Admit only `pass_proof_verification_func_means_prefix_001` with its original pass/proof_verification/vc_generation, functors.means_definition and semantic reference, sole active_proof_verification tag and additive means-functor snapshot reference/path. Reserve exact source/sidecar identities across stages and reject missing/duplicate inventory or auxiliary payloads. Require authentic pending existence/uniqueness, the actual application equality theorem, complete Core/VcSet reruns and fixed snapshot. Generation never treats an empty correctness clause as accepted or supplies its pending defining equation to the theorem.

## Bounded equals-infix functor admission

Admit only `pass_proof_verification_func_equals_infix_operator_001` at unchanged pass/proof_verification/vc_generation, functors.equals_definition and original semantic reference, with sole active_proof_verification tag and additive equals-functor snapshot reference/path. Exact source/sidecar identities, missing/duplicate inventory, aliases and auxiliary payloads remain fail-closed. Require complete deterministic Core/VcSet reruns and fixed baseline, actual RHS coherence and application theorem graphs, independent binder/operator renaming and hostile replay controls. This is pending-goal generation only.

## Bounded false-postcondition admission

Activate only `fail_proof_verification_algorithm_ensures_unprovable_001` with unchanged fail/proof_verification/verification, proof_failure, algorithms.contracts, algorithms.ensures.unprovable and empty diagnostics. Retain its original ensures reference first; append `spec.en.mizar_vc.vc_ir.algorithm_ensures_failure_snapshot` and `snapshots/vc/fail_proof_verification_algorithm_ensures_unprovable_001.vc_ir.snap`, with source_vc_decomposition trace section `VC Tasks 43/50; bounded runtime Pick false-postcondition VcIr snapshot`. Exact admission and deterministic full Core/CFG/VcSet replay are required; the existing negative observer must return the actual postcondition VcId after baseline comparison. Unsupported generation or observer errors cannot satisfy the oracle. The inhabitation VC remains Open and termination unresolved; metadata and baseline-corruption controls preserve all existing activations.

## Narrower-loci predicate admission

Reserve the exact narrower-loci source/sidecar identity and unchanged pass/type_elaboration/type_check oracle with its sole semantic reference and sole active tag. In one immutable source orchestration, freshly prove the authored registration through the existing checker/Core/VC/kernel/policy path, then invoke the [declaration checker](../../mizar-checker/en/type_checker.md#narrower-loci-predicate-declaration-checking) with that local output. Require the shared written type gate and both formal bindings, actual strictly narrower root correspondence, typed swapped equality bodies and one pending coherence request. Source, registration/choice/citation, policy, binding, normalized-signature, root-order/ambiguity and coherence-owner mutations must reject or retain the accurately changed pending goal; no caller acceptance, fabricated database, proof success or new VC snapshot is admitted.
