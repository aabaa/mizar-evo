...
| `snapshots` | no | Transitional parse-only `SurfaceAst` baseline path, the one exact Core Task-31 `CoreIr` baseline, the one exact VC Task-31 VcIr baseline, or the [bounded algorithm return baseline](harness.md#algorithm-return-contract-admission), when applicable. |
