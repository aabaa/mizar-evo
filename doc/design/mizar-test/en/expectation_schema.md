...
| `snapshots` | no | Transitional parse-only `SurfaceAst` baseline path or the [exact reduction failure VcIr baseline](harness.md) or [contradictory-assertion baseline](harness.md#contradictory-assertion-admission) at `verification`; the Core Task-31 exception remains pass-only. |
...
and task-116 positive `empty`/builtin-`set` bridge to use
`failure_category = "external_dependency_gap"` with
...
Task 113 supersedes task 103 for the exact
`ImportedAttributeAssertionPayloadBoundary: 1 is empty` source. It uses
`rejection_reason = "imported_attribute_assertion_formula_payload_gap"` and
`stable_detail_key =
