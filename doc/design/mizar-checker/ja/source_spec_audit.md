...
attributed builtin head を missing-evidence diagnostic へ、task 55/56 外の same-module local
mode head（attributed local mode、mixed attributed/bare local mode source、attributed chain
dependency を含む）を missing mode-expansion diagnostic へ、same-module local structure head と attributed local
structure head を missing evidence-query diagnostic へ運ぶ。
