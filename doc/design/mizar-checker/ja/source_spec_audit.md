...
same-module local mode head を missing mode-expansion diagnostic へ運ぶ。これは checker の
新しい raw-syntax dependency ではなく、non-builtin declaration、imported symbol、
attribute / mode argument、structure、term、formula、overload、CoreIr、ControlFlowIr、
VC payload、proof evidence の AST-wide source-to-checker gap を閉じるものでもない。
...
| MC-G020 | `external_dependency_gap` / `deferred` | task 7-11 と後続 consumer の semantic pass fixture を妨げる source-to-checker extraction blocker として active。reserve-only builtin declaration slice は checker-owned syntax-free producer seam を使って `BindingEnv`、`DeclarationInput`、`DeclarationChecker` handoff data を作り、successful bare builtin について active runner で `TypedAst` と `ResolvedTypedAst` まで到達する。active fail slice は same-module attributed builtin reserve head を missing-evidence diagnostic へ、same-module local mode head を missing mode-expansion diagnostic へ運ぶ。より広い non-builtin declaration、imported attribute / mode、attribute argument、attributed mode head、mode argument、structure、term、formula、coercion、overload、recorded fact、CoreIr、ControlFlowIr、VC、proof payload extraction は未解決のまま。 |
