...
持たないものも successful pass execution に含まれる。active fail slice は same-module
attributed builtin head を missing-evidence diagnostic へ、task 55 外の same-module local
mode head（attributed local mode や mixed attributed/bare local mode source を含む）を
missing mode-expansion diagnostic へ、same-module local structure head と attributed local
...
parameterized / contextual mode expansion、term、formula、overload、CoreIr、
...
| MC-G020 | `external_dependency_gap` / `deferred` | task 7-11 と後続 consumer の semantic pass fixture を妨げる source-to-checker extraction blocker として active。reserve-only builtin declaration slice は checker-owned syntax-free producer seam を使って `BindingEnv`、`DeclarationInput`、`DeclarationChecker` handoff data を作り、successful bare builtin と、real AST-derived bare-builtin RHS expansion を持つ task-55 same-module no-argument local mode について active runner で `TypedAst` と `ResolvedTypedAst` まで到達する。active fail slice は same-module attributed builtin reserve head を missing-evidence diagnostic へ、task 55 外の same-module local mode head と attributed same-module local mode head を missing mode-expansion diagnostic へ、same-module local structure head を missing evidence-query diagnostic へ、attributed same-module local structure head を full attributed-type missing-evidence diagnostic へ運ぶ。より広い non-builtin declaration、imported attribute / mode / structure、attribute argument、mode / structure argument、structure base-shape / full attributed-type existential evidence、broader / imported / attributed / parameterized / contextual mode expansion、term、formula、coercion、overload、recorded fact、CoreIr、ControlFlowIr、VC、proof payload extraction は未解決のまま。 |
