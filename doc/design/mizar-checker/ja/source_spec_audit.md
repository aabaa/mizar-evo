...
Task 87 addendum: active `mizar-test` type-elaboration runner は
`theorem TermFormulaPayloadBoundary: 1 = 1;` という term-bearing theorem formula
source を parser / resolver 実行後に観測し、
`type_elaboration.external_dependency.ast_payload_extraction` を報告する。これは
Chapter 13 numeral term と Chapter 14 builtin equality の source-derived
term/formula theorem extraction-gap boundary だけを credit する。checker
term/formula payload extraction、term inference、formula checking、recorded fact、
theorem acceptance、dedicated `formula_statement` runner、CoreIr、ControlFlowIr、
VC、proof payload coverage は主張しない。この addendum は MC-G020 row の
formula/theorem gap を task-86/task-87 boundary にだけ狭め、semantic term/formula
payload extraction はその boundary を超えて unavailable のままにする。

Task 98 addendum: active `mizar-test` type-elaboration runner は
`theorem ImportedPredicateFunctorPayloadBoundary: 1 divides (1 ++ 2);` という
imported predicate/functor theorem formula source を parser / resolver 実行後に観測し、
`type_elaboration.external_dependency.ast_payload_extraction` を報告する。これは
Chapter 11、12、13、14、16 の source-derived imported predicate/functor
term-formula extraction-gap boundary だけを credit する。imported module AST
extraction、imported predicate/functor semantic payload、checker term/formula
payload extraction、term inference、formula checking、recorded fact、theorem
acceptance、dedicated `formula_statement` runner、CoreIr、ControlFlowIr、VC、
proof payload coverage は主張しない。

Task 100 addendum: active `mizar-test` type-elaboration runner は
`theorem BuiltinMembershipPayloadBoundary: 1 in 1;` という builtin membership
theorem formula source を parser / resolver 実行後に観測し、
`type_elaboration.external_dependency.ast_payload_extraction` を報告する。これは
Chapter 13、14、16 の source-derived builtin membership term-formula
extraction-gap boundary だけを credit する。checker term/formula payload
extraction、membership operand type inference/checking、formula checking、
recorded fact、theorem acceptance、dedicated `formula_statement` runner、CoreIr、
ControlFlowIr、VC、proof payload coverage は主張しない。

Task 101 addendum: active `mizar-test` type-elaboration runner は
`theorem BuiltinInequalityPayloadBoundary: 1 <> 2;` という builtin inequality
theorem formula source を parser / resolver 実行後に観測し、
`type_elaboration.external_dependency.ast_payload_extraction` を報告する。これは
Chapter 13、14、16 の source-derived builtin inequality term-formula
extraction-gap boundary だけを credit する。checker term/formula payload
extraction、inequality desugaring または equality semantic checking、formula
checking、recorded fact、theorem acceptance、dedicated `formula_statement` runner、
CoreIr、ControlFlowIr、VC、proof payload coverage は主張しない。

Task 102 addendum: active `mizar-test` type-elaboration runner は
`theorem BuiltinTypeAssertionPayloadBoundary: 1 is set;` という builtin
type-assertion theorem formula source を parser / resolver 実行後に観測し、
`type_elaboration.external_dependency.ast_payload_extraction` を報告する。これは
Chapter 13、14、16 の source-derived builtin type-assertion term-formula
extraction-gap boundary だけを credit する。checker term/formula payload
...
Task 103 addendum: active `mizar-test` type-elaboration runner は
`import parser.type_fixtures; theorem ImportedAttributeAssertionPayloadBoundary: 1 is empty;`
という imported attribute assertion theorem formula source を parser / resolver
実行後に観測し、`type_elaboration.external_dependency.ast_payload_extraction`
を報告する。これは Chapter 6、11、12、13、14、16 の source-derived imported
attribute assertion formula extraction-gap boundary だけを credit する。
checker term/formula payload extraction、imported attribute assertion
...
acceptance、imported module AST extraction、theorem formula 向け checker
`AttributeInput` payload extraction、dedicated `formula_statement` runner、
CoreIr、ControlFlowIr、VC、proof payload coverage は主張しない。

Task 104 addendum: active `mizar-test` type-elaboration runner は
`import parser.type_fixtures; theorem ImportedNonEmptyAttributeAssertionPayloadBoundary: 1 is non empty;`
という attribute-level `non empty` imported attribute assertion theorem formula
source を parser / resolver 実行後に観測し、
`type_elaboration.external_dependency.ast_payload_extraction` を報告する。これは
Chapter 6、11、12、13、14、16 の source-derived attribute-level `non empty`
imported attribute assertion formula extraction-gap boundary だけを credit する。
checker term/formula payload extraction、imported attribute-level non-empty
...
checker `AttributeInput` payload extraction、dedicated `formula_statement`
runner、CoreIr、ControlFlowIr、VC、proof payload coverage は主張しない。

Task 105 addendum: active `mizar-test` type-elaboration runner は
`theorem SetEnumerationPayloadBoundary: {1, 2} = {1, 2};` という
set-enumeration theorem formula source を parser / resolver 実行後に観測し、
`type_elaboration.external_dependency.ast_payload_extraction` を報告する。これは
Chapter 13、14、16 の source-derived set-enumeration theorem formula
extraction-gap boundary だけを credit する。checker set-enumeration term
payload extraction、term/formula payload extraction、term inference、
...
Task 99 addendum: active `mizar-test` type-elaboration runner は
`theorem FormulaConnectiveQuantifierPayloadBoundary: contradiction implies for x being set holds not contradiction;`
という formula connective / quantifier theorem source を parser / resolver 実行後に
観測し、`type_elaboration.external_dependency.ast_payload_extraction` を報告する。
これは theorem declaration 内の source-derived Chapter 14 connective / quantifier
formula extraction-gap boundary だけを credit する。formula payload extraction、
