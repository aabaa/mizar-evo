...
task 72 の two-edge bare local-mode chain も successful pass execution に含まれる。
active fail slice は same-module
attributed builtin head を missing-evidence diagnostic へ、task 55/56/57/58/59/60/61/62/63/64/65/66/72 外の same-module local
mode head（mixed attributed/bare local mode source、attributed chain
dependency、cold / cached three-edge local-mode chain を含む）を missing mode-expansion diagnostic へ、same-module local structure head と attributed local
...
local-mode chain を既存の pass readiness path へ通し、cold path と cached reuse の
three-edge chain を checker missing mode-expansion diagnostic に残す。
