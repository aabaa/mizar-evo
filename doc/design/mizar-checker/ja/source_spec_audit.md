...
task 72 の two-edge bare local-mode chain と task 73 の three-edge bare local-mode
chain も successful pass execution に含まれる。
active fail slice は same-module
attributed builtin head を missing-evidence diagnostic へ、task 55/56/57/58/59/60/61/62/63/64/65/66/72/73 外の same-module local
mode head（mixed attributed/bare local mode source、attributed chain
dependency、cold / cached four-edge local-mode chain を含む）を missing mode-expansion diagnostic へ、same-module local structure head と attributed local
...
cold path と cached reuse の four-edge chain は checker missing mode-expansion diagnostic に残す。
