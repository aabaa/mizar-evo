...
task 101 は同じ term/formula boundary の builtin inequality variant を記録する:
`theorem BuiltinInequalityPayloadBoundary: 1 <> 2;` は Chapter 13 の numeral term と
Chapter 14 の builtin inequality predicate を通じて parser / resolver 実行まで
到達するが、real term/formula payload extraction、inequality desugaring または
equality semantic checking、formula checking、recorded fact、theorem acceptance、
