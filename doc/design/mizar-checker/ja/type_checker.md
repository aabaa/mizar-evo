...
task 229 は ordered bare definition 7 個 `BaseObjectMode -> object`、`ChainObjectMode1 -> BaseObjectMode` から `ChainObjectMode6 -> ChainObjectMode5`、`ChainObjectMode6` reserve 1 個、`LongLocalObjectModeFourHopAssertedHeadPayloadBoundary: x is ChainObjectMode2` だけを対象とする。Task 74 は real AST-derived object expansion、Task 200 は real formula/checker consumer、Task 221 は byte-for-byte unchanged closed `BindingFourHopRadix` を供給する。Tasks 225/227 は two/three-hop object long-chain guard、Task 222 は object-terminal relation guard、Task 228 は depth-matched set guard とする。

active route は distinct `ChainObjectMode6` subject/`ChainObjectMode2` asserted symbol/site/range、ordinal 1 / `BindingId(0)`、exact expansion 7 個を保持し、pairwise-distinct bare `ChainObjectMode6 -> ChainObjectMode5 -> ChainObjectMode4 -> ChainObjectMode3 -> ChainObjectMode2` link を直接検証する。残る tail は cycle-safe object-terminal normalization のみに使う。known entry 3 個は BaseObjectModeDef-RHS `BuiltinObject` identity 1 個へ normalize し、inferred variable 1 個と object/set coercion なしの constraint/candidate/fact/diagnostic/deferred 0 個の checked type assertion 1 個を生成する。全5,039 nonidentity order、finite structural/provenance/corruption coverage、先行 owner 53 件、immutable output、focused sibling、real sidecar が route を保護する。focused、relevant-crate、workspace verification は成功した。generic reachability、set-terminal/broader chain、proof/CoreIr/ControlFlowIr/VC、downstream payload は deferred のままとし、checker source/module-layout change は不要であった。

## task 230 active exact seven-expansion set-terminal formula-side five-hop local-mode asserted-head checker bridge

task 230 は ordered bare definition 7 個 `BaseMode -> set`、`ChainMode1 -> BaseMode` から `ChainMode6 -> ChainMode5`、`ChainMode6` reserve 1 個、`LongLocalModeFiveHopAssertedHeadPayloadBoundary: x is ChainMode1` だけを対象とする。Task 74 は real AST-derived set expansion、Task 199 は real formula/checker consumer を供給する。新規 closed `BindingFiveHopRadix` が written link 5 本を直接検証し、Tasks 224/226/228 は shorter-distance guard、Task 229 は latest terminal sibling とする。

active route は distinct `ChainMode6` subject/`ChainMode1` asserted symbol/site/range、ordinal 1 / `BindingId(0)`、exact expansion 7 個を保持し、pairwise-distinct bare `ChainMode6 -> ChainMode5 -> ChainMode4 -> ChainMode3 -> ChainMode2 -> ChainMode1` link を直接検証する。`ChainMode1 -> BaseMode -> set` は cycle-safe terminal normalization のみに使う。known entry 3 個は BaseModeDef-RHS `BuiltinSet` identity 1 個へ normalize し、inferred variable 1 個と constraint/candidate/fact/diagnostic/deferred 0 個の checked type assertion 1 個を生成する。全5,039 nonidentity order、finite structural/provenance/corruption coverage、先行 owner 54 件、immutable output、focused sibling、real sidecar で route を保護する。focused、relevant-crate、workspace verification は成功した。generic reachability、object-terminal five-hop、broader chain、proof/CoreIr/ControlFlowIr/VC、downstream payload は deferred のままとし、checker source/module-layout change は不要であった。

## task 231 active exact seven-expansion object-terminal formula-side five-hop local-mode asserted-head checker bridge

task 231 は ordered bare definition 7 個 `BaseObjectMode -> object`、`ChainObjectMode1 -> BaseObjectMode` から `ChainObjectMode6 -> ChainObjectMode5`、`ChainObjectMode6` reserve 1 個、`LongLocalObjectModeFiveHopAssertedHeadPayloadBoundary: x is ChainObjectMode1` だけを対象とする。Task 74 は real AST-derived object expansion、Task 200 は real formula/checker consumer、Task 230 は byte-for-byte unchanged closed `BindingFiveHopRadix` を供給する。Task 229 は shorter-distance object sibling、Task 230 は depth-matched set sibling とする。

active route は distinct `ChainObjectMode6` subject/`ChainObjectMode1` asserted symbol/site/range、ordinal 1 / `BindingId(0)`、exact expansion 7 個を保持し、pairwise-distinct bare `ChainObjectMode6 -> ChainObjectMode5 -> ChainObjectMode4 -> ChainObjectMode3 -> ChainObjectMode2 -> ChainObjectMode1` link を直接検証する。`ChainObjectMode1 -> BaseObjectMode -> object` は cycle-safe terminal normalization のみに使う。known entry 3 個は BaseObjectModeDef-RHS `BuiltinObject` identity 1 個へ normalize し、inferred variable 1 個と object/set coercion なしの constraint/candidate/fact/diagnostic/deferred 0 個の checked type assertion 1 個を生成する。全5,039 nonidentity order、finite structural/provenance/corruption coverage、先行 owner 55 件、immutable output、focused sibling、real sidecar で route を保護する。focused、relevant-crate、workspace verification は成功した。generic reachability、imported-positive definition、broader chain、proof/CoreIr/ControlFlowIr/VC、downstream payload は deferred のままとし、checker source/module-layout change は不要であった。

## task 233 active exact transparent parenthesized builtin-object reserved-variable equality checker bridge

task 233 は `reserve x for object;` と、それに続く `ParenthesizedReservedObjectVariableEqualityPayloadBoundary: (x) = x;` だけを対象とする。Chapter 13 は real unrecovered `ParenthesizedTerm` が inner identifier の型を保持し、FOL meaning を追加しないことを要求する。Task 223 は exact single-left-wrapper producer、Task 188 は real object reserve extraction、`BindingEnv`、builtin-object projection、equality consumption を供給する。active implementation は private parenthesized extraction/output assertion path だけを exact static config で parameterize する。Task 223 は object reserve を、Task 188 は parenthesized operand を引き続き reject し、新 public route は両者の exact intersection だけを所有する。

route は独立 wrapper/inner-reference/direct-right site/range を保持し、二つの `x` use を source-order ordinal 1/2 で `BindingId(0)` へ解決し、canonical `BuiltinObject` identity 1 個を inferred variable 2 個、ordered expected constraint 2 個、fact/candidate/diagnostic/deferred 0 個の checked equality 1 個へ透明に再利用する。独立 wrapper type/value、object/set coercion、axiom、fact、FOL node、child payload は作らない。finite contract は non-exact wrapper/reserve/theorem/formula shape を reject し、provenance/lookup/type/matched-output corruption、immutable output、先行 binary-formula owner 53 件との bidirectional isolation、real frontend/resolver sidecar を検証する。arbitrary parenthesis/operand/precedence、formula grouping、closure/order materialization、equality truth/fact、theorem acceptance、proof/CoreIr/ControlFlowIr/VC、child graph、broader semantics は deferred。checker source/module-layout change は不要であった。

## task 234 active exact seven-expansion set-terminal formula-side six-hop local-mode asserted-head checker bridge

task 234 は ordered bare definition 7 個 `BaseMode -> set` から `ChainMode6 -> ChainMode5`、`ChainMode6` reserve 1 個、`LongLocalModeSixHopAssertedHeadPayloadBoundary: x is BaseMode` だけを対象とする。specs 03/04/07/13/14/16 は mode unfolding、reserved theorem-variable typing、asserted type の static admissibility を直接要求する。Task 74 は real AST-derived expansion 7 個すべて、Task 199 は real reserve/BindingEnv/type-assertion checker consumer を供給する。

active implementation は pairwise-distinct bare link 6 本 `ChainMode6 -> ChainMode5 -> ChainMode4 -> ChainMode3 -> ChainMode2 -> ChainMode1 -> BaseMode` を直接検証する closed `BindingSixHopRadix` relation を追加し、`BaseMode -> set` は terminal-normalization evidence のみに使う。distinct raw subject/asserted symbol/site/range、ordinal 1 の `BindingId(0)`、BaseModeDef-RHS `BuiltinSet` 1 個へ normalize する known type entry 3 個、inferred variable 1 個、constraint/fact/candidate/diagnostic/deferred 0 個の checked assertion 1 個を保持する。contract は全5,039 nonidentity expansion order、finite structural/provenance/removal/corruption case、immutable output、先行 owner route 56 件との bidirectional isolation、focused sibling、real frontend/resolver sidecar を検証する。generic reachability/widening/`qua`、object-terminal/imported-positive/attributed/argument-bearing head、acceptance、truth/fact、child graph、proof/CoreIr/ControlFlowIr/VC、general chain semantics は deferred。checker source/API/module-layout change は不要であった。
...
Task 267はTask 268にliteral proof-intent valueをtrustさせず、
`CheckedStatementOwner`を拡張させる。`validate_exact_local_theorem`はexact
resolver `SymbolEntry`が`Visibility::Public`かつ`ExportStatus::Exported`で、
theorem definitionがalready validated origin/contributionとmatching public
visibilityを持つことを独立validationし、2 resolver factを
`CheckedStatementOwner`に保持する。Public/exportedはname-visibility propertyで
ありtheorem acceptanceではない。
Task 268はstored fieldをprivateに保ち、public `visibility()`/
`export_status()` getterを公開する。

Task-268 proof bundleはauthenticated ownerをreferenceで受ける。
`ResolvedTypedAst` assemblyはproof tableをpublishする前にproof-intent rowのowner
visibility/export statusをpreserved owner fieldと比較する。testはauthenticated
...
`TermFormulaChecker::check_source_predicate_statements(&SurfaceResolvedArena, &SymbolEnv, &TypedArena)` は local な binary builtin-set predicate と theorem header・proof-local の両 call を認証し、既存の `(BindingEnv, TermFormulaInferenceOutput, TypedAst)` を返す。
source/environment を再照合し、実 declaration・順序付き loci・等式または否定等式の本体・別個の量化/proof binding を検査する。既存 inference で実引数型と期待型が Known で同じ正規化済み identity を持つことを確定し、open predicate candidate を完全な検査結果とは扱わない。
返却 AST は既存 primary/atomic handoff を install し、各 call に同じ実 predicate identity と各自の positive または `does not` polarity site を保持する。本体の否定で call の否定を相殺せず、header/proof の一致や theorem の真偽は検証しない。
この直接操作だけが限定 statement-check 結果を与え、transport request は未解決データのままである。unfolding・fact・証明受理・Task274・Core/VC・新たな source-family install authority は与えない。
...
`check_source_attribute_widening_types(&SurfaceResolvedArena, &TypedArena, &SymbolEnv, &RegistrationDatabase)` は既存 `(ExistentialGateOutput, CoercionCheckingOutput, OverloadCollectionOutput, CandidateViabilityOutput)` を `Result<_, String>` で返し、新 payload・proof receipt は追加しない。
根拠は Chapters 3 §§3.5/3.7、6 §§6.1–2/6.8、17 §§17.1/17.3.4。local attribute/registration/functor/theorem 全体、中立 node・range・recovery、実 identity body/result/coherence wrapper と citation を認証する。同名の四宣言は resolver の異なる binding identity で解決する。
database は確立済み typing premise であり、自身の kernel authenticity を証明しない。runner は同じ不変 source/nodes/symbols で [proof status](../../mizar-proof/ja/status.md#source-existential-registration-proof) を fresh 実行した直後に本 API を呼ぶ。外部・再利用・捏造 database を経路の権威にせず、構造的に foreign/pending/mismatched な入力も拒否する。
実際の quantifier/proof-let 両 attributed type site で attribute SymbolId と builtin-set の全 pattern を導き、登録 block 完了後の一致する activated entry を要求する。両 gate はその entry を伴う Satisfied とし、bare-set base evidence で代用せず、定理と証明内の context/binding を分離する。
