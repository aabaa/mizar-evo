...
boxed optional `source_proof_local_given_condition_use_term`、getter、consuming
one-shot installer、`InvalidSourceProofLocalGivenConditionUseTerm`だけを追加。
complete GCT、2-term/2-reference fingerprint、6-node arena、slotの順にvalidateし、
generic/all proof-local siblingとboth-order mutual exclusion、atomic rollback。
Displayは`source proof-local given-condition-use term handoff is invalid`。
nodes/context/type/fact/coercion/initial obligation/diagnostic/`TypedAstParts`
とgeneric source-termは不変。
...
`TypedAst` field/public API/node/tableを変更しない。future descendant ownerは
別frozen contractとmutual-exclusion matrixを必要とする。
...
## Task 269SDT Typed Ownership

[central Task-269SDT contract](../../task_contracts/ja/269SDT.md) の下で、
`TypedAst` はboxed optional `source_proof_local_given_descendant_type`、exact
getter
...
を追加する。errorは `TypedAstError::InvalidSourceProofLocalGivenDescendantType`、
displayは `source proof-local given-descendant type handoff is invalid`。

SDT ownerはSDCをby valueで含み、standalone SDCまたは先行10 proof-local
owners、すなわちdeclaration、Let binding/type、Given binding/type、Given-use
type/term、Given-condition binding/type/use-termと両install順でcoexistできない。
全current generic/source installerはreciprocal availability guardを持つ。
