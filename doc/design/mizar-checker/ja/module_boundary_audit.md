...
| `src/source_proof_local_declaration.rs` | 4188 | syntax-free proof-local declaration producer | `source_proof_local_declaration.md` | no | no | cohesiveなTask-269A/B named-witness、Task-269C proof-`let`、Task-269G proof-`given` binding input/row/table/handoff、exact lower authentication、resolver-local lexical binding transition/ordinal lookup replay、deterministic rendering、phase-ordered replay、owner validation。syntax/type/condition/fact/proof semantic ownershipなし。 |
