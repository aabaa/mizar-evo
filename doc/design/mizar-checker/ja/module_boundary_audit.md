...
| `src/typed_ast.rs` | 5752 | typed AST data model | `typed_ast.md` | no | no | mutually exclusive Task-259--264 definition transactionとseparate Task-269A/B、private boxed Task-269C/269CT proof-local ownerを含むcohesive typed-AST table/validation/rendering/one-shot handoff。 |
| `src/binding_env.rs` | 3166 | binding environment and resolver shell boundary | `binding_env.md` | no | no | source-formula、Task-258B1 statement-context identity、Task-258B2がreuseするunchanged context contract、cfg(test)-only Task-269A installed-local corruption seamを含むcohesiveなbinding/context data layer。behavior-neutral splitは不要。 |
...
| `src/source_term.rs` | 2263 | syntax-free source primary-term producer | `source_term.md` | no | no | cohesiveなTask-252 term/reference/request table、binding/parent validation、deterministic rendering、Task-258A dependency revalidation、cfg(test)-only generic Task264 raw-term corruption seam。split不要。 |
...
| `src/source_proof_local_declaration.rs` | 2827 | syntax-free proof-local declaration producer | `source_proof_local_declaration.md` | no | no | cohesiveなTask-269A/B named-witnessとTask-269C proof-`let` binding input/row/table/handoff、exact lower authentication、resolver-local binding transition/ordinal lookup replay、deterministic rendering、phase-ordered replay、owner validation。syntax/proof semantic ownershipなし。 |
...
| `src/source_type.rs` | 8517 | syntax-free source-type application producer | `source_type.md` | no | no | cohesiveなTask-249 flat/extension family、Task-269CT exact proof-local composite、environment/arena/form/graph/provenance validation、deterministic rendering、exhaustive corruption test、cfg(test)-only Task-263 lower-corruption seam。split不要。 |
...
| `src/resolved_typed_ast.rs` | 7939 | final resolved typed AST assembly | `resolved_typed_ast.md` | no | no | Task-259--264 definition handoffのclone-preserveとmutually exclusiveなTask-269A/B、private boxed Task-269C/269CT complete proof-local replayを含むcohesive final projection。 |
