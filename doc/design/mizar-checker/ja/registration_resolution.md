...
Core の目標生成、証明、受理状態の伝達は別の責務に留める。
直接の構文照合はこの認証済みソース入口に限定し、既存の payload 消費側の境界は維持する。
この限定経路はソースごとに登録 1 件、ブロックごとに set 引数を高々 1 個まで対象とする。
簡約項では透過的な括弧と、実際の要素を型検査した単元集合の列挙も扱う。
列挙の型は builtin set とし、括弧は意味上のサイズに含めず、単元集合は 1 ノード分を加える。
...
task 47 は同じ explicit-payload seam を spec §17.3.4 の unattributed base-shape
inhabitation に拡張する。`ExistentialGateBaseEvidence` record は、evidence pattern と
normalized pattern が完全に一致する unattributed gate だけを満たせる。built-in `object` と
`set` は `Builtin` coverage と guard なしで使う。accepted mode application と bare schema
type parameter は `CompleteGuardSet` coverage と少なくとも 1 つの visible consumable guard
fact を要求し、accepted mode obligation または schema-context assumption を明示する。
structure constructor witness は guard なしの `ZeroFieldStructure`、または 1 つ以上の
field-inhabitation guard を持つ `CompleteGuardSet` のどちらかを使う。空の complete guard set は
nonzero structure について invalid である。base evidence は requested attribute を持つ gate を
満たせない。attributed type は full attributed pattern に対する activated existential
registration を引き続き要求する。
...
2. valid candidate がすべての required guard evidence を持つか、valid base evidence が
   すべての required guard evidence を持つなら `Satisfied`;
3. valid candidate または valid base evidence はあるが guard が missing、invisible、または
   non-consumable なら `BlockedGuard`;
4. candidate または base evidence はあるが valid なものがないなら `InvalidCandidate`;
5. candidate も base evidence も存在しなければ `MissingExistential`。

verified downstream fact を seed してよいのは `Satisfied` gate だけである。
base-evidence で satisfied になった gate は checker registration id を持たないが、base evidence
kind / coverage と、必要な場合は visible guard fact id を記録する。missing、blocked-guard、
invalid-candidate、degraded-recovery gate は checker-local diagnostic を発行し、後続 diagnostic
用の degraded output だけを保持してよいが、inhabited metadata を export したり verified fact を
seed したりしてはならない。
...
task 47 では source-derived base-shape execution も deferred のままにする。real source payload
から accepted mode tuple、完全な structure-field coverage、schema-context assumption、activation
item order を証明できる source-to-checker extraction が必要である。現在の実装は explicit
checker-owned base evidence だけを消費し、type spelling や resolver opaque shell から不足 guard
を推論してはならない。
...
spec §17.7.1 の制限された no-argument cluster adjective grammar が closure の
language-level termination argument である。task-17 の configured saturation bound は防御的な
diagnostic と cache-key input であり、成功した truncated fixed point の export を許可するもの
ではない。矛盾する derived attribute は registration-resolution diagnostic であり、一方を捨てて
黙って調停してはならない。spec §17.7.3 は closure-time contradiction を fatal かつ
non-recoverable な soundness-boundary failure にする。checker は contextual diagnostic を
報告してよいが、影響を受けた fact を export 可能な partial output へ degrade して続行しては
ならない。現在の checker-local diagnostic class は、MC-G005 が public checker diagnostic code
を割り当てるまで `cluster_contradiction` のままである。public diagnostic contract はこの family
を Chapter 22 の cluster-inconsistency code へ対応づけ、別の semantic condition を作っては
ならない。
...
task 46 は checker-owned explicit-payload trace layer をこの signature に合わせる。replay は
stable な type、attribute、`such` evidence を要求し、discharged side-condition set は記録済み
reduction trace identity の一部であり、`such` evidence は strategy-audit selection key から
除外されたままである。source-derived normalization result、rule search、guard extraction は、
実 source payload と active advanced-semantics runner が存在するまで
MC-G020/MC-G021/MC-G023/MC-G030 の下で deferred のままとする。
...
| `ExistentialGateBaseEvidenceKind` | 前方互換; source extraction が追加の type shape を公開すると base-shape inhabitation evidence kind は増える可能性がある。 |
| `ExistentialGateBaseEvidenceCoverage` | 前方互換; structure / mode payload が豊かになると base-evidence coverage category は増える可能性がある。 |
...
## Narrower-predicate source-proof profile

単一の局所等式属性・登録に通常の二項 predicate と narrower-loci 再定義が続く source 全体の replay を拡張する。後続両宣言・formal 参照・属性付き head・coherence/proof owner を認証し、ここでは root 選択や coherence 証明を行わない。既存の単一登録 seal は実際の属性依存、記述された二つの bare-set choice、独立 builtin gate のみを保持し、切り出した source・後続 predicate の前提・新たな accepted 入力を登録証明に使わない。
