...
`TypedAst` はboxed optional `source_proof_local_given_descendant_type`、
exact getter `source_proof_local_given_descendant_type() -> Option<&SourceProofLocalGivenDescendantTypeHandoff>`、
consuming installer `with_source_proof_local_given_descendant_type(...) -> Result<Self, TypedAstError>`
を追加する。errorは
`TypedAstError::InvalidSourceProofLocalGivenDescendantType`、displayは
`source proof-local given-descendant type handoff is invalid`。
`ResolvedTypedAst` は同じgetterとbuilder-finish replayを追加し、
`ResolvedTypedAstError::InvalidSourceProofLocalGivenDescendantType` のdisplayは
`resolved typed AST source proof-local given-descendant type handoff is invalid`。
separately replaceableなResolved inputはない。

SDTはSDC dependencyをby valueで所有し、standalone SDC slotをco-installしない。
Typed/Resolvedは両順序でexact 11 predecessors、すなわちproof-local
declaration、Let binding/type、Given binding/type、Given-use type/term、
Given-condition binding/type/use-term、Given-descendant bindingとのcoexistenceを
rejectする。全current generic/source ownerもreciprocal availability guardを
得る。failureはpre-call debug bytes、owner state、3 arena nodes、emptyな
context/type/fact/coercion/initial-obligation/diagnostic tablesを保持する。
