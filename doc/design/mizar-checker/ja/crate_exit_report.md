...
task 48-84 は crate plan、TODO、source/spec audit、corpus traceability で追跡する
post-closeout continuation task である。task 74 は task 73 の three-edge bridge の後に
AST-bounded structural bare local-mode chain pass slice を追加し、task 75 は
forward local-mode reserve head の lower-stage active-range boundary を追加し、task 76 は
forward local-structure reserve head の対応する boundary を追加し、task 77 は
forward local-attribute reserve type expression の対応する boundary を追加する。task
78 は imported structure reserve-head extraction-gap boundary を追加し、task 79 は
matching imported mode reserve-head extraction-gap boundary を追加し、task 80 は
matching imported attribute reserve extraction-gap boundary を追加する。task 81 は
argument-bearing local-attribute reserve extraction-gap boundary と
declaration-symbol suffix projection を追加し、task 82 は documented imported mode
head を checker type-head provenance bridge へ昇格し、task 83 は documented
imported structure `R` だけを対応する provenance/type-head bridge へ昇格し、
task 84 は documented imported attribute `TypeCaseAttr` だけを builtin `set`
上の対応する provenance / attribute-payload bridge へ昇格する。これらは
original score を変更せず、imported module AST extraction、positive imported
structure/mode/attribute elaboration、AST-wide extraction、
CoreIr/ControlFlowIr/VC、proof payload promotion を主張しない。
