...
| SSA-007 | medium | 17.10/3.3 | task 41 で解決済み: cluster 停止性は引数なしの制限 adjective 文法に依存 |
| SSA-008 | medium | 17.7.3 | task 41 で解決済み: 矛盾する derived attribute は closure 時 fatal cluster 診断 |
| SSA-009 | medium | 17.6.4 | task 42 で解決済み: reduce 決定性シグネチャに解消済み side-condition 集合を含める |
...
| SSA-013 | medium | 7.8.1 | task 43 で解決済み: parameterized `sethood` は正規化済み argument tuple ごと |
| SSA-014 | medium | 7.8/17.3.4 | task 43 で解決済み: inhabitation table が無属性基底と組み込み型を覆う |
| SSA-015 | medium | 8.2 | task 44 で解決済み: justification 省略 `reconsider` は proof-free widening、inheritance/view、cluster-closure、local fact のみ |
| SSA-016 | low | 19.2.3 | task 37 で解決済み: 特異度は閉包同値で商を取る前は前順序 |
| SSA-017 | low | 6.7/19.4.1 | task 44 で解決済み: 候補複数の `coherence with` 省略は ambiguous redefinition target |
| SSA-018 | low | 19.6.4 | 貪欲 `of`/`over` 構文解析がスコープ内アリティ集合に依存 |
| SSA-019 | low | 19.6.1 | task 37 で解決済み: 重複した導入文を削除 |
| SSA-020 | medium | 3.3/6.2 | task 41 で解決済み: `attr(args)` は use-site application 形式であり cluster adjective ではない |
...
### SSA-007 (medium, 解決済み `spec_gap`) — cluster 閉包の停止性が adjective 文法に依存
...
**解決(task 41):** §17.7.1 はこれを load-bearing な停止性前提として明記した。
cluster closure は引数なしの制限 `adjective` 文法を使い、term 引数を合成しない。
将来 cluster adjective を任意の term 引数へ拡張する場合は、新しい停止性論証が必要である。
spec 19.2.1 はこの前提を参照し、architecture 04 は saturation bound を成功した
truncated semantics ではなく防御的な failure diagnostic として扱う。

### SSA-008 (medium, 解決済み `spec_gap`) — 矛盾する導出属性はどこで検出されるか
...
**解決(task 41):** §17.7.3 は closure 時検出を fatal な `cluster` 診断として
規定した。1 つの registration の正規化済み consequent が同じ subject 上に
`A` と `non A` を置く static case も含む。残余の ATP 可視な不整合は後段の
proof-system failure であり、cluster-resolution trigger ではない。
inactive `fail_cluster_contradictory_consequent_001` seed はこの closure 時 fatal
diagnostic に対応する。

### SSA-009 (medium, 解決済み `spec_gap`) — reduce の決定性対 `such` 側条件
...
**解決(task 42):** §17.6.4 は normalization を、項、スコープ内の activated
reduction rule、rewrite site で利用可能な解消済み side-condition 集合の
決定的関数として規定する。`such` side condition は安定した local fact、trace、
citation evidence から読む applicability guard であり、specificity の入力では
ない。rule selection は pattern-first、guard-second である。厳密な `LHS`
pattern subsumption は guard 比較の前に勝者となり、それ以外では対応する
pattern-variable の type/attribute guard を §19.2.3 preorder で位置ごとに
比較する。対応欠落、mixed winner、同等または比較不能な一致は
lexicographically smallest rule FQN へ落ちる。checker
`registration_resolution.md` の reduction 節も同じ規則を反映し、task-18 の
`such` guard は applicability-only のままとした。
...
### SSA-013 (medium, 解決済み `spec_gap`) — 依存 mode の `sethood`
...
**解決(task 43):** §7.8.1 は非パラメータ形とパラメータ化 `sethood`
義務を両方与える。parameterized sethood は宣言済み parameter guard で
guard され、§13.4.2 の comprehension generator が使う同じ resolved mode と
正規化済み argument tuple で検査される。witness set は parameter に依存して
よい。accepted public/exported sethood は module-interface semantic property
fact だが、具体的な witness term は使用可能な値として export されない。
既存 inactive comprehension seed が sethood evidence 欠落の negative coverage のまま残る。

### SSA-014 (medium, 解決済み `spec_gap`) — 無属性基底型と組み込み型の存在要件
...
**解決(task 43):** §7.8 はすべての mode 宣言の正規化済み RHS 全体に
inhabitation evidence を要求し、parameterized mode には parameter guard
付きの存在義務を与える。§17.3.4 は、可視 active existential registration を
要求する属性付き型式と、built-in/base-shape table を使う無属性基底を区別する。
表は `object`、`set`、accepted mode application、bare structure radix、template
本体内の bare schema type parameter を含む。bare-structure 行は無条件の
inhabitation ではなく constructor-witness rule である: parameter substitution、
inheritance、default を考慮した後、必要な field 引数型すべてが inhabitation
evidence を持たなければならない。property は除外し、field なし structure は
自明に inhabited である。schema-parameter 行は §18.10.2 の schema-context 仮定に
支えられる。§18 の template actual も同じ表へ委譲し、F2 の inhabitation gate を
維持する。

### SSA-015 (medium, 解決済み `spec_gap`) — 正当化なしの `reconsider`

**該当箇所:** 8.2 EBNF(「完全に省略可」), 8.2.2。

task 44 以前、justification 省略時に narrowing 義務が cluster closure のみで
解消されるのか、ATP に送られるのか、エラーなのかを仕様は述べていなかった。

**解決(task 44):** 省略形は構文上は受理可能だが、各 target-type 義務が、その
source point で利用可能な proof-free widening、inheritance、cluster-closure、
または既に記録済みの local type fact だけで discharge できる場合に限り合法で
ある。省略形は proof search を開始せず、暗黙の `by` step も作らない。いずれか
の item が新しい proof を必要とする場合、checker は
`type.narrowing_requires_proof` を報告する。spec 04、08、15、Appendix A、22 は
同じ grammar/semantic split を持ち、Chapter 15/Appendix A は以前から記述されて
いた proof-block `reconsider` form を `reconsider_tail` で明示する。inactive seed
`fail_types_reconsider_omitted_justification_001` が missing-proof negative case を
固定する。省略構文と proof-block `reconsider` をまだ拒否する既存 parser 挙動は、
parser task 47 の `source_drift` / `test_expectation_drift` として記録する。
...
### SSA-017 (low, 解決済み `spec_gap`) — 曖昧な `coherence with` 省略

task 44 以前、§19.4.1 は `coherence with` なしの `redefine` を「シグネチャを
研ぎ澄ます唯一の先行定義」に割り当てていた。複数該当する場合のエラー名・挙動は
未規定だった。

**解決(task 44):** `coherence with` 省略時の target inference は、use-site の
overload 解決より前、宣言検査時に行う。同じ symbol kind と arity を持つ可視の
先行 ordinary root のうち、redefinition signature によって厳密に精密化される
ものがちょうど 1 つの場合だけ成功する。複数 root が該当する場合、checker は
`resolve.ambiguous_redefinition_target` を報告し、宣言順、import 順、return type
で選んではならない。inactive seed
`fail_overload_redefine_ambiguous_target_001` がこの negative case を固定し、
advanced-semantics runner と source-to-checker redefinition payload が存在するまで
inactive のままとする。
...
### SSA-020 (medium, 解決済み `spec_gap`) — `attr(args)` は使用可能だが宣言不能
...
task 41 は欠けていた扱いを定義した。`attr_pattern` は唯一の宣言形式のままであり、
`attribute_name(args)` は matching arity を持つ宣言済み parameterized attribute の
use-site positional application である。cluster registration adjective は意図的に
引数なしの制限 subset を使う。argument-bearing registration adjective の active parser
recovery seed は `spec.en.17.clusters.restricted_adjective_grammar.parser` を trace する。
...
seed を追加する。task 39 は property-overlap coherence seed を追加し、task 40 は
non-retroactive activation seed を新しい traceability row の下で保持する。
tasks 41-43 の contradiction / reduction / sethood seed は source-derived payload
support が存在するまで inactive のままにし、task 44 は justification 省略
`reconsider` と `coherence with` 省略時の ambiguous target seed を追加する。
...
| `fail/types/fail_types_reconsider_omitted_justification_001` | justification 省略 `reconsider` は proof-free evidence のみを使う | 8.2.2, 15.5.1 |
...
| `fail/overload/fail_overload_redefine_ambiguous_target_001` | `coherence with` 省略は複数の精密化 root を拒否する | 19.4.1, 19.5 |
...
- **解決済み仕様タスク:** SSA-001(constructor/extensionality) は task 35 で
  ...
  traceability row を含む。SSA-007、SSA-008、SSA-020 は task 41 で解決済み。
  spec 03/06/17/19/Appendix A の同期編集、architecture 04 の saturation wording、
  closure-time contradiction seed、restricted-adjective parser trace row を含む。
  SSA-009 は task 42 で解決済み。spec 17 の reduction determinism を同期編集した。
  SSA-013 と SSA-014 は task 43 で解決済み。spec 07/13/17/18 の同期編集、
  sethood/existential trace note 更新、既存 reject-first corpus seed の inactive
  維持を含む。SSA-015 と SSA-017 は task 44 で解決済み。spec
  04/08/15/19/22 と Appendix A の同期編集、justification 省略 `reconsider` と
  `coherence with` 省略時の ambiguous target inference に対する inactive
  reject-first seed を含む。
- **残りの仕様タスク(checker 意味論の続行前):** Step 2 SSA decision wave には
  残りなし。checker/core implementation task は source-derived payload gap を
  先取りせず、これらの決定を Step 4 で消費できる。
- **Task 19/20(registration ゲート、existential ゲート):** source-derived
  accepted status と payload が存在するようになったら、task 43 の
  built-in/base-shape inhabitation table に照らして再訪する。SSA-006 の
  活性化契約は task 40 で、task-19 activation policy を暫定的な保守近似として
  記録した。
- **Task 16-18(閉包、ループ、reduce):** task 41 は SSA-007 の文法基準の
  停止性論証と SSA-008 の closure-time contradiction rule を、既存の
  `cluster_trace.md`/`registration_resolution.md` behavior と同期した。task 42 は
  reduce 決定性を、task-18 の explicit discharged-guard evidence と
  applicability-only の `such` side condition に同期した。
...
canonical Given block scopeはowning blockの全unshadowed descendantでouter
bindingをvisibleにする。exact SDP/SDC production transactionがmaterialize
するのはone child contextとone outer Given bindingで、synthetic matrixは
second same-proof childもinheritすることを証明する。SDCはwitnessの
existential/Skolem meaning、
written type guard、child RHS occurrence/capture、Set equality/substitution、
conclusion/proof/obligationを全てzero meaningとする。Ch.4/15 Set差が
nonblockingなのは全効果をexcludeするためで、`z`/`q` row追加はauthority-free
lower expansionかつboundary violation。
