...
| Id | 重大度 | 領域 | 概要 |
|---|---|---|---|
| SSA-001 | critical | 5.5/5.8 | コンストラクタの property 引数とフィールド限定の外延性が論理を崩壊させる |
| SSA-002 | high | 5.3/5.4 | ダイアモンドのメンバー同一性が名前基準だがリネームで名前照合が壊れる |
...
| SSA-011 | medium | 5.4 vs 19.2.2 | 整合なダイアモンドでも暗黙アップキャストが遮断; 「path」対「embedding」が未定義 |
| SSA-012 | medium | 5.3 | 継承の非循環性が前提(13.8.7)なのに checker 規則として明文化されていない |
...
### SSA-001 (critical, `spec_gap`) — コンストラクタの property 引数対フィールド限定外延性

**該当箇所:** 05.structures.md §5.5.1, §5.8.4, §5.8.5; 07.modes.md §7.4.1。

§5.5.1 はデフォルトコンストラクタに **property** 値の供給を許し
(`OneStr(carrier: A, one: b)`)、§5.8.4 はその射影公理
(`one(Agg_OneStr(A, b)) = b`)を生成する。§5.8.5 の外延性は**フィールドのみ**
を対象とする。両者は不整合である: property 型の任意の `b1, b2` について
`Agg_OneStr(A, b1)` と `Agg_OneStr(A, b2)` は全フィールドで一致するため外延性
が両者の同一性を強制し、射影公理から `b1 = b2` が導かれる — すべてのキャリア
が高々 1 要素に崩壊し、偽の命題が証明可能になる。また property のコンストラ
クタ引数は §7.4.1 の `means`/`equals` 実装と、値の供給源として調停なく競合
する。
...
**解決案:** メンバー同一性を **`from` チェーンで到達する根宣言**で定義し、
子メンバー型は各親メンバー型に対し `⊑` であることを要求、構文的に同一で
ない親ごとに coherence 義務を 1 件課す。起源の異なる名前衝突はリネームで
分離しない限りエラーとする。

**コーパス:** `fail_structure_diamond_member_type_conflict_001`、
`fail_structure_inherit_uncovered_member_001` が既規定の拒否ケースを固定。
...
仕様が明言すべきである。現行文は構文的を含意しており実装可能だが、§5.4 と
並べると意外であり、選択の記録が必要。

**コーパス:** `fail_overload_inheritance_path_ambiguity_001` が規定済みの
構文的挙動を test-first で固定。
...
extends A;` を禁止しておらず診断名もない。**解決案:** §5.3 に明示的な非循環
要件と診断を追加。**コーパス:** `fail_structure_inherit_cycle_001`。
...
| `fail/structures/fail_structure_diamond_member_type_conflict_001` | ダイアモンドのメンバー型衝突 | 5.3.1, 5.4 |
...
新規 traceability 要件: `spec.en.05.structures.inheritance.semantic`、
...
- **新規仕様タスク(checker 意味論の続行前):** SSA-001(コンストラクタ/
  外延性)、SSA-002+SSA-011+SSA-012(継承の同一性・パス一意性・非循環性)、
  SSA-003(テンプレートタイブレーク)、SSA-004(functorial `for` の意味論)、
  SSA-005(property 実装の coherence)に各 1 タスク。いずれも
  `doc/spec/en/` + `ja/` を同一変更で更新。
