...
- **残りの仕様タスク(checker 意味論の続行前):** SSA-009、
  SSA-013、SSA-014、SSA-015、SSA-017 が tasks 42-44 に残る。
...
- **Task 16-18(閉包、ループ、reduce):** task 41 は SSA-007 の文法基準の
  ...
  `cluster_trace.md`/`registration_resolution.md` behavior と同期した。
  reduce の決定性には引き続き SSA-009 の修正済み関数シグネチャが必要。
