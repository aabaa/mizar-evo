...
一覧を持つ。この決定は `mizar-core` task 26 / F7 と整合するが、それを
完了させない。省略テンプレート引数の推論は Phase A の source payload task
のままであり、欠落 payload から推測してはならない。
...
- **解決済み仕様タスク:** SSA-001(constructor/extensionality) は task 35 で
  ...
  same-signature corpus seed を含む。
- **残りの仕様タスク(checker 意味論の続行前):** SSA-004(functorial `for` の
  意味論)と SSA-005(property 実装の coherence)に各 1 タスク。いずれも
...
- **Task 23-26(テンプレート、viability、選択):** task 37 は Phase B の
  ...
  比較 evidence を推測してはならず、`mizar-core` task 26 / F7 は Phase A の
  省略テンプレート引数推論の決定性を引き続き所有する。
- **Task 29 コーパス記録:** deferred の advanced_semantics コーパス要件
  ...
  すべきである。task 37 が追加した overload ambiguity row と deferred
  same-return declaration-conflict row も含める。
