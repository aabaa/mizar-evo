...
checker-local template expansion data layer を実装する。tasks 24-26 が残りの named section を
実装する。task 28 は後で `resolved_typed_ast.md` が仕様化する final `ResolvedTypedAst`
data shape を組み立てる。
...
task 23 は以下を意図的に deferred のままにする。

- tasks 24-26 の Rust implementation;
