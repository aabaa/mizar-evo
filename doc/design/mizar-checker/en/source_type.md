...
This is the durable derived checker ABI for the
...
The common source-type input has exactly two dense application/expression
rows: reserve `set@14..17` at binding/ordinal/root `0/0/0`, and Given
`set@95..98` at `1/1/1`. Both use normal bare `BuiltinSet` heads with
expression/head roles on typed nodes 0 and 1. Arguments and all extension
tables are empty.
...
The composite owns the SDC dependency by value and cannot co-install its
standalone slot. Atomic publication and reciprocal exclusion are owned by the
[Typed](./typed_ast.md#task-269sdt-typed-ownership) and
[Resolved](./resolved_typed_ast.md#task-269sdt-resolved-ownership) sections.
