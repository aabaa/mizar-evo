...
Task 191 adds only the exact distinct-binding shared-builtin-object
equality `reserve x, y for object; theorem
DistinctReservedObjectVariableEqualityPayloadBoundary: x = y;`. The
production route must compose the real one-item/two-binding shared-range
reserve handoff with the existing builtin-object binary-formula consumer,
resolve source-order uses at ordinals 2 and 3 to `BindingId(0)` and
...
`Checked` `Equality`. Exact/near-miss tests must reject another label, reversed,
same, or unknown operands, another operator, status/recovery/extra items,
single/extra/separate reserve bindings, and `set`, mode, structure, attributed,
argument-bearing, imported, or ambiguous reserve heads. Task-local corruption
probes must reject collapsed binding identity, ordinal drift, `BuiltinSet`
substitution, collapsed roles, wrong shared source projection, or a missing
expected input; positive count/status/constraint/canonical-source assertions
plus the shared binary-formula output validator must pin the immutable checker
tables. A real frontend/resolver sidecar must guard the active fixture. This is
type/well-formedness checking only: equality truth, object/set coercion, facts,
implicit closure/order, theorem acceptance, proof execution, and CoreIr/
...
Task 195 is limited to the exact three-edge set-terminal same-outer-mode
asserted-head source with four ordered definitions `Outer -> Middle -> Inner ->
Base -> set`, one reserve of `OuterThreeEdgeModeAssertedHead`, and
`ThreeEdgeLocalModeAssertedHeadPayloadBoundary: x is
OuterThreeEdgeModeAssertedHead;`. The production route composes Task 73's real
four-expansion AST handoff with Task 186's same-symbol formula-side asserted-
head consumer. It resolves ordinal 1 to `BindingId(0)`, preserves independent
raw reserve-subject and formula asserted-type sites/ranges for the outer
symbol, and consumes exactly four local bare argument/attribute/context/
recovery-free expansions. Three known type entries normalize to one
`BuiltinSet` identity canonically anchored at the base definition RHS. The
immutable result contains one `Inferred` variable, zero expected constraints,
zero candidates/facts/diagnostics/deferred reasons, and one normalized-
reflexive `Checked` `TypeAssertion`. Exact structural and provenance guards
reject shorter, deeper, object-terminal, malformed, unrelated-local, imported,
and ambiguous asserted-head shapes; independent corruption probes, positive
output checks, route isolation, and a real frontend/resolver sidecar guard
active runner 143. This does not establish mode declaration acceptance,
...
Task 197 is limited to the exact four-edge set-terminal same-outermost-mode
asserted-head source with five ordered definitions `TooDeep -> Outer -> Middle
-> Inner -> Base -> set`, one reserve of `TooDeepFourEdgeModeAssertedHead`, and
`FourEdgeLocalModeAssertedHeadPayloadBoundary: x is
TooDeepFourEdgeModeAssertedHead;`. The production route composes Tasks 74/152's
real five-expansion AST handoff with Tasks 186/195's same-symbol formula-side
asserted-head consumer. It resolves ordinal 1 to `BindingId(0)`, preserves
independent raw reserve-subject and formula asserted-type sites/ranges for the
outermost symbol, and consumes exactly five local bare argument/attribute/
context/recovery-free expansions. Three known type entries normalize to one
`BuiltinSet` identity canonically anchored at the base definition RHS. The
immutable result contains one `Inferred` variable, zero expected constraints,
zero candidates/facts/diagnostics/deferred reasons, and one normalized-
reflexive `Checked` `TypeAssertion`. Exact structural and provenance guards
reject shorter, deeper, reordered, object-terminal, malformed, unrelated-
local, imported, and ambiguous asserted-head shapes; independent
`BuiltinObject`/canonical-source corruption probes, positive output checks,
route isolation, and a real frontend/resolver sidecar guard active runner 145.
...
general chain semantics. No checker source or module-layout change was
required.

Task 198 is limited to the exact four-edge object-terminal same-outermost-mode
asserted-head source with five ordered definitions `TooDeep -> Outer -> Middle
-> Inner -> Base -> object`, one reserve of
`TooDeepFourEdgeObjectModeAssertedHead`, and
`FourEdgeLocalObjectModeAssertedHeadPayloadBoundary: x is
TooDeepFourEdgeObjectModeAssertedHead;`. The production route composes Tasks
74/153's real five-expansion AST handoff with Tasks 187/196's same-symbol
formula-side asserted-head consumer. It resolves ordinal 1 to `BindingId(0)`,
preserves independent raw reserve-subject and formula asserted-type sites and
ranges for the outermost symbol, and consumes exactly five local bare argument/
attribute/context/recovery-free expansions. Three known type entries normalize
to one `BuiltinObject` identity canonically anchored at the base definition
RHS. The immutable result contains one `Inferred` variable, zero expected
constraints, zero candidates/facts/diagnostics/deferred reasons, and one
normalized-reflexive `Checked` `TypeAssertion`, without object/set coercion.
Exact structural and provenance guards reject shorter, deeper, full-reordered,
set-terminal, malformed, unrelated-local, imported, and ambiguous asserted-
head shapes; independent `BuiltinSet`/canonical-source corruption probes,
positive output checks, route isolation, and a real frontend/resolver sidecar
guard active runner 146. This does not establish mode declaration acceptance,
reachability/widening/`qua`, assertion truth or facts, implicit closure/order,
theorem acceptance, broader term/formula/child-graph semantics, proof/CoreIr/
...
Task 201 covers only the exact source with `BaseModeRadixAssertedHeadDef` defining `BaseModeRadixAssertedHead` as builtin `set`, `OuterModeRadixAssertedHeadDef` defining `OuterModeRadixAssertedHead` as that base mode, one `x` reserve of the outer mode, and `ChainedLocalModeRadixAssertedHeadPayloadBoundary: x is BaseModeRadixAssertedHead;`. The runner composes Tasks 56/146's two real AST-derived expansions with Task 184's formula-side local asserted-type consumer. A closed `Builtin` / `SameMode` / `BindingImmediateRadix` relation preserves every existing route and admits only the resolved base symbol that is the outer binding expansion's real immediate radix.

The route preserves distinct raw Outer/Base symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes exactly two expansions; and normalizes three known entries to one `BaseModeRadixAssertedHeadDef` RHS-anchored `BuiltinSet` identity. Its immutable output has one `Inferred` variable, zero expected constraints/candidates/facts/diagnostics/deferred reasons, and one normalized-reflexive `Checked` `TypeAssertion`. Exact structural/provenance, independent payload corruption, Task 146/184 isolation, and real frontend/resolver-sidecar guards protect active runner 149. Object-terminal, deeper, unrelated, imported, attributed, and argument-bearing asserted heads, general reachability/widening/`qua`, declaration or theorem acceptance, truth/facts, closure/order, broader child-graph semantics, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.


## Task 202 Exact One-Edge Object-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 202 covers only `BaseObjectModeRadixAssertedHead -> object`, `OuterObjectModeRadixAssertedHead -> BaseObjectModeRadixAssertedHead`, one outer-mode `x` reserve, and `ChainedLocalObjectModeRadixAssertedHeadPayloadBoundary: x is BaseObjectModeRadixAssertedHead;`. It composes Tasks 56/147's real two-expansion object normalization, Task 185's object formula consumer, and Task 201's closed `BindingImmediateRadix` relation without changing that relation or any existing route.

The route preserves distinct Outer/Base resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes exactly two expansions; and normalizes three known entries to one `BaseObjectModeRadixAssertedHeadDef` RHS-anchored `BuiltinObject`. Its immutable output has one `Inferred` variable, zero expected constraints/candidates/facts/diagnostics/deferred reasons, and one `Checked` `TypeAssertion` without object/set coercion. Exact structural/provenance/corruption, real Tasks 147/185/201 bidirectional isolation, and frontend/resolver-sidecar guards protect active runner 150. Additional set-terminal/deeper/unrelated/imported/attributed/argument-bearing heads, reachability/widening/`qua`, acceptance/truth/facts, child graphs, proof/CoreIr/ControlFlowIr/VC, and general chains remain deferred. No checker source or module-layout change was required.


## Task 203 Exact Two-Edge Set-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 203 covers only three exact local definitions, `BaseTwoEdgeModeRadixAssertedHead -> set`, `MiddleTwoEdgeModeRadixAssertedHead -> BaseTwoEdgeModeRadixAssertedHead`, and `OuterTwoEdgeModeRadixAssertedHead -> MiddleTwoEdgeModeRadixAssertedHead`, one outer-mode `x` reserve, and `TwoEdgeLocalModeRadixAssertedHeadPayloadBoundary: x is MiddleTwoEdgeModeRadixAssertedHead;`. It composes Task 72's real three-expansion set producer, Task 186's formula consumer, and Tasks 201/202's unchanged closed `BindingImmediateRadix` relation. Only the real immediate Outer-to-Middle edge is accepted; no two-hop reachability is introduced.

The route preserves distinct Outer-subject and Middle-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes exactly three expansions; and normalizes three known entries to the `BaseTwoEdgeModeRadixAssertedHeadDef` RHS-anchored `BuiltinSet` identity. Its immutable output has one `Inferred` variable, zero expected constraints/candidates/facts/diagnostics/deferred reasons, and one normalized-reflexive `Checked` `TypeAssertion`. Exact structural/provenance/corruption checks, all nonidentity definition reorderings, duplicate/spelling/imported/ambiguous/deeper near misses, real Tasks 122/148/149/186/187/201/202 bidirectional isolation, and a frontend/resolver sidecar protect active runner 151. Assertion of Base across both links, the object-terminal sibling, broader heads and semantics, proof/CoreIr/ControlFlowIr/VC, and general chain behavior remain deferred. No checker source or module-layout change was required.


## Task 204 Exact Two-Edge Object-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 204 covers only three exact local definitions, `BaseTwoEdgeObjectModeRadixAssertedHead -> object`, `MiddleTwoEdgeObjectModeRadixAssertedHead -> BaseTwoEdgeObjectModeRadixAssertedHead`, and `OuterTwoEdgeObjectModeRadixAssertedHead -> MiddleTwoEdgeObjectModeRadixAssertedHead`, one outer-mode `x` reserve, and `TwoEdgeLocalObjectModeRadixAssertedHeadPayloadBoundary: x is MiddleTwoEdgeObjectModeRadixAssertedHead;`. It composes Task 72's real three-expansion object producer, Task 187's formula consumer, and Tasks 202/203's unchanged closed `BindingImmediateRadix` relation. Only the real immediate Outer-to-Middle edge is accepted; no two-hop reachability is introduced.

The route preserves distinct Outer-subject and Middle-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes exactly three expansions; and normalizes three known entries to the `BaseTwoEdgeObjectModeRadixAssertedHeadDef` RHS-anchored `BuiltinObject` identity. Its immutable output has one `Inferred` variable, zero expected constraints/candidates/facts/diagnostics/deferred reasons, and one normalized-reflexive `Checked` `TypeAssertion` without object/set coercion. Exact structural/provenance/corruption checks, every nonidentity definition order, duplicate/spelling/imported/ambiguous/deeper near misses, real Tasks 189/145/147/149/187/202 and set-terminal Tasks 148/186/203 bidirectional isolation, and a frontend/resolver sidecar protect active runner 152. Assertion of Base across both links, broader heads and semantics, proof/CoreIr/ControlFlowIr/VC, and general chain behavior remain deferred. No checker source or module-layout change was required.

## Task 205 Exact Three-Edge Set-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 205 covers only four exact local definitions, `BaseThreeEdgeModeRadixAssertedHead -> set`, `InnerThreeEdgeModeRadixAssertedHead -> BaseThreeEdgeModeRadixAssertedHead`, `MiddleThreeEdgeModeRadixAssertedHead -> InnerThreeEdgeModeRadixAssertedHead`, and `OuterThreeEdgeModeRadixAssertedHead -> MiddleThreeEdgeModeRadixAssertedHead`, one outer-mode `x` reserve, and `ThreeEdgeLocalModeRadixAssertedHeadPayloadBoundary: x is MiddleThreeEdgeModeRadixAssertedHead;`. It composes Task 73's real four-expansion set-terminal producer, Task 195's formula consumer, and Tasks 201/203/204's unchanged closed `BindingImmediateRadix` relation. Only the real immediate Outer-to-Middle edge is accepted; no multi-hop Inner/Base reachability is introduced.

The route preserves distinct Outer-subject and Middle-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes exactly four expansions; and normalizes three known entries to the `BaseThreeEdgeModeRadixAssertedHeadDef` RHS-anchored `BuiltinSet` identity. Its immutable output has one `Inferred` variable, zero expected constraints/candidates/facts/diagnostics/deferred reasons, and one normalized-reflexive `Checked` `TypeAssertion`. Exact structural/provenance/corruption checks, all 23 nonidentity definition orders, missing/duplicate/label/spelling/radix and imported/ambiguous/deeper/multi-hop near misses, bidirectional isolation against set Tasks 122/138/146/148/150/195/201/203 and object Tasks 189/145/147/149/151/196/202/204, and a frontend/resolver sidecar protect active runner 153. Multi-hop Inner/Base assertion, the matching object sibling, broader heads and semantics, proof/CoreIr/ControlFlowIr/VC, and general chain behavior remain deferred. No checker source or module-layout change was required.

## Task 206 Exact Three-Edge Object-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 206 covers only four exact local definitions, `BaseThreeEdgeObjectModeRadixAssertedHead -> object`, `InnerThreeEdgeObjectModeRadixAssertedHead -> BaseThreeEdgeObjectModeRadixAssertedHead`, `MiddleThreeEdgeObjectModeRadixAssertedHead -> InnerThreeEdgeObjectModeRadixAssertedHead`, and `OuterThreeEdgeObjectModeRadixAssertedHead -> MiddleThreeEdgeObjectModeRadixAssertedHead`, one outer-mode `x` reserve, and `ThreeEdgeLocalObjectModeRadixAssertedHeadPayloadBoundary: x is MiddleThreeEdgeObjectModeRadixAssertedHead;`. It composes Task 73's real four-expansion object-terminal producer, Task 196's formula consumer, and Tasks 201/204/205's unchanged closed `BindingImmediateRadix` relation. Only the real immediate Outer-to-Middle edge is accepted; no multi-hop Inner/Base reachability or object/set coercion is introduced.

The route preserves distinct Outer-subject and Middle-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes exactly four expansions; and normalizes three known entries to the `BaseThreeEdgeObjectModeRadixAssertedHeadDef` RHS-anchored `BuiltinObject` identity. Its immutable output has one `Inferred` variable, zero expected constraints/candidates/facts/diagnostics/deferred reasons, and one normalized-reflexive `Checked` `TypeAssertion`. Exact structural/provenance/corruption checks, all 23 nonidentity definition orders, per-definition missing/duplicate/label/spelling/radix and imported/ambiguous/deeper/multi-hop/local-other near misses, bidirectional isolation against set Tasks 122/138/146/148/150/195/201/203/205 and object Tasks 189/145/147/149/151/196/202/204, and a frontend/resolver sidecar protect active runner 154. Multi-hop Inner/Base assertion, broader heads and semantics, proof/CoreIr/ControlFlowIr/VC, and general chain behavior remain deferred. No checker source or module-layout change was required.

## Task 207 Exact Four-Edge Set-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 207 covers only five ordered local definitions, `BaseFourEdgeModeRadixAssertedHead -> set`, `InnerFourEdgeModeRadixAssertedHead -> BaseFourEdgeModeRadixAssertedHead`, `MiddleFourEdgeModeRadixAssertedHead -> InnerFourEdgeModeRadixAssertedHead`, `OuterFourEdgeModeRadixAssertedHead -> MiddleFourEdgeModeRadixAssertedHead`, and `TooDeepFourEdgeModeRadixAssertedHead -> OuterFourEdgeModeRadixAssertedHead`, one TooDeep-mode `x` reserve, and `FourEdgeLocalModeRadixAssertedHeadPayloadBoundary: x is OuterFourEdgeModeRadixAssertedHead;`. It composes Task 74's real five-expansion set-terminal producer, Task 197's formula consumer, and Tasks 201/203/205/206's unchanged closed `BindingImmediateRadix` relation. Only the real immediate TooDeep-to-Outer edge is accepted; no multi-hop Middle/Inner/Base reachability is introduced.

The route preserves distinct TooDeep-subject and Outer-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes exactly five expansions; and normalizes three known entries to the `BaseFourEdgeModeRadixAssertedHeadDef` RHS-anchored `BuiltinSet` identity. Its immutable output has one `Inferred` variable, zero expected constraints/candidates/facts/diagnostics/deferred reasons, and one normalized-reflexive `Checked` `TypeAssertion`. Exact structural/provenance/corruption checks, all 119 nonidentity definition orders, every per-definition and asserted-head near miss, imported/ambiguous checks for all five symbols, bidirectional isolation against the 20 declared set/object owner routes, and a frontend/resolver sidecar protect active runner 155. Multi-hop Middle/Inner/Base assertions, the matching object sibling, broader heads and semantics, proof/CoreIr/ControlFlowIr/VC, and general chain behavior remain deferred. No checker source or module-layout change was required.

## Task 208 Exact Four-Edge Object-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 208 covers only five ordered local definitions, `BaseFourEdgeObjectModeRadixAssertedHead -> object`, `InnerFourEdgeObjectModeRadixAssertedHead -> BaseFourEdgeObjectModeRadixAssertedHead`, `MiddleFourEdgeObjectModeRadixAssertedHead -> InnerFourEdgeObjectModeRadixAssertedHead`, `OuterFourEdgeObjectModeRadixAssertedHead -> MiddleFourEdgeObjectModeRadixAssertedHead`, and `TooDeepFourEdgeObjectModeRadixAssertedHead -> OuterFourEdgeObjectModeRadixAssertedHead`, one TooDeep-mode `x` reserve, and `FourEdgeLocalObjectModeRadixAssertedHeadPayloadBoundary: x is OuterFourEdgeObjectModeRadixAssertedHead;`. It composes Task 74/153's real five-expansion object-terminal producer, Task 198's formula consumer, and Tasks 202/204/206/207's unchanged closed `BindingImmediateRadix` relation. Only the real immediate TooDeep-to-Outer edge is accepted; no multi-hop Middle/Inner/Base reachability or object/set coercion is introduced.

The route preserves distinct TooDeep-subject and Outer-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes exactly five expansions; and normalizes three known entries to the `BaseFourEdgeObjectModeRadixAssertedHeadDef` RHS-anchored `BuiltinObject` identity. Its immutable output has one `Inferred` variable, zero expected constraints/candidates/facts/diagnostics/deferred reasons, and one normalized-reflexive `Checked` `TypeAssertion`. Exact/corruption/all-119-orders/per-definition/reserve/formula/asserted-head/deeper/provenance guards, an unrelated-import positive, imported/ambiguous checks for all five symbols, expansion removal/corruption checks, bidirectional isolation against the 21 declared owner routes, immutable output, and a frontend/resolver sidecar protect active runner 156. Multi-hop Middle/Inner/Base assertions, broader heads and semantics, proof/CoreIr/ControlFlowIr/VC, and general chain behavior remain deferred. No checker source or module-layout change was required.

## Task 209 Exact Seven-Expansion Set-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 209 covers only the ordered `BaseMode -> set`, `ChainMode1 -> BaseMode`, through `ChainMode6 -> ChainMode5` definitions, one `ChainMode6` reserve, and `LongLocalModeRadixAssertedHeadPayloadBoundary: x is ChainMode5;`. Task 74 supplies the seven real AST-derived expansions, Task 199 the formula consumer, and Task 175 the builtin asserted-type sibling and reusable output guards. The unchanged closed `BindingImmediateRadix` relation accepts only the real bare `ChainMode6 -> ChainMode5` immediate edge; it does not add multi-hop reachability.

The route preserves distinct ChainMode6-subject and ChainMode5-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes seven expansions; and normalizes three known entries to one `BaseModeDef` RHS-anchored `BuiltinSet` identity. Its immutable output has one `Inferred` variable, one normalized-reflexive `Checked` `TypeAssertion`, and zero constraints/candidates/facts/diagnostics/deferred reasons. All 5,039 nonidentity orders, the declared finite source/provenance/corruption matrix, bidirectional isolation against all 34 pre-existing owners, immutable output, and a real frontend/resolver sidecar protect active runner 157. Multi-hop ChainMode4 through BaseMode assertions, the object sibling, imported-positive/attributed/argument-bearing/parameterized/contextual shapes, widening/`qua`, acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 210 Exact Seven-Expansion Object-Terminal Formula-Side Immediate-Radix Local-Mode Asserted-Head Checker Bridge

Task 210 covers only the ordered `BaseObjectMode -> object`, `ChainObjectMode1 -> BaseObjectMode`, through `ChainObjectMode6 -> ChainObjectMode5` definitions, one `ChainObjectMode6` reserve, and `LongLocalObjectModeRadixAssertedHeadPayloadBoundary: x is ChainObjectMode5;`. Task 74 supplies the seven real AST-derived object-terminal expansions, Task 200 the formula consumer, Task 179 the builtin-object asserted-type sibling and reusable output guards, and Task 209 the matching set-terminal immediate-radix sibling. The unchanged closed `BindingImmediateRadix` relation accepts only the real bare `ChainObjectMode6 -> ChainObjectMode5` immediate edge; it adds neither multi-hop reachability nor object/set coercion.

The route preserves distinct ChainObjectMode6-subject and ChainObjectMode5-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes seven expansions; and normalizes three known entries to one `BaseObjectModeDef` RHS-anchored `BuiltinObject` identity. Its immutable output has one `Inferred` variable, one normalized-reflexive `Checked` `TypeAssertion`, and zero constraints/candidates/facts/diagnostics/deferred reasons. All 5,039 nonidentity orders, the declared finite source/provenance/corruption matrix, bidirectional isolation against all 35 pre-existing owners, immutable output, and a real frontend/resolver sidecar protect active runner 158. Multi-hop ChainObjectMode4 through BaseObjectMode assertions, imported-positive/attributed/argument-bearing/parameterized/contextual shapes, widening/`qua`, object/set coercion, acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 211 Exact Two-Edge Set-Terminal Formula-Side Two-Hop Local-Mode Asserted-Head Checker Bridge

Task 211 covers only `BaseTwoHopModeAssertedHead -> set`, `MiddleTwoHopModeAssertedHead -> BaseTwoHopModeAssertedHead`, `OuterTwoHopModeAssertedHead -> MiddleTwoHopModeAssertedHead`, one Outer reserve, and `TwoEdgeLocalModeTwoHopAssertedHeadPayloadBoundary: x is BaseTwoHopModeAssertedHead;`. Task 72 supplies three real AST-derived expansions, and the existing reserved-variable formula bridge passes the raw Outer subject and independently resolved Base asserted type to the checker. A separate closed `BindingTwoHopRadix` relation validates both actual bare links with pairwise-distinct symbols and the exact Base-to-set terminal; generic terminal traversal alone is not relation evidence.

The route preserves distinct Outer-subject and Base-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes three expansions; and normalizes three known entries to one `BaseTwoHopModeAssertedHeadDef` RHS-anchored `BuiltinSet` identity. Its immutable output has one `Inferred` variable, one normalized-reflexive `Checked` `TypeAssertion`, and zero constraints/candidates/facts/diagnostics/deferred reasons. All five nonidentity definition orders, the finite structural/provenance/corruption matrix, bidirectional isolation against all 36 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 159. The object sibling, other distances, imported-positive/attributed/argument-bearing/parameterized/contextual shapes, widening/`qua`, general reachability, acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 212 Exact Two-Edge Object-Terminal Formula-Side Two-Hop Local-Mode Asserted-Head Checker Bridge

Task 212 covers only `BaseTwoHopObjectModeAssertedHead -> object`, `MiddleTwoHopObjectModeAssertedHead -> BaseTwoHopObjectModeAssertedHead`, `OuterTwoHopObjectModeAssertedHead -> MiddleTwoHopObjectModeAssertedHead`, one Outer reserve, and `TwoEdgeLocalObjectModeTwoHopAssertedHeadPayloadBoundary: x is BaseTwoHopObjectModeAssertedHead;`. Task 72 supplies three real AST-derived object expansions, and the existing reserved-variable formula bridge passes the raw Outer subject and independently resolved Base asserted type to the checker. Task 211's closed `BindingTwoHopRadix` relation validates both actual bare links with pairwise-distinct symbols and the exact Base-to-object terminal; generic terminal traversal alone is not relation evidence.

The route preserves distinct Outer-subject and Base-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes three expansions; and normalizes three known entries to one `BaseTwoHopObjectModeAssertedHeadDef` RHS-anchored `BuiltinObject` identity. Its immutable output has one `Inferred` variable, one normalized-reflexive `Checked` `TypeAssertion`, and zero constraints/candidates/facts/diagnostics/deferred reasons without object/set coercion. All five nonidentity definition orders, the finite structural/provenance/corruption matrix, bidirectional isolation against all 37 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 160. Other distances, imported-positive/attributed/argument-bearing/parameterized/contextual shapes, widening/`qua`, general reachability, object/set coercion, acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 213 Exact Three-Edge Set-Terminal Formula-Side Two-Hop Local-Mode Asserted-Head Checker Bridge

Task 213 covers only `BaseThreeEdgeModeTwoHopAssertedHead -> set`, `InnerThreeEdgeModeTwoHopAssertedHead -> BaseThreeEdgeModeTwoHopAssertedHead`, `MiddleThreeEdgeModeTwoHopAssertedHead -> InnerThreeEdgeModeTwoHopAssertedHead`, `OuterThreeEdgeModeTwoHopAssertedHead -> MiddleThreeEdgeModeTwoHopAssertedHead`, one Outer reserve, and `ThreeEdgeLocalModeTwoHopAssertedHeadPayloadBoundary: x is InnerThreeEdgeModeTwoHopAssertedHead;`. Task 73 supplies four real AST-derived expansions, and Task 195's reserved-variable formula bridge passes the raw Outer subject and independently resolved Inner asserted type to the checker. The refined `BindingTwoHopRadix` relation still validates the actual bare Outer-to-Middle and Middle-to-Inner links with pairwise-distinct symbols; only the remaining Inner-to-Base-to-set tail uses cycle-safe terminal traversal, which is not relation evidence.

The route preserves distinct Outer-subject and Inner-asserted resolved symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes four expansions; and normalizes three known entries to one `BaseThreeEdgeModeTwoHopAssertedHeadDef` RHS-anchored `BuiltinSet` identity. Its immutable output has one `Inferred` variable, one normalized-reflexive `Checked` `TypeAssertion`, and zero constraints/candidates/facts/diagnostics/deferred reasons. All 23 nonidentity definition orders, the finite structural/provenance/corruption matrix, focused Task 211/212 regressions, bidirectional isolation against all 38 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 161. The object sibling, Base/full-distance assertion, deeper/imported-positive/attributed/argument-bearing/parameterized/contextual shapes, widening/`qua`, general reachability, acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 214 Exact Three-Edge Object-Terminal Formula-Side Two-Hop Local-Mode Asserted-Head Checker Bridge

Task 214 covers only `BaseThreeEdgeObjectModeTwoHopAssertedHead -> object`, `InnerThreeEdgeObjectModeTwoHopAssertedHead -> BaseThreeEdgeObjectModeTwoHopAssertedHead`, `MiddleThreeEdgeObjectModeTwoHopAssertedHead -> InnerThreeEdgeObjectModeTwoHopAssertedHead`, `OuterThreeEdgeObjectModeTwoHopAssertedHead -> MiddleThreeEdgeObjectModeTwoHopAssertedHead`, one Outer reserve, and `ThreeEdgeLocalObjectModeTwoHopAssertedHeadPayloadBoundary: x is InnerThreeEdgeObjectModeTwoHopAssertedHead;`. Task 73 supplies four real AST-derived object expansions, and Task 196's formula bridge passes the raw Outer subject and independently resolved Inner asserted type to the checker. The unchanged `BindingTwoHopRadix` relation directly validates the pairwise-distinct Outer-to-Middle and Middle-to-Inner bare links; the remaining Inner-to-Base-to-object tail uses cycle-safe terminal normalization only and is never relation evidence.

The route preserves distinct Outer-subject and Inner-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes four expansions; and normalizes three known entries to one `BaseThreeEdgeObjectModeTwoHopAssertedHeadDef` RHS-anchored `BuiltinObject` identity. Its immutable output has one `Inferred` variable, one normalized-reflexive `Checked` `TypeAssertion`, and zero constraints/candidates/facts/diagnostics/deferred reasons without object/set coercion. All 23 nonidentity definition orders, the finite structural/provenance/corruption matrix, focused Task 211/212/213 regressions, bidirectional isolation against all 39 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 162. Base/full-distance assertion, deeper/imported-positive/attributed/argument-bearing/parameterized/contextual shapes, widening/`qua`, general reachability, acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, object/set coercion, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 215 Exact Four-Edge Set-Terminal Formula-Side Two-Hop Local-Mode Asserted-Head Checker Bridge

Task 215 covers only `BaseFourEdgeModeTwoHopAssertedHead -> set`, `InnerFourEdgeModeTwoHopAssertedHead -> BaseFourEdgeModeTwoHopAssertedHead`, `MiddleFourEdgeModeTwoHopAssertedHead -> InnerFourEdgeModeTwoHopAssertedHead`, `OuterFourEdgeModeTwoHopAssertedHead -> MiddleFourEdgeModeTwoHopAssertedHead`, `TooDeepFourEdgeModeTwoHopAssertedHead -> OuterFourEdgeModeTwoHopAssertedHead`, one TooDeep reserve, and `FourEdgeLocalModeTwoHopAssertedHeadPayloadBoundary: x is MiddleFourEdgeModeTwoHopAssertedHead;`. Task 74 supplies five real AST-derived set expansions, and Task 197's formula bridge passes the raw TooDeep subject and independently resolved Middle asserted type to the checker. The byte-for-byte unchanged `BindingTwoHopRadix` relation directly validates the pairwise-distinct TooDeep-to-Outer and Outer-to-Middle bare links; the remaining Middle-to-Inner-to-Base-to-set tail uses cycle-safe terminal normalization only and is never relation evidence.

The route preserves distinct TooDeep-subject and Middle-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes five expansions; and normalizes three known entries to one `BaseFourEdgeModeTwoHopAssertedHeadDef` RHS-anchored `BuiltinSet` identity. Its immutable output has one `Inferred` variable, one normalized-reflexive `Checked` `TypeAssertion`, and zero constraints/candidates/facts/diagnostics/deferred reasons without object/set coercion. All 119 nonidentity definition orders, the finite structural/provenance/corruption matrix, focused Tasks 211-214 regressions, bidirectional isolation against all 40 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 163. One test-first fixture with five shared plus one dedicated backlink accounts for 378 cases, 342 requirements, type-elaboration coverage 210/198, and pass/fail 194/184 without changing existing expectations. The object sibling, Inner three-hop/Base full-distance assertions, deeper/imported-positive/attributed/argument-bearing/parameterized/contextual shapes, widening/`qua`, general reachability, acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, object/set coercion, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 216 Exact Four-Edge Object-Terminal Formula-Side Two-Hop Local-Mode Asserted-Head Checker Bridge

Task 216 covers only `BaseFourEdgeObjectModeTwoHopAssertedHead -> object`, `InnerFourEdgeObjectModeTwoHopAssertedHead -> BaseFourEdgeObjectModeTwoHopAssertedHead`, `MiddleFourEdgeObjectModeTwoHopAssertedHead -> InnerFourEdgeObjectModeTwoHopAssertedHead`, `OuterFourEdgeObjectModeTwoHopAssertedHead -> MiddleFourEdgeObjectModeTwoHopAssertedHead`, `TooDeepFourEdgeObjectModeTwoHopAssertedHead -> OuterFourEdgeObjectModeTwoHopAssertedHead`, one TooDeep reserve, and `FourEdgeLocalObjectModeTwoHopAssertedHeadPayloadBoundary: x is MiddleFourEdgeObjectModeTwoHopAssertedHead;`. Task 74 supplies five real AST-derived object expansions, and Task 198's formula bridge passes the raw TooDeep subject and independently resolved Middle asserted type to the checker. The byte-for-byte unchanged `BindingTwoHopRadix` relation directly validates the pairwise-distinct TooDeep-to-Outer and Outer-to-Middle bare links; the remaining Middle-to-Inner-to-Base-to-object tail uses cycle-safe terminal normalization only and is never relation evidence.

The route preserves distinct TooDeep-subject and Middle-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes five expansions; and normalizes three known entries to one `BaseFourEdgeObjectModeTwoHopAssertedHeadDef` RHS-anchored `BuiltinObject` identity. Its immutable output has one `Inferred` variable, one normalized-reflexive `Checked` `TypeAssertion`, and zero constraints/candidates/facts/diagnostics/deferred reasons without object/set coercion. All 119 nonidentity definition orders, the finite structural/provenance/corruption matrix, focused Tasks 211-215 regressions, bidirectional isolation against all 41 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 164. One test-first fixture with five shared plus one dedicated backlink accounts for 379 cases, 343 requirements, type-elaboration coverage 211/199, and pass/fail 195/184 without changing existing expectations. Inner three-hop/Base full-distance assertions, deeper/imported-positive/attributed/argument-bearing/parameterized/contextual shapes, widening/`qua`, general reachability, acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, object/set coercion, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 217 Exact Three-Edge Set-Terminal Formula-Side Three-Hop Local-Mode Asserted-Head Checker Bridge

Task 217 is limited to `BaseThreeEdgeModeThreeHopAssertedHead -> set`, `InnerThreeEdgeModeThreeHopAssertedHead -> BaseThreeEdgeModeThreeHopAssertedHead`, `MiddleThreeEdgeModeThreeHopAssertedHead -> InnerThreeEdgeModeThreeHopAssertedHead`, `OuterThreeEdgeModeThreeHopAssertedHead -> MiddleThreeEdgeModeThreeHopAssertedHead`, one Outer reserve, and `ThreeEdgeLocalModeThreeHopAssertedHeadPayloadBoundary: x is BaseThreeEdgeModeThreeHopAssertedHead;`. Task 73 supplies four real AST-derived set expansions and Task 195 supplies the real formula/checker consumer. The new closed `BindingThreeHopRadix` relation directly validates pairwise-distinct Outer-to-Middle, Middle-to-Inner, and Inner-to-Base bare links; Base-to-set is cycle-safe terminal normalization only and is never relation evidence.

The route preserves distinct Outer-subject and Base-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes four expansions; and normalizes three known entries to one `BaseThreeEdgeModeThreeHopAssertedHeadDef` RHS-anchored `BuiltinSet` identity before one inferred variable and one zero-constraint/fact/candidate/diagnostic/deferred checked type assertion. All 23 nonidentity definition orders, the finite structural/provenance/corruption matrix, focused Tasks 211-216 regressions, bidirectional isolation against all 42 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 165. One test-first fixture with five shared plus one dedicated backlink accounts for 380 cases, 344 requirements, type-elaboration coverage 212/200, and pass/fail 196/184 without changing existing expectations. The object sibling, other depths, generic reachability/widening/`qua`, declaration/theorem acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 218 Exact Three-Edge Object-Terminal Formula-Side Three-Hop Local-Mode Asserted-Head Checker Bridge

Task 218 is limited to `BaseThreeEdgeObjectModeThreeHopAssertedHead -> object`, `InnerThreeEdgeObjectModeThreeHopAssertedHead -> BaseThreeEdgeObjectModeThreeHopAssertedHead`, `MiddleThreeEdgeObjectModeThreeHopAssertedHead -> InnerThreeEdgeObjectModeThreeHopAssertedHead`, `OuterThreeEdgeObjectModeThreeHopAssertedHead -> MiddleThreeEdgeObjectModeThreeHopAssertedHead`, one Outer reserve, and `ThreeEdgeLocalObjectModeThreeHopAssertedHeadPayloadBoundary: x is BaseThreeEdgeObjectModeThreeHopAssertedHead;`. Task 73 supplies four real AST-derived object expansions and Task 196 supplies the real formula/checker consumer. The byte-for-byte unchanged `BindingThreeHopRadix` relation directly validates pairwise-distinct Outer-to-Middle, Middle-to-Inner, and Inner-to-Base bare links; Base-to-object is cycle-safe terminal normalization only and is never relation evidence.

The route preserves distinct Outer-subject and Base-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes four expansions; and normalizes three known entries to one `BaseThreeEdgeObjectModeThreeHopAssertedHeadDef` RHS-anchored `BuiltinObject` identity before one inferred variable and one zero-constraint/fact/candidate/diagnostic/deferred checked type assertion without object/set coercion. All 23 nonidentity definition orders, the finite structural/provenance/corruption matrix, focused Tasks 211-217 regressions, bidirectional isolation against all 43 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 166. One test-first fixture with five shared plus one dedicated backlink accounts for 381 cases, 345 requirements, type-elaboration coverage 213/201, and pass/fail 197/184 without changing existing expectations. Other depths, generic reachability/widening/`qua`, declaration/theorem acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, object/set coercion, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 219 Exact Four-Edge Set-Terminal Formula-Side Three-Hop Local-Mode Asserted-Head Checker Bridge

Task 219 is limited to `BaseFourEdgeModeThreeHopAssertedHead -> set`, `InnerFourEdgeModeThreeHopAssertedHead -> BaseFourEdgeModeThreeHopAssertedHead`, `MiddleFourEdgeModeThreeHopAssertedHead -> InnerFourEdgeModeThreeHopAssertedHead`, `OuterFourEdgeModeThreeHopAssertedHead -> MiddleFourEdgeModeThreeHopAssertedHead`, `TooDeepFourEdgeModeThreeHopAssertedHead -> OuterFourEdgeModeThreeHopAssertedHead`, one TooDeep reserve, and `FourEdgeLocalModeThreeHopAssertedHeadPayloadBoundary: x is InnerFourEdgeModeThreeHopAssertedHead;`. Task 74 supplies five real AST-derived set expansions and Task 197 supplies the real formula/checker consumer. The byte-for-byte unchanged `BindingThreeHopRadix` relation directly validates pairwise-distinct TooDeep-to-Outer, Outer-to-Middle, and Middle-to-Inner bare links; the Inner-to-Base-to-set tail is cycle-safe terminal normalization only and is never relation evidence.

The route preserves distinct TooDeep-subject and Inner-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes five expansions; and normalizes three known entries to one `BaseFourEdgeModeThreeHopAssertedHeadDef` RHS-anchored `BuiltinSet` identity before one inferred variable and one zero-constraint/fact/candidate/diagnostic/deferred checked type assertion. All 119 nonidentity definition orders, the finite structural/provenance/corruption matrix with independent guards for an unconnected unsupported deeper asserted head and an actual connected sixth-definition/sixth-edge asserted head, focused Task 207 and Tasks 211-218 regressions, bidirectional isolation against all 44 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 167. One active fixture with five shared plus one dedicated backlink accounts for 382 cases, 346 requirements, type-elaboration coverage 214/202, and pass/fail 198/184 without changing existing expectations. The object sibling, Base full-distance assertion, generic reachability/widening/`qua`, declaration/theorem acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 220 Exact Four-Edge Object-Terminal Formula-Side Three-Hop Local-Mode Asserted-Head Checker Bridge

Task 220 is limited to `BaseFourEdgeObjectModeThreeHopAssertedHead -> object`, `InnerFourEdgeObjectModeThreeHopAssertedHead -> BaseFourEdgeObjectModeThreeHopAssertedHead`, `MiddleFourEdgeObjectModeThreeHopAssertedHead -> InnerFourEdgeObjectModeThreeHopAssertedHead`, `OuterFourEdgeObjectModeThreeHopAssertedHead -> MiddleFourEdgeObjectModeThreeHopAssertedHead`, `TooDeepFourEdgeObjectModeThreeHopAssertedHead -> OuterFourEdgeObjectModeThreeHopAssertedHead`, one TooDeep reserve, and `FourEdgeLocalObjectModeThreeHopAssertedHeadPayloadBoundary: x is InnerFourEdgeObjectModeThreeHopAssertedHead;`. Task 74 supplies five real AST-derived object expansions and Task 198 supplies the real formula/checker consumer; Task 208 guards the immediate-edge sibling. The byte-for-byte unchanged `BindingThreeHopRadix` relation directly validates pairwise-distinct TooDeep-to-Outer, Outer-to-Middle, and Middle-to-Inner bare links; the Inner-to-Base-to-object tail is cycle-safe terminal normalization only and is never relation evidence.

The route preserves distinct TooDeep-subject and Inner-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes five expansions; and normalizes three known entries to one `BaseFourEdgeObjectModeThreeHopAssertedHeadDef` RHS-anchored `BuiltinObject` identity before one inferred variable and one zero-constraint/fact/candidate/diagnostic/deferred checked type assertion without object/set coercion. All 119 nonidentity definition orders, the finite structural/provenance/corruption matrix with independent guards for an unconnected unsupported deeper asserted head and an actual connected sixth-definition/sixth-edge asserted head, focused Tasks 208 and 211-219 regressions, bidirectional isolation against all 45 prior owners, immutable output, and a real frontend/resolver sidecar protect active runner 168. One active fixture with five shared plus one dedicated backlink accounts for 383 cases, 347 requirements, type-elaboration coverage 215/203, and pass/fail 199/184 without changing existing expectations. The Base full-distance assertion, generic reachability/widening/`qua`, declaration/theorem acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, object/set coercion, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 221 Exact Four-Edge Set-Terminal Formula-Side Four-Hop Local-Mode Asserted-Head Checker Bridge

Task 221 is limited to `BaseFourEdgeModeFourHopAssertedHead -> set`, `InnerFourEdgeModeFourHopAssertedHead -> BaseFourEdgeModeFourHopAssertedHead`, `MiddleFourEdgeModeFourHopAssertedHead -> InnerFourEdgeModeFourHopAssertedHead`, `OuterFourEdgeModeFourHopAssertedHead -> MiddleFourEdgeModeFourHopAssertedHead`, `TooDeepFourEdgeModeFourHopAssertedHead -> OuterFourEdgeModeFourHopAssertedHead`, one TooDeep reserve, and `FourEdgeLocalModeFourHopAssertedHeadPayloadBoundary: x is BaseFourEdgeModeFourHopAssertedHead;`. Task 74 supplies five real AST-derived set expansions and Task 197 supplies the real formula/checker consumer. The active closed `BindingFourHopRadix` relation directly validates pairwise-distinct TooDeep-to-Outer, Outer-to-Middle, Middle-to-Inner, and Inner-to-Base bare links; Base-to-set is cycle-safe terminal normalization only and is never relation evidence.

The active route preserves distinct TooDeep-subject and Base-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes five expansions; and normalizes three known entries to one `BaseFourEdgeModeFourHopAssertedHeadDef` RHS-anchored `BuiltinSet` identity before one inferred variable and one zero-constraint/fact/candidate/diagnostic/deferred checked type assertion. The finite contract covers all 119 nonidentity definition orders, every definition's structural variants, the complete reserve/formula/head/provenance/removal/corruption matrix including all four relation links and terminal corruption, independent unconnected-deeper and actual connected fifth-link guards, focused Task 207 and Tasks 211-220 regressions, bidirectional isolation against all 46 prior owners, immutable output, and a real frontend/resolver sidecar. One active fixture with five shared plus one dedicated backlink accounts for active runner 169 within 384 cases and 348 requirements, type-elaboration coverage 216/204, and pass/fail 200/184 without changing existing expectations. The object sibling, longer chains, imported-positive definitions, attributed/argument-bearing shapes, generic reachability/widening/`qua`, declaration/theorem acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. Checker source and module layout were unchanged.

## Task 222 Exact Four-Edge Object-Terminal Formula-Side Four-Hop Local-Mode Asserted-Head Checker Bridge

Task 222 is limited to `BaseFourEdgeObjectModeFourHopAssertedHead -> object`, `InnerFourEdgeObjectModeFourHopAssertedHead -> BaseFourEdgeObjectModeFourHopAssertedHead`, `MiddleFourEdgeObjectModeFourHopAssertedHead -> InnerFourEdgeObjectModeFourHopAssertedHead`, `OuterFourEdgeObjectModeFourHopAssertedHead -> MiddleFourEdgeObjectModeFourHopAssertedHead`, `TooDeepFourEdgeObjectModeFourHopAssertedHead -> OuterFourEdgeObjectModeFourHopAssertedHead`, one TooDeep reserve, and `FourEdgeLocalObjectModeFourHopAssertedHeadPayloadBoundary: x is BaseFourEdgeObjectModeFourHopAssertedHead;`. Task 74 supplies five real AST-derived object expansions and Task 198 supplies the real formula/checker consumer. The unchanged closed `BindingFourHopRadix` relation directly validates pairwise-distinct TooDeep-to-Outer, Outer-to-Middle, Middle-to-Inner, and Inner-to-Base bare links; Base-to-object is cycle-safe terminal normalization only and is never relation evidence.

The active route preserves distinct TooDeep-subject and Base-asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; consumes five expansions; and normalizes three known entries to one `BaseFourEdgeObjectModeFourHopAssertedHeadDef` RHS-anchored `BuiltinObject` identity before one inferred variable and one zero-constraint/fact/candidate/diagnostic/deferred checked type assertion without object/set coercion. The finite contract covers all 119 nonidentity definition orders, every definition's structural variants, the complete reserve/formula/head/provenance/removal/corruption matrix including all four relation links and `BuiltinSet` terminal corruption, independent unconnected-deeper and actual connected fifth-link guards, focused Task 208 and Tasks 211-221 regressions, bidirectional isolation against all 47 prior owners, immutable output, and a real frontend/resolver sidecar. One active fixture with five shared plus one dedicated backlink accounts for active runner 170 within 385 cases and 349 requirements, type-elaboration coverage 217/205, and pass/fail 201/184 without changing existing expectations. Relevant-crate and workspace verification passed. Longer chains, imported-positive definitions, attributed/argument-bearing shapes, generic reachability/widening/`qua`, object/set coercion, declaration/theorem acceptance, truth/facts, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source or module-layout change was required.

## Task 223 Exact Transparent Parenthesized Reserved-Variable Equality Checker Bridge

Task 223 is limited to `reserve x for set;` followed by `ParenthesizedReservedVariableEqualityPayloadBoundary: (x) = x;`. Chapter 13 requires the parenthesized expression to preserve the inner expression's type and contribute no change to FOL encoding. The active route consumes one real unrecovered `ParenthesizedTerm` with direct `(` / `)` tokens, exactly one nested `TermExpression`, and exactly one identifier `TermReference`. Task 119 supplies the real reserve extraction, `BindingEnv`, builtin-set type projection, and equality consumer.

The active route preserves independent wrapper, inner-reference, and direct-right sites/ranges in source payload metadata; resolves the inner and right `x` uses at source-order ordinals 1/2 to `BindingId(0)`; and transparently lowers the wrapper by reusing the inner reference's reserve-derived builtin-set type and value in the existing equality path. No separate parenthesis type, axiom, fact, FOL node, or synthetic child payload is permitted. The finite contract rejects direct/right/both/nested/empty/non-identifier/recovered/malformed variants and non-exact reserve/theorem/formula shapes; covers independent provenance, lookup, type-input, matched-output, and immutable-output corruption; and isolates all 52 prior reserved-variable binary-formula owners bidirectionally with a real frontend/resolver sidecar. One active fixture with four shared plus one dedicated backlink accounts for active runner 171 within 386 cases and 350 requirements, type-elaboration coverage 218/206, and pass/fail 202/184 without changing existing expectations. Focused, relevant-crate, and workspace verification passed. Arbitrary nesting/operands/precedence, formula grouping, implicit-closure materialization, equality truth/facts, theorem acceptance, proof/CoreIr/ControlFlowIr/VC, general child graphs, and broader term/formula semantics remain deferred. No checker source or module-layout change was required.

## Task 224 Exact Seven-Expansion Set-Terminal Formula-Side Two-Hop Local-Mode Asserted-Head Checker Bridge

Task 224 is limited to the seven ordered bare definitions `BaseMode -> set`, `ChainMode1 -> BaseMode`, through `ChainMode6 -> ChainMode5`, one `ChainMode6` reserve, and `LongLocalModeTwoHopAssertedHeadPayloadBoundary: x is ChainMode4`. Task 74 supplies real AST-derived expansions, Task 199 the real formula/checker consumer, and Task 211 the byte-for-byte unchanged closed `BindingTwoHopRadix`; Task 209 is an immediate-edge sibling guard only.

The active route preserves distinct `ChainMode6` subject and `ChainMode4` asserted symbols/sites/ranges, resolves ordinal 1 to `BindingId(0)`, consumes exactly seven expansions, and directly validates pairwise-distinct bare `ChainMode6 -> ChainMode5 -> ChainMode4` links. The remaining tail is cycle-safe terminal normalization only. Three known entries normalize to the BaseModeDef-RHS `BuiltinSet` identity before one inferred variable and one checked type assertion with zero constraints/candidates/facts/diagnostics/deferred reasons. All 5,039 nonidentity orders, finite structural/provenance/corruption coverage, all 48 prior owners, immutable output, focused siblings, and a real sidecar protect the route. Focused, relevant-crate, and workspace verification passed. Generic reachability, broader chains, proof/CoreIr/ControlFlowIr/VC, and other downstream payloads remain deferred. No checker source or module-layout change was required.

## Task 225 Exact Seven-Expansion Object-Terminal Formula-Side Two-Hop Local-Mode Asserted-Head Checker Bridge

Task 225 is limited to the seven ordered bare definitions `BaseObjectMode -> object`, `ChainObjectMode1 -> BaseObjectMode`, through `ChainObjectMode6 -> ChainObjectMode5`, one `ChainObjectMode6` reserve, and `LongLocalObjectModeTwoHopAssertedHeadPayloadBoundary: x is ChainObjectMode4`. Task 74 supplies real AST-derived object expansions, Task 200 the real formula/checker consumer, and Task 211 the byte-for-byte unchanged closed `BindingTwoHopRadix`; Task 210 is an immediate-edge sibling and Task 224 the set-terminal two-hop sibling.

The active route preserves distinct `ChainObjectMode6` subject and `ChainObjectMode4` asserted symbols/sites/ranges, resolves ordinal 1 to `BindingId(0)`, consumes exactly seven expansions, and directly validates pairwise-distinct bare `ChainObjectMode6 -> ChainObjectMode5 -> ChainObjectMode4` links. The remaining tail is cycle-safe object-terminal normalization only. Three known entries normalize to the BaseObjectModeDef-RHS `BuiltinObject` identity before one inferred variable and one checked type assertion with zero constraints/candidates/facts/diagnostics/deferred reasons and no object/set coercion. All 5,039 nonidentity orders, finite structural/provenance/corruption coverage, all 49 prior owners, immutable output, focused siblings, and a real sidecar protect the route. Focused, relevant-crate, and workspace verification passed. Generic reachability, broader chains, proof/CoreIr/ControlFlowIr/VC, and other downstream payloads remain deferred. No checker source or module-layout change was required.

## Task 226 Exact Seven-Expansion Set-Terminal Formula-Side Three-Hop Local-Mode Asserted-Head Checker Bridge

Task 226 is limited to the seven ordered bare definitions `BaseMode -> set`, `ChainMode1 -> BaseMode`, through `ChainMode6 -> ChainMode5`, one `ChainMode6` reserve, and `LongLocalModeThreeHopAssertedHeadPayloadBoundary: x is ChainMode3`. Task 74 supplies real AST-derived set expansions, Task 199 the real formula/checker consumer, and Task 217 the byte-for-byte unchanged closed `BindingThreeHopRadix`; Task 219 is the set-terminal three-hop longer-tail sibling, while Tasks 209/224 are the immediate/two-hop long-chain guards.

The active route preserves distinct `ChainMode6` subject and `ChainMode3` asserted symbols/sites/ranges, resolves ordinal 1 to `BindingId(0)`, consumes exactly seven expansions, and directly validates pairwise-distinct bare `ChainMode6 -> ChainMode5 -> ChainMode4 -> ChainMode3` links. The remaining tail is cycle-safe set-terminal normalization only. Three known entries normalize to the BaseModeDef-RHS `BuiltinSet` identity before one inferred variable and one checked type assertion with zero constraints/candidates/facts/diagnostics/deferred reasons. All 5,039 nonidentity orders, finite structural/provenance/corruption coverage, all 50 prior owners, immutable output, focused siblings, and a real sidecar protect the route. Focused, relevant-crate, and workspace verification passed. Generic reachability, object-terminal and broader chains, proof/CoreIr/ControlFlowIr/VC, and other downstream payloads remain deferred. No checker source or module-layout change was required.

## Task 227 Active Exact Seven-Expansion Object-Terminal Formula-Side Three-Hop Local-Mode Asserted-Head Checker Bridge

Task 227 is limited to the seven ordered bare definitions `BaseObjectMode -> object`, `ChainObjectMode1 -> BaseObjectMode`, through `ChainObjectMode6 -> ChainObjectMode5`, one `ChainObjectMode6` reserve, and `LongLocalObjectModeThreeHopAssertedHeadPayloadBoundary: x is ChainObjectMode3`. Task 74 supplies real AST-derived object expansions, Task 200 the real formula/checker consumer, and Task 217 the byte-for-byte unchanged closed `BindingThreeHopRadix`; Task 220 is the object-terminal three-hop longer-tail sibling, Task 226 the depth-matched set sibling, and Tasks 210/225 the immediate/two-hop object long-chain guards.

The active route preserves distinct `ChainObjectMode6` subject and `ChainObjectMode3` asserted symbols/sites/ranges, resolves ordinal 1 to `BindingId(0)`, consumes exactly seven expansions, and directly validates pairwise-distinct bare `ChainObjectMode6 -> ChainObjectMode5 -> ChainObjectMode4 -> ChainObjectMode3` links. The remaining tail is cycle-safe object-terminal normalization only. Three known entries normalize to the BaseObjectModeDef-RHS `BuiltinObject` identity before one inferred variable and one checked type assertion with zero constraints/candidates/facts/diagnostics/deferred reasons and no object/set coercion. All 5,039 nonidentity orders, finite structural/provenance/corruption coverage, all 51 prior owners, immutable output, focused siblings, and a real sidecar protect the route. Focused, relevant-crate, and workspace verification passed. Generic reachability, set-terminal and broader chains, proof/CoreIr/ControlFlowIr/VC, and other downstream payloads remain deferred. No checker source or module-layout change was required.

## Task 228 Exact Seven-Expansion Set-Terminal Formula-Side Four-Hop Local-Mode Asserted-Head Checker Bridge

Task 228 is limited to the seven ordered bare definitions `BaseMode -> set`, `ChainMode1 -> BaseMode`, through `ChainMode6 -> ChainMode5`, one `ChainMode6` reserve, and `LongLocalModeFourHopAssertedHeadPayloadBoundary: x is ChainMode2`. Task 74 supplies real AST-derived set expansions, Task 199 the real formula/checker consumer, and Task 221 the byte-for-byte unchanged closed `BindingFourHopRadix`; Tasks 224/226 are the two/three-hop long-chain guards, Task 222 the object-terminal relation guard, and Task 227 the latest terminal guard.

The active route preserves distinct `ChainMode6` subject and `ChainMode2` asserted symbols/sites/ranges, resolves ordinal 1 to `BindingId(0)`, consumes exactly seven expansions, and directly validates pairwise-distinct bare `ChainMode6 -> ChainMode5 -> ChainMode4 -> ChainMode3 -> ChainMode2` links. The remaining tail is cycle-safe set-terminal normalization only. Three known entries normalize to the BaseModeDef-RHS `BuiltinSet` identity before one inferred variable and one checked type assertion with zero constraints/candidates/facts/diagnostics/deferred reasons. All 5,039 nonidentity orders, finite structural/provenance/corruption coverage, all 52 prior owners, immutable output, focused siblings, and a real sidecar protect the route. Focused, relevant-crate, and workspace verification passed. Generic reachability, object-terminal and broader chains, proof/CoreIr/ControlFlowIr/VC, and other downstream payloads remain deferred. No checker source or module-layout change was required.

## Task 229 Active Exact Seven-Expansion Object-Terminal Formula-Side Four-Hop Local-Mode Asserted-Head Checker Bridge

Task 229 is limited to the seven ordered bare definitions `BaseObjectMode -> object`, `ChainObjectMode1 -> BaseObjectMode`, through `ChainObjectMode6 -> ChainObjectMode5`, one `ChainObjectMode6` reserve, and `LongLocalObjectModeFourHopAssertedHeadPayloadBoundary: x is ChainObjectMode2`. Task 74 supplies real AST-derived object expansions, Task 200 the real formula/checker consumer, and Task 221 the byte-for-byte unchanged closed `BindingFourHopRadix`; Tasks 225/227 are the two/three-hop object long-chain guards, Task 222 the object-terminal relation guard, and Task 228 the depth-matched set guard.

The active route preserves distinct `ChainObjectMode6` subject and `ChainObjectMode2` asserted symbols/sites/ranges, resolves ordinal 1 to `BindingId(0)`, consumes exactly seven expansions, and directly validates pairwise-distinct bare `ChainObjectMode6 -> ChainObjectMode5 -> ChainObjectMode4 -> ChainObjectMode3 -> ChainObjectMode2` links. The remaining tail is cycle-safe object-terminal normalization only. Three known entries normalize to the BaseObjectModeDef-RHS `BuiltinObject` identity before one inferred variable and one checked type assertion with zero constraints/candidates/facts/diagnostics/deferred reasons and no object/set coercion. All 5,039 nonidentity orders, finite structural/provenance/corruption coverage, all 53 prior owners, immutable output, focused siblings, and a real sidecar protect the route. Focused, relevant-crate, and workspace verification passed. Generic reachability, set-terminal and broader chains, proof/CoreIr/ControlFlowIr/VC, and other downstream payloads remain deferred. No checker source or module-layout change was required.

## Task 230 Active Exact Seven-Expansion Set-Terminal Formula-Side Five-Hop Local-Mode Asserted-Head Checker Bridge

Task 230 is limited to the seven ordered bare definitions `BaseMode -> set`, `ChainMode1 -> BaseMode`, through `ChainMode6 -> ChainMode5`, one `ChainMode6` reserve, and `LongLocalModeFiveHopAssertedHeadPayloadBoundary: x is ChainMode1`. Task 74 supplies real AST-derived set expansions and Task 199 the real formula/checker consumer. The new closed `BindingFiveHopRadix` validates the exact five written links; Tasks 224/226/228 remain shorter-distance guards and Task 229 remains the latest terminal sibling.

The active route preserves distinct `ChainMode6` subject and `ChainMode1` asserted symbols/sites/ranges, resolves ordinal 1 to `BindingId(0)`, consumes exactly seven expansions, and directly validates pairwise-distinct bare `ChainMode6 -> ChainMode5 -> ChainMode4 -> ChainMode3 -> ChainMode2 -> ChainMode1` links. `ChainMode1 -> BaseMode -> set` is cycle-safe terminal normalization only. Three known entries normalize to the BaseModeDef-RHS `BuiltinSet` identity before one inferred variable and one checked type assertion with zero constraints/candidates/facts/diagnostics/deferred reasons. All 5,039 nonidentity orders, finite structural/provenance/corruption coverage, all 54 prior owners, immutable output, focused siblings, and a real sidecar protect the route. Focused, relevant-crate, and workspace verification passed. Generic reachability, object-terminal five-hop, broader chains, proof/CoreIr/ControlFlowIr/VC, and other downstream payloads remain deferred. No checker source or module-layout change was required.

## Task 231 Active Exact Seven-Expansion Object-Terminal Formula-Side Five-Hop Local-Mode Asserted-Head Checker Bridge

Task 231 is limited to the seven ordered bare definitions `BaseObjectMode -> object`, `ChainObjectMode1 -> BaseObjectMode`, through `ChainObjectMode6 -> ChainObjectMode5`, one `ChainObjectMode6` reserve, and `LongLocalObjectModeFiveHopAssertedHeadPayloadBoundary: x is ChainObjectMode1`. Task 74 supplies real AST-derived object expansions, Task 200 the real formula/checker consumer, and Task 230 the byte-for-byte unchanged closed `BindingFiveHopRadix`; Task 229 remains the shorter-distance object sibling and Task 230 the depth-matched set sibling.

The active route preserves distinct `ChainObjectMode6` subject and `ChainObjectMode1` asserted symbols/sites/ranges, resolves ordinal 1 to `BindingId(0)`, consumes exactly seven expansions, and directly validates pairwise-distinct bare `ChainObjectMode6 -> ChainObjectMode5 -> ChainObjectMode4 -> ChainObjectMode3 -> ChainObjectMode2 -> ChainObjectMode1` links. `ChainObjectMode1 -> BaseObjectMode -> object` is cycle-safe terminal normalization only. Three known entries normalize to the BaseObjectModeDef-RHS `BuiltinObject` identity before one inferred variable and one checked type assertion with zero constraints/candidates/facts/diagnostics/deferred reasons and no object/set coercion. All 5,039 nonidentity orders, finite structural/provenance/corruption coverage, all 55 prior owners, immutable output, focused siblings, and a real sidecar protect the route. Focused, relevant-crate, and workspace verification passed. Generic reachability, imported-positive definitions, broader chains, proof/CoreIr/ControlFlowIr/VC, and other downstream payloads remain deferred. No checker source or module-layout change was required.

## Task 233 Active Exact Transparent Parenthesized Builtin-Object Reserved-Variable Equality Checker Bridge

Task 233 is limited to `reserve x for object;` followed by `ParenthesizedReservedObjectVariableEqualityPayloadBoundary: (x) = x;`. Chapter 13 requires the real unrecovered `ParenthesizedTerm` to preserve its inner identifier's type and add no FOL meaning. Task 223 supplies the exact single-left-wrapper producer; Task 188 supplies real object reserve extraction, `BindingEnv`, builtin-object projection, and equality consumption. The active implementation parameterizes only the private parenthesized extraction/output assertion path by an exact static config. Task 223 continues rejecting object reserves, Task 188 continues rejecting parenthesized operands, and the new public route owns only their exact intersection.

The route preserves independent wrapper, inner-reference, and direct-right sites/ranges; resolves the two `x` uses at source-order ordinals 1/2 to `BindingId(0)`; and transparently reuses one canonical `BuiltinObject` identity for two inferred variables, two ordered expected constraints, and one fact/candidate/diagnostic/deferred-free checked equality. It creates no independent wrapper type/value, object/set coercion, axiom, fact, FOL node, or child payload. The finite contract rejects non-exact wrapper/reserve/theorem/formula shapes, covers provenance/lookup/type/matched-output corruption, proves immutable output, isolates all 53 prior binary-formula owners bidirectionally, and uses a real frontend/resolver sidecar. Arbitrary parentheses/operands/precedence, formula grouping, closure/order materialization, equality truth/facts, theorem acceptance, proof/CoreIr/ControlFlowIr/VC, child graphs, and broader semantics remain deferred. No checker source or module-layout change was required.

## Task 234 Active Exact Seven-Expansion Set-Terminal Formula-Side Six-Hop Local-Mode Asserted-Head Checker Bridge

Task 234 is limited to the seven ordered bare definitions from `BaseMode -> set` through `ChainMode6 -> ChainMode5`, one `ChainMode6` reserve, and `LongLocalModeSixHopAssertedHeadPayloadBoundary: x is BaseMode`. Specs 03/04/07/13/14/16 directly require mode unfolding, reserved theorem-variable typing, and static admissibility of the asserted type. Task 74 supplies all seven real AST-derived expansions and Task 199 supplies the real reserve/BindingEnv/type-assertion checker consumer.

The active implementation adds a closed `BindingSixHopRadix` relation that directly validates the six pairwise-distinct bare links `ChainMode6 -> ChainMode5 -> ChainMode4 -> ChainMode3 -> ChainMode2 -> ChainMode1 -> BaseMode`; `BaseMode -> set` remains terminal-normalization evidence only. It preserves distinct raw subject/asserted symbols, sites, and ranges; resolves ordinal 1 to `BindingId(0)`; normalizes all three known type entries to one BaseModeDef-RHS `BuiltinSet`; and records one inferred variable plus one zero-constraint/fact/candidate/diagnostic/deferred checked assertion. The contract covers all 5,039 nonidentity expansion orders, finite structural/provenance/removal/corruption cases, immutable output, all 56 prior owner routes bidirectionally, focused siblings, and a real frontend/resolver sidecar. Generic reachability/widening/`qua`, object-terminal/imported-positive/attributed/argument-bearing heads, acceptance, truth/facts, child graphs, proof/CoreIr/ControlFlowIr/VC, and general chain semantics remain deferred. No checker source/API/module-layout change was required.

## Task 236 Active Exact Seven-Expansion Object-Terminal Formula-Side Six-Hop Local-Mode Asserted-Head Checker Bridge
...
The same operation admits one preceding bare structure with a required builtin-set field, a type formal bounded by that actual structure, and a selector-valued set-result functor. Existing definition-only structure checking authenticates the real constructor/member and required-field witness; source formal identities remain distinct from global symbols.
...
`check_source_algorithm_types(&SurfaceResolvedArena, &TypedArena, &SymbolEnv)` returns opaque borrowed `SourceAlgorithmCheck` after complete source/environment/neutral-typed and algorithm-owner authentication. Existing BindingEnv and term/formula inference preserve the object parameter, sequential initialized var/const/ghost-var bindings, declaration/use identity, inferred types and expected return type. The flat contract profile additionally admits direct-variable assignment, equality or single prefix-negated equality assertions and a final return; destination references carry actual BindingIds without reading their values, and result is visible only in optional equality ensures. Declaration ordinals govern shadowing and forward references; formula ownership distinguishes assertions from ensures without assuming either. Prefix negation is supported for assertions and the bounded while guard: retain its real node, equality child and separately checked operand occurrences; no negated ensures/requires or formula shortcut is admitted. The seal exposes immutable typed/binding/inference/owner data, never raw resolver authority. Dotted destinations, annotations beyond the bounded while invariant, justifications, other contracts, control flow beyond the bounded while, nested snapshots, calls and literals remain unsupported. A separate empty-interface profile authenticates an earlier zero-parameter void algorithm with bare return and the later claim target through the actual local algorithm symbol; its empty binding/inference tables describe only that algorithm. The nested theorem remains owned by `check_theorem_skeletons`; no proof, totality or registration approval follows.

## Source attributed argument widening

`check_source_attribute_widening_types(&SurfaceResolvedArena, &TypedArena, &SymbolEnv, &RegistrationDatabase)` returns existing `(ExistentialGateOutput, CoercionCheckingOutput, OverloadCollectionOutput, CandidateViabilityOutput)` in `Result<_, String>`; no new payload or proof receipt is introduced.
Authority is Chapters 3 §§3.5/3.7, 6 §§6.1–2/6.8 and 17 §§17.1/17.3.4. Authenticate the complete local attribute/registration/functor/theorem profile, neutral nodes, source ranges, recovery, actual identity body/result/coherence wrapper and citations. Resolve the four same-spelling declarations by their distinct resolver binding identities.
The database is an established typing premise, not evidence of its own kernel authenticity. The admitted runner must freshly obtain it from [proof status](../../mizar-proof/en/status.md#source-existential-registration-proof) on the same immutable source, nodes and symbols and immediately invoke this operation; externally supplied, reused or fabricated databases cannot authorize that route. Reject structurally foreign, pending or mismatched databases.
At both actual quantifier/proof-let attributed type sites, derive the complete normalized attribute-symbol/builtin-set pattern and require its matching activated entry after the complete registration block. Both gates must be Satisfied with that entry, not bare-set base evidence; theorem and proof-local contexts/bindings stay separate.
...
**Bounded symbolic extension.** Extend the existing predicate producer and theorem-seal replay for a binary set predicate whose body is one object quantifier with membership antecedent and consequent. Authenticate the actual symbolic head, nested binder and formal references, quantifier/st/holds structure, object/set operand inference and both positive self-applications. The nested context inherits both definition formals; theorem and proof binders remain independent. Derive body bindings from declaration/occurrence correspondence, not operand order or equal numeric IDs. Extra quantifiers, connectives, properties, citations and foreign references remain unsupported; no seal field or public API is added.
...
## Bounded equals-infix functor theorem checking

Extend the existing statement producer and theorem replay for one fresh binary set functor with a formal-reference equals RHS, one empty coherence clause, its actual left-associative precedence-80 infix declaration and two binary applications. Authenticate declaration/occurrence correspondence, ordered arguments, result typing, operator spelling/tokens, the existing infix association payload against the left declaration, and six separate formal/theorem/local bindings. Replay the complete term/application/equality graph and clause ownership. The existing seal carries the clause to Core; do not misclassify it as a registration/narrowing obligation or add a public initial-obligation kind. Static RHS typing grants no coherence proof, defining-equation premise or accepted definition.

## Narrower-loci predicate declaration checking

The bounded source producer consumes the same-call registration database from [proof status](../../mizar-proof/en/status.md#source-existential-registration-proof), authenticates its full source-local pattern and checks the actual attributed type site shared by both grouped formals through the existing inhabitation gate. Normalize the original and narrower ordered signatures under the existing radix/attribute-closure relation; infer the unique visible earlier ordinary root of the same predicate head/arity only when both loci are no wider and at least one is strict. Type-check both actual equality bodies with independent declaration bindings and retain one Pending PredicateRedefinitionCoherence request for their equivalence under the actual narrower guards, linked to the root, new declaration and complete coherence clause. A written `thus thesis` supplies no acceptance. No use-site refinement, defining axiom, theorem/Core/VC credit or general Task274 import follows.
