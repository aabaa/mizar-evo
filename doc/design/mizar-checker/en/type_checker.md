...
`check_source_algorithm_types(&SurfaceResolvedArena, &TypedArena, &SymbolEnv)` returns opaque borrowed `SourceAlgorithmCheck`. It authenticates complete source/environment/neutral typed correspondence and the actual algorithm owner, then uses existing `BindingEnv` and term inference for the object parameter, sequential initialized var/ghost-var locals, variable returns and break profile. It preserves real declaration/use identity, inferred local types, expected return types and source order. The seal exposes only existing immutable typed, binding, inference and owner data to its runner/Core consumers, never raw resolver authority. The separate one-return profile admits optional equality ensures: an object-typed generated result binding is visible only in contract scope, actual ordered operand references are checked without assuming the equality, and body returns cannot read result. Other contracts, assignments, loops, const, snapshots/claim, calls and literals fail closed; no proof, totality or registration approval follows.
