...
|---|---:|---|---|---|---|---|
| `src/lib.rs` | 39 | crate boundary and public module exports | `00.crate_plan.md` and `source_spec_audit.md` | no | no | Keep as the crate root; it only exposes documented modules and test-only determinism support. |
| `src/typed_ast.rs` | 3885 | typed AST data model | `typed_ast.md` | no | no | Large but cohesive typed-AST tables, ids, validation, rendering, tests, and bidirectional Task-253/254 installation checks; monitor ergonomics after downstream use. |
...
| `src/resolved_typed_ast.rs` | 6820 | final resolved typed AST assembly | `resolved_typed_ast.md` | no | no | Cohesive final projection module, including exact Task-180 singleton statement/proof/direct-terminal validation and Task-251/252/253/254 clone-preserving handoffs; no behavior-neutral split required. |
...
| `tests/lint_policy.rs` | 1826 | cross-cutting policy and audit guards | `source_spec_audit.md`, `bilingual_sync_audit.md`, and `module_boundary_audit.md` | no | no | Large support test but intentionally centralizes repository-policy guardrails; no split required for task 34. |
