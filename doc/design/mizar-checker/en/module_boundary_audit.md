...
|---|---:|---|---|---|---|---|
| `src/lib.rs` | 50 | crate boundary and public module exports | `00.crate_plan.md` and `source_spec_audit.md` | no | no | Keep as the crate root; it exports the documented syntax-free formula-composition and predicate/functor/attribute/mode/structure/property-implementation source modules. |
| `src/typed_ast.rs` | 5455 | typed AST data model | `typed_ast.md` | no | no | Large but cohesive typed-AST tables, validation, rendering, and one-shot handoffs, including mutually exclusive Task-259--264 definition transactions; monitor ergonomics after downstream use. |
...
| `src/resolved_typed_ast.rs` | 7727 | final resolved typed AST assembly | `resolved_typed_ast.md` | no | no | Cohesive final projection module, including clone-preserving Task-259--264 handoffs and mutual-exclusion guards; no behavior-neutral split required. |
...
| `tests/lint_policy.rs` | 1931 | cross-cutting policy and audit guards | `source_spec_audit.md`, `bilingual_sync_audit.md`, and `module_boundary_audit.md` | no | no | Centralized policy guardrails include Task-259--264 module/spec/public-enum coverage and the unchanged production syntax boundary. |
