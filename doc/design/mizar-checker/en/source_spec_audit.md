...
Task 78 addendum: the active `mizar-test` type-elaboration runner observes a
source-derived reserve head whose structure symbol comes from the documented
`parser.type_fixtures` import summary and reports
`type_elaboration.external_dependency.ast_payload_extraction`. This credits
only the Chapter 3/5/11/12 imported-structure reserve-head diagnostic boundary;
it does not treat the import summary as real imported module AST extraction and
does not claim imported structure provenance, structure type-head payload
extraction, base-shape or constructor-witness evidence, positive structure
type elaboration, CoreIr, ControlFlowIr, VC, or proof coverage.

Task 79 addendum: the active `mizar-test` type-elaboration runner observes a
source-derived reserve head whose mode symbol comes from the documented
`parser.type_fixtures` import summary and reports
`type_elaboration.external_dependency.ast_payload_extraction`. This credits
only the Chapter 3/7/11/12 imported-mode reserve-head diagnostic boundary; it
does not treat the import summary as real imported module AST extraction and
does not claim imported mode provenance, mode type-head payload extraction,
`ModeExpansion` payloads, positive mode elaboration, CoreIr, ControlFlowIr, VC,
or proof coverage.

Task 80 addendum: the active `mizar-test` type-elaboration runner observes a
source-derived reserve type whose attribute symbol comes from the documented
`parser.type_fixtures` import summary and reports
`type_elaboration.external_dependency.ast_payload_extraction`. This credits
only the Chapter 3/6/11/12 imported-attribute reserve diagnostic boundary; it
does not treat the import summary as real imported module AST extraction and
does not claim imported attribute provenance, `AttributeInput` payload
