...
and no definition-local context; active fail slices also carry same-module
attributed builtin heads to the missing-evidence diagnostic and same-module
local mode heads outside task 55, including attributed or mixed attributed/bare
local mode sources, to the missing mode-expansion diagnostic, and same-module
...
broader/imported/attributed/parameterized/contextual mode expansion, terms, formulas, overloads, CoreIr,
...
| MC-G020 | `external_dependency_gap` / `deferred` | Active source-to-checker extraction blocker for semantic pass fixtures across tasks 7-11 and later consumers. The reserve-only builtin declaration slice now uses a checker-owned syntax-free producer seam for `BindingEnv`, `DeclarationInput`, and `DeclarationChecker` handoff data and reaches `TypedAst` and `ResolvedTypedAst` through the active runner for successful bare builtins plus task-55 same-module no-argument local modes with real AST-derived bare-builtin RHS expansions. Active fail slices now also carry same-module attributed builtin reserve heads to the missing-evidence diagnostic, same-module local mode heads outside task 55 and attributed same-module local mode heads to the missing mode-expansion diagnostic, same-module local structure heads to the missing evidence-query diagnostic, and attributed same-module local structure heads to the full attributed-type missing-evidence diagnostic. Broader non-builtin declarations, imported attributes/modes/structures, attribute arguments, mode/structure arguments, structure base-shape/full attributed-type existential evidence, broader/imported/attributed/parameterized/contextual mode expansion, terms, formulas, coercions, overloads, recorded facts, CoreIr, ControlFlowIr, VC, and proof payload extraction remains open. |
