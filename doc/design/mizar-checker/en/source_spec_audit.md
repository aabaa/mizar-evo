...
local mode heads outside task 55/56, including attributed or mixed
...
diagnostic. This is not a new checker raw-syntax
...
| The reserve-only source-derived producer seam consumes syntax-free reserve payloads and builds checker-owned binding/declaration handoff data without importing raw syntax. | `SourceReserveDeclarationBridge`, `SourceReserveBindingInput`, `SourceReserveDeclarationHandoff`. | `source_reserve_declaration_bridge_builds_checker_owned_handoff`, `source_reserve_declaration_bridge_validates_local_symbol_heads_and_mismatched_inputs`, active `mizar-test` type-elaboration runner regressions. | Implemented for successful bare builtin `set`/`object` reserve declarations, task-55 same-module no-argument local mode reserve declarations whose real AST-derived expansion has a bare builtin RHS, and task-56 one-edge same-module local-mode expansion chains whose dependency mode has that accepted builtin RHS expansion. Active diagnostic slices cover same-module attributed builtin reserve heads, same-module local mode heads outside task 55/56 missing real expansion payloads, attributed same-module local mode heads and attributed chain dependencies missing real expansion payloads, same-module local structure heads missing real base-shape evidence, attributed same-module local structure heads missing full attributed-type existential evidence, and task-57 real local-mode expansions whose RHS is a same-module local structure head but whose base-shape/constructor-witness evidence is still absent; broader AST-wide extraction remains MC-G020. |
