...
task 72's two-edge bare local-mode chains and task 73's three-edge bare
local-mode chains that end in builtin `set` / `object`.
Active fail slices also carry same-module
attributed builtin heads to the missing-evidence diagnostic and same-module
local mode heads outside task 55/56/57/58/59/60/61/62/63/64/65/66/72/73, including
mixed attributed/bare local mode sources, attributed chain dependencies, and
cold or cached four-edge local-mode chains, to the
...
chains ending in builtin `set` / `object`. Both cold and cached four-edge
chains stay on the checker missing mode-expansion diagnostic.
