...
local structure heads to the missing evidence-query diagnostic. This is not a
new checker raw-syntax dependency and does not close the AST-wide
source-to-checker gaps for non-builtin declarations, imported symbols,
attribute or mode/structure arguments, structure base-shape evidence, terms,
formulas, overloads, CoreIr, ControlFlowIr, VC payloads, or proof evidence.
...
| MC-G020 | `external_dependency_gap` / `deferred` | Active source-to-checker extraction blocker for semantic pass fixtures across tasks 7-11 and later consumers. The reserve-only builtin declaration slice now uses a checker-owned syntax-free producer seam for `BindingEnv`, `DeclarationInput`, and `DeclarationChecker` handoff data and reaches `TypedAst` and `ResolvedTypedAst` through the active runner for successful bare builtins. Active fail slices now also carry same-module attributed builtin reserve heads to the missing-evidence diagnostic, same-module local mode heads to the missing mode-expansion diagnostic, and same-module local structure heads to the missing evidence-query diagnostic. Broader non-builtin declarations, imported attributes/modes/structures, attribute arguments, attributed mode or structure heads, mode/structure arguments, structure base-shape evidence, terms, formulas, coercions, overloads, recorded facts, CoreIr, ControlFlowIr, VC, and proof payload extraction remains open. |
