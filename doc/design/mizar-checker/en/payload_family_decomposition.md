...
| 274 | **Partial prerequisite implemented:** bounded module-local existential proof association in [proof status](../../mizar-proof/en/status.md#source-existential-registration-proof); general accepted verifier/artifact import remains blocked-reserved. | Existing validated registration input, authentic Core/VC/normal-kernel leaves and unchanged ProofObligation policy; only the bounded source-proof producer is executable. | No caller-supplied Accepted/result receipt; original source and normalized full pattern stay bound. MC-G004 import schema and broader cluster/reduction activation remain deferred. |
...
- **Gate A1 — accepted registration status:** the bounded source existential
  association is implemented by [proof status](../../mizar-proof/en/status.md#source-existential-registration-proof),
  including fresh kernel checking, same-environment activation and corruption tests.
  Only this bounded profile may execute; general accepted-status import
  remains blocked-reserved and MC-G004 artifact schema remains unnamed.
