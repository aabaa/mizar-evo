...
Extend complete-source replay for the single local equality attribute/registration followed by an ordinary binary predicate and its narrower-loci redefinition. Authenticate both trailing declarations, formal references, attributed heads and coherence/proof ownership without choosing a root or proving coherence here. The existing single-registration seal retains only its actual attribute dependency, both written bare-set choices and independent builtin gates; no truncated source, later predicate premise or new accepted input enters registration proof.
