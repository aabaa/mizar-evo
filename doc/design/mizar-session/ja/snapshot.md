...
- Internal: `SourceId` で keyed され、`SourceVersion` の同一性と併せて consume
  される座標テーブルのための `source_map`
- External: パス正規化、ハッシュ計算、パッケージメタデータ、LSP のドキュメントバージョン型
- Shared: `SnapshotLease.reason` は本 snapshot／共有リース層で定義する `RetentionReason` を用いる。`retention` モジュールはこれを再定義せず、再エクスポートして再利用する。
...
`SnapshotLease` は、retention collection が、外部の利用側がまだスナップショットを参照している可能性がある間、そのスナップショットを生存させるために使う snapshot 層のハンドルです。
レジストリは `RetentionReason` ごとに live lease 数を追跡します。
`create_snapshot` が返す active-build lease も、`acquire_lease` で取得した
lease と同じ方法で計上されます。

`acquire_lease` は lease id を要求する前に未知のスナップショットを拒否します。
...
retention モジュールは、同じ lease 状態をソースとソースマップの保持へ bridge します。
...
このレジストリは、その結果として得られた snapshot input を記録・検証するだけで、
ディスクやエディタバッファからソーステキストを読み込みません。expected-vs-actual の
open-buffer staleness は、リクエストのドキュメントバージョンメタデータを所有する
source-loading 層で検証します。
...
- 診断、LSP、IR、cache、artifact writer のリースを含め、生存中リースが参照していない
- それを指名する retention の current mark がない

回収は、retention accounting metadata として記録されたインメモリのソーステキストと
