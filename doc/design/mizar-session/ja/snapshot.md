...
正確な綴りは upstream build-plan layer の責務です。`mizar-session` に渡される
package id は、同期済みのパッケージ管理仕様とモジュール名前空間仕様の規則、すなわち
小文字の `snake_case` (`[a-z][a-z0-9]*(?:_[a-z0-9]+)*`) で、ハイフン正規化を行わない、
という規則をすでに満たしているべきです。
...
- 開いている editor buffer は source snapshot であり、published artifact ではない。
- 古くなった result を再利用できるのは、対象 `BuildSnapshotId` に対する通常の cache validation を通した場合だけである。
