...
- This module is the session-level implementation point of the resident-set memory model (spec [§12.6.3](../../../spec/en/12.modules_and_namespaces.md#1263-memory-model), [§23.7.9](../../../spec/en/23.package_management_and_build_system.md#2379-memory-design-principles); internal [00.internal_overview.md](../../internal/en/00.internal_overview.md), [06.ir_storage_and_snapshot_handles.md](../../internal/en/06.ir_storage_and_snapshot_handles.md)): it bounds the resident set to the source text, source maps, and snapshot metadata that active work still references, and collects the rest. `mizar-ir` holds `PhaseOutputReference` leases here so IR-output lifetime composes with source/snapshot lifetime under one collection policy.
