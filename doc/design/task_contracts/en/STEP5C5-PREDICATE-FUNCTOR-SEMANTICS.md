...
| Field | Value |
|---|---|
| Status | Partial: fourteen outcomes complete; three gaps deferred |
| Tier | Full: semantic-credit and expectation-tag changes |
| Owner / consumer | `mizar-checker` owns definition/type and bounded predicate-statement checks; `mizar-resolve` owns identities and ordinary functor/predicate signature conflicts; `mizar-core` owns guarded phrase/symbolic-definition and theorem lowering; existing `mizar-vc` intake generates Open VCs; `mizar-test` admits and compares |
| Dependencies | Prior thirteen C5 outcomes; existing predicate-statement checking, source theorem seal, Core and VC intake |
...
The [phrase checker](../../mizar-checker/en/type_checker.md#source-phrase-predicate-theorem-checking) and [Core owner](../../mizar-core/en/elaborator.md#source-phrase-predicate-theorem) retain actual binders, guarded definition and predicate goal; one Open VC gives no proof or accepted-definition credit. Their bounded symbolic extension owns the actual quantified-membership body and existing terminal VC kind; [primitive membership](../../mizar-core/en/core_ir.md#primitive-membership-formula) is a required Core representation gap, not new proof semantics. Preserve thirteen active rows; add only symbolic predicate generation with sole `active_proof_verification` and its additive snapshot trace. Three other gaps retain their records, inactive sidecars, phases, outcomes and keys, including narrower-loci registration deferral. Except for the approved N2MarkedExists witness registration and additive phrase/symbolic snapshot references, preserve `.miz`, expectation outcome/phase/key, trace status/order, activation map, oracle scope, soundness policy, 23-case certificate rejection corpus, Task 277B state, and archive.
...
warnings-denied Clippy, and full tests. Exit requires fourteen activations and three unchanged gaps, retained controls plus actual phrase/symbolic bindings/definition/goal/context, independent renaming, forged receipt/scope rejection, complete deterministic snapshots and a task-only local commit.
