...
| Field | Value |
|---|---|
| Status | Partial: fifteen outcomes complete; means-prefix correctness remains pending; two gaps deferred |
| Tier | Full: semantic-credit and expectation-tag changes |
| Owner / consumer | `mizar-checker` owns definition/type and bounded predicate-statement checks; `mizar-resolve` owns identities and ordinary functor/predicate signature conflicts; `mizar-core` owns guarded phrase/symbolic-definition and bounded means-functor correctness/theorem lowering; existing `mizar-vc` intake generates Open VCs; `mizar-test` admits and compares |
| Dependencies | Prior fourteen C5 outcomes; existing source theorem seal, term/application typing, Core definition/proof and VC intake |
| Audit impact | Update Chapters 10/16 for pending means-functor correctness and theorem Core/VC ownership; preserve other coverage and `doc/spec` |
...
The [phrase checker](../../mizar-checker/en/type_checker.md#source-phrase-predicate-theorem-checking) and [Core owner](../../mizar-core/en/elaborator.md#source-phrase-predicate-theorem) retain actual binders, guarded definition and predicate goal; one Open VC gives no proof or accepted-definition credit. Their bounded symbolic extension owns the actual quantified-membership body and existing terminal VC kind; [primitive membership](../../mizar-core/en/core_ir.md#primitive-membership-formula) is a required Core representation gap, not new proof semantics. The means-prefix extension lowers both authenticated source correctness clauses into pending Core goals with an Opaque body; its defining equation is not a theorem premise. Preserve fourteen active rows; add only means-prefix functor generation with sole `active_proof_verification` and its additive snapshot trace. Two other gaps retain their records, inactive sidecars, phases, outcomes and keys, including narrower-loci registration deferral. Except for the approved N2MarkedExists witness registration and additive phrase/symbolic/means-functor snapshot references, preserve `.miz`, expectation outcome/phase/key, trace status/order, activation map, oracle scope, soundness policy, 23-case certificate rejection corpus, Task 277B state, and archive.
...
warnings-denied Clippy, and full tests. Exit requires fifteen activations and two unchanged gaps, retained controls plus actual phrase/symbolic/means-functor bindings, pending correctness and theorem graphs, independent renaming, forged receipt/scope rejection, complete deterministic snapshots and a task-only local commit.
