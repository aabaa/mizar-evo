...
| Field | Value |
|---|---|
| Status | Bounded: seventeen mapped outcomes complete; narrower-loci coherence Pending, no accepted refinement |
| Tier | Full: semantic-credit and expectation-tag changes |
| Owner / consumer | `mizar-checker` owns definition/type and bounded predicate-statement checks; `mizar-resolve` owns identities and ordinary functor/predicate signature conflicts; `mizar-core` owns guarded phrase/symbolic-definition and bounded means/equals-functor correctness/theorem lowering; existing `mizar-vc` intake generates Open VCs; `mizar-test` admits and compares |
| Dependencies | Prior sixteen C5 outcomes; authentic local existential-registration proof and attributed gates |
| Audit impact | Update Chapters 9/17/19 for bounded narrower-loci typing and pending coherence ownership; preserve other coverage and the approved specification |
...
[§10.6.1](../../../spec/en/10.functors.md#1061-functor-properties), [§16.6.2](../../../spec/en/16.theorems_and_proofs.md#1662-coherence-and-compatibility), [§19.1](../../../spec/en/19.overload_resolution.md#191-overview), [§9.9.6](../../../spec/en/09.predicates.md#996-negation-encoding), [§14.2.1](../../../spec/en/14.formulas.md#1421-predicate-applications),
...
| `pass_proof_verification_pred_symbolic_infix_001` | `vc_generation` / pass |
| `pass_proof_verification_func_means_prefix_001` | `vc_generation` / pass |
| `pass_proof_verification_func_equals_infix_operator_001` | `vc_generation` / pass |
| `pass_type_elaboration_pred_redefine_narrower_loci_001` | `type_check` / pass |
...
The [phrase checker](../../mizar-checker/en/type_checker.md#source-phrase-predicate-theorem-checking) and [Core owner](../../mizar-core/en/elaborator.md#source-phrase-predicate-theorem) retain actual binders, guarded definition and predicate goal; one Open VC gives no proof or accepted-definition credit. Their bounded symbolic extension owns the actual quantified-membership body and existing terminal VC kind; [primitive membership](../../mizar-core/en/core_ir.md#primitive-membership-formula) is a required Core representation gap, not new proof semantics. The means-prefix extension uses two pending correctness goals and an Opaque body; its defining equation is not a theorem premise. The [equals checker](../../mizar-checker/en/type_checker.md#bounded-equals-infix-functor-theorem-checking) and [Core owner](../../mizar-core/en/elaborator.md#bounded-equals-infix-functor-theorem) retain actual RHS coherence and an Opaque term body without equation premises. Preserve sixteen predecessor rows; add only narrower-loci declaration typing with sole `active_type_elaboration`, authentic same-source registration proof and Pending redefinition coherence under the [declaration owner](../../mizar-checker/en/type_checker.md#narrower-loci-predicate-declaration-checking). Means/equals generation retain their pending Core/VC boundaries; no accepted refinement or Task274 import. Except for the approved N2MarkedExists witness registration and additive phrase/symbolic/means/equals-functor snapshot references, preserve `.miz`, expectation outcome/phase/key, trace status/order, activation map, oracle scope, soundness policy, 23-case certificate rejection corpus, Task 277B state, and archive.
...
warnings-denied Clippy, and full tests. Exit requires seventeen mapped activations, retained controls plus actual phrase/symbolic/means/equals-functor bindings, pending correctness and theorem graphs, independent renaming, forged receipt/scope rejection, complete deterministic snapshots and a task-only local commit.
