...
Pending -> Ready -> CacheHit  (task 18 cache integration 後)
Pending -> Blocked
Ready   -> Blocked
Running -> Failed
...
running work を supersede してよい。cancellation が実装されるまでは、scheduler tests は
driver watch APIs ではなく explicit synthetic cancellation seam responses を使う。
...
Cache-aware scheduling は optimization seam である。

admitted ready task を実行する前に、scheduler は external cache seam に validated outcome
を問い合わせてよい:
...
- `ValidatedHit` は task 18 が real `mizar-cache` public contract を接続した後だけ
  有効である。task を `CacheHit` に遷移させ、execution を skip してよい。
- `Miss` と `Unavailable` は task を通常どおり実行する。
- `Error` は、後続の explicit cache-required mode が cache diagnostic を出すと定めない
  限り、`Miss` として扱う。
...
task 10 ではこの seam を `Unavailable` または `Miss` に default し、`CacheHit` を生成しては
...
correctness dependents だけを block する。independent tasks は継続してよい。詳細な
failure classes、blocked-state diagnostics、degraded-mode permissions は tasks 15-16 の
`failure_state.md` が定義する。
