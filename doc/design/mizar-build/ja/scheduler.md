...
| SCH-G003 | `external_dependency_gap` | `mizar-driver` crate は存在するが、request/session/registry/event authority は `mizar-build` の外側にある。 | caller-supplied graph/session-like inputs と dispatcher callbacks を受け取り、driver dependency や placeholder driver API を追加しない。 |
| SCH-G004 | `external_dependency_gap` | real IR sealed output handles と storage adapters は build-owned seam 経由では利用できない。 | scheduler tests では synthetic in-memory task outputs を使い、IR storage API を創作しない。 |
...
| SCH-G010 | `source_drift` / `test_gap` | scheduler はこれまで modeled synthetic outcomes だけを公開し、driver registry boundary を通じた real phase execution 用の scheduler-selected callback を公開していなかった。 | task 27 は [phase_dispatch.md](./phase_dispatch.md) が定義する callback seam と focused tests を追加する。phase input identities、producer outputs、diagnostics bridges、artifact tokens、LSP bridges が欠ける場合は placeholder ではなく external owner gaps として残す。 |
...
explicit synthetic cancellation seam responses を使う。driver watch integration は
build-owned ではないためである。
...
dispatch callback seam は [phase_dispatch.md](./phase_dispatch.md) が定義する。
`run_scheduler_with_dispatcher` と `SchedulerTaskDispatcher` は `SchedulerInput` から
分離された API として公開される。

scheduler は、ready task が cache seam を miss または decline し、
cancellation-before-start を通過し、worker/resource limits で admit された後にだけ
dispatcher を呼ぶ。validated cache hit は dispatcher を呼ばない。
running-checkpoint cancellation は callback execution を引き続き防ぎ、
completed-before-publication freshness は stale callback results を引き続き suppress する。

callback は phase semantics ではなく scheduler outcomes (`Complete`, `Failed`,
`Blocked`, `Skipped`, `Cancelled`) を返す。`mizar-build` はそれを task states、
failure/block records、resource releases、deterministic events/results へ写す。
`mizar-build` は registry phase outputs を IR handles、artifact publication tokens、
proof status、cache compatibility、diagnostics identity、LSP payloads へ変換しない。
...
- task-27 dispatch seam behavior: scheduler-selected callback は readiness、cache
  fallback、cancellation-before-start、resource admission の後にだけ実行されること。
  cache hits は callback を skip すること。callback blocks は synthetic outputs なしで
  propagate すること。running-checkpoint cancellation は callback execution を防ぐこと。
...
| Enum | Policy |
|---|---|
| `TaskState` | `#[non_exhaustive]`; downstream callers は wildcard match arms を含めなければならない。 |
| `SchedulerMode` | `#[non_exhaustive]`; downstream callers は wildcard match arms を含めなければならない。 |
| `CacheSchedulingPolicy` | `#[non_exhaustive]`; downstream callers は wildcard match arms を含めなければならない。 |
| `SyntheticTaskStatus` | `#[non_exhaustive]`; downstream callers は wildcard match arms を含めなければならない。 |
| `CompletionOrder` | `#[non_exhaustive]`; downstream callers は wildcard match arms を含めなければならない。 |
| `SchedulerQueue` | `#[non_exhaustive]`; downstream callers は wildcard match arms を含めなければならない。 |
| `SchedulerEventKind` | `#[non_exhaustive]`; downstream callers は wildcard match arms を含めなければならない。 |
| `SchedulerDiagnosticKind` | `#[non_exhaustive]`; downstream callers は wildcard match arms を含めなければならない。 |
