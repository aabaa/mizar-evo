...
19. **batch ビルド統合スイート。** [x]
    ...
    - task 19 scope: fixture はその時点で利用可能だった public `mizar-build`
      boundaries を覆い、`mizar-build` が消費できなかった
      driver/IR/producer-token owner seams を `external_dependency_gap` として記録する。
      fake driver APIs、IR handles、producer publication tokens、proof authority は
      追加してはならない。
...
20. **決定性スイート。** [x]
    ...
    - task 20 scope: implemented `mizar-build` seams を table-driven fixtures
      で覆う。`mizar-build` が消費できなかった driver/IR/producer-token
      clean/incremental owner seams は `external_dependency_gap` として記録し、
      placeholders は追加しない。
...
27. **Scheduler-selected real phase dispatch callback seam.** [x]
    - `SchedulerInput` から分離された `mizar-build` scheduler-selected dispatch
      callback API を公開する。これにより scheduler input は deterministic build data の
      ままで、external dispatcher が selected task を実行できる。
    - `mizar-build` は readiness、dependency ordering、resource admission、
      cancellation checkpoints、cache-aware decision consumption、deterministic
      collation を保つ。`mizar-driver` は scheduler-selected callback からだけ
      phase registry を呼んで seam を消費する。
    - Tests: deterministic dispatch order、cache-hit skip、blocked callback
      propagation、cancellation の focused scheduler callback tests。driver contract
      tests は registry-backed consumption を示し、scheduler semantics を複製しない。
    - Deps: D-022 までの completed `mizar-driver` closeout、既存 registry/query
      boundary、task 18 cache seam。Spec: `phase_dispatch.md`、`scheduler.md`、
      internal 01、architecture 14/22、mizar-driver `driver.md` と `registry.md`。
    - Forbidden: `mizar-build` に `mizar-driver` dependency を追加しない。fake phase
      adapter、stub producer output、provisional artifact token、cache/proof authority、
      artifact serialization、diagnostics identity allocation、LSP protocol conversion を
