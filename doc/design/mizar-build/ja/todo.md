...
第 B 波の module specs は `failure_state` まで存在する。残りの source 実装は
引き続き専用 task に従う。モジュール名は
...
- **cache-aware スケジューリングの時期: 未解決。task 18 で解決する。**
  タスク実行前のキャッシュ参照は `mizar-cache` の validation decision を
  消費し、将来は driver が所有する必須の `salsa` query 境界
  （`mizar-driver` task 4〜5）から呼び出せなければならない。それまでは
  build-side seam を用意した上で非キャッシュ実行とする。
