...
専用の仕様タスクが（英語と日本語を同じ変更で）執筆する。初期の `planner`
ソースは、同期済みの package manifest 仕様が要求する package-name 検証の
限定スライスとしてのみ存在する。モジュール名は
...
resolver のモジュール索引入力が workspace スタブを実際の planner 出力で
置き換えるため早期に着地させる。**第 B 波**（タスクグラフ、スケジューラ、
リソース、キャンセル、failure 状態）はスケジュール対象の検証 phase と
ともに到来し、合成タスクに対して開発できる。
...
3. **manifest と lockfile の構文解析。** [~]
   - パッケージ/workspace の manifest と lockfile を構文解析・検証し、
     manifest エラー診断を出す。
   - package manifest の `name` spelling 検証が、最初の限定スライスとして
     着地済み: package id は小文字の `snake_case`
     (`[a-z][a-z0-9]*(?:_[a-z0-9]+)*`) でなければならず、hyphenated spelling
     は拒否され、hyphen から underscore への正規化は行わない。完全な TOML
     構文解析、workspace manifest 検証、lockfile 検証は未完。
   - テスト: 有効/無効な manifest のフィクスチャ。lockfile 不一致の
     診断。決定的なエラー順。
   - 依存: 2。仕様: `planner.md`。

4. **依存グラフ解決と `BuildPlan` の生成。** [ ]
...
6. **モジュール索引の構築。** [ ]
   - `BuildPlan` とソースレイアウトからモジュール索引を構築し、resolver の
     プロバイダー契約を実装して `mizar-resolve` の workspace スタブを
     置き換え可能にする（同 task 7 の暫定 seam をクローズする）。
   - テスト: 複数パッケージのフィクスチャ。別名に依存しないモジュール
     識別。resolver スタブのフィクスチャとのプロバイダー同等性。
   - 依存: 4、5、`mizar-resolve` task 7。仕様: `module_index.md`。
