...
| RES-G002 | `source_drift` / `test_gap` | task 12 の前は `src/resource.rs` と resource-budget tests が存在しなかった。task 12 がこの spec に沿って accounting/admission source と focused tests を追加する。 | budget scopes が発展するとき、scheduler/resource source、tests、この spec を同期し続ける。 |
...
| RES-G004 | `external_dependency_gap` | driver request/session/watch integration は `mizar-build` の外側にある。 | budgets を input-driven かつ session-agnostic に保ち、driver dependency や placeholder driver API を追加しない。 |
...
`worker_count` は scheduler の synthetic executor dispatch cap のままである。
Resource budgets は追加の modeled admission layer であり、ready task は dispatch cap と
該当するすべての resource budgets が admit する場合だけ start する。
...
per-obligation backend fanout limit は `ResourceBudget` に置く。これは宣言済み
`BackendRun` tasks に対する build-side scheduling constraint であり、
`VcTaskDescriptor` の新しい field でも proof policy でもない。
...
- task id と queue。
- requested units。
- admitted、delayed、impossible、released status。
- blocking budget scope。
- deterministic admission attempt order。

release telemetry は、release される reservation の deterministic admission order を
再利用する。すでに reserved の task に対する duplicate admission attempt は idempotent
であり、二重に reserve せず original reservation order を報告する。
