...
で約束された public API family と挙動を source と tests へ trace する。task 27 は
mizar-driver closeout item DRIVER-G-011 に必要な scheduler-selected phase dispatch
callback seam についてこの監査を拡張する。
...
[phase_dispatch.md](./phase_dispatch.md)、[batch_integration.md](./batch_integration.md)、および
...
task 26 は移動された unit-test modules に対する source-layout 部分を再実行し、
追加で [module_boundary_refactor_gate.md](./module_boundary_refactor_gate.md) を監査する。
task 27 は新しい dispatch callback design について scheduler、driver-boundary、
bilingual audit portions を再実行する。

分類結果:

- `spec_gap`: 実装済み `mizar-build` behavior には見つからない。
- `test_gap`: `sorted_manifest_updates` の direct standalone coverage に対する
  BUILD-G-016 は残る。commit-order behavior は `commit_manifest_updates` を通じて
  すでに covered である。DISPATCH-G001 / BUILD-G-011 は task-27
  scheduler-selected callback coverage item であり、task-27 worktree の
  implementation と driver-consumption tests により解決済みである。
- `design_drift`: 実装済み `mizar-build` behavior には見つからない。
- `source_drift`: BUILD-G-018 は oversized inline unit-test modules に対する
  layout-only source drift として開かれ、解決済みである。DISPATCH-G001 /
  BUILD-G-011 は task 27 以前に missing scheduler-selected callback を記録し、
  `mizar-driver` dependency を追加せず build-owned dispatcher API を追加することで
  解決済みである。
...
- `external_dependency_gap`: owner-provided phase input identities、parent output
  handles、diagnostics/output publisher handles、IR producer outputs、artifact
  publication tokens、cache compatibility decisions、proof authority、artifact
  serialization、LSP conversion は下記のとおり残る。BUILD-G-019 は real phase
  dispatch に必要な owner inputs を分類し、それらを `mizar-build` で fabricate しては
  ならない。
...
| [phase_dispatch.md](./phase_dispatch.md) driver-consumption contract | build-owned scheduler callback は、driver-side readiness、dependency ordering、resource admission、cache-decision consumption、cancellation semantics を持たずに、`mizar-driver` から registry-backed dispatcher 経由で消費できる | `crates/mizar-driver/src/driver.rs`, `crates/mizar-driver/src/driver/scheduler.rs`; driver contract tests | driver tests は missing owner dispatch inputs が dispatch gap のままであること、owner-provided inputs が scheduler-selected callback から registry-backed phase execution を可能にすることを示す。source guards は scheduler semantics を `mizar-driver` front-door code から、fake outputs/tokens を seam から締め出す。 | No finding. |
...
| Scheduler-selected phase dispatch は build-owned readiness、dependency ordering、cache miss/fallback、cancellation-before-start、resource admission の後にだけ実行される。scheduler states だけを返し、phase semantics、proof、cache compatibility、artifact、diagnostics identity、LSP authority を `mizar-build` へ移さない。 | `scheduler.rs` dispatch API、task-27 scheduler tests、driver registry-consumption tests。 | No finding. |
...
| Module-boundary source layout は public modules を安定させたまま、大きな inline unit-test bodies を private child modules へ移動する。 | task-26 private test modules は [module_boundary_refactor_gate.md](./module_boundary_refactor_gate.md) に記録する。`cargo test -p mizar-build` は、移動された unit tests が同じ parent modules を引き続き実行することを確認する。 | No finding. |
| crate は `mizar-driver` に依存せず、driver-owned requests、sessions、event streams、registry semantics、phase input identity production、`salsa` query storage を実装しない。scheduler は driver-to-build 方向から driver が消費できる callback seam を公開する。 | `Cargo.toml` dependency tree、`scheduler.rs`、`tests/batch_integration.rs`、task-27 scheduler tests、driver boundary guard tests。 | No finding. |

## 残る gaps

task 27 source/test implementation 後には、新しい blocking/high `spec_gap`、`test_gap`、
`design_drift`、`source_undocumented_behavior`、`test_expectation_drift`、
`boundary_violation`、`repo_metadata_conflict` は残っていない。BUILD-G-018 は layout-only `source_drift`
として task 26 で開かれ、大きな inline unit-test bodies を private child modules へ
移動することで解決済みである。DISPATCH-G001 / BUILD-G-011 は missing
scheduler-selected dispatch seam として開かれ、task 27 source/tests により解決済みである。
...
| BUILD-G-002 | `external_dependency_gap` | real driver-owned requests、sessions、event streams、cache-query adapter、`salsa` query storage は `mizar-build` で実装しない。`mizar-driver` は存在し、driver-to-build 方向から build を消費する。`mizar-build` は entry-point agnostic のままで、`mizar-driver` dependency を持たない。 |
| BUILD-G-003 / BUILD-G-012 | `external_dependency_gap` | real sealed output handles、output storage、snapshot-handle rehydration は `mizar-build` の外側が所有する。実装済み scheduler tests は synthetic immutable output refs だけを使い、dispatch callback は IR handles を fabricate しない。 |
...
| BUILD-G-019 / DISPATCH-G002 / DISPATCH-G003 | `external_dependency_gap` | owner-provided phase input identities、parent output handles、diagnostics/output publisher handles、producer outputs、artifact publication tokens は full real phase output publication に必要である。task 27 は scheduler-selected dispatch callback だけを公開し、missing owner inputs を gaps として記録する。fake adapters、stub outputs、provisional tokens は作らない。 |
...
VC / proof crate tests によりすでに検証している。task 26 は Rust source layout
のみを変更し、その verification は `cargo test -p mizar-build`、`cargo clippy -p
mizar-build --all-targets -- -D warnings`、`cargo fmt --check`、追加で利用可能な
adjacent checks の `mizar-cache`、`mizar-artifact`、`mizar-vc`、`mizar-proof` に合格
した。task 27 verification は task commit とともに記録し、focused `mizar-build`
scheduler tests、`mizar-driver` contract tests、利用可能な adjacent seam checks、
fmt、clippy、full workspace tests、diff checks を含む。
