...
- `source_drift`: BUILD-G-018 は oversized inline unit-test modules に対する
  layout-only source drift として開かれ、解決済みである。実装済み behavior drift
  は残っていない。
...
task 26 後には、新しい blocking/high `spec_gap`、`test_gap`、`design_drift`、
`source_drift`、`source_undocumented_behavior`、`test_expectation_drift`、
`boundary_violation`、`repo_metadata_conflict` は残っていない。BUILD-G-018 は
layout-only `source_drift` として task 26 で開かれ、大きな inline unit-test
bodies を private child modules へ移動することで解決済みである。
...
した。`mizar-driver` はこの checkout には存在せず、`external_dependency_gap` のまま
残る。
