...
task 19 は、`mizar-driver`、`mizar-ir`、real producer publication token が
まだ存在しない段階で利用できる build-side integration path を対象にする。
...
| BUILD-G-011 | `external_dependency_gap` | `mizar-driver` が存在しないため、real `BuildRequest`、build session、phase registry、event stream、driver-owned salsa boundary を消費できない。 | integration test は entry-point agnostic に保つ。driver-owned APIs や `mizar-driver` dependency を追加しない。 |
| BUILD-G-012 | `external_dependency_gap` | `mizar-ir` が存在しないため、real phase output handles と snapshot-handle rehydration が利用できない。 | scheduler integration では scheduler-owned synthetic output refs だけを使う。placeholder IR handles を作らない。 |
