...
`operator_fixity` は依存字句サマリが公開しているデータからのみ埋める。現在の summary shape がまだ fixity を公開していない場合、stub path では `OperatorFixityTable { entries: Vec::new() }` を使い、実 Pratt/fixity テストは `mizar-parser` / `mizar-syntax` の availability に gate する。`StringRequiredContext::None` は通常の source-to-token foundation mode であり、`UniformForTest` は字句解析器全体を意図的に `ParserLexContext::string_required()` で実行する bounded test 専用である。位置別 string-required span と parser-driven symbol-kind filter はこの初期型では表現しない。real source input で文法位置の文字列リテラルが必要になる前に、parser-assisted lexing contract で追加する。
...
統合がパーサー駆動の曖昧性解消を用いる場合、パーサーは狭い文脈オブジェクトを通じて文字列必須位置または symbol-kind filter を Step 4 へ供給する。その契約が landing するまでは、実 parser 統合は source-level の注釈／演算子文字列リテラルトークン化を要求してはならない。それらのテストは synthetic token stream を使うか gate されたままにする。パーサーが字句解析器のカーソルを直接駆動することは決してない。
...
- stub source-to-token path では `ParserInputs` が空の `OperatorFixityTable` と `StringRequiredContext::None` を使う。
...
- 注釈引数位置の `StringLiteral` トークンが注釈ノードへ解析され、文字列必須位置での文字列リテラルの欠落が期待される構文診断を生む。real source text に対する parser-assisted lexing contract が確定するまでは synthetic token stream を使う。
