...
- モジュールとタスク状態は同期済みである。task 1-21 は完了済みであり、
  follow-up task 22-24 は未着手のままである。
...
- loading-map preservation、粗い raw-scan recovery、provider provenance
  ...
  source-load location、resident-set の `ModuleLexicalSummary` 境界、task 19 の
  cache-key storage/computation boundary、task 20 の parser-assisted
  lexing-plan boundary、および task 21 の durable lint-policy guard について、
  挙動の約束は同期済みである。
...
| [preprocess.md](../en/preprocess.md) | [preprocess.md](./preprocess.md) | 公開 API、comment/doc-comment handling、import stub、粗い raw import recovery、診断、parser-assisted string-argument handling を同期済み。 |
...
| [source_spec_correspondence.md](../en/source_spec_correspondence.md) | [source_spec_correspondence.md](./source_spec_correspondence.md) | task 16 の監査文は、task 19 の cache-key wiring、task 20 の parser-assisted lexing、task 21 の durable lint enforcement、残 task 22-24 を記録するよう同期済み。 |
| [todo.md](../en/todo.md) | [todo.md](./todo.md) | タスク状態と follow-up 記録を同期済み。task 21 はこの結果とともに完了済み。 |
...
この監査では新しい follow-up task は追加していない。task 18、task 19、task 20、
task 21 はその後完了した。既存の未着手 follow-up は次のとおり。
