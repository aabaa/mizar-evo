...
- モジュールとタスク状態は同期済みである。task 1-26 は完了済みである。
...
- loading-map preservation、精密な recoverable raw-scan recovery、provider provenance
  ...
  および task 26 の公開 API rustdoc summary 方針について、挙動の約束は同期済みである。
...
task 21、task 22、task 23、task 24、task 25、task 26 はその後完了した。現在予約されている
