...
- モジュールとタスク状態は同期済みである。task 1-24 は完了済みである。
...
- loading-map preservation、精密な recoverable raw-scan recovery、provider provenance
  ...
  task 22 の raw-scan recovery boundary、および task 24 の予約 diagnostic
  surface 方針について、挙動の約束は同期済みである。
...
task 21、task 22、task 23、task 24 はその後完了した。現在予約されている fallback
variant に対する具体的 producer を将来の non-exhaustive lexer/session/parser
