...
range-backed 診断はフェーズ優先度、次に第一 `SourceRange` の開始で統合され、順序は実行間で安定し内部スケジューリングと独立である。
...
クラス内では、range-backed 診断を第一スパンの開始、次に診断コードで順序付ける。source-load 診断は `FrontendError` として返り、フェーズ成果物が存在しないため、返却される `FrontendOutput` の統合順には参加しない。バッチ表示などで複数の source-load failure を並べる場合は、安定な source-load location key、次に診断コードで順序付ける。副次 `SourceAnchor` は表示・説明用に保持するが、ソートには参加しない。後の診断が先の回復に影響されうる場合は回復ノートを付ける。
...
- 返却される出力診断の統合順序は決定的であり、range-backed 診断についてはスパンキー付けである。
