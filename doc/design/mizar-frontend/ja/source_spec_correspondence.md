...
この監査は、task 15 後の `mizar-frontend` 実装を、まず英語正本の
...
- task 1-16 が約束する公開 API とエラー／診断 variant について、欠落した
  実装は見つからなかった。
