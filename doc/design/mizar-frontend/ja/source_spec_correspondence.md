...
| [source.md](./source.md) | `SourceUnit`, `SourceUnit::{canonical_disk_bytes, from_canonical_disk_bytes}`, `SourceUnitRequest`, `SourceUnitLoader`, `SourceUnitLoader::load_source_unit`, `FrontendSourceLoader`, `FrontendSourceLoader::new`, `source_unit_from_loaded`, `register_source_unit` | `crates/mizar-frontend/src/source.rs` | inline tests が再計算しない projection、loader forwarding、BOM/CRLF loading map、identity load、open-buffer origin/version、open-buffer `file://` path decode/fallback、generated source、bridge registration、`SourceLoadError` の非再分類伝播を確認する。 Disk codec の往復・再束縛・拒否テストを含む。 |
...
99/100。exact staging/cached review と implementation commit
