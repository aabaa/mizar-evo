...
`ResolvedImport`、`ModuleLexicalSummary`、`ActiveLexicalEnvironment`、`LexicalEnvironmentFingerprint`、`LexicalEnvironmentError` は `mizar-lexer` から再エクスポートされる。`LexicalSummaryProvider` は、ビルドプランナー／解決器が解決済みインポートと字句サマリを供給するための継ぎ目である。フロントエンドはそれらを構築するためにモジュール IR へ踏み込まない。
