...
`SourceUnit` はアーキテクチャのインターフェースを反映し、`origin` を追加して、以降のフェーズ（前処理診断、LSP オーバーレイ）が session レコードを読み直さずにディスク・オープンバッファ・生成テキストを区別できるようにする。
...
- 外部: `mizar-session`（`SourceLoader`、`LoadedSource`、`SourceInput`、`SourceId`、`LineMap`、`LoadingMap`、`SourceOrigin`、`SessionIdAllocator`、`BuildSnapshotId`）、session ローダー経由のファイルシステムとパッケージメタデータ。
...
`SourceUnit` は 1 つの `.miz` ファイルに対する不変かつソース忠実な読み込み済みレコードである。`source_text` は `mizar-session` が生成した検証済み・ソース読み込み正規化済みテキストそのものである。`source_hash`、`line_map`、`loading_map` は session の値であり、再計算せずにコピーする。`file_path` は診断用のローカル表示パスであり、公開される同一性は session レコード内の正規化パスを用いる。

`SourceUnit` は [architecture/en/02.source_and_frontend.md](../../architecture/en/02.source_and_frontend.md) の「Incrementality」における Step 1 のキャッシュキーの基点であり、そのキーはファイルパスとソースバイトで、いずれも session の `SourceVersion` が既に捕捉している。
...
3. 返された `LoadedSource` を `SourceUnit` へ射影し、`source_id`、`package_id`、`module_path`、テキスト、ハッシュ、line map、loading map、origin をそのまま保持する。
4. 読み込んだ `LineMap` / `LoadingMap` を `SourceId` の下で `span_bridge` レジストリに登録し、後のフェーズがスパンを変換できるようにする。
5. `SourceUnit` を返す。
...
- ディスクの `LoadedSource` が、同一の `source_id`・`source_hash`・`line_map`・`loading_map` を持つ `SourceUnit` へ射影される（再計算なし）。
...
- `source_hash`・`line_map`・`loading_map` はフロントエンドが再計算しない。
