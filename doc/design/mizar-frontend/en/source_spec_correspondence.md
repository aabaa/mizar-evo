...
Status: completed through task 29 and updated for the mizar-lexer
source-position-aware operator metadata integration.
...
- No missing implementation was found for the public APIs and error/diagnostic
  ...
  policy, the task-28 parser-growth follow-through, the task-29 real-parser
  frontend fuzz coverage, or the mizar-lexer source-position-aware operator
...
- No remaining stale English canonical spec text was found for tasks 1-29 or the
  source-position-aware operator metadata integration.
...
| [source.md](./source.md) | `SourceUnit`, `SourceUnit::{canonical_disk_bytes, from_canonical_disk_bytes}`, `SourceUnitRequest`, `SourceUnitLoader`, `SourceUnitLoader::load_source_unit`, `FrontendSourceLoader`, `FrontendSourceLoader::new`, `source_unit_from_loaded`, `register_source_unit` | `crates/mizar-frontend/src/source.rs` | Inline tests cover projection without recomputation, loader forwarding, BOM/CRLF loading maps, identity loads, open-buffer origin/version, open-buffer `file://` path decode/fallback, generated sources, bridge registration, and unchanged `SourceLoadError` propagation. Disk codec tests cover roundtrip/rebinding and rejection. |
