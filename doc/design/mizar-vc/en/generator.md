...
`generate_source_algorithm_postconditions(&CoreIr, BuildSnapshotId, &GenerationSchemaVersion, &VcSchemaVersion) -> Result<VcSet, String>` validates the single-object-parameter flat profile, derives CFG/handoff and preserves existing intake/normalization. The immutable one-return case retains its Core-term equality representation. Initialized var/const, typed local writes and equality assertions use authenticated program-value versions: evaluate RHS before a write, preserve old facts and independent storage, then add the new equality/type guard. A single prefix-negated equality assertion retains existing Not(Generated(ProgramEquals)) rows with actual variable/write identities and both source formula ranges; all rows remain accounted for, and ensures stays equality-only. Each assertion goal sees its pre-context; later contexts carry that exact goal only as a pending dependency on its assertion handoff. Return substitutes the current value for result. Raw normalized contexts remain internal; complete source/owner/maps/provenance, write order and dependency links are validated atomically. Assertions and ensures yield open VCs, while metadata termination retains original-status NoConcreteVc accounting; no obligations yields the complete zero-VC result. Unsupported control flow, ghost picks or unsupported ghost flow, malformed or diagnostic input fail closed. Anchors and unresolved dependencies stay reuse-ineligible; no discharge, acceptance or terminating promotion occurs.
...
The implemented snapshot extension uses `generate_source_algorithm_postconditions` after complete Core/CFG capture validation. Initialized ghost var/const use the existing program-value generations; runtime sinks still reject ghost dependencies. Snapshot capture leaves state and assumptions unchanged; the ordinary return postcondition remains Open.
Preserve every handoff row: ghost initialization retains its existing GhostErasure/NoConcreteVc accounting and termination its original unresolved status. A capture is neither a proof nor a ghost-erasure certificate; snapshot claim substitution, loops beyond the bounded while and postcondition failure beyond the bounded return Pick remain unsupported.

## Bounded while postconditions

`generate_source_algorithm_postconditions` additionally validates this one-loop Core graph and fresh CFG/handoff, including loop-owned header/backedge sites and the actual final return. Compute MayWrite from the authenticated body destination; at the cutpoint freshen only that local and preserve the immutable parameter. Entry initialization facts remain entry-only. Emit Open entry and preservation invariant VCs and an Open return VC; preservation uses the havoced invariant/guard context followed by the actual old-state assignment, while return uses the cutpoint invariant and negated guard, never body-write facts. Authenticate both invariant handoffs and the exact instantiated exit summary inside this source generator. Use existing LoopInvariantAvailable for the exit hypothesis and PremiseRef::ConservativeUnknown for unresolved dependencies, keeping real dependency slices reuse-ineligible; no typed dependency links or detached VcSet source authentication are promised; the body invariant is the induction hypothesis, not an accepted proof or a self-dependency. Preserve every handoff row and unresolved metadata-only termination; no decreasing VC, evaluation, discharge or promotion follows.

## Bounded runtime Pick postconditions

`generate_source_algorithm_postconditions` additionally authenticates the complete source-derived Pick/Return pair, contained ranges, distinct parameter/result/Pick/logical binders, bare-set predicate, single ExistingCore inhabitation seed and fresh CFG/handoff. Emit an Open `GeneratedNonEmptiness` VC for the actual `exists q. is_set(q)` goal without assuming the selected value, and an Open postcondition over exactly object(a), set(p), where p is keyed by the actual Pick statement. Substitute p for result; never infer p=a from type compatibility or distinctness from IDs. Preserve unresolved algorithm termination accounting; no builtin suppression, StableChoice origin, Discharged evidence or generic CFG redesign. Whole-result replay owns semantic authentication, and dependency/reuse remains conservative.
