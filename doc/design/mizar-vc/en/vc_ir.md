...
| VCIR-G003 | `external_dependency_gap` / `deferred` | Task 2 originally treated `mizar-atp`, `mizar-kernel`, `mizar-proof`, and `mizar-cache` as inactive consumers. `mizar-kernel` tasks 23-29 now provide the checker-side formula/substitution evidence path, and post-closeout VC tasks 24-26 provide the producer-side handoff builder plus reuse identity integration, but ATP candidate production, proof/cache consumers, and artifact witness consumers remain later work. | Keep `VcIr` prover-independent and record only untrusted deterministic evidence references, status states, reuse-candidate data, and task-24 handoff rules. Do not add SAT clauses, backend traces, or trusted acceptance fields. |
