...
`VcProgramValue { var: CoreVarId, definition: Option<CoreAlgorithmStmtId> }` identifies an immutable parameter with None or a value after its actual declaration/write with Some; Some(loop statement) additionally identifies the arbitrary cutpoint value of a validated MayWrite local, without implying an assignment equality; the generator validates owner, destination, source order and definition-before-use against Core.
