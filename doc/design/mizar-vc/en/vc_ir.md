...
| VCIR-G003 | `external_dependency_gap` / `deferred` | Task 2 originally treated `mizar-atp`, `mizar-kernel`, `mizar-proof`, and `mizar-cache` as inactive consumers. `mizar-kernel` tasks 23-30 now provide the checker-side formula/substitution evidence path and goal-polarity binding, and post-closeout VC tasks 24-28 provide the producer-side handoff builder, explicit goal-polarity declaration/rejection, reuse identity integration, and context-identity payload, but ATP candidate production, kernel-side F2 membership verification, proof/cache consumers, and artifact witness consumers remain later work. | Keep `VcIr` prover-independent and record only untrusted deterministic evidence references, status states, reuse-candidate data, and task-24/28 handoff rules. Do not add SAT clauses, backend traces, or trusted acceptance fields. |
...
| VCIR-G006 | `deferred` | Discharge evidence and dependency slices are now implemented by later VC tasks, and post-closeout VC tasks 24-28 implement the producer-side kernel-evidence handoff, explicit goal-polarity declaration/rejection, reuse identity integration, and context-identity payload. ATP translation, downstream proof/cache/artifact or corpus consumers, kernel-side F2 verification, and source-derived corpus runner activation remain later tasks or external seams. | Task 9 must not generate discharge evidence, compute dependency slices, translate ATP problems, invoke kernel/proof/cache/corpus paths, or fabricate downstream integration records. |
