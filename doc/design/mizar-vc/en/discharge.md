...
| DIS-G003 | `external_dependency_gap` / `deferred` | Task 10 originally treated ATP/kernel/proof/cache/corpus consumers as unwired. `mizar-kernel` now owns corrected formula/substitution evidence checking, but `mizar-vc` still lacks the task-25 handoff builder and downstream ATP/proof/cache/artifact consumers. | This spec records only prover-independent statuses, untrusted evidence, and deferred downstream integration points. Discharge rule names or evidence hashes are not trusted kernel acceptance material. |
