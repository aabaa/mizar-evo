...
```text
Continue mizar-vc autonomous STEP 5 development from the completed docs-only
VC Task 30 commit. Before editing, verify its commit hash and a clean worktree,
then re-read `doc/design/mizar-vc/en/source_vc_decomposition.md`, `generator.md`,
`vc_ir.md`, `00.crate_plan.md`, `todo.md`, `task_ledger.md`, the exact Core
Task-31 authority, and the prepared mizar-test harness/snapshot/traceability
contracts. Implement VC Task 31 only: validate the exact marker-free Task-180
CoreIr/direct TerminalGoal relation, empty ControlFlowOutput, one ExistingCore
handoff/intake row, and produce exactly one `VcId(0)`
`TerminalProofGoal`/`Open` VC with preserved owner/source/path/origin, empty
context/premises/hints/generated formulas, and an incomplete anchor missing
only `CanonicalGoalHash`. Add the distinct
`pass_proof_verification_contradiction_formula_constant_001` proof-verification
source/sidecar, first runner/tag/guard, complete double-generated
`VcSet::debug_text()` VcIr baseline, exact frozen trace row, and all specified
admission/corruption tests in the same logical task. Do not inject a terminal
marker, alter or admit the existing type-elaboration sidecar, fabricate a goal
hash, discharge, transition to NeedsAtp, verify/accept the theorem, publish a
fact, implement broader VC families, or promote Steps 6/7. Run focused tests,
fmt, full-feature Clippy, crate/workspace tests, four CLI oracles, count/hash
oracles, and diff checks; complete all review-only phases before the one Task-31
commit.
```

Rationale: Task 31 is the first real source-to-VC phase-11 vertical slice and
crosses structural identity, seed accounting, anchor honesty, runner admission,
and traceability boundaries. Keep `xhigh`; lower reasoning is appropriate only
for a separately authorized documentation-only typo fix.
