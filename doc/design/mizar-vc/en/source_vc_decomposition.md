...
|---|---|---|---|
| 32 | General theorem proof-step and terminal-goal VCs, stable formula payloads, quantified binders, and ordered local contexts. Specs 04.5, 14, 15, and 16.1-16.5/16.7. | Core 33-35/37; `MT10-VC-PV/VC32`. | Task 180 remains VC 31. No proof search, closure, acceptance, or verified-premise publication. The [bounded computation request](generator.md#bounded-computation-request-generation), under Spec 20.9.2, retains its open equality and source options; its narrow trace section is `VC Task 32; bounded source computation request VcIr snapshot`. |
...
| 39 | Reduction `reducibility` universal-equality obligation. Specs 17.6 and 17.9.4. | Core 39; `MT10-VC-PV/VC39`. | Simplification-order/size/variable checks are structural registration-time rejection rules and never VCs. No rewrite activation or formula synthesized from labels/order alone. The bounded identity/singleton profile retains `verification`/`fail` as an explicit exception to the shared `vc_generation` phase, comparing its open Reduction VcIr baseline before the [failure observation](./discharge.md). Its narrow trace section is `VC Task 39; bounded identity-singleton reduction VcIr snapshot`; broader reduction verification remains deferred. |
...
| 43 | `requires` as callee-body context; return-result substitution, postcondition, and assertion VCs. Specs 20.4-20.5 and 20.13.1/20.13.3. | Core 42-43/46/48/52-53; `MT10-VC-PV/VC43`. | A declared entry `requires` is not an `AlgorithmPrecondition` proof VC. No return/postcondition fact without its generated goal. Bounded equality ensures and flat variable/const/assignment/assertion contexts use the [source return generator](./generator.md#bounded-source-return-postconditions); requires and wider control-flow coverage remain deferred. The state trace section is VC Task 43; bounded straight-line state and assertion VcIr snapshot. The exact contradictory-assertion row is a `verification`/`fail` exception to the shared `vc_generation` phase: compare the full open VcIr before the [bounded failure observation](./discharge.md). Its required snapshot trace section is `VC Task 43; bounded contradictory assertion VcIr snapshot`; no generic verification or termination acceptance is credited. The implemented [flat snapshot profile](generator.md#flat-snapshot-postconditions) retains the ordinary return goal and unresolved ghost accounting; its snapshot trace section is `VC Task 43; bounded ghost snapshot return VcIr snapshot`. |
...
| 52 | Recursive and mutually recursive component-decrease obligations. Specs 20.7-20.8 and 20.13.4. | Core 46/52-53; `MT10-VC-PV/VC52`. | No promotion to `func`, call-graph acceptance, or termination fact. Promotion premises belong to VC 57 and 59. |
...
| 54 | Snapshot/claim theorem obligations with captured program context and universal valid-execution quantification. Specs 20.6 and 20.13. | Core 47/51-53; `MT10-VC-PV/VC54`. | No state theorem, old-state substitution, or claim acceptance without explicit captured payload. The [bounded void-claim producer](generator.md#bounded-void-claim-generation) validates the actual empty execution interface and algorithm-linked proof obligation; broader capture/substitution stays deferred. |
...
| 56 | Induction-hypothesis context for a `terminating` algorithm with a header measure: the contract of the algorithm and its recursive group at strictly smaller lexicographic measure, in every VC of its body. Specs 20.4.1 and 20.13.4. | Core 46/52; VC 43-44, 47, and 51-52; `MT10-VC-PV/VC56`. | No hypothesis at the current arguments and none without a header measure. No termination acceptance or promotion premise. |
| 57 | Module-local premise availability: totality, contract, and defining-equation axioms (Core 54) of an algorithm declared in the same module, as cited or policy-exposed premises after its termination obligations are freshly accepted inside one authenticated source orchestration, following the bounded [source existential-registration proof](../../mizar-proof/en/status.md#source-existential-registration-proof). Specs 16.5.1, 20.7.2, and 20.13.2. | Core 54; VC 52; resolver algorithm citations; mizar-proof module-local acceptance; `MT10-VC-PV/VC57`. | Nothing is available before genuine acceptance; acceptance is never derived from obligation presence, order, or local checking, and the accepted state is not a transferable receipt. Item order is preserved. |
| 59 | **Blocked-reserved imported premise availability:** promotion axioms of an algorithm exported by another module. Specs 12, 16.5.1, 20.13.2, and 23. | VC 57; unexecuted `MT10-VC-PV/VC59`. Current canonical authority names no producer, schema, or authentication rules for transferable accepted status. | No imported premise without authenticated transferable acceptance; re-proving another module's obligations is not a substitute. Blocked until that authority exists. |
| 58 | Uniqueness obligation of a `by computation` step for each value-returning algorithm in its computation closure that contains a non-ghost Pick site or a `for ... in` loop, with the syntactic `result = t` / `result = true iff φ` exemption. Specs 20.2.4, 20.3, and 20.9.2. | Existing bounded computation request; Core 42, 44, and 46; VC 44; `MT10-VC-PV/VC58`. | The closure is computed within the same package; an imported callee available only through a computational export leaves it unestablished and the step open. No MVM evaluation, computation acceptance, public E0330 projection, or canonical iteration order. Ghost-only Pick is not a source. |
