...
| 43 | `requires` as callee-body context; return-result substitution, postcondition, and assertion VCs. Specs 20.4-20.5 and 20.13.1/20.13.3. | Core 42-43/46/48/52-53; `MT10-VC-PV/VC43`. | A declared entry `requires` is not an `AlgorithmPrecondition` proof VC. No return/postcondition fact without its generated goal. Bounded one-return equality ensures uses the [source return generator](./generator.md#bounded-source-return-postconditions); requires/assert/stateful coverage remains deferred. |
