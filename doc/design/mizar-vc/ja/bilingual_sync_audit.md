...
| `bilingual_sync_audit.md` | audit scope、method、pair inventory、classification、Task 19/21/22 sync edits。 | この paired audit document により同期済み。 |
...
| `task_ledger.md` | Task status、Task 21 までに利用可能な commit hash、review outcome、verification summary、deferred notes、handoff prompt。 | Task 22 で Task 21 hash を backfill し Task 22 audit checkpoint を記録した後に同期済み。 |
| `todo.md` | ordered task list、完了 task、残り closeout scope、recommended verification、notes。 | Task 22 で module-boundary refactor gate を完了にした後に同期済み。 |
...
- `deferred`: 大きい `vc_ir`、`generator`、`dependency_slice` implementation file 内の
  ...
  として残る。closeout は final quality review と crate-exit reporting を所有する。
