...
これらの hash は reuse invalidation input にすぎない。`mizar-vc` が kernel caller になるわけではなく、
handoff package を proof acceptance へ昇格させるものでもない。
...
- canonical formula-envelope hash と task-28 context-identity hash を含み、task 25 後の
  proof-reuse candidate を invalidation するための optional kernel evidence handoff
  identity entry。
...
- `kernel_evidence`: caller が提供した task-25 canonical kernel evidence handoff hash、
  task-28 context-identity hash と rows、target binding、schema/encoding/profile identity、
  imported formula context requirements;
...
- 同じ VC に対する task-25/28 `VcKernelEvidenceHandoff` が供給され、その canonical
  formula-envelope hash と `context_identity_hash()` が proof-reuse key payload に
  含まれていること;
