...
Task 26 は [kernel_evidence_handoff.md](./kernel_evidence_handoff.md) が生成する
canonical kernel evidence handoff hash を、slice と proof-reuse identity boundary に追加する。
Task 28 はその boundary に非 import source-binding の `context_identity_hash()` を追加する。
Task 29 は imported formula evidence が供給された場合の imported-statement projection data を
kernel-evidence dependency payload に追加する。これらの field は reuse invalidation input に
すぎない。`mizar-vc` が kernel caller になるわけではなく、handoff package を proof acceptance へ
昇格させるものでもない。
...
- canonical formula-envelope hash、task-28 context-identity hash、task-29
  imported-statement projection payload を含み、task 25 後の proof-reuse candidate を
  invalidation するための optional kernel evidence handoff identity entry。
...
- kernel checking、SAT solving、ATP backend execution、proof-witness persistence、
  cache lookup;
...
- caller が dependency slice と proof-reuse key に kernel evidence identity を含めたい場合の、
  `VcId` で key 付けされた optional task-25 `VcKernelEvidenceHandoff`;
...
- `kernel_evidence`: caller が提供した task-25 canonical kernel evidence handoff hash、
  task-28 context-identity hash と rows、task-29 imported-statement projection payload、
  target binding、schema/encoding/profile identity、imported formula context requirements;
...
- handoff が供給された場合の kernel evidence handoff hash、context-identity hash、
  imported-statement projection payload、imported formula context requirements;
...
candidate-key helper を追加する。Task 26 はこの helper を強化し、現在の task-25 kernel
evidence handoff が供給され、その canonical hash が slice/key identity に参加しない限り
candidate key を利用不能にした。Task 28 は同じ boundary に `context_identity_hash()` を
追加する。この helper は次がすべて成り立つ場合にだけ key を返す:
...
- 渡された `DependencySliceSet` が、その同じ `VcSet` と `DischargeOutput` から新しく計算した
  slice、および kernel evidence handoff と fingerprint、completeness、kind、status で一致すること;
- `ObligationAnchor` が complete で slice も complete であること;
- canonical VC fingerprint と local-context fingerprint が利用可能であること;
- 同じ VC に対する task-25/28/29 `VcKernelEvidenceHandoff` が供給され、その canonical
  formula-envelope hash、`context_identity_hash()`、imported-statement projection payload
  boundary が、slice が他の条件で complete な場合の proof-reuse key payload に含まれていること;
...
この helper は kernel evidence handoff identity がない場合、preserved / pre-existing discharged
status、欠けている replay data、missing または unstable evidence hash、incomplete anchor、
incomplete slice、stale slice set、non-discharged status、policy/evidence mismatch では
key を返してはならない。Proof-witness hash、cache lookup、kernel acceptance、
ATP certificate validation、artifact consumer は downstream
...
Task 26 は Rust coverage として次を追加する:

- canonical kernel evidence handoff hash が dependency-slice fingerprint と
  proof-reuse candidate key に参加すること;
- kernel evidence handoff がない場合に proof-reuse key が利用不能であること;
- canonical kernel evidence hash が変わると proof-reuse が invalidation されること;
- duplicate、unknown、または selected-VC と mismatch する kernel evidence handoff input が
  fail closed になること;
- downstream proof/cache/artifact consumer が external/deferred のままであること。

Task 28 は Rust coverage として次を追加する:

- context-identity hash が dependency-slice fingerprint と proof-reuse candidate key に
  参加すること;
- canonical evidence hash が変わらないまま context-identity hash が変わると proof-reuse が
  invalidation されること;
- local-hypothesis、cited-premise、generated-VC-fact row が context identity で cover され、
  imported axiom/theorem entry は除外されること。

Task 29 は Rust coverage として次を追加する:

- imported formula evidence 由来の imported-statement projection payload が
  `kernel_evidence` dependency payload に参加すること;
