...
この hash は reuse invalidation input にすぎない。`mizar-vc` が kernel caller になるわけではなく、
...
- task 25 後の proof-reuse candidate を invalidation するための optional kernel
  evidence handoff identity entry。
...
- handoff が供給された場合の kernel evidence handoff hash と imported formula context requirements;
...
candidate key を利用不能にする。この helper は次がすべて成り立つ場合にだけ key を返す:
...
- 同じ VC に対する task-25 `VcKernelEvidenceHandoff` が供給され、その canonical hash が
  proof-reuse key payload に含まれていること;
...
- kernel evidence handoff hash が dependency-slice fingerprint と proof-reuse candidate key に
  参加すること;
