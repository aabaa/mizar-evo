...
- `deferred`: proof-witness hash、ATP/kernel/proof/cache validation、artifact consumer、
  ...
  candidate key の外で受理する前に実装しなければならない。
- `deferred`: Task 22 は module-boundary refactor gate、closeout は final quality review と
  crate-exit status を記録する。
