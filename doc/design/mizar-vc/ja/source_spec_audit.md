...
- `CoreGenerationInput`, `VcNormalizationInput`,
  ...
  `CoreGenerationNoCandidate`, `GeneratorError`, `generate_source_algorithm_postconditions`, `generate_source_void_claim`, `generate_source_computation_request`
...
| proof-obligation handoff は explicit refutation polarity を宣言し、canonical package assembly 前に consistency polarity を拒否する。 | `KernelEvidenceHandoffInput::goal_polarity`、現在の `VcKind` 全列挙の `required_goal_polarity`、`KernelEvidenceHandoffError::GoalPolarityMismatch`、`KernelFinalGoalEvidence::polarity`。 | `consistency_goal_polarity_for_proof_obligation_fails_closed` と deterministic handoff polarity assertion。 | producer-side の現在 VC kind 向けに実装済み。checker-side acceptance binding は `mizar-kernel` task 30 で実装済み。 |
...
| 非 import の local-hypothesis、cited-premise、generated-VC-fact source binding は、target VC と canonical evidence hash に bind された producer-side context identity row を持つ。 | `KernelContextIdentityPayload`、`KernelContextIdentityEntry`、`KernelContextIdentitySource`、`VcKernelEvidenceHandoff::context_identity_hash`、`context_identity_hash_input`。 | `context_identity_covers_non_imported_source_bindings`、`context_identity_binding_breaks_when_source_label_is_mutated`、dependency-slice kernel-evidence identity coverage。 | producer-side task 28 向けに実装済み。kernel-side membership verification は `mizar-kernel` task 31 で実装済み。 |
| imported context / payload data は、empty context provenance、unsupported imported-statement / formula fingerprint algorithm、stale statement projection、formula-projection mismatch、same-requirement projection conflict、empty または noncanonical imported-statement projection payload、missing context/payload data で fail closed する。返される context requirement は canonical sorted/deduplicated である。 | `validate_import_requirement`、`validate_imported_statement_projection`、`imported_payload_map`、`canonical_context_requirements`。 | imported context missing/mismatch、empty context provenance、unsupported algorithm、statement-projection mismatch、projected-formula mismatch、conflicting-projection、empty/noncanonical-projection、duplicate-context tests。 | 現在の explicit imported payload 向けに実装済み。kernel-side trusted validation と pass fixture は対になる `mizar-kernel` task 33 で実装済み。 |
...
`mizar-kernel` task 30 で実装済みである。

## Task 28 context identity follow-up

Task 28 は `VcKernelEvidenceHandoff` に producer-side `context_identity` payload を追加する。
この payload は各非 import local-hypothesis、cited-premise、generated-VC-fact source
binding に対する deterministic row を記録し、それらの row を選択 target VC と canonical
evidence hash に bind し、dependency-slice / proof-reuse identity 用に
`context_identity_hash()` を expose する。

この task は kernel call、checker/core semantic change、ATP backend integration、
source payload の捏造、proof row、cache promotion、artifact witness publication、
corpus runner activation、`doc/spec` edit、`.miz` fixture、expectation update を追加しない。
Kernel-side F2 membership verification は `mizar-kernel` task 31 で実装済みであり、
`mizar-vc` producer-side source surface の外に残る。
...
declaration/rejection contract を閉じ、Task 28 は producer-side F2 context-identity hash を
...
- `external_dependency_gap`: upstream explicit/stable payload は不完全だが、Task 30 は
  theorem/formula/context、definition/property、term/`qua`、registration/trace-context/
  direct-template、algorithm、termination、snapshot/claim、
  ghost-isolation zero-VC の全 family を exact Core 33-53 dependency 付き bounded VC
  32-55 owner に割り当てた。VC 40 は complete 済み VC 37/39 output と Core 40/A1
  により blocked のまま。missing scheme/theorem role は direct VC 41 の外で S1 の背後に残る。
- `spec_gap`: VC 53 は separately blocked。canonical authority は termination-evidence
  producer/reference identity/schema、authentication contract/rule、owning test を命名しない。
  transport payload/authentication mechanism を捏造しない。
...
Task 265/Core Task-32 ownership addendum: Core Tasks 31-32は完了し、完了した
docs-only VC Task 30はexact Task-180
...
Core Tasks 33-53、VC 40 向け complete 済み VC 37/39 output と Gate A1、Task-53
canonical-authority gap、direct VC 41 外の missing role に対する Gate S1、VC-owned
concrete call/result substitution は mandatory dependency/boundary のまま。descendant
の命名は implemented を意味しない。

Task-30 completion addendum: accepted paired
[source VC decomposition](./source_vc_decomposition.md) は VC Tasks 31-55 を
割り当てる。Task 31 は exact structural Task-180 terminal-goal mapping と
`MT10-VC-T180` だけを所有し、Tasks 32-41 は theorem/definition/property/term/
registration/direct-template family、Tasks 42-55 は shared `MT10-VC-PV/VC<n>`
contract により全 Chapter-20/SCA-005 VC/zero-VC integration family を所有する。
VC 40 は VC 37/39 と Core 40/A1、VC 53 は将来の canonical authority が evidence
transport/authentication contract を命名するまで blocked のまま。missing
scheme/theorem role は direct VC 41 の外で S1 の背後に残る。
current terminal-marker mismatch、entry-`requires` mapping、range/exit treatment、
simplification-order/partial-termination/ghost-isolation non-generation boundary、missing
honest kind、generated formula/context limitation は descendant `test_gap` を
伴う assigned `design_drift` / `source_drift` と分類する。dependency-ready descendant を
block する `spec_gap` はないが、上記 bounded missing authority は VC 53 を block する。
current `boundary_violation`、`repo_metadata_conflict` はない。Task 30 は ownership metadata
だけを変更し、最初の source/trace row/VC baseline は Task 31 が所有する。
