...
| DIS-G002 | `source_drift` / `test_gap` | `src/discharge.rs`、discharge evidence data、discharge tests はまだ存在しない。 | Tasks 11-12 がこの仕様に従って source と tests を実装する。 |
...
| DIS-G005 | `deferred` | concrete numeric computation limit、evidence serialization、dependency-slice fingerprint、corpus fixture、kernel/proof/cache validation は後続 task または crate が所有する。 | Task 10 は必要な shape と invariant を定義し、task 11-12 と後続 dependency/consumer task が実装詳細を埋める。 |
...
- explicit replayable trace または premise reference を持つ cluster、registration、reduction fact;
- VC の unfold request と policy input が許す definitional reduction;
- requested computation と limit policy が explicit な場合の bounded `by computation` または verification-time computation。
...
Task 10 はこの shape を固定するが、numeric default は選ばない。Task 11 が concrete default と
policy ごとの limit 解釈を記録する。limit exceeded は failed proof ではない。VC は stable
explanation 付きで `NeedsAtp` または deferred のままになり、`Discharged` evidence は持たない。
...
各 discharged VC は次を記録しなければならない:
