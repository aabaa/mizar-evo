...
canonical v1 `KernelImportedStatementProjection` payload によって引用先 formula-tree
fingerprint と接続されなければならない。この payload は
`MIZAR_KERNEL_IMPORTED_STATEMENT_PROJECTION\0` に imported-statement fingerprint の
canonical bytes と formula fingerprint の canonical bytes を続けたものである。unsupported
formula / imported-statement algorithm id、空または非正準の projection payload、stale statement
fingerprint、formula-projection mismatch は、bytes を再解釈する合図ではなく fail-closed
builder error である。
...
formula-projection mismatch、same-requirement projection conflict、空または非正準の
projection payload、context/payload 不足が fail closed することを示す Rust coverage を追加する。
...
根拠: task 34 は、F6 が producer と kernel-consumer の両側で実装された後に残る
