...
- local hypothesis、cited premise、generated VC fact、imported-statement
  projection payload を持つ accepted imported fact、policy-bounded built-in の
  formula evidence entry;
...
Task 28 は `context_identity` を stable な非 diagnostic handoff section として追加する。
これは意図的に `canonical_evidence` の外に置く。`canonical_hash()` は引き続き
formula/substitution/provenance envelope hash を指し、context section は非再帰的な
独自の `context_identity_hash()` を持つ。context hash input は target VC fingerprint、
canonical evidence hash、local-hypothesis / cited-premise / generated-VC-fact の各
formula evidence entry に対する deterministic row を記録する。各 row は source class/id を
formula evidence row id、formula fingerprint、producer `VcFormulaRef` に bind する。
Imported axiom/theorem entry は代わりに `formula_context_requirements` で扱い、
`context_identity` には現れない。

v1 `context_identity_hash()` input は `mizar-kernel` task 31 と共有する UTF-8 line grammar
である:
...
Rows は source kind/id、formula id、formula fingerprint、producer formula ref の順で sort
する。Source spelling は Rust debug form の
`LocalHypothesis { local_context_id: N }`、
`CitedPremise { local_context_id: N }`、`GeneratedVcFact { vc_fact_id: N }` を使い、
producer ref は `Core(CoreFormulaId(N))` または
`Generated(VcGeneratedFormulaId(N))` を使う。Kernel task 31 は
`canonical_evidence_hash` を不透明な `mizar-vc` canonical formula-envelope handoff hash として
扱い、この context-identity hash だけを再計算する。
...
imported statement projection payload を明示的に運ばなければならない。requirement 側の
imported-statement fingerprint は architecture-18 の imported-statement algorithm を使い、
引用される formula tree は kernel formula fingerprint algorithm を保つ。projection はこの 2 つの
fingerprint を bind し、imported entry が emit される前に producer 側で fail-closed に検査される。

`diagnostic_inputs` は explainability 用の任意 producer-side detail である。後続 spec が
stable field として明示的に昇格しない限り、canonical kernel evidence bytes、stable
handoff identity、proof reuse identity から除外する。snapshot-local な `VcId`、generated formula
...
| 選択された `VcIr.kind` と `KernelEvidenceHandoffInput.goal_polarity` | builder は現在の全 VC kind を列挙し、proof obligation には `AssertFalseForRefutation` を要求する。検証済みの explicit polarity は `final_goal.polarity` にコピーされる。consistency-polarity request は canonical evidence bytes や package hash の構築前に失敗する。kernel-side acceptance binding は `mizar-kernel` task 30 で実装済みである。 |
| formula ref を持つ `LocalContext` entries | local-hypothesis または cited-premise source binding を持つ formula evidence entry と、context-entry id、formula evidence row id、formula fingerprint、target VC、canonical evidence hash を bind する `context_identity` row。stable formula payload または provenance を欠く entry は捏造せず missing payload と記録する。 |
| `PremiseRef::LocalContext` と `PremiseRef::GeneratedFact` | 対応する local-hypothesis、cited-premise、generated-VC-fact formula evidence entry への reference。非 import source binding には対応する `context_identity` row が必要である。 |
| `PremiseRef::ImportedFact` | package/module/exported item identity、imported-statement fingerprint、imported-statement-to-formula projection payload、required proof-status requirement、matching `FormulaEvidenceContext` input が利用可能な場合だけ candidate imported axiom/theorem formula entry にする。`mizar-vc` は imported fact が accepted であるとは認定せず、proof/kernel-owned context がそれを行わなければならない。それ以外は `external_dependency_gap` または fail-closed builder error。 |
...
| `DependencySlice` と proof-reuse candidate data | tasks 26 / 28 の identity / invalidation input。canonical formula-envelope hash と context-identity hash の両方を含むが、VC を証明せず、kernel checking を置き換えない。 |
...
formula-tree fingerprint は、この handoff version では kernel formula fingerprint algorithm を
使わなければならない。imported-statement fingerprint は formula-tree fingerprint ではなく、
architecture-18 dependency fingerprint 用に定義された imported-statement algorithm id を使い、
nonempty `KernelImportedStatementProjection` payload によって引用先 formula-tree fingerprint と
接続されなければならない。unsupported formula / imported-statement algorithm id、空の projection
payload、stale statement fingerprint、formula-projection mismatch は、bytes を再解釈する合図ではなく
...
- resolved `VC-HANDOFF-G006`: task 27 は handoff input に explicit `goal_polarity` を追加し、
  ...
  enforcement は `mizar-kernel` task 30 で実装済みである。
- resolved `VC-HANDOFF-G007`: task 28 は local-hypothesis、cited-premise、
  generated-VC-fact formula evidence row 向けの producer-side `context_identity`
  payload と hash を追加する。この payload は target VC と canonical evidence hash に bind され、
  dependency-slice / proof-reuse identity に参加する。kernel-side membership verification は
  `mizar-kernel` task 31 で実装済みである。
- resolved `VC-HANDOFF-G008`: task 29 は imported axiom/theorem entry 向けの
  producer-side `KernelImportedStatementProjection` payload を追加する。handoff は
  architecture-18 imported-statement fingerprint を kernel formula-tree fingerprint とは別に
  検査し、projection を canonical evidence rendering/hash input に記録し、同じ payload を
  dependency slice に export する。kernel-side trusted validation と pass fixture は対になる
...
Task 28 は、`context_identity` がすべての非 import local-hypothesis、cited-premise、
generated-VC-fact source binding を cover し、imported premise を除外し、stable かつ
self-consistent であり、canonical source binding を mutate すると stale になることを示す
Rust coverage を追加する。

Task 29 は、imported fact が distinct な imported-statement fingerprint と formula-tree
fingerprint を運べること、projection が canonical evidence/debug rendering と dependency slice に
参加すること、unsupported imported-statement algorithm、stale statement projection、
formula-projection mismatch、same-requirement projection conflict、空の projection payload、
context/payload 不足が fail closed することを示す Rust coverage を追加する。
...
根拠: mizar-vc task 29 は F6 の producer half だけを閉じる。次の Step 3 task は fake corpus data
に頼らず、projection payload の trusted kernel validation contract を決めて強制しなければならない。
