...
doc/design/mizar-vc/en/kernel_evidence_handoff.md. Implement kernel-side F6
validation and the pass/reject fixture only from real mizar-vc projection
payloads. Do not fabricate projection bytes, weaken formula fingerprint
checks, change checker/core semantics, activate unverified source fixtures, or
rebaseline expectations to match current behavior.
```

Rationale: mizar-vc task 29 は F6 の producer half だけを閉じる。次の Step 3 task は fake
corpus data に頼らず、projection payload の trusted kernel validation contract を決めて強制しなければならない。
この boundary は imported fact を引用可能にしつつ imported-statement identity を formula-tree
identity へ潰さないためのものなので `xhigh` を保つ。typo-only documentation synchronization だけなら
