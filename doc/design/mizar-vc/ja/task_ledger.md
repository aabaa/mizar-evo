...
Rationale: mizar-vc task 28 は producer-side F2 context-identity payload だけを閉じる。
trusted membership verification は `mizar-kernel` task 31 に残る。次 task は trusted boundary を
編集するため `xhigh` を保つ。typo-only documentation synchronization だけなら lower reasoning が
適切である。
