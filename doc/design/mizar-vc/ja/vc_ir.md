...
| VCIR-G003 | `external_dependency_gap` / `deferred` | Task 2 は当初 `mizar-atp`、`mizar-kernel`、`mizar-proof`、`mizar-cache` を inactive consumer として扱っていた。`mizar-kernel` task 23-30 は checker-side formula/substitution evidence path と goal-polarity binding を提供し、post-closeout VC task 24-28 は producer-side handoff builder、explicit goal-polarity declaration/rejection、reuse identity integration、context-identity payload を提供するが、ATP candidate production、kernel-side F2 membership verification、proof/cache consumer、artifact witness consumer は後続 work のままである。 | `VcIr` は prover-independent に保ち、untrusted deterministic evidence reference、status state、reuse-candidate data、task-24/28 handoff rule だけを記録する。SAT clause、backend trace、trusted acceptance field を追加しない。 |
...
| VCIR-G006 | `deferred` | Discharge evidence と dependency slice は後続 VC task で実装済みであり、post-closeout VC task 24-28 は producer-side kernel-evidence handoff、explicit goal-polarity declaration/rejection、reuse identity integration、context-identity payload を実装済みである。ATP translation、downstream proof/cache/artifact または corpus consumer、kernel-side F2 verification、source-derived corpus runner activation は後続 task または external seam のままである。 | Task 9 は discharge evidence 生成、dependency slice 計算、ATP problem translation、kernel/proof/cache/corpus path 呼び出し、downstream integration record の fabricate をしてはならない。 |
