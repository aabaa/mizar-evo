...
| VCIR-G003 | `external_dependency_gap` / `deferred` | Task 2 は当初 `mizar-atp`、`mizar-kernel`、`mizar-proof`、`mizar-cache` を inactive consumer として扱っていた。`mizar-kernel` task 23-29 は checker-side formula/substitution evidence path を提供し、post-closeout VC task 24-27 は producer-side handoff builder、explicit goal-polarity declaration/rejection、reuse identity integration を提供するが、ATP candidate production、proof/cache consumer、artifact witness consumer は後続 work のままである。 | `VcIr` は prover-independent に保ち、untrusted deterministic evidence reference、status state、reuse-candidate data、task-24/27 handoff rule だけを記録する。SAT clause、backend trace、trusted acceptance field を追加しない。 |
