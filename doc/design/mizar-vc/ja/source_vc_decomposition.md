...
本文書は mizar-vc Task 30 の確定成果である。Task 31 が実装した Task-180 theorem
obligation から VC への厳密な写像を固定し、その他すべてのソース由来 VC family を
bounded follow-up task へ分解する。decomposition は task、依存関係、consumer の
authority のままであり、以下の Task-31 completion record は later family を広げず
exact implementation/coverage だけを記録する。
...
Task 31 は public borrowed adapter `generate_exact_task180_vc` で上記 mapping を
実装する。adapter は exact CoreIr、snapshot、explicit generation/VC schema version
`mizar-vc-generation-task31-v1` / `mizar-vc-vcset-task31-v1` を受け取る。Core の
package/module identity を length-framed `VcModuleRef` として encode し、empty CFG、
handoff、intake を内部構築し、fully validated `VcSet` または typed atomic error だけを
返す。

distinct `pass_proof_verification_contradiction_formula_constant_001.miz` sidecar だけを
`active_proof_verification` に admit する。runner は complete
source-to-checker-to-Core-to-VC path を2回実行し、complete `VcSet::debug_text()` bytes を
committed VcIr baseline と verify-compare する。admission test は wrong stage、
missing/duplicate/wrong tag、wrong phase、absent snapshot、unchanged type-elaboration
Task-180 sidecar の exclusion を覆う。snapshot/report test は absent/missing/unreadable/
mismatched baseline、failure diagnostic、deterministic report bytes、CLI summary を覆う。

VC corruption matrix は exact Core table allowlist とすべての owner/formula/proof/
terminal/seed identity、kind、status、path、source、source-map、Core-reference、
provenance、handoff、flow-site、fresh-intake boundary を覆う。また classification が
structural/marker-free であることを証明する。この snapshot に exactly one covered
trace requirement を追加し、broad proof-verification row は deferred のままにする。
existing type-elaboration source/sidecar/Core snapshot/expectation bytes は不変であり、
downstream verification/acceptance は claim しない。
...
|---|---|---|---|
| 32 | General theorem proof-step/terminal-goal VC、stable formula payload、quantified binder、ordered local context。Specs 04.5, 14, 15, 16.1-16.5/16.7。 | Core 33-35/37; `MT10-VC-PV/VC32`. | Task 180 は VC 31 のまま。proof search、closure、acceptance、verified premise publication なし。 Spec 20.9.2 の[限定 computation 要求](generator.md#bounded-computation-request-generation) は open equality と source option を保持し、限定 trace section は `VC Task 32; bounded source computation request VcIr snapshot` とする。 |
...
| 39 | Reduction `reducibility` universal-equality obligation。Specs 17.6, 17.9.4。 | Core 39; `MT10-VC-PV/VC39`. | simplification-order/size/variable check は structural registration-time rejection rule で、決して VC ではない。rewrite activation や label/order だけからの formula synthesis なし。 限定 identity/singleton profile は shared `vc_generation` phase の明示的な例外として `verification`/`fail` を維持し、[失敗観測](./discharge.md)前の open Reduction VcIr baseline を比較する。限定 trace section は `VC Task 39; bounded identity-singleton reduction VcIr snapshot` とし、広範な簡約検証は未対応のまま。 |
...
| 43 | callee-body context としての `requires`、return-result substitution、postcondition、assertion VC。Specs 20.4-20.5, 20.13.1/20.13.3。 | Core 42-43/46/48/52-53; `MT10-VC-PV/VC43`. | declared entry `requires` は `AlgorithmPrecondition` proof VC ではない。generated goal なしの return/postcondition fact なし。 限定した等式 ensures と平坦な var/const/assignment/assertion 文脈は [source return generator](./generator.md#bounded-source-return-postconditions) を使用し、requires と広い制御フローは保留する。状態 trace 節は VC Task 43; bounded straight-line state and assertion VcIr snapshot。 正確な矛盾 assertion 行だけは shared `vc_generation` phase の例外として `verification`/`fail` を維持し、[限定失敗観測](./discharge.md)の前に完全な open VcIr を比較する。required snapshot trace 節は `VC Task 43; bounded contradictory assertion VcIr snapshot` とし、一般的な検証や終了受理を与えない。 実装済みの [平坦 snapshot profile](generator.md#flat-snapshot-postconditions) は通常の返却目標と未解決 ghost accounting を保持し、trace section を `VC Task 43; bounded ghost snapshot return VcIr snapshot` とする。 |
...
| 52 | recursive/mutually recursive component-decrease obligation。Specs 20.7-20.8, 20.13.4。 | Core 46/52-53; `MT10-VC-PV/VC52`. | `func` promotion、call-graph acceptance、termination fact なし。昇格公理の前提は VC 57 と 59 が扱う。 |
...
| 54 | captured program context と universal valid-execution quantification を持つ snapshot/claim theorem obligation。Specs 20.6, 20.13。 | Core 47/51-53; `MT10-VC-PV/VC54`. | explicit captured payload なしの state theorem、old-state substitution、claim acceptance なし。 [限定 void-claim producer](generator.md#bounded-void-claim-generation) は実際の空実行 interface と algorithm に結び付く証明義務を検証し、一般の capture・代入は保留する。 |
...
| 56 | 見出しに測度を持つ `terminating` algorithm の帰納法の仮定の文脈: algorithm とその再帰グループの、辞書式測度が厳密に小さい引数での契約を、本体のすべての VC に置く。Specs 20.4.1, 20.13.4。 | Core 46/52; VC 43-44, 47, 51-52; `MT10-VC-PV/VC56`. | 現在の引数での仮定はなく、見出しの測度がなければ仮定もない。termination acceptance や昇格公理の前提は扱わない。 |
| 57 | モジュール内の前提利用: 同じモジュールで宣言された algorithm の全体性・契約・定義式 (Core 54) の公理を、1 つの認証された source orchestration の中で termination の義務をその場で受理した後に、引用または policy による前提として利用する。限定的な [source existential-registration proof](../../mizar-proof/ja/status.md) の前例に従う。Specs 16.5.1, 20.7.2, 20.13.2。 | Core 54; VC 52; resolver の algorithm 引用; mizar-proof のモジュール内受理; `MT10-VC-PV/VC57`. | 真の受理の前には何も利用できず、受理を義務の存在・順序・局所検査から導かず、受理状態を持ち運べる受領証にしない。項目の順序を保つ。 |
| 59 | **Blocked-reserved の import した前提の利用:** 他のモジュールが export した algorithm の昇格公理。Specs 12, 16.5.1, 20.13.2, 23。 | VC 57; 未実行の `MT10-VC-PV/VC59`. 現在の canonical authority は、持ち運べる受理状態の producer、schema、認証規則を定めていない。 | 認証された持ち運べる受理なしに import した前提は使わず、他のモジュールの義務を証明し直すことで代用しない。その authority ができるまで block する。 |
| 58 | `by computation` のステップについて、計算閉包の中でゴーストでない Pick サイトまたは `for ... in` ループを含む値を返す各 algorithm の一意性の義務。`result = t` / `result = true iff φ` の構文的な免除を含む。Specs 20.2.4, 20.3, 20.9.2。 | 既存の限定 computation 要求; Core 42, 44, 46; VC 44; `MT10-VC-PV/VC58`. | 閉包は同じパッケージの中で求める。計算用の公開を通してしか本体が得られない import 先の callee は閉包を確定させず、ステップは open のままとする。MVM 評価、計算結果の受理、public E0330 projection、正規の走査順序は扱わない。ゴーストだけの Pick は源にならない。 |
...
Task-30 boundary では次の sequential STEP 5 task は VC Task 31 だった。Steps 6/7 は
deferred のままで、Tasks 31-55 の命名は proof-verification/acceptance credit を
与えなかった。

その後 VC Task 31 はこの exact slice だけを実装・実行する。covered snapshot row は
broad proof-verification row に credit を与えず、Task-180 の `test_gap` と
`source_drift` を閉じる。Tasks 32-55 は dependency-paced のままである。
Core-33-to-VC-32 dependency chain では checker Task 248 が次の prerequisite かつ
recommended continuation で、その後に Core 33 と descendant が続く。top-level TODO は
