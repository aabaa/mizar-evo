...
|---|---|---|---|
| `mizar-resolve` | The resolver closeout records R-G001: public resolver diagnostic code ownership is absent, and task R-013/R-015 keep name/import/label diagnostics crate-local/internal. | `external_dependency_gap` / `deferred` | Spec chapter 22 allocates or delegates resolver diagnostic codes, and `mizar-resolve` defines a real producer-side adoption task with corpus or `.miz` coverage. |
...
| `mizar-driver` | `crates/mizar-driver` is not a workspace member in this checkout; driver request/session/event-stream tasks are still future work. | `external_dependency_gap` | `mizar-driver` is scaffolded and owns the session orchestration and publication boundary that can collect phase batches. |
