...
Task 18 は explain-owned public enums を downstream forward compatibility のため
...
将来の variant は lazy-reference boundary を維持しなければならない。explanation は新しい
handle kind、subject、source、resolution state を追加してよいが、proof status、artifact
mutation、LSP routing、driver orchestration をこの crate へ移してはならない。
