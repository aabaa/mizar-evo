...
serialization ではない。draft、validated registry descriptor、handle、freshness から
構築された record は、すべての shared draft field を正確に保存し、descriptor から
`semantic_name` と `severity` を copy し、clone、equality、accessor、deterministic
debug snapshot を通じて同じ値を公開しなければならない。`SourceId` と
`BuildSnapshotId` は session-local identity のままである。task 5 API は JSON、LSP、
artifact、cross-session serialization を約束しない。
