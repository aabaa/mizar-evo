...
|---|---|---|---|
| `mizar-resolve` | resolver closeout は R-G001 を記録している: 共有 registry は広い `Resolution` family を reserve しているが、resolver の name/import/label descriptor と migration/adoption behavior はなく、task R-013/R-015 はそれらの diagnostics を crate-local/internal に保つ。 | `external_dependency_gap` / `deferred` | `mizar-resolve` R-030 または同等の real producer-side adoption task が、具体的 descriptor、numeric code または alias、migration behavior、corpus または `.miz` coverage を定義する。 |
