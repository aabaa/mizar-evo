...
| `mizar-driver` | この checkout では `crates/mizar-driver` は workspace member ではない。driver request/session/event-stream tasks は future work のままである。 | `external_dependency_gap` | `mizar-driver` が scaffold され、phase batches を collect できる session orchestration と publication boundary を所有する。 |
