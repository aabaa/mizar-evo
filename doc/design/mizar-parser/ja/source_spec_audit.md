...
`repo_metadata_conflict` は見つかっていない。task 42 までに実装済みの parser
挙動は、source、unit test、active parse-only corpus case、traceability metadata に
表現されている。この監査では、下記の deferred operator-declaration gap を 1 件
記録した。
