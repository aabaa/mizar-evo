...
- module と task の状態は同期済みである。parser task 1-44 は完了済み、task 45
  は pending のまま、task 46 は concrete operator declaration と operator 予約語
  corpus coverage のために明示的に deferred のままである。
...
|---|---|---|
| [README.md](../en/README.md) | [README.md](./README.md) | crate boundary、task 44 までの parser status、audit list、task 45 pending state、task 46 deferred state を同期済み。 |
| [grammar.md](../en/grammar.md) | [grammar.md](./grammar.md) | grammar inventory、syntax-only responsibility、public enum policy note、task 35 までの status、deferred operator-declaration wording を同期済み。 |
...
| [todo.md](../en/todo.md) | [todo.md](./todo.md) | task status と follow-up record は task 44 まで同期済み。task 45 は pending、task 46 は deferred のまま両言語で残る。 |
...
task 45 は public enum forward-compatibility policy follow-up として pending のまま
残る。task 46 は concrete operator declaration と operator 予約語 corpus coverage
の deferred task として残る。
