...
変更しない。従来の94/100 closeoutは、別のpost-Task-46 closeoutが全hard gateを
再実行するまでhistoricalである。
...
状態: この crate は、session `SourceRange` を持つ frontend 適合済み token transfer object を消費し、`mizar_syntax::SurfaceAst` と構文診断を返す parser entry point を公開している。Tasks 1-48は実装済みである。P-043-01/P-046はTask 46によりclosed、P-265-47Dはnonblocking human-owned wording gapのままである。Task 46はglobal Step 5を閉じず、Task 49やSteps 6/7を許可しない。
