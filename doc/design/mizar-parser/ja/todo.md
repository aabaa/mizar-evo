...
| recovery | [recovery.md](./recovery.md) | `src/recovery.rs` | [~] task 12 の recovery と task 28 の nested block-end matching は現在 `src/lib.rs` にある |
...
2. **パーサー基盤: cursor、期待トークン診断、同期。** [ ]
   ...
   - task 12 の recovery と task 28 の block-stack matching（`end` 欠落、
     文字列リテラル欠落、回復不能入力、文脈付き block opener）を、観測可能な
     挙動を変えずにこれらのヘルパーへ一般化する。
...
39. **パーサー fuzz ターゲット。** [ ]
    - 任意の UTF-8 上でトークン化と構文解析を駆動するワークスペース fuzz
      ...
      アサートする。`mizar-frontend` task 29 の real-parser fuzz follow-up と
      調整し、可能なら frontend のターゲットと一緒に着地させる。
    - 依存: 36。仕様: [recovery.md](./recovery.md)、
      [../../mizar-frontend/ja/todo.md](../../mizar-frontend/ja/todo.md)
      task 29。

40. **frontend パススルーのフォロースルー。** [ ]
    - 現在の task 28 surface を超える文法の成長では、
...
- 現在の task 28 surface 後に文法が成長する場合は、fuzz coverage、recovery
  marker passthrough、診断 merge ordering、`SurfaceAstCacheKey` invalidation
  のための新しい `mizar-frontend` follow-up を開く。
