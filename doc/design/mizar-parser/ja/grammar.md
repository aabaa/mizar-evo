...
既に skip 済みの library annotation prefix token があればそれら、`private` または
`public` token を包む `VisibilityMarker` 1 個、後続 target item node である。
表現済み theorem / lemma target は concrete な `TheoremItem` / `LemmaItem` node を使う。
notation target、短い legacy theorem fragment、deferred な template predicate argument
を含む theorem payload は `PlaceholderItem` target のままである。合法 target start は
...
`LemmaItem` node である。formula payload が deferred template predicate argument を含む
source は、task 31 / S-016 まで legacy token-preserving `PlaceholderItem` path に残す。
...
concrete theorem item であり、template predicate argument だけが task 31 / S-016 に
deferred のままである。formula を term syntax 内へ埋め込む Fraenkel / set-builder term は
task 15 で実装済みである。
...
`StatementItem` wrapper なしで直接所有する。statement-level annotation はこの
...
である。この task 17 では、次の citation separator までの前に `[` が続く reference は、
部分的な template invocation ではなく recoverable malformed justification syntax として扱う。
...
`simple_justification` を使う host（`let`、`consider`、`reconsider`、iterative equality
step、`per cases`）は引き続き task-17 の `by` clause だけを受け入れる。
...
または label の後に formula start が見える missing-colon form から始まる。formula payload が
deferred predicate template argument を含む場合も task 31 / S-016 まで placeholder に残す。
...
その branch は symbol table に依存するため resolver-owned のまま残る。
