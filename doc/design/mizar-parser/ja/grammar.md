...
および task 34 の algorithm verification clause は実装済み。annotation と
...
loop verification clause は task 34 で実装済みである。statement-level / library
annotation は task 35 に残す。frontend scope skeleton は algorithm header を単一
...
限定される。statement-level / library annotation は task 35 に残す。
