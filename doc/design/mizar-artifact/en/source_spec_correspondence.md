...
| [proof_witness.md](./proof_witness.md) | `PROOF_WITNESS_REF_SCHEMA_FAMILY`; `ProofWitnessRef`, `KernelAcceptanceMetadata`, `ProofStatus`, `EvidenceKind`, `ProofWitnessReadOptions`, `ProofWitnessError`; `current_schema_version`, `schema_version_support`, `write_proof_witness_ref`, `proof_witness_ref_json`, `read_proof_witness_ref` | `crates/mizar-artifact/src/proof_witness.rs` | Unit tests cover round-trip canonical JSON, deterministic writer behavior, schema-version rejection, hash-domain/digest validation, missing/unknown/empty fields, unsafe witness paths, formula/substitution evidence metadata, legacy certificate-field rejection, and supplied witness artifact hash mismatches. | No finding. |
...
| [all module specs](./todo.md#hardening-and-cross-cutting-follow-ups) | Public enum forward-compatibility decisions for every current public enum | Source attributes in non-test `src/**/*.rs`; per-module "Public Enum Forward Compatibility" sections | `tests/lint_policy.rs` guards that every public enum in non-test source files is `#[non_exhaustive]` and documented in both English and Japanese module specs. | No finding. |
| [module_boundary_refactor.md](./module_boundary_refactor.md) | Public module roots remain unchanged while inline unit tests move to private `src/<module>/tests.rs` files. | `crates/mizar-artifact/src/*.rs`; private `crates/mizar-artifact/src/*/tests.rs`; `crates/mizar-artifact/tests/lint_policy.rs` | Unit tests still run through the parent modules; the public-enum guard scans source recursively while excluding test-only files. | No finding. |
...
| Module-boundary refactor preserves public API paths and artifact-facing behavior | Parent modules keep the same public roots and only include private `tests` submodules under `#[cfg(test)]` | Existing crate tests, clippy, and formatting after task 22 | No finding. |
...
| ART-G-017 | `external_dependency_gap` | Task 17 reevaluation is recorded in [phase15_emission_reevaluation.md](./phase15_emission_reevaluation.md). `mizar-proof` now exists and owns witness staging/publication metadata, but real producer projection output, committed publication reachability integration, and an artifact-owned end-to-end emission handoff are absent, so task 17 remains deferred. |
