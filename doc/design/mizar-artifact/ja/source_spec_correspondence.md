...
後に proof-witness と verified-artifact の範囲を再実行する。この監査は module spec から
source と tests へ、public API と約束された artifact-facing behavior を trace する。
...
- `deferred`: task 17 emission integration は task 23 の再評価後も real producer
  publication output が存在しないため deferred のまま。task 22 が source-layout
  refactor decision を所有する。
...
| `ProofWitnessRef` は proof payload を load/accept/replay せず、formula/substitution kernel evidence reference と kernel-acceptance metadata を保存する | `src/proof_witness.rs` の schema と validation helpers | proof-witness round-trip、hash mismatch、formula/substitution evidence matrix、legacy certificate rejection、path、missing/unknown field、hash-domain tests | No finding. |
...
| ART-G-012 | `external_dependency_gap` | concrete witness payload publication と proof producer integration は upstream に残る。task 23 は artifact 側の formula/substitution reference schema を定義し、legacy certificate material を拒否する。 |
...
| ART-G-017 | `external_dependency_gap` | task 23 は witness schema update 後に phase 15 emission を再評価した。この checkout では `mizar-proof` が workspace crate ではなく、real producer publication output もないため、task 17 は deferred のままである。 |

## Verification

task 23 は Rust source を変更し、proof-witness boundary についてこの audit scope を再実行する。
Rust verification は task 23 commit とともに記録する。documentation verification には
引き続き `git diff --check` を含める。
