...
task 6 はこの形だけを仕様化する。task 7 が canonical JSON として serialize し、
validating reader/writer を追加する。
...
optional field は存在しない場合も JSON `null` として出力する。reader は identity、label、
kind、trigger、summary、parameter、trace-id field の空文字列、unknown enum value、start が end
より大きい range、duplicate `trace_ids`、`accepted` 以外の `accepted_status` を拒否する。pending
または unaccepted registration は alternate status value ではなく、この summary に存在しないことで
表現する。
...
task 6 はこの仕様だけを追加する。source implementation は task 7 に deferred とする。task 7 は
`RegistrationSummary` schema、writer、validating reader、および round-trip、trace reference の
hash 解決、deterministic ordering、incompatible-version read、registration/hash mismatch rejection の
test を追加する。

checker producer integration、concrete `ResolutionTrace` artifact production、proof acceptance、
manifest/file I/O は external または later-task work に残り、task 6 で stub してはならない。
