...
この document は task 8 の仕様だけを定義する。task 9 が schema、writer、
validating reader、test を実装する。
...
crate と kernel crate が所有し、`mizar-artifact` は所有しない。
...
`proof-witnesses/` の下になければならない。absolute path、空 path、`..` traversal は許可しない。
task 9 reader は `CanonicalJson` boundary でこの lexical path validation を行う。symlink と
artifact-root escape の検査には filesystem access が必要であり、artifact-store I/O に属する。
manifest は publication index である。file が `proof-witnesses/` の下に存在するだけでは
publish されたことにならない。
...
task 8 は reference と validation contract だけを仕様化する。atomic write、manifest
visibility、byte-level corruption diagnostic は artifact store と manifest task に deferred
のままとする。
...
task 8 はこの仕様だけを追加する。task 9 は `ProofWitnessRef` schema、canonical value writer、
validating `CanonicalJson` reader、および round-trip と hash mismatch detection の test を実装する。
