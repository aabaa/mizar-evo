...
display、publication status を保持するが、VC IR、ATP problem、proof certificate、backend log、
kernel state は埋め込まない。
...
kernel evidence handoff identity に commit する。task 24 の再点検後、trusted
formula/substitution witness producer は、explicit proof-obligation goal polarity、
canonical formula-envelope handoff hash、存在する場合の task-28 non-imported-source
`context_identity_hash()`、および存在する場合の imported formula context/projection
identity を含む、修正後の kernel-evidence identity からこの composite を導出しなければならない。
`VerifiedArtifact` はこの field の hash class と spelling を検証し、witness consistency
に使う。real producer integration が入るまでは、composite value の構築責務は VC/proof
producer に残る。
...
1 つの `ProofWitnessRef.obligation_id` に解決されなければならない。task 23 では、accepted trusted
witness は kernel がすでに受理した formula/substitution/provenance/target-binding evidence を表す。
legacy backend certificate、resolution trace、built-in certificate discharge、kernel-primitive
placeholder は trusted `ProofWitnessRef` value ではない。
...
- witness の `kernel_acceptance.target_binding_hash`、`formula_evidence_hash`、
  `substitution_evidence_hash`、`provenance_hash`、optional な `formula_context_hash`、
  `accepted_result_hash` は producer/kernel boundary が供給する interface hash であり、
  参照された `ProofWitnessRef` を通じて含まれる。
- task 24 の goal-polarity と context-identity coverage は間接的である。これは
  producer-owned な `obligation_fingerprint`、proof validation identity、accepted-result
  metadata の一部である。`VerifiedArtifact` は `mizar-vc` context-identity hash の再計算や
  kernel acceptance replay を行わない。

`VerifiedArtifact` が検証するのは projected field 同士の reference consistency だけである。
witness replay、kernel acceptance の再計算、proof authority の決定は行わない。backend proof
method、backend log、caller-supplied instantiated formula または SAT problem も trusted
material として扱わない。
...
- backend proof method、resolution trace、SMT proof object、backend log、schema-version `1.0`
  certificate reference を trusted acceptance material として扱わない。
