...
- witness の `obligation_fingerprint` は obligation の `vc_fingerprint` と等しい。
- witness の `kernel_acceptance.verifier_policy_fingerprint` は obligation の
  `verifier_policy_fingerprint` と等しい。
- obligation の `local_context_fingerprint` と `dependency_slice_fingerprint` は obligation
  metadata 内の interface fingerprint として存在し、`implementation_hash` participation に含まれる。
...
`interface_hash` は importer-visible projection に対して計算される。

- import interpretation に影響する module identity field。
- exported signature、visibility、dependency-facing fingerprint。
- importer が観測できる exported theorem/registration fact の accepted proof status。
- importer に影響する dependency interface hash と schema version。

これは expression metadata、local diagnostic、implementation body、diagnostic/navigation
`source_range` field、source path metadata、`source_hash`、`verified_at`、proof witness path と
byte hash（accepted exported status が dependency-facing fingerprint を要求する場合を除く）、
local provenance、`cache_key`、`implementation_hash` を除外する。

`implementation_hash` は、hash-excluded local field と stored `interface_hash` / `implementation_hash`
field 自身を除いた、source file の full stable published projection に対して計算される。hash field 自身を
除くのは self-reference を避けるためである。expression metadata、obligation、diagnostic、stable
provenance input（`toolchain`、`language_edition`、`lockfile_hash`、`verifier_config_hash`、
`dependency_artifact_hashes`）、`ProofWitnessRef` value は含まれる。`verified_at`、`cache_key`、
将来の local/cache-only provenance field は除外する。
