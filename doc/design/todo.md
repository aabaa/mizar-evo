...
| mizar-parser | yes | Grammar, Pratt parsing, syntax recovery, parse-only corpus execution | [~] grammar tasks 1-36, recovery consolidation task 37, `SurfaceAst` snapshot baselines task 38, determinism task 39, fuzz target task 40, and frontend passthrough audit task 41 complete; hardening/audit tasks 42-45 remain | [todo](./mizar-parser/en/todo.md) |
...
determinism task 39, parser-owned fuzz target task 40, and frontend passthrough
audit task 41, but remains [~] because hardening/audit tasks 42-45 are still
open.
...
3. **mizar-parser hardening** - finish module split, source/spec audit,
   bilingual sync, and public-enum policy tasks 42-45.
...
- **Parser grammar status: main growth complete through task 36; recovery
  consolidation complete through task 37; snapshot baselines, determinism, and
  fuzz target plus frontend passthrough audit complete through tasks 38-41.**
  Remaining parser work is module split, audits, and public-enum policy.
