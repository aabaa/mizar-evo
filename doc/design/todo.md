...
| mizar-test | yes | Corpus discovery, expectation sidecars, staged model, traceability, snapshots, harness behavior | [~] foundation complete through task 16; 2026-07 kernel-audit tasks 17-18 (soundness vocabulary, corpus-root naming) are pending; broader consumer-runner support remains paced by source-derived semantic/core/VC bridges | [todo](./mizar-test/en/todo.md) |
...
Revised 2026-07-06 after the July audit wave. Steps 1 and 2 are new and come
before further pipeline growth because they close audited soundness holes and
settle semantic decisions that later implementation must not preempt; steps
3-6 are the previous waves, re-paced around them.
...
1. **Soundness contract closure** - implement the kernel-audit corrections on
   ...
   ([mizar-test tasks 17-18](./mizar-test/en/todo.md)). These have no
...
3. **Source-derived semantic bridge** - continue widening real
   source-derived payload extraction through `mizar-checker`, then lowering
   confirmed checker payloads through `mizar-core` and `mizar-vc`. The
