...
| mizar-test | Corpus discovery, expectations, staged model, traceability, harness | [~] foundation complete through task 22; Tasks 265-268, Core-31, and the Task 269SDC dormant consumer are implemented and synchronized | After the central migration commit, implement only the dormant private route in [Task 269SDT](./task_contracts/en/269SDT.md) | [todo](./mizar-test/en/todo.md) |
| mizar-checker | Type checking, cluster/registration resolution, overload resolution | [~] explicit-payload milestone and source producers/consumers through Task 269SDC binding/context are implemented and synchronized | After the central migration commit, implement only Given source type in [Task 269SDT](./task_contracts/en/269SDT.md); descendant occurrence stays later and capture remains blocked on `set` reconciliation | [todo](./mizar-checker/en/todo.md) |
