...
| mizar-test | Corpus discovery, expectations, staged model, traceability, harness | [~] foundation complete through task 22; Tasks 265-268, Core-31, and the dormant [Task 269SDT](./task_contracts/en/269SDT.md) consumer are implemented and synchronized | Select the successor only from fresh post-commit inventory; active dispatch remains excluded | [todo](./mizar-test/en/todo.md) |
| mizar-checker | Type checking, cluster/registration resolution, overload resolution | [~] explicit-payload milestone and source producers/consumers through [Task 269SDT](./task_contracts/en/269SDT.md) are implemented and synchronized | Descendant occurrence stays later and capture remains blocked on `set` reconciliation; select the successor only from fresh post-commit inventory | [todo](./mizar-checker/en/todo.md) |
