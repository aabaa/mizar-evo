...
| `20.algorithm_and_verification.md` | Parser/syntax, core control-flow, VC generation/discharge, and documentation/extraction docs cover the current algorithm pipeline. [Step 5C.14](./task_contracts/en/STEP5C14-ALGORITHM-SEMANTICS.md) adds bounded source/Core/CFG static observations for illegal break and runtime initializer/assignment/return ghost leakage and immutable-write rejection, plus concrete equality postcondition/assertion VCs with typed local writes, versioned program-point contexts and pending assertion dependencies; no acceptance credit. | partial | Branch/match/range/collection-loop payloads, term-derived/recursive termination, partial-call termination-evidence admission, Pick non-emptiness, ghost-isolation static integration beyond the bounded initializer/assignment/return profile, MVM/code-extraction backends, computational-export publication/loading and equivalence-aware compatibility (§20.9.1, §23.2), and source-derived payloads remain TODO/deferred. |
