...
| `16.theorems_and_proofs.md` | Core, VC, ATP, kernel, proof, cache, artifact, and diagnostics docs cover the current proof pipeline boundaries. Checker task 86 adds active type-elaboration boundary coverage for the theorem formula slot using `theorem FormulaPayloadBoundary: thesis;`, but keeps theorem/formula payload extraction and proof semantics on the external gap. | partial | End-to-end theorem acceptance, proof/cache/artifact consumer integration, recorded facts, proof skeletons, and `formula_statement` execution are still split across evidence-pipeline follow-ups. |
