...
| `19.overload_resolution.md` | Architecture 05 and checker overload docs cover candidate collection, template expansion, viability, specificity, root selection, refinement join, and `qua` insertion. Checker task 36 pins implicit upcast path uniqueness as syntactic over resolved `inherit` declaration paths. | covered | Artifact projection and source-corpus coverage remain external, but the design contract exists. |
