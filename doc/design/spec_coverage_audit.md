...
| `16.theorems_and_proofs.md` | Core, VC, ATP, kernel, proof, cache, artifact, and diagnostics docs cover the current proof pipeline boundaries. Checker task 86 adds active type-elaboration boundary coverage for the theorem formula slot using `theorem FormulaPayloadBoundary: thesis;`, checker task 87 adds the term-bearing equality variant `theorem TermFormulaPayloadBoundary: 1 = 1;`, checker task 98 adds the imported predicate/functor theorem formula variant, checker task 100 adds the builtin membership theorem formula variant, checker task 101 adds the builtin inequality theorem formula variant, checker task 102 adds the builtin type-assertion theorem formula variant, checker task 103 adds the imported attribute assertion theorem formula variant, checker task 104 adds the attribute-level non-empty imported attribute assertion theorem formula variant, checker task 105 adds the set-enumeration theorem formula variant, checker task 99 adds the connective/quantifier theorem formula variant, checker task 88 adds the proof-block variant `theorem ProofSkeletonPayloadBoundary: thesis proof thus thesis; end;`, checker task 89 adds a statement-proof theorem variant with labeled and final proof-justified statements, checker task 93 adds a theorem proof containing proof-local declarations, and checker task 94 adds a theorem proof containing proof-local inline definitions, but all fourteen keep theorem/formula/proof-skeleton/statement, proof-local declaration, inline definition, imported predicate/functor semantic payloads, builtin membership operand checking, builtin inequality desugaring/equality checking, builtin type-assertion payload/checking, set-enumeration term payload extraction, equality checking, imported attribute assertion payload/checking, imported attribute-level non-empty assertion payload/checking, connective/quantifier formula payloads, quantifier binder/context payloads, and proof semantics on the external gap. | partial | End-to-end theorem acceptance, proof/cache/artifact consumer integration, recorded facts, proof skeleton payloads, statement proof payloads, proof-local declaration payloads, inline definition formal/body payloads, term/formula checking, proof contexts, local abbreviation expansion, reconsider coercion/obligation evidence, and `formula_statement` execution are still split across evidence-pipeline follow-ups. |
...
Task87 addendum for chapters `13.term_expression.md`, `14.formulas.md`, and
`16.theorems_and_proofs.md`: checker task 87 adds active source-derived
diagnostic coverage for a term-bearing theorem equality formula after parser
and resolver execution. The active runner observes
`type_elaboration.external_dependency.ast_payload_extraction`, so this credits
only the term/formula theorem extraction-gap boundary. It does not credit
checker term/formula payload extraction, term inference, formula checking,
