...
supersedes that documented `R` portion by carrying imported structure
provenance/type-head payloads to the checker evidence-query diagnostic.
Broader imported-structure reserve heads outside the task-83 `R` bridge remain
deferred until a non-`R` source-derived fixture exists. This credits no real
...
Task83 addendum for chapters `03.type_system.md`, `05.structures.md`,
`11.symbol_management.md`, and `12.modules_and_namespaces.md`: checker task 83
adds active source-derived diagnostic coverage for a reserve head whose
imported structure `R` comes from the documented `parser.type_fixtures` import
summary. The active runner observes
...
extraction for that fixture, not imported module AST extraction, base-shape
...
checker proof-local declaration payload extraction, local proof context,
