...
| `17.clusters_and_registrations.md` | Architecture 04/17, checker registration/cluster trace docs, artifact registration summaries, and cache cluster-db docs cover the current data layers. Checker task 38 pins functorial-cluster `for` as a result-type applicability guard in bilingual spec text with inactive semantic corpus and traceability. Checker task 40 pins item-ordered activation with asynchronous acceptance and the non-retroactive ordering seed. | partial | Accepted status production/import, positive accepted-local activation in source-derived passes, artifact publication, persistent cluster-db materialization, and active source-derived functorial-cluster execution remain deferred/external in owner TODOs. |
