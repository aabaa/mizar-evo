...
Task86 / Task115 addendum for chapters `14.formulas.md` and
...
The active runner now observes
`type_elaboration.checker.checker.formula.external.formula_payload` and
`type_elaboration.checker.checker.formula.unsupported_payload` for that exact
fixture; non-exact formula-only theorem shapes remain on
`type_elaboration.external_dependency.ast_payload_extraction`. This credits
only exact formula constant site/range recovery handoff, not formula constant
semantics, child-formula graph payloads, checker theorem/formula semantic
