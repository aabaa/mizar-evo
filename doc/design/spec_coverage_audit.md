...
| `13.term_expression.md` | Parser/syntax covers terms; checker/core cover typed terms, inserted views, and lowering. Checker task 43 pins Fraenkel sethood lookup to the resolved mode and normalized instantiated argument tuple. Core task 27 adds explicit-payload `qua` reduct term lowering with distinct renamed/multi-path view terms and no-reduct identity/cluster reuse. Kernel task 35 confirms those view terms remain ordinary normalized term subjects for kernel atom identity; the kernel does not infer or collapse `qua` paths. Core task 30 adds explicit-payload Fraenkel sethood gating for template type parameters by cross-referencing accepted bound/constraint sethood records and preserving bare parameters as missing sethood. Checker task 87 adds active type-elaboration boundary coverage for numeral terms inside a theorem equality formula, checker task 98 adds the same boundary for imported predicate/functor term applications inside a theorem formula, and checker task 100 adds the builtin membership theorem formula variant with numeral operands, but all three keep term payload extraction and inference on the checker source-to-payload extraction gap. | partial | Some source-derived payloads, positive source-derived sethood evidence flow, real checker view-functor/sethood extraction, semantic selector/constructor facts, and term inference for source-derived formulas remain owner-gated. Tasks 87, 98, and 100 do not credit term payload extraction, imported predicate/functor semantic payloads, membership operand type inference/checking, quantifier binder/context payloads, formula payloads, or downstream semantic payloads. |
| `14.formulas.md` | Parser/syntax covers formulas; checker/core/VC cover typed formulas, erasure, proof goals, and generated obligations. Checker task 86 adds active type-elaboration boundary coverage for a formula-only theorem source that reaches parser/resolver execution and then remains on the checker source-to-payload extraction gap. Checker task 87 adds the same boundary for a term-bearing builtin equality theorem formula, task 98 adds the imported predicate/functor theorem formula boundary, task 100 adds the builtin membership theorem formula boundary, task 99 adds the connective/quantifier theorem formula boundary, task 88 adds the proof-block theorem variant whose `thus thesis;` conclusion still depends on formula/proof payload extraction, and task 89 adds the statement-level proof-justification variant with nested proof blocks. | partial | Complete source-derived formula payloads, term inference, membership operand type inference/checking, formula checking, facts, proof contexts, `formula_statement` execution, proof skeleton extraction, and proof/VC integration remain external. Tasks 86, 87, 98, 100, 99, 88, and 89 do not credit formula payload extraction, imported predicate/functor semantic payloads, builtin membership operand checking, quantifier binder/context payloads, formula payloads, or downstream semantic payloads. |
