...
- choosing overload resolution results;
...
Import resolution is intentionally split because lexing needs an active lexical environment before full semantic resolution.
...
The active lexical environment may contain symbolic names from candidate imports before the import is fully validated. Full validation happens during semantic import resolution, and invalid imports make later symbol uses unresolved even if tokenization succeeded.
...
- symbolic spellings contributed to downstream module lexical summaries;
...
- overload resolution result selection;
...
| algorithm name in a citation | export surface of the algorithm | promotion-axiom citations (spec 16.5.1), VC generation |
...
- module lexical summary for downstream active lexical environments.

Responsibilities:

- build visible symbol tables for the checker;
- build exported module summary for importers;
- record synonym and antonym links at the name level, including functor synonyms whose original target is an algorithm (spec 11.1.2);
- record declaration dependency edges, including edges from algorithm bodies, for the definition-cycle check (spec 10.3.3);
...
Name and label references are stored in separate tables in
`doc/design/mizar-resolve/en/resolved_ast.md`, which is authoritative for the
payload shapes. `DeferredSelector` is used when the parser produced dotted
syntax and the resolver can identify a term base but cannot decide the selector
...
`SymbolEnv` is the resolver/checker boundary. The detailed index family and
contribution-tracking invariants are defined in
`doc/design/mizar-resolve/en/env.md`. The visible symbol and label projections
contain the current module's local entries plus all entries made visible by
imports and legal re-exports. `module_summaries` is an in-memory index of loaded
dependency summaries, keyed by `ModuleId`. Together these fields should be
sufficient for type checking without re-reading dependency source files.
...
[Spec chapter 22](../../../spec/en/22.error_handling_and_diagnostics.md#2235-module-import-resolution) assigns semantic import failures E0220–E0225 within the existing Resolution family and owns their reporting cardinality and retained-source locations. Other name, label, visibility, and declaration diagnostics remain unallocated. Registry adoption, authentic semantic input transport, and public emission remain separate work.
...
- downstream modules that import a changed exported symbol, exported label, re-export edge, or module lexical summary entry;
...
Combining parsing and name resolution would simplify early implementation, but it would make active lexical environment bootstrapping, syntax recovery, and LSP partial parsing harder. Keeping the parser semantic-free preserves a clean syntax boundary.

### Checker-Owned Name Resolution

Letting type checking resolve names would make selector and overload cases feel local, but it would blur symbol identity with type inference. The adopted design gives the checker a stable `SymbolEnv` while still allowing checker-owned decisions for selectors, coercions, overload roots, and active refinements.
...
- `doc/design/mizar-build/en/planner.md`
- `doc/design/mizar-build/en/module_index.md`
- `doc/design/mizar-resolve/en/resolved_ast.md`
- `doc/design/mizar-resolve/en/imports.md`
...
- `doc/design/mizar-artifact/en/module_summary.md`
- `doc/design/mizar-lsp/en/navigation.md`
...
- Active lexical environment construction may use candidate import summaries, but only semantic import resolution validates whether those imports are legal.
