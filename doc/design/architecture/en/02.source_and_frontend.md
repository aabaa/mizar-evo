...
| `mizar-frontend` | Loads sources, builds source maps, coordinates preprocessing, active lexical environment construction, lexing, parser invocation, and combined `FrontendOutput`. |
...
Produced by `mizar-frontend` after invoking the lexer and `mizar-parser`.
