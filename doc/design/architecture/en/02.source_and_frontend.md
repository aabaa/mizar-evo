...
struct ImportStub {
    path: RawModulePath,
    alias: Option<IdentifierText>,
    source_segments: Vec<SourceRange>,
    span: SourceRange,
}
```

`ImportStub` is not a resolved import and does not carry an import-kind field.
It is enough to request lexicon entries and to produce good diagnostics if
lexicon loading fails.
