...
この文書では、Mizar Evo のフロントエンドが `.miz` ソースファイルを読み込み、コメント、トークン、ソース形状を保った構文木へ変換し、後続のモジュール解決と名前解決へ渡す流れを定義する。

これは [00.pipeline_overview.md](./00.pipeline_overview.md) のフェーズ 1-3 を詳細化する文書である。特に、文脈依存の字句解析、active lexical environment、ユーザー定義シンボル、ドットの扱い、字句解析器が完全に切り出す文字列リテラル、doc comment、annotation attachment の境界を明確にする。

## 関連文書

- [00.pipeline_overview.md](./00.pipeline_overview.md) — 全体パイプライン。この文書はフェーズ 1-3 を詳細化する
...
フロントエンドは、意味論的な名前解決や型検査を実行してはならない。

frontend が行ってよいこと:

- UTF-8 と code-region ASCII constraints を検証する。
- comments と doc comments を識別する。
- active lexical environment を要求できるだけの import declarations を pre-scan する。
...
imported modules は active lexical environment を拡張するため、フロントエンドは最終的な tokenization の前に import information を必要とする。ただし、完全な import resolution は module resolver の責務である。
...
Import pre-scan は raw lexer output を consume する。raw scanning 自体は imports を解釈せず、active lexical environment も構築しない。

build planner / module resolver はこれらの `ImportStub` から resolved imports と lightweight module lexical summaries を用意する。lexer はそれらから `ActiveLexicalEnvironment` を構築し、その imports から見える symbols で final disambiguation を進める。

### Lexing は raw scan を先に行い、その後で文脈に応じて確定する
...
1. active lexical environment 由来の user-defined symbols
...
| user-defined symbols containing `.` | disambiguator | active lexical environment、parser context、scope override rules、longest-match により認識する |
...
それでも frontend は parsing 前に complete `TokenStream` を生成することを目標にする。現在の lexer API では、この依存関係を narrow な `ParserLexContext` として表現する。Frontend/parser integration は、任意の parser state を lexer に公開せず、grammar-derived な string-required context を渡してよい。
...
これらの位置以外では、quote characters は active lexical environment と longest-match rule に従う通常の symbol characters である。
...
- source bytes を読む。
- UTF-8 を検証する。
- loaded source text を作る前に、package-authored source から先頭 UTF-8 BOM を一つ取り除く。
- `source_hash` を計算する。
- file path から module path を導出する。
- Unicode scalar column rules を使って `LineMap` を構築する。
- loading によって offsets が変わった場合、loaded-text offsets から original file byte offsets または editor-provided text offsets への `LoadingMap` を保持する。
...
現在の `mizar-lexer` crate は、comment stripping、lexical precondition diagnostics、module-source naming、raw scan、import pre-scan の helper API を提供する。Frontend は file I/O、UTF-8 validation、line map、全体の orchestration を担当する。
...
`ActiveLexicalEnvironment` は、resolved imports と lightweight `ModuleLexicalSummary` から構築される。これは lexer-local な構造であり、lexical shape と provenance を記録するだけで、full module IR や semantic applicability は保持しない。
...
- 可能な場合、malformed spans に `TokenKind::ErrorRecovery` を emit する。
...
- import edits は active lexical environment、token stream、AST、その source file のすべての semantic layers を invalidate する。
...
これは lexer に precise symbol identities を与えるが、cycle を作る。module contents を理解するには parsing が必要であり、lexing には imported symbols が必要である。採用した shallow pre-scan は、この cycle を壊しつつ active lexical environment を構築するのに十分な情報を保つ。
...
3. shallow imports と dependency lexical summaries から active lexical environment を構築する。
...
- `doc/design/mizar-lexer/ja/import_prescan.md` — raw lexer output に対する import pre-scan
- `doc/design/mizar-lexer/ja/lexical_environment.md` — imports と dependency summaries からの active lexical environment assembly
- `doc/design/mizar-lexer/ja/scope_skeleton.md` — scoped identifier override のための lexical scope skeleton
- `doc/design/mizar-lexer/ja/disambiguator.md` — final token disambiguation
...
- Active lexical environment は token boundaries に影響し得るため、tokenization は dependency lexical-summary fingerprints に依存する。
