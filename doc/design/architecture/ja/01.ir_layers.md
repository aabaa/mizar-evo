...
- doc comment blocks
- active lexical environment を構築するのに十分な import pre-scan result

Does not own:

- fully resolved imports
- final active lexical environment に依存する token classification
...
`TokenStream` は active lexical environment を適用した後の parser input である。
...
- user-defined symbolic names は active lexical environment に対して認識される。
