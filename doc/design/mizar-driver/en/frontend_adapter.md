...
Keep the existing non-Complete output suppression and failed task/session semantics. A later transport design must distinguish retained diagnostic input from ordinary dependent outputs and bind it to the same current SourceUnit, source version and captured index; no generic Recoverable-to-Complete remapping or replay of unauthenticated decoded data. Reuse FrontendOutput; a new public API or representation is not specified here.
Tests for future adoption: live E0022-only unrecovered AST with missing root/module, relative escape, duplicate alias and root-alias conflict; co-reporting with original E0022; clean path unchanged; reject every other code/class pair (including E0023 in the same class) and mixed E0022/other vectors, absent key/AST, parser errors without recovery flags, recovered nodes, malformed import framing, invalid source/ranges/index/parents, stale snapshot and cancellation. Verify failed resolver conversion emits no semantic batch but retains validated frontend diagnostics; preserve failed build, no normal dependent output/cache/artifact credit, and no E0225 from incomplete graph inputs. No runtime or corpus coverage is claimed by this design.
