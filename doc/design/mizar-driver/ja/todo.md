...
| registry | `registry.md`（task 4） | `src/registry.rs`; private helper は `src/registry/catalog.rs` | [x] |
| driver | `driver.md`（task 7） | `src/driver.rs`; private helper は `src/driver/{event_log,scheduler,watch}.rs`; unit test は `src/driver/tests.rs` | [x] |
...
- **CLI の表面: task 12 で解決済み。** バイナリ名は `mizar`、実装済み batch
  entry point は `mizar build` である。将来の subcommand は実装前に `cli.md` に
  文書化する。
...
16. **エンドツーエンド決定性スイート。** [x]
    ...
    - task D-016 で完了: `tests/determinism.rs` は、同一の phase-zero
      ...
      multi-task source / module work については、real phase dispatch が owner gap だった
      pre-task-27 の historical deterministic blocking behavior を記録する。superseded
...
17. **公開 enum の前方互換性ポリシー。** [x]
    ...
    - 依存: 13。仕様: 全モジュール仕様。
    - task D-017 で完了: `request`、`registry`、`driver`、`events`、`cli` の
      現在の public enum はすべて、owning module spec で downstream-facing な
      `#[non_exhaustive]` boundary type として分類し、exhaustive exception はないと
      記録した。既存の enum declaration はすでにその policy を満たしていたため、
      変更ではなく監査した。変更した source は、完了済み policy を表す lint-policy
      guard の文言だけである。frontend task-25 procedure の source/spec correspondence
      table 部分は、driver-wide source/spec correspondence audit である task 18 に残す。
...
18. **ソース/仕様対応監査。** [x]
    ...
    - task D-018 で完了: [source_spec_correspondence.md](source_spec_correspondence.md) は
      実装済みの `request`、`registry`、`driver`、`events`、`cli` の public API と
      promised behavior を source / tests へ trace する。新しい未解決の blocking、high、
      medium source/spec drift は見つからなかった。既存の `external_dependency_gap`、
      `deferred`、report-only `repo_metadata_conflict` は、この監査で source change にせず、
      分類済み follow-up のまま残す。

19. **二言語ドキュメント同期監査。** [x]
    - `doc/design/mizar-driver/en/` の各英語正本と日本語版を比較し、
      内容を同期する。
    - 依存: 18。仕様: リポジトリのドキュメント方針。
    - task D-019 で完了: [bilingual_documentation_sync.md](bilingual_documentation_sync.md) は
      driver design corpus が crate plan、TODO、module spec、adapter readiness、
      source/spec correspondence、新規 sync audit 自体を含め、`en/` と `ja/` で
      1 対 1 に paired であることを記録する。未解決の blocking/high EN/JA
      documentation drift は見つからなかった。既存の `external_dependency_gap`、
      `deferred`、report-only `repo_metadata_conflict` record は変更せず同期済みのまま。

20. **architecture-22 フォローアップ監査。** [x]
    ...
    - 依存: 16、19。仕様: 全モジュール仕様、本 TODO、リポジトリの
      ドキュメント方針。
    - task D-020 で完了: [architecture_22_follow_up_audit.md](architecture_22_follow_up_audit.md)
      は、実装済み driver seam に未解決の blocking/high architecture-22 drift がないことを
      記録する。query boundary、stale-output suppression、diagnostics owner-gap handling、
      artifact-boundary non-ownership、worker-count determinism は既存 test と doc で
      cover される。full real clean / incremental / parallel equivalence、
      scheduler-selected phase dispatch、LSP bridge、semantic/proof/artifact adapter は
      `deferred` または `external_dependency_gap` のままである。
...
21. **module 境界リファクタリング gate。** [x]
    ...
    - task D-021 で完了: [module_boundary_refactor_gate.md](module_boundary_refactor_gate.md) は
      source-layout audit と private helper split を記録する。`cli` output rendering は
      `src/cli/output.rs`、driver event construction、scheduler helper、watch helper、
      unit test は private な `src/driver/` child、registry phase catalog / hash helper は
      `src/registry/catalog.rs` へ移動した。public module、public API、deterministic
      rendering、diagnostics/artifact-facing schema、owner boundary は変更していない。

22. **crate exit report と quality review。** [x]
    - hard-gate evidence、task commit hash、quality score、分類済み residual gap、
      final verification、next-phase handoff で autonomous task stream を close する。
    - 依存: 21。仕様: autonomous crate protocol と本 TODO。
    - task D-022 で完了: [crate_exit_report.md](crate_exit_report.md) は task commit
      list、milestone scope、included/excluded item、最終 driver ownership shape、
      hard-gate status、score cap、score breakdown、verification、94/100 quality score、
