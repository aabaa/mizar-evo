...
- **`cache_key` の純粋性: internal 01 により解決済み。**
  ...
  テストする。
- **salsa query 境界: task 4 で仕様化済み。実装は task 5 に残る。** `salsa` は
  最終的な query / cache 層に必須である。`registry.md` は `PhaseService` の入力、
  出力、cancellation / versioning、cache-key intent を driver-owned salsa database へ
  どう対応付けるかを定義し、phase crate には driver / query-engine への直接依存を
  持ち込まない。
