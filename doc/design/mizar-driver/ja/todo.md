...
- **`cache_key` の純粋性: internal 01 により解決済み。**
  ...
  テストする。
- **salsa query 境界: 未解決。task 4〜5 で解決する。** `salsa` は最終的な
  query / cache 層に必須である。`PhaseService` の入力、出力、
  cancellation / versioning、cache key を salsa database へどう対応付けるかを
  `registry.md` で決定し、phase crate には driver / query-engine への直接依存を
  持ち込まない。
...
2. **仕様: `request.md`。** [ ]
   - リクエストの仕様を執筆する（英語と日本語、コードなし）:
     ...
     watch/LSP セッションからの obsolete publication を拒否する
     current-snapshot 境界。
...
3. **`BuildRequest` と `BuildSession`。** [ ]
   - `mizar-session`/`mizar-ir` の識別を通じた snapshot 捕捉を備えた
     ...
     セッションからの obsolete publication を拒否する current-snapshot 境界を
     含める。
