...
7. **仕様: `driver.md`。** [x]
   ...
   - task D-007 で完了: [driver.md](driver.md) は `CompilerDriver` の
     ...
     task-graph と scheduler submission の ownership、`mizar-build::CancellationPolicy`
     経由の cancellation、diagnostics と artifact handoff 境界、missing phase service を
     synthetic scheduler output ではなく classified gap として残す規則を定義する。
...
8. **driver のコア。** [x]
   ...
   - task D-008 で完了: `src/driver.rs` は `CompilerDriver` の session submission、
     ...
     suppression、最小の protocol-agnostic event stream handle、`mizar-build`
     `CancellationPolicy` 経由の idempotent cancellation を実装する。public scheduler
     submission result は output-free な task-state / event / diagnostic summary だけを
     ...
     dispatch は DRIVER-G-011 `external_dependency_gap` のままである。snapshot-wide
     explicit / shutdown cancellation policy propagation も、`mizar-build` がその reason 用の
     driver-owned mutator を公開していないため cancellation-flow task へ deferred とする。
