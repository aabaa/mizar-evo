...
この module の public enum はすべて downstream-facing な registry boundary type であり、
...
将来の phase owner、status、cache intent、registry error は、phase semantics、cache
compatibility、proof acceptance、artifact publication、LSP authority を driver に移さずに
追加できる。
...
execution に必要な owner handle を含む。`PhaseInputIdentities`、
`PhaseDispatchInputBundle`、`SealedParentOutputHandle` は `mizar-ir` が所有する。
driver registry は scheduler-selected dispatch で IR 所有 bundle を消費する front door
にすぎない。各 field は owner seam への参照である:
...
- seal 済み parent output は、snapshot/currentness validation 後に `mizar-ir`
  dispatch input handle を通る。

output publisher と diagnostics handle は、real producer adapter がまだ external
dependency gap であるため optional である。parent output handle は、IR 所有
`PhaseDispatchInputProvider` が scheduler-selected task の validated snapshot-bound
dispatch bundle を供給する場合にだけ存在する。real adapter は registry context が供給する
owner handle を使うか、`external_dependency_gap` を報告しなければならない。private
sink、output store、生の parent-output hash、publication token で context を迂回しては
ならない。
