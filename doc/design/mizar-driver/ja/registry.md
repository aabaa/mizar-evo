...
| durable cache lookup と compatibility は `mizar-cache` が所有する。 | task 5 integration までは `external_dependency_gap`、以後 owner seam | driver query は cache API を呼び得るが、compatibility と proof-reuse decision は `mizar-cache` に残す。 |
...
- `mizar-cache` が返す cache lookup request/result;
- cache lookup が miss した、または disabled の場合の phase execution result;
