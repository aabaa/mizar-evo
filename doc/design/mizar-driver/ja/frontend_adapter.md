...
将来の限定経路は [仕様 22.3.5](../../../spec/ja/22.error_handling_and_diagnostics.md) に従う。Frontend は元の実 aggregate と診断変換、IR は sealed storage/lineage、driver は currentness・取消し・限定呼出し、resolver は AST 候補検証と型付き path 失敗を所有する。通常の成功 Frontend phase 出力や workspace summary provider とはしない。
...
型付き path 失敗列全体を driver の Frontend service 内で非公開に変換する。E0220–E0224 に対応する class だけを Resolver phase draft にし、未割当 class があれば意味的 batch 全体を拒む。source 結合・range・UTF-8 境界・anchor・class/code 対応・安定 detail を全件検証してから別の Resolver sink に出す。意味的変換の失敗はその batch だけを捨て、検証済み Frontend batch は残す。返却前に取消しと publisher/親の現行性を再確認し、後段の lane guard は superseded 報告を抑える。出力 ref が空の `Recoverable` とし、既存 `PhaseResult.diagnostics` に Frontend batch と任意の Resolver batch を載せる。registry は従来どおり query observation を付けるが、`NoKey` は cache 再利用や成功 credit を与えない。scheduler は task を失敗にし、通常の後続を skip する。新しい出力型・registry resource・失敗出力 handle・Recoverable から Complete への変更は不要。
E0220–E0224 の registry descriptor は利用可能だが、ソース結合済み変換と runtime adoption は後続とする。全体の `CompilerDriver::submit` は後段 service 不足を dispatch 前に検査するため、直接の service と scheduler fixture で限定経路を確認しても全体 build 実行は主張しない。E0225 と完全な workspace summary は後続とする。
将来の採用テストは、E0022 のみで回復のない実 AST を使った root/module 欠落・相対 path 逸脱・別名重複・root 別名衝突、元の E0022 との併記、正常経路維持、他の全 code/class 組（同じ class の E0023 を含む）・E0022 と他の診断の混在・key/AST 欠落・回復 flag のない parser error・回復 node・不正 import 構造・AST 候補と stub の欠落/余剰/不一致・token 本文/綴り/span の不一致・不正 source/range/index/親・古い snapshot・取消しの拒否を対象とする。resolver 変換失敗時に意味的診断 batch を出さず検証済み frontend 診断を保持すること、build 失敗の保持、通常の依存先出力/cache/artifact の受理がないこと、不完全な graph 入力から E0225 を出さないことを検証する。この設計で実装や corpus coverage を主張しない。
