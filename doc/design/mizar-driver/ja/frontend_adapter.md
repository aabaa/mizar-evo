...
実 Frontend service はこの限定経路でソース結合済み E0220–E0224 変換を行う。全体の `CompilerDriver::submit` は後段 service 不足を dispatch 前に検査するため、直接の service と scheduler fixture で限定経路を確認しても全体 build 実行は主張しない。E0225 と完全な workspace summary は後続とする。
