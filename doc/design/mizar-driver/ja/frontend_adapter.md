...
既存の非 Complete 出力の抑制と task/session の失敗を維持する。後続の受渡し設計は、診断用の保持入力と通常の依存先出力を区別し、同じ現行 SourceUnit・source version・捕捉 index に結合する。Recoverable を一律に Complete へ変更せず、未認証の復元データを再実行しない。FrontendOutput を再利用し、新たな公開 API や表現はここでは規定しない。
将来の採用テストは、E0022 のみで回復のない実 AST を使った root/module 欠落・相対 path 逸脱・別名重複・root 別名衝突、元の E0022 との併記、正常経路維持、他の全 code/class 組（同じ class の E0023 を含む）・E0022 と他の診断の混在・key/AST 欠落・回復 flag のない parser error・回復 node・不正 import 構造・不正 source/range/index/親・古い snapshot・取消しの拒否を対象とする。resolver 変換失敗時に意味的診断 batch を出さず検証済み frontend 診断を保持すること、build 失敗の保持、通常の依存先出力/cache/artifact の受理がないこと、不完全な graph 入力から E0225 を出さないことを検証する。この設計で実装や corpus coverage を主張しない。
