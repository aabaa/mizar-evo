...
| [registry.md](registry.md) | phase registry と query boundary: `PhaseDescriptor`, `PhaseRequirement`, `PhaseInput`, IR-owned `PhaseInputIdentities`, `PhaseContext`, `PhaseCacheContext`, `PhaseExecutionContext`, `PhaseExecutionResources`, `SourceLoadInputs`, `PhaseResult`, `PhaseCacheObservation`, `PhaseCacheQueryResult`, `PhaseExecutionQueryResult`, `PhaseRegistryBuilder`, `PhaseRegistry`, `DriverQueryBoundary`, `DriverQueryDatabase`, `PhaseService`, `required_phase_services`; enum `PhaseOwner`, `PhaseServiceAvailability`, `PhaseStatus`, `PhaseCacheIntent`, `PhaseRegistryError`。 | `crates/mizar-driver/src/registry.rs` | `tests/registry.rs` が phase service table、deterministic registration、duplicate rejection、descriptor normalization、missing owner seam、cache-key purity、`mizar-ir` bundle 由来の dependency/parent query identity、execution context 内の parent handle、salsa/query-boundary execution、semantic/proof/cache/artifact/LSP authority を持たない source guard を cover する。`tests/lint_policy.rs` は phase owner crate が driver または salsa dependency を得ないことを確認する。 `tests/source_load.rs` は実 SourceLoad registration、input binding、typed publication を cover する。 |
| [driver.md](driver.md) | driver front door と watch orchestration: `BuildSubmission`, `DriverSubmitInput`, re-exported IR `PhaseDispatchInputProvider`, `CompilerDriver`, `WatchSubmission`, `WatchSubmitControl`, `WatchSupersededSession`, `WatchSnapshotReplacement`, `WatchModeGap`, `WatchSubmitFailure`, `DriverSchedulerRun`, `DriverMissingPhaseService`, `DriverCancelOutcome`; enum `WatchSnapshotReplacementStatus`, `WatchModeGapOwner`, `WatchOwnerSeam`, `WatchSubmitError`, `DriverSubmissionStatus`, `DriverCancelReason`, `DriverSubmitError`; `CompilerDriver::{new, with_output_publisher, registry, session, cancellation_policy, submit_watch_change, submit, cancel, events}`。 | `crates/mizar-driver/src/driver.rs`, `crates/mizar-driver/src/driver/scheduler.rs` | `tests/driver.rs` が phase-0 bootstrap、`mizar-build` planner / index / task graph の利用、scheduler submission / result consumption、missing phase-service gap、owner inputs がない場合の synthetic output なしの dispatch-gap blocking、IR-owned bundle と seal 済み parent handle が供給される場合の registry-backed scheduler-selected dispatch、stale same-lane suppression、failed module-index session storage、cancellation、non-owner authority を持たない source guard を cover する。`tests/watch.rs` は watch replacement、superseded replay、missing watcher/LSP/publisher seam、publisher replacement failure、previous-session validation を cover する。 `tests/source_load.rs` は captured submission input に対する SourceLoad binding と、後続 service 欠如による submit blocking を確認する。 |
...
| Driver は `mizar-build` の planning、task graph、scheduler、cancellation authority を消費し、scheduler semantics を複製しない。 | `driver.rs` は `mizar-build` planner、module index、task graph builder、scheduler を呼ぶ。owner IR dispatch input bundle の欠落は、scheduler-selected task が registry-backed dispatcher に到達した場合にだけ報告される。`mizar-ir` `PhaseDispatchInputProvider<BuildTask>` が bundle を供給する場合、bundle は scheduler snapshot と照合され、その後 scheduler-selected task は `run_scheduler_with_dispatcher`、`RegistrySchedulerDispatcher`、`PhaseRegistry` を通り、readiness、dependency ordering、cache、resource、cancellation decisions は replay しない。invalid IR bundle は欠落 bundle とは別に fail closed する。parent output handle は IR-owned のまま execution context を通り、生 hash から再構築しない。 | `registered_services_execute_from_scheduler_selected_dispatch`、`cache_hit_tasks_do_not_require_phase_dispatch_inputs`、`registry_dispatch_statuses_map_to_scheduler_outcomes_without_publication` を含む `tests/driver.rs`、`tests/registry.rs`、`tests/determinism.rs`、D-016/task 27 で実行した `cargo test -p mizar-build` coverage。 |
