...
| [driver.md](driver.md) | driver front door と watch orchestration: `BuildSubmission`, `DriverSubmitInput`, `PhaseDispatchInputProvider`, `CompilerDriver`, `WatchSubmission`, `WatchSubmitControl`, `WatchSupersededSession`, `WatchSnapshotReplacement`, `WatchModeGap`, `WatchSubmitFailure`, `DriverSchedulerRun`, `DriverMissingPhaseService`, `DriverCancelOutcome`; enum `WatchSnapshotReplacementStatus`, `WatchModeGapOwner`, `WatchOwnerSeam`, `WatchSubmitError`, `DriverSubmissionStatus`, `DriverCancelReason`, `DriverSubmitError`; `CompilerDriver::{new, registry, session, cancellation_policy, submit_watch_change, submit, cancel, events}`。 | `crates/mizar-driver/src/driver.rs`, `crates/mizar-driver/src/driver/scheduler.rs` | `tests/driver.rs` が phase-0 bootstrap、`mizar-build` planner / index / task graph の利用、scheduler submission / result consumption、missing phase-service gap、owner inputs がない場合の synthetic output なしの dispatch-gap blocking、owner inputs が供給される場合の registry-backed scheduler-selected dispatch、stale same-lane suppression、failed module-index session storage、cancellation、non-owner authority を持たない source guard を cover する。`tests/watch.rs` は watch replacement、superseded replay、missing watcher/LSP/publisher seam、publisher replacement failure、previous-session validation を cover する。 |
...
| Driver は `mizar-build` の planning、task graph、scheduler、cancellation authority を消費し、scheduler semantics を複製しない。 | `driver.rs` は `mizar-build` planner、module index、task graph builder、scheduler を呼ぶ。owner phase-input identities の欠落は、scheduler-selected task が registry-backed dispatcher に到達した場合にだけ報告される。`PhaseDispatchInputProvider` が identities を供給する場合、scheduler-selected task は `run_scheduler_with_dispatcher`、`RegistrySchedulerDispatcher`、`PhaseRegistry` を通り、readiness、dependency ordering、cache、resource、cancellation decisions は replay しない。 | `registered_services_execute_from_scheduler_selected_dispatch`、`cache_hit_tasks_do_not_require_phase_dispatch_inputs`、`registry_dispatch_statuses_map_to_scheduler_outcomes_without_publication` を含む `tests/driver.rs`、`tests/determinism.rs`、D-016/task 27 で実行した `cargo test -p mizar-build` coverage。 |
