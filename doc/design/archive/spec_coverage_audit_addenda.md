# Frozen Log: Specification Coverage Audit Archived Task Addenda (2026)
...
Moved verbatim from `doc/design/spec_coverage_audit.md` on 2026-09-02
(compaction batch CPT-16, rules in
...
location, and the registered `legacy_compactions.tsv` completion-redirect
line stays in the live audit (the ledger pins it there). Frozen: do not
extend; the live coverage authority is
[../spec_coverage_audit.md](../spec_coverage_audit.md). The authoritative
per-task detail is the paired task contract under
...
route, Core identity, parameter/argument, generated origin, or Task-277B
readiness is added. Core 33 retains later snapshot-local identity allocation,
