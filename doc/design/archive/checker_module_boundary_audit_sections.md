# Frozen Log: mizar-checker Module-Boundary Audit Archived Sections (2026)
...
Moved verbatim from `doc/design/mizar-checker/en/module_boundary_audit.md` on 2026-09-02
(compaction batch CPT-13, rules in
...
stay in the live document (the ledger pins them there), so archived section
bodies exclude those lines. Frozen: do not extend; the live authority is
[../mizar-checker/en/module_boundary_audit.md](../mizar-checker/en/module_boundary_audit.md). The authoritative per-task
detail is the paired task contract under
...
review passed. The task-only commit, immediate post-commit proof, and accepted
fresh-inventory
disposition are closed in the language-local [historical
...
The completed [C4B](../task_contracts/en/CHECKER-FRAENKEL-GENERATOR-BOUND-USE-257C4B.md)
...
this owner alone grows to 7958 lines / SHA-256
...
focused/package verification, formatting, package Clippy, and diff checks
pass. Full-workspace Clippy/tests, metadata/public-enum suites, and five
unchanged CLI replays also pass without widening the boundary. Independent
bilingual/boundary review is **NO FINDINGS** after the sole Low
...
The public family is distinct from exact-F5 C4A/C4B and transports only
...
state would be a `boundary_violation`. The implementation adds no module or
facade and leaves the production path set at 32 paths; the cohesive owner is
`9358` lines / SHA-256
...
keeps the one-to-one graph-owner receipt in the existing cohesive
`source_formula_composition.rs` owner. The only other Rust changes are the
checker public-enum lint row and one test in the existing private mizar-test
leaf. No module, `lib.rs` export, Cargo edge, resolver production API,
Typed/Resolved/Core field, installer, active route, diagnostic, semantic
owner, or protected artifact changes.

The boundary retains the unchanged C4C8 graph and Task33R receipt by value,
compares only exact retained resolver and resolved owner identities, and adds
no table or new id domain. Moving the receipt to Typed/Resolved/Core, exposing
an installer, or choosing parameter/argument/GeneratedOrigin transport would
be a `boundary_violation` in this task. The cohesive owner remains the correct
location; no behavior-neutral split is justified.
...
Task 257C4C6 leaves the production layout at 32 paths / 197561 lines, with path
SHA-256
...
Implemented Task 257C4C6 changes no module, file path, crate dependency, or
production owner. It extends the existing cohesive
`source_formula_composition.rs`, `typed_ast.rs`, and
`resolved_typed_ast.rs` boundaries with one private validation seam and boxed
owner plus the frozen Typed/Resolved public methods and error variants. The
three row measurements and 32-path content hash below are final; the path count
and path-list hash remain fixed. No new public module or enum type, syntax/Core dependency, runner route,
diagnostic boundary, or private split is introduced. See the paired
[C4C6 contract](../task_contracts/en/CHECKER-FRAENKEL-NESTED-CAPTURE-IDENTITY-INSTALLATION-257C4C6.md).
Independent implementation and bilingual/boundary re-reviews report **NO
FINDINGS** after the zero-semantic table guard and frozen-test repairs.
...
| `src/typed_ast.rs` | 7094 | typed AST data model | `typed_ast.md` | no | no | Large but cohesive typed-AST tables, validation, rendering, and one-shot handoffs, including definition transactions, separate proof-local owners, the neutral Task-277A source-template slot, and the boxed C4C6 receipt destination; monitor ergonomics after downstream use. |
...
| `src/source_formula_composition.rs` | 12475 | syntax-free cross-family formula composition producer | `source_formula_composition.md` | no | no | Cohesive Task-257B1/B2/B3 atomic-edge/bound-use associations, separate Task-257C2/C3 transactions, completed Task-257C4A/C4B exact-F5 transports, completed Task-257C4C3 one-row nested binder/use identity transport, completed Task-257C4C5 exactly-one capture-identity receipt, C4C6 final-owner/zero-semantic validation, C4C8 standalone normalized graph, and Task33C scalar graph-owner receipt with exact dependency/association validation; no split required. |
...
| `src/source_term.rs` | 7583 | syntax-free source primary-term producer | `source_term.md` | no | no | Cohesive Task-252 term/reference/request tables plus the exact Task-269GU/269GCU/269SDU proof-local composites and completed C4C4 nested-mapper `1/1/0` profile, C4C5 complete-validation seam, binding and parent validation, deterministic rendering, dependency/fingerprint/arena corruption matrices, and predecessor-ownership checks; production remains syntax-free and no split is required. |
...
| `src/source_statement.rs` | 52284 | syntax-free source statement producer | `source_statement.md` | no | no | Cohesive Task-258 statement/witness transactions and their corruption matrices; the same four Task-269 tests reuse the private exact B3N/B3M1 fixtures, including all-field arena and isolated cross-profile rejection, while production proof-local ownership remains in its dedicated module. |
...
| `src/resolved_typed_ast.rs` | 8998 | final resolved typed AST assembly | `resolved_typed_ast.md` | no | no | Cohesive final projection module, including clone-preserving definition and proof-local handoffs, neutral Task-277A source-template replay, and the syntax-only C4C6 receipt clone; no behavior-neutral split required. |
...
| `tests/lint_policy.rs` | 1989 | cross-cutting policy and audit guards | `source_spec_audit.md`, `bilingual_sync_audit.md`, and `module_boundary_audit.md` | no | no | Centralized policy guardrails include Task-259--264, Task-269A, Task-277A, Task-277B-L module/spec/public-enum coverage, the Task33C scalar/no-installer guard, and the unchanged production syntax boundary. |
...
| `tests/support/source_property_implementation_unit.rs` | 2516 | test-only Task-264 unit-test support | `source_property_implementation.md` and this audit | no | no | Non-integration child support for exact equals/means construction, carrier-identity and equals-selector-identity corruption, nonempty-baseline transactionality, final replay, orphan/extra rejection, and actual Task-259 isolation. |
...
Task264D remains inside that same `source_property_implementation.rs` owner:
construction borrows the existing resolver environment and retains only
complete syntax-free checker handoffs plus a private identity receipt. The
existing Task264 producer, Task254 generic owner, Typed/Resolved destinations,
Core, runner production route, Cargo graph, and protected artifacts remain
unchanged.
...
implemented checker change to `source_template.rs`, its export, neutral
...
The immediate post-implementation read-only inventory recorded `31/187955`
with the path/content hashes above.
...
no successor is selected. Task 277B is not ready with zero semantic credit.
