# Frozen Log: mizar-test TODO Archived Bodies (2026)
...
Moved verbatim from `doc/design/mizar-test/en/todo.md` on 2026-09-02
(compaction batch CPT-10, rules in
...
stay in the live TODO (the ledger pins them there), so archived section
bodies exclude those lines. Frozen: do not extend; the live sequencing
authority is [../mizar-test/en/todo.md](../mizar-test/en/todo.md). The
authoritative per-task detail is the paired task contract under
[../task_contracts/en/](../task_contracts/en/).

## Ordered Task List — Archived Completed Task Bodies
...
2. **Source/spec gap audit and status sync.** [x]
   ...
   - Audit record: [00.crate_plan.md](../mizar-test/en/00.crate_plan.md) "Known Gaps And
     Drift" and [Task 2 Audit Baseline](../mizar-test/en/todo.md#task-2-audit-baseline).
   - Deps: 1. Spec: all module specs.

3. **Runner modes and CLI completion.** [x]
   - Complete the CLI beyond `plan` per
     [minimal_crate.md](../mizar-test/en/minimal_crate.md) "CLI"/"Exit Codes" and
     [harness.md](../mizar-test/en/harness.md) "Runner Modes": validation mode over the
   ...
   - Reconcile the `parser.type_fixtures` import-summary exception with
     [harness.md](../mizar-test/en/harness.md): document it explicitly or remove the fixture
     symbol injection.
...
4. **Snapshot module: API and canonicalization.** [x]
   - Add `src/snapshot.rs` implementing the snapshot kinds, public API, and
     canonicalization rules of [snapshot.md](../mizar-test/en/snapshot.md) (stable paths,
     normalized line endings, no nondeterministic fields).
   - Tests: canonicalization fixtures; comparison failures carry precise
     diffs.
   - Deps: 2. Spec: [snapshot.md](../mizar-test/en/snapshot.md) "Public
...
5. **Snapshot update policy and determinism checks.** [x]
   - Implement the baseline update flow (resolving the update-mechanism
     ...
     [snapshot.md](../mizar-test/en/snapshot.md) (repeat-render comparison).
   - Tests: update flow round-trips; accidental-update protection;
     determinism check catches injected nondeterminism.
   - Deps: 4. Spec: [snapshot.md](../mizar-test/en/snapshot.md) "Update
...
6. **Coverage and pass/fail-mix reporting.** [x]
   ...
   - Tests: report fixtures over synthetic corpora; deterministic report
     bytes.
   - Deps: 3. Spec: [traceability.md](../mizar-test/en/traceability.md),
...
7. **Stage-prerequisite validation.** [x]
   ...
   - Tests: prerequisite-violation fixtures fail validation with stable
     diagnostics.
   - Deps: 6. Spec: [staged_model.md](../mizar-test/en/staged_model.md) "Stage Rules".

8. **Fail/soundness contract support.** [x]
   - Implement the expected-failure contract of
     [fail_soundness.md](../mizar-test/en/fail_soundness.md): required-case bookkeeping
   ...
   - Deps: 6. Spec: [fail_soundness.md](../mizar-test/en/fail_soundness.md).

9. **Corpus size and review-rule validation.** [x]
   - Validate the corpus-growth rules of [miz_corpus.md](../mizar-test/en/miz_corpus.md):
   ...
   - Deps: 3. Spec: [miz_corpus.md](../mizar-test/en/miz_corpus.md).
...
11. **Determinism suite.** [x]
    ...
    - Deps: 6. Spec: [harness.md](../mizar-test/en/harness.md) "Determinism Requirements".
...
13. **Bilingual documentation sync audit.** [x]
    - Compare each English canonical document under
      ...
      synchronize content.
    - Completion: [bilingual_sync_audit.md](../mizar-test/en/bilingual_sync_audit.md)
...
15. **Architecture-22 follow-up audit.** [x]
    ...
    - Completion: task 15 updates
      [bilingual_sync_audit.md](../mizar-test/en/bilingual_sync_audit.md) and
      [00.crate_plan.md](../mizar-test/en/00.crate_plan.md) with the post-task-14 audit.
...
16. **Source-derived builtin type-expression bridge.** [x]
    ...
    - Deps: 10, `mizar-checker` task 12. Spec: [harness.md](../mizar-test/en/harness.md),
      [expectation_schema.md](../mizar-test/en/expectation_schema.md),
      [traceability.md](../mizar-test/en/traceability.md), checker MC-G020.
...
17. **Source-derived builtin `ResolvedTypedAst` bridge.** [x]
    ...
    - Keep declaration extraction, non-builtin type heads, attributes, terms,
      ...
      payloads.
    - Deps: 16, `mizar-checker` task 28. Spec: [harness.md](../mizar-test/en/harness.md),
...
18. **Source-derived reserve declaration semantic bridge.** [x]
    ...
    - Deps: 16, 17, checker MC-G011/MC-G016/MC-G020. Spec:
      [harness.md](../mizar-test/en/harness.md), [expectation_schema.md](../mizar-test/en/expectation_schema.md),
...
19. **Reserve bridge core summary readiness and builtin declaration
    ...
    - Deps: 18, `mizar-core` elaborator summary API. Spec:
      [harness.md](../mizar-test/en/harness.md), [expectation_schema.md](../mizar-test/en/expectation_schema.md),
      [traceability.md](../mizar-test/en/traceability.md), core `elaborator.md`.
...
## Addenda — Archived Closed Section Bodies

Section headings match the live TODO; pinned redirect lines are
not duplicated here.
...
- [x] Complete spec review to **NO FINDINGS**, all measured docs-only gates,
  uncapped final quality with all nine gates PASS at `100/100`, and exact
  prerequisite commit `b43081161b31fcc4bc23ac2fd42c5c42e772ab78`.
...
- [x] Complete repeated spec/bilingual review with **NO FINDINGS**, all nine
  docs gates uncapped at `>=90/100`, exact 42-file staging, and docs commit.
- [x] Fresh-preflight and implement only SDC; keep source type and descendant
  occurrence separate and all Set closure/capture blocked.
- [x] Complete all implementation reviews/gates, commit, and fresh-select the
...
[semantic_bridge_corpus_map.md](../mizar-test/en/semantic_bridge_corpus_map.md) (coverage,
