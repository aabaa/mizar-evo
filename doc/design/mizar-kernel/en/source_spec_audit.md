...
| KERNEL34-G001 | F9 `design_drift` / `test_gap` closed by task 34 | Before task 34, the legacy tautology-marker effect was profile-dependent and thinly specified after the migration/audit gate. | Task 34 pins the marker to explicit legacy audit replay data only, documents that it is not an empty contradiction or trusted acceptance material, and adds mutation coverage that a tautology-marker final goal rejects as `invalid_sat_proof` before successful-audit wrapping. |

## Verification Plan

Task 34 refreshes this audit while pinning legacy tautology-marker semantics in
the audit-only resolution-trace path. Required verification is:
...
Task 34 changes legacy audit-path tests and docs, but not `mizar-core`,
