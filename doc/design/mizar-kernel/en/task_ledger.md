...
```text
Continue mizar-kernel autonomous crate development from the completed task 20
source/spec correspondence audit commit. Before starting task 21, verify a
clean worktree, confirm the task 20 commit exists in git log, and re-read
...
doc/design/mizar-kernel/ja/source_spec_audit.md, and the paired files under
doc/design/mizar-kernel/en/ and doc/design/mizar-kernel/ja/. Implement task 21
only: perform the bilingual documentation sync audit, add paired
`bilingual_sync_audit.md` files, and synchronize English/Japanese companion
content where the audit finds drift. Do not change Rust behavior, public APIs,
...
cluster search, implicit coercion insertion, fallback inference, global
mutable state, or downstream ATP/proof/cache/artifact integration. Use
review-only agents for the required AGENTS.md review phases. Run at least
`git diff --check` and `git diff --cached --check`; run crate verification only
if the sync audit changes Rust source or executable lint behavior. Commit task
21 by itself with explicit path staging.
```

Rationale: task 21 is documentation-audit work, but it guards the canonical
English/Japanese contract before the module-boundary gate and closeout. Keep
`xhigh` because the user requested it for this autonomous crate run; lower only
for typo-only follow-up, and raise only if the bilingual docs reveal
contradictory trusted-kernel requirements.
