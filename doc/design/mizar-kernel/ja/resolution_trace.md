...
unused context entry を scan して拒否してはならない。Compatibility validation は unbounded
count/size limit を使ってよく、profile、symbol、variable、canonical-form agreement だけを
検査する。使われた imported clause が compatibility validation に失敗する場合は
