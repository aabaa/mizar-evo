...
Encoded problem はすべての evidence formulas を true として assert し、target goal を
refutation のため false として assert する。Equivalent caller order は同一の canonical SAT
bytes を生成しなければならない。
