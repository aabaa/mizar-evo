...
- deterministic interruption に必要な最小 callback 以外、`batsat` の DRAT/proof、
  theory、statistics-printing、logging、model enumeration、callback surface を
  呼び出さない;
...
public kernel-facing shape は [sat_checker.md](./sat_checker.md) が定義したまま:
...
- deterministic callback-based step limit を実装して test し、interruption を
  `timeout` または `resource_exhaustion` へ map する; または
- exact solver-step limit は dependency が support しないものとして分類し、support
  される deterministic input limit だけを expose し、unsupported step budget を
  要求する caller request を拒否する。
...
Task 27 は `sat_checker` source module が public になる時点で source/spec audit
record を更新しなければならない。それまでは、この audit が authoritative
dependency decision であり、dependency source はまだ統合されていない。

## Failure Mapping

Wrapper は UNSAT 以外のすべての outcome を stable kernel rejection detail へ map する:

| Wrapper condition | Kernel detail |
|---|---|
| derived SAT problem が satisfiable | `invalid_sat_refutation` |
...
| deterministic callback interruption / step-budget stop | [rejection.md](./rejection.md) に従い `timeout` または `resource_exhaustion` |
...
`SatCheckResult::Unsat` だけが caller に formula/substitution evidence の受理を
許可する。
...
|---|---|---|---|
| KERNEL24-G001 | `deferred` | `batsat` は public exact conflict/propagation budget setter を持たない。 | Task 27 は callback interruption を証明・test するか、supported input limit だけを expose し、unsupported step-budget request を拒否する。 |
| KERNEL24-G002 | `source_drift` | `sat_checker` source はまだ統合されておらず、manifest も task-1 dependency set のままである。 | Task 27 が `batsat` を統合し、lint policy を更新し、wrapper test を追加する。 |
...
| KERNEL24-G004 | `deferred` | `batsat` は host random state を読まないが、deterministic pseudo-random heuristic option を expose する。 | Task 27 は wrapper-owned solver construction でそれらを pin/disable し、caller に expose してはならない。 |
