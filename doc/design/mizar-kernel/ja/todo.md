...
- **証明書スキーマの所有権: 未解決。task 4 で解決する。** アーキテクチャ
  15 が証明書フォーマットを定義する。既定候補は、この crate がスキーマ型
  （構文解析を含む）を所有し、`mizar-atp` が候補を構築するために
  `mizar-kernel` に依存する形であり、kernel は証拠の生産者に決して依存
  しない。決定を `certificate_parser.md` とトップレベルに記録する。
- **trusted ベースラインの crate ポリシー: 未解決。task 1 で解決する。**
  信頼されるコードへの追加の厳格さ（例: `#![forbid(unsafe_code)]`、
  サードパーティ実行時依存の禁止、モジュールごとの trust 文の必須化）を
  決め、lint 方針ガードに符号化する。
- **discharge 証拠の検証範囲: 未解決。保留。** `mizar-vc` の pre-ATP
  discharge 証拠を kernel が再生するか、ポリシーレベルの証拠として受理
  するかは `mizar-proof`（そのポリシー仕様）とともに決める。トップレベルで
  追跡する。
...
1. **crate の足場と trusted ベースライン lint 方針。** [ ]
...
9. **resolution trace checker の実装。** [ ]
...
12. **alpha 変換と自由変数検査の実装。** [ ]
    - alpha 同値の主張と自由変数の側条件を検査する。
    - テスト: 同値のフィクスチャ。自由変数条件違反の拒否。
