...
再検査する binder 契約）に依存する。予定される唯一の production dependency
追加は、SAT dependency audit task が選ぶ、review 済みの pure-Rust
MiniSAT-compatible SAT checker である。それは deterministic、in-process、
resource-bounded で、backend process execution を含まないものでなければならない。
その他の依存追加には記録された正当化が必要である。`mizar-atp` と
`mizar-proof` がこの crate に依存するのであって、逆は決してない。アーキテクチャ:
...
- **formula/substitution evidence correction: 未解決。task 23-29 が所有する。**
  ...
  `source_drift` と分類する。
- **trusted SAT dependency: 未解決。task 24 が所有する。** kernel は small
  kernel の一部として review 済み pure-Rust MiniSAT-compatible SAT checker を
  信頼してよいが、外部 SAT/ATP process を呼び出したり ATP search を実装したり
  してはならない。source integration より前に、dependency choice、version
  pinning、determinism、limit、unsafe usage、audit note を記録しなければならない。
- **証明書スキーマの所有権: task 4 で解決済み。** アーキテクチャ 15 が
  ...
  であり、task 23-29 で formula/substitution evidence schema へ移行しなければならない。
- **trusted ベースラインの crate ポリシー: task 1 で解決済み、task 24 で改訂予定。**
  ...
  lint/dependency policy を、`mizar-session` と `mizar-core` に加えて audit 済み
  pure-Rust SAT checker dependency をちょうど 1 つだけ許す形へ改訂するか、
  dependency を追加しない理由を記録しなければならない。
...
23. **仕様: kernel evidence format correction。** [ ]
...
24. **仕様と監査: trusted SAT checker dependency。** [ ]
...
25. **Formula/substitution evidence schema and parser。** [ ]
    ...
    - 依存: 23、24。仕様: `formula_evidence.md`。
