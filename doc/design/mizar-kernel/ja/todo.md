...
| 所見 | 処置 |
|---|---|
| F1(goal polarity) | architecture 15 修正済み。kernel 実装は task 30 で実装済み。producer-side polarity declaration/rejection は [mizar-vc task 27](../../mizar-vc/en/todo.md) で解決済み |
...
| F6(imported-statement projection) | task 33 で実装済み(mizar-vc task 29 と対) |
| F7(mizar-test soundness 語彙) | [mizar-test task 21](../../mizar-test/en/todo.md) で解決済み |
| F8(corpus ディレクトリ命名) | [mizar-test task 22](../../mizar-test/en/todo.md) で解決済み |
| F9(レガシー tautology marker) | task 34 で実装済み |
| reduct-view soundness 再監査 | task 35 で完了 |

30. **check service における goal-polarity 束縛(F1、不変条件 B4)。** [x]
...
31. **非 import ソース束縛の context-identity 検証(F2、P クラス不変条件)。** [x]
    - local-hypothesis / cited-premise / generated-VC-fact ソース束縛の検証
      データを仕様化・実装する: `FormulaEvidenceContext`(または immutable
      kernel context)を拡張して task-28 `context_identity_hash()` payload を
      canonical な `mizar-vc` formula-envelope handoff hash とともに運び、
      非 import 束縛それぞれがその payload に対して検証可能であることを
      受理の前提にする。payload を欠く evidence への現行の fail-closed
      `missing_provenance` 挙動は維持する。canonical `mizar-vc` handoff hash は
      parser envelope bytes から再計算せず、不透明な caller context として扱う。
      task-28 context-identity hash は architecture-15 v1 line grammar から再計算する。
      `formula_evidence.md` と architecture 15 を同一変更で更新する。producer 側 payload は
      対となる mizar-vc タスク。
    - 受け入れ条件:
      ...
      goal を hypothesis とラベルする ATP 側 mutation が拒否される。golden-vector test が
      task-28 context-identity line grammar と hash domain を固定する。
    - 検証: `cargo test -p mizar-kernel`、`cargo test -p mizar-test`、
      `cargo clippy -p mizar-kernel --all-targets -- -D warnings`。
...
33. **imported-statement projection の仕様化(F6)。** [x]
    - arch-18 の imported statement fingerprint(リッチな式)から evidence
      ...
      Task 33 は一時的な fingerprint 等値規則を、`formula_evidence.md` と
      architecture 15 の canonical imported-statement projection validation で
      置き換える。paired `mizar-vc` producer payload contract は完了済みであり、
      残る ATP/downstream production は捏造 payload ではなく、文書化済み
      projection contract を使わなければならない。
...
34. **レガシー tautology marker の意味論(F9、low)。** [x]
    - Task 34 は legacy resolution-trace tautology marker を廃止せず確定した。
      Marker は明示的な migration/audit `ClauseTautologyPolicy::Marker` profile の下でだけ
      到達可能である。Audit replay data では zero-literal checked step として現れてよいが、
      `empty` contradiction clause ではなく、final-goal checking を満たせず、corrected
      formula/substitution evidence path では trusted acceptance material にはならない。
    - 受け入れ条件: 意味論は `resolution_trace.md` の節と mutation-rejection tests を持つ。
      Ordinary replayed clause を `tautology` と誤ラベルしても premise 弱化に留まる。
      Tautology marker を final goal として名指しした場合は、successful-audit wrapper が
      生成される前に final-goal location の `invalid_sat_proof` として拒否される。
    - 検証: `cargo test -p mizar-kernel`。
    - 依存: 29。仕様: `resolution_trace.md`; soundness_argument.md F9、
      L クラス不変条件。

35. **reduct-view エンコードに対する soundness argument の再訪。** [x]
    - テンプレートエンコーディング監査
    ...
    - 受け入れ条件: `soundness_argument.md`(英日)が再監査結果を記録する。
      view choice は atom subject の正規化 term bytes であるため、kernel
      invariant や corpus sidecar の変更は不要である。将来、不変条件が
      変わる場合は、corpus sidecar note を同一変更で更新する(本文書の
      constraint 節に従う)。
    - 検証: `cargo test -p mizar-kernel`、`cargo test -p mizar-test`。
