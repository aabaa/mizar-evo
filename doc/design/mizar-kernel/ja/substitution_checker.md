...
Frame は `canonical_index` で sorted、`binder_id` と `variable_id` で unique、かつ
`source_term` / `target_term` が使う normalized term encoding と compatible でなければ
ならない。Free / schematic variable list は sorted unique である。Display name と
...
free-variable replay は task 12 に残す。Task 12 は同じ module に alpha-conversion、
