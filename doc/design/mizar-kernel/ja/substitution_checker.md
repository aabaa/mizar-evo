...
Task 12 は architecture 16 strategy の certificate-visible な具象化を用いる。Freshness
witness が semantically valid なのは、次をすべて満たす場合だけである:

- `binder_path` が `source_term` 内の binder node を選択する;
- 同じ normalized path が `target_term` 内の binder node を選択する;
- source binder frame が original bound variable と role を与える;
- target binder frame の role が `generated fresh binder` であり、
  `variable_id == generated_variable_id` である;
- `avoided_variables` が、source binder body の free variables から original bound
  variable を除いた集合と、その binder の下へ挿入される replacement actual term の
  free variables の和として再計算した sorted set と完全に一致する;
- `generated_variable_id` が、その再計算された avoided set に含まれない;
- `deterministic_counter` が、再計算された avoided set を除外した certificate variable
  manifest の stable id 昇順 candidate stream における `generated_variable_id` の
  zero-based position である。

Witness owner と binder path は、architecture 16 が要求する source または certificate
owner と role data の replay envelope の一部である。この module は、欠けた owner、role、
source identity を埋めるために producer state を参照してはならない。利用可能な normalized
evidence から上記 candidate stream を再計算できない場合、その witness は
`invalid_substitution` である。
...
- 2つの normalized term の差分が decoded binder context 内の consistent bound-variable
  renaming だけである;
- rename された各 binder は、同一の normalized source / target `binder_path` position と
  1つの referenced freshness witness によって対応付けられる;
- source binder の frame が original variable id を与え、その source binder scope 内の
  bound occurrence はすべて target generated variable id に rewrite される一方で、
  replacement actual term は変更されない;
...
- free のままでなければならない variable は replay 後の recorded term path で free の
  ままでなければならない;
- checker は recorded target path 上の active target binder stack から capture set を
  再計算し、constraint predicate を受理する前に recorded `capture_set` がその再計算された
  sorted set と完全に一致することを要求する;
- capture set から absent でなければならない variable は、その再計算された set に
  出現してはならない;
...
- `test_gap`: task 11 は valid replay、explicit payload validation、target mismatch、
  capture violation、malformed binder context、missing provenance、resource limits、
  deterministic report の direct-substitution replay coverage を閉じた。Task 12 は
  alpha-equivalence、deterministic freshness、free-variable / capture-set、
  alpha-resource、shuffled-context coverage をこの module 内で閉じる。Source-derived
  corpus fixture は後続 checker/source integration task まで deferred のままである。
