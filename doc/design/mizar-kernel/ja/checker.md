...
promotion、artifact `kernel_verified` status を populate してはならない。Resolution
replay が報告する legacy tautology marker は audit data に限られる。その marker が
final goal として名指しされた場合、checker は successful-audit の
`unsupported_certificate_format` wrapper record を生成する前に、final-goal location の
`kernel_rejection/invalid_sat_proof` としてその input を拒否する。
...
9. `final_goal` が参照する generated clause、resolution step、または supported derived fact を解決し、
   ...
   Legacy tautology marker は empty obligation ではなく、受理されない。
...
Corrected formula-evidence path では、task 33 が caller-owned
`FormulaImportedFactEvidence` row に `formula_evidence.md` で検証される
imported-statement projection を追加する。Checker はまず package、module、exported item、
architecture-18 statement fingerprint によって imported source binding を解決し、その後
context row の projection が parsed formula source projection と完全に一致することを要求する。
Stale または mismatched な context projection は SAT encoding 前に
`formula.imported_statement_projection` の `unresolved_symbol` になる。Parser 側の
projection failure(unsupported algorithm id、空または非正準 payload、stale source projection、
formula-projection mismatch)は `missing_provenance` のままであり、checker の import lookup には
到達しない。この projection check は legacy imported-clause
`normalized_clause_fingerprint` path を変更しない。
...
- legacy `tautology` marker は canonical empty clause ではない。tautology marker を指す
  final goal は `invalid_sat_proof` として拒否される;
...
Task 33 は以下の Rust tests を追加しなければならない:

- architecture-18 statement fingerprint が kernel formula-tree fingerprint と異なるが、
  canonical projection payload が一致する projected imported statement。
- stale projection statement fingerprint、formula-projection mismatch、非正準 projection
  payload が parser で reject されること。
- context-side projection mismatch が SAT encoding 前に `unresolved_symbol` として reject
  されること。
