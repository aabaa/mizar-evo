...
module へ昇格させる。`sat_encoding` と `sat_checker` は planned/unimplemented design
surface のままであり、tasks 26-27 が対応する exported module を追加するまでは
executable source-backed guard の対象には意図的に含めない。
...
現在の source inventory は task-25 public surface であり、formula/substitution evidence
parser を追加する。一方で legacy `check_kernel_certificate` path は corrected evidence
format に対する `source_drift` / `design_drift` として残る。Tasks 26-29 は、通常 proof
policy が corrected pipeline に依存できるようになる前に、SAT encoding/check/service path を
追加し legacy resolution-trace acceptance を gate または retire しなければならない。
