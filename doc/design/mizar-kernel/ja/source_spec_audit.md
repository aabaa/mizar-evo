...
新しい task-23 `formula_evidence`、`sat_encoding`、`sat_checker` specs は
planned/unimplemented design surface である。対応する exported module を tasks 25-27 が
追加するまでは、executable source-backed guard の対象には意図的に含めない。
...
Legacy `check_kernel_certificate` path は corrected evidence format に対する
`source_drift` / `design_drift` として分類され、target normal proof policy として扱っては
ならない。
