...
Rationale: task 21 は documentation-audit work だが、module-boundary gate と
closeout 前に英語正本/日本語 companion の contract を守る。今回の autonomous
crate run では user が要求したため `xhigh` を維持する。typo-only follow-up
だけなら下げてよく、bilingual docs が trusted-kernel requirements の矛盾を
明らかにする場合だけ上げる。
