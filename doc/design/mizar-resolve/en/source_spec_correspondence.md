...
- Source behavior that remains outside the current executable corpus is already
  ...
  its current concrete remainder is refined by `R-G007`. Task 265 classifies
  the exact same-signature/same-return conflict as R-G008, owned by R-031.
  R-G003 is resolved by R-024.
...
| R-G008 | `source_drift` / `test_gap` | Chapter 19 §19.1 requires ordinary declarations with the same symbol kind, spelling, arity, and argument signature to conflict even when return signatures match; the active runner covers only the different-return slice and the same-return seed remains deferred. | Active R-031 owns the exact resolver collection/diagnostic and declaration-symbol consumer. Preserve identities/order, avoid checker overload selection, and do not rebaseline the different-return expectation. |
