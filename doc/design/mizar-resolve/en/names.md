...
Ordinary callable uses also admit direct `DefinitionParameter` qualified segments (including two unique comma-separated binders), unary non-template functor-pattern arguments and the two locus positions of a plain binary predicate pattern under the same declaration-identity, scope and ordering checks. Radix interpretation and field-selector typing remain checker-owned; no prior template or registration profile changes.
