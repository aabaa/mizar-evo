...
Ordinary callable uses also admit direct `DefinitionParameter` qualified segments and unary non-template functor-pattern arguments under the same declaration-identity, scope and ordering checks. Radix interpretation and field-selector typing remain checker-owned; no prior template or registration profile changes.
