...
`names.rs` owns authenticated `SurfaceAst` variable scope. It derives binding,
...
Ordinary callable uses also admit direct `DefinitionParameter` qualified segments, two unique comma-separated binders in ordinary definitions/theorem quantifiers/proof lets, unary non-template functor-pattern arguments, and the outer loci of plain binary functor/predicate/notation patterns under the same declaration-identity, scope and ordering checks. Radix interpretation and field-selector typing remain checker-owned; no prior template or registration profile changes.
A type formal may carry the exact `type extends <TypeExpression>` declaration tail; lookup still returns its real binder token for type positions and selector bases. Bound resolution and object-versus-element interpretation remain checker-owned.
