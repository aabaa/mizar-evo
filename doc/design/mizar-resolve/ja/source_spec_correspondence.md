...
状態: task R-027 audit complete。

## 範囲

この監査は、R-026 までの non-deferred な `mizar-resolve` task 実装を英語正本の
resolver design spec、すなわち [resolved_ast.md](./resolved_ast.md)、
...
task R-024 は `external_dependency_gap` として明示的に deferred であるため、
deferral record と resolver-owned artifact schema / reader が存在しないことだけを監査する。
この監査は executable test を置き換えない。また、実装に合わせる目的で `doc/spec`、
既存 `.miz` source、expectation sidecar を変更しない。

## 結果

- R-026 までの non-deferred な公開 API と behavior contract について、missing
  implementation は見つからなかった。
...
- 現在の executable corpus 外に残る source behavior はすでに分類済みである:
  public resolver diagnostic code（`R-G001`）、artifact-backed `ModuleSummary` reuse
  （`R-G003`）、parser/syntax の scheme declaration exposure（`R-G006`）、
  より広い semantic `.miz` runner assertion である。広い履歴的 corpus gap は
  `R-G002` として残り、現在の具体的な残りは `R-G007` が精緻化する。
...
| [env.md](./env.md) deterministic environment debug rendering | `SymbolEnv::snapshot_text` と sorted index/contribution section | `crates/mizar-resolve/src/env.rs`, crate-root determinism test | `symbol_env_snapshot_text_is_stable_and_covers_index_families`, `resolver_public_seams_are_deterministic_for_equivalent_inputs` | No finding |
...
| [names.md](./names.md) preliminary symbol-name resolution と internal diagnostics | name projection、built-in projection、reference candidate、`SymbolNameResolver`、`NameDiagnosticCollector`、`NameDiagnosticReport`、diagnostic root/cascade | `crates/mizar-resolve/src/names.rs` | `unqualified_lookup_uses_declaration_point_shadowing_and_builtins`, `duplicate_import_aliases_drive_ambiguous_namespace_payloads_deterministically`, `unresolved_import_dependency_produces_one_primary_name_diagnostic`, `name_diagnostics_preserve_ambiguous_candidate_order`, `name_diagnostics_order_same_range_by_class_spelling_and_candidate_key`, `name_diagnostics_use_mixed_root_ordering`, `recovered_inputs_do_not_emit_name_diagnostic_roots` | No finding |
...
| Artifact boundary | R-024 は deferred のままである。source には resolver-owned `ModuleSummary` schema、artifact reader、writer、fallback format は存在しない。 |
...
| R-024 ModuleSummary reuse | `R-G003` / `external_dependency_gap` として明示的に deferred。source audit では resolver-owned artifact schema、reader、shim は見つからなかった。 |
...
| R-G003 | `external_dependency_gap` / deferred | canonical `ModuleSummary` artifact から dependency module を消費する経路。 | deferred R-024。解除条件は `mizar-artifact` task 1〜5 が schema、writer、validating reader、version compatibility policy を提供すること。 |
