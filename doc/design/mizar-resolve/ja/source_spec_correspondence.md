...
overlay complete; Task 265 R-031 ownership addendum active。
...
- 現在の executable corpus 外に残る source behavior はすでに分類済みである:
  ...
  残りは `R-G007` が精緻化する。Task 265はexact same-signature/same-return
  conflictをR-G008に分類し、R-031へ割り当てる。R-G003はR-024で解消済み。
...
| [symbols.md](./symbols.md), [mizar-test staged model](../../mizar-test/ja/staged_model.md) declaration-symbol runner | active `declaration_symbol` corpus stage、public resolver diagnostic code を作らず internal detail-key expectation を比較し、表現済み kind、visibility、export status について exact SymbolEnv-derived pass payload assertion を持つ経路 | `crates/mizar-test/src/runner.rs`, `tests/miz/pass/resolve/pass_resolve_declaration_symbol_smoke_001.*`, `tests/miz/fail/resolve/fail_resolve_duplicate_theorem_symbol_001.*`, `tests/miz/fail/resolve/fail_resolve_same_signature_return_conflict_001.*`, `tests/coverage/spec_trace.toml` | `cargo run -p mizar-test -- plan --tests-root tests --manifest tests/coverage/spec_trace.toml`; `cargo run -p mizar-test -- declaration-symbol --tests-root tests --manifest tests/coverage/spec_trace.toml`; R-023 と post-task-20 R-G007 increments の `cargo test -p mizar-test`; `active_declaration_symbol` tag 付き expectation sidecar | represented runner pathにfindingなし。R-G008/R-031がdeferred same-return conflictを所有する。より広いimport/name/dot-chain/label assertionはR-G007。 |
...
| R-019 to R-023 symbols and corpus runner | `symbols.md` は `src/symbols.rs` に実装済み。unit tests は opaque / parser-backed signature、represented duplicate/conflict、overload grouping、registration、recovery、context-only shell、deterministic diagnostics を cover する。R-023 は active declaration-symbol pass/fail corpus seed と traceability metadata を追加し、R-G008がexact Chapter-19 same-return exceptionを記録する。 |
...
| R-G007 | `test_gap` | active signature-conflict と pass-payload increment 後に残る R-G002 の具体的な残り: R-009〜R-019 の import graph、namespace/name resolution、dot-chain、label-reference fact について、より広い active semantic `.miz` assertion を追加する。 | R-027 には non-blocking。unit tests が挙動を cover し、R-023 は declaration-symbol runner と初期 traceable active set を導入済み。`doc/spec/en` を超える挙動を創作せず、将来の runner assertion 拡張で coverage を増やす。 |
| R-G008 | `source_drift` / `test_gap` | Chapter 19 §19.1は同じsymbol kind、spelling、arity、argument signatureを持つordinary declarationがreturn signature一致時もconflictすることを要求する。active runnerはdifferent-return sliceだけをcoverし、same-return seedはdeferred。 | Active R-031がexact resolver collection/diagnosticとdeclaration-symbol consumerを所有する。identity/orderを保存し、checker overload selectionを行わず、different-return expectationをrebaselineしない。 |
