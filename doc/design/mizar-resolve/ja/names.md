...
ambiguous record である。task R-014 は preliminary `NameSymbolProjection` seam 上で
ordinary symbol-name lookup を実装する。qualified / unqualified visibility、
declaration-point filtering、current-module shadowing、overload-group placeholder、
enabled builtin、deterministic `NameRefTable` outcome が対象である。task R-015 は
...
identity である。R-014 はこの seam を `NameSymbolProjection` として公開する。
R-019〜R-021 は同じ identity を kind-specific signature、overload family、
relation link、exported summary data で精緻化する。declaration を semantic declaration
として表現できない場合、resolver は `SymbolId` を創作せず unresolved または ambiguous
result を記録する。
...
task R-015 は `NameRefTable` を name outcome の source of truth として保ち、
crate-local diagnostic report を追加する。この report は unresolved / ambiguous name
reference、unresolved namespace path、unresolved import-alias dependency、ambiguous
namespace / import-alias root に deterministic `NameDiagnosticRootId` を割り当てる。
...
primary diagnostic を出さない。namespace failure が unresolved import alias に起因する
...
recovered namespace record と recovered reference origin は resolver table に保持するが、
internal name diagnostic root / cascade からは除外する。
...
- 各 internal record の primary / cascade role。

diagnostic ordering は root primary range、primary-before-cascade role、failure /
ambiguity class name、attempted spelling、stable candidate key、record-local
range/name-reference tie-breaker を使う。public numeric code は
...
通常の callable 参照も、同じ宣言同一性・スコープ・順序検査の下で直接の `DefinitionParameter` qualified segment、通常定義・定理量化・proof let の重複のない comma 区切り2束縛、unary な非 template 関手パターン引数、plain binary functor/predicate/notation pattern の両端 loci を受理する。基底型の解釈と field selector の型検査は checker が所有し、従来 template・registration profile は変更しない。
型仮引数は厳密な `type extends <TypeExpression>` 宣言末尾も持てる。型位置と selector 基底の参照は実際の宣言トークンへ解決し、上界の解決と object・element の区別は checker が所有する。
