...
contract を仕様化する。実行可能な pass は導入しない。public resolver diagnostic
向けの code range は既知の `spec_gap` のままであるため、本書は diagnostic class、
payload、ordering を仕様化するが、public numeric diagnostic code は創作しない。
...
- `imports.md` 由来の resolved import graph と alias binding。
