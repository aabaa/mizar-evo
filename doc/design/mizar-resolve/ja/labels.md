...
状態: task R-017 は task R-018 に向けた resolver-owned label-resolution contract
を仕様化する。専用 label scope family、proof block nesting rule、forward-reference
policy、citation lookup behavior、後続の proof / checker / VC / artifact phase が使う
normalized label-origin path を記録する。本 task は design-only であり、実行可能な
label collection と `LabelRefTable` への投入は R-018 で行う。
...
item ごとに 1 つの label-resolution outcome を生成する。shared module prefix の失敗は
unresolved namespace / module dependency として 1 回記録し、各 grouped item はそれに依存する
unresolved label outcome を記録する。
...
R-017 は documentation-only であるため executable test を追加しない。R-018 は次の unit test を
追加しなければならない。
...
- parser が該当 syntax をすでに生成する範囲での simple、qualified、grouped citation lookup。
