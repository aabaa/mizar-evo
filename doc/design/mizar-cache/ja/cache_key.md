...
`CacheKey` 内の vector はすべて hashing 前に sort する:
...
同一 canonical key かつ同一 payload の重複は coalesce する。同一 canonical key で異なる
payload の重複は invalid key request であり、`CacheKey` を作る前に reject する。
...
domain と digest bytes を encode する。
...
`CacheKeyBuildOutcome::Uncacheable` を生成してもよいが、その場合は uncacheable
validation input も設定しなければならない。そのような key は reusable hit を生成しては
ならない。structurally invalid request、unsupported cache-key schema、conflicting
duplicate は `CacheKeyBuildOutcome::NoKey` を生成する。
