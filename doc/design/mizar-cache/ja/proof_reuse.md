...
- `mizar-proof` が export する cache-local accepted goal-polarity key。task 24
  は `assert_false_for_refutation` だけを support し、future/unknown key は
  この spec と cache-side schema が更新されるまで miss とする;
...
|---|---|---:|
| `KernelVerified` | selected proof witness hash、selected evidence hash、accepted goal-polarity key、proof-reuse validation hash、`ObligationAnchor`、canonical VC/local-context/dependency-slice fingerprint、policy fingerprint、schema version、dependency artifact がすべて一致する。 | no |
| `DischargedBuiltin` | deterministic discharge hash、selected evidence hash、accepted goal-polarity key、proof-reuse validation hash、`ObligationAnchor`、canonical VC/local-context/dependency-slice fingerprint、policy fingerprint、schema version、dependency artifact がすべて一致する。 | no |
...
task 24 は cache-side proof-reuse schema を v2 に上げる。v2 validator は reusable
trusted class に対して cache-local accepted goal-polarity key を要求する。この key を
欠く監査前 record、または現在の cache schema が support しない key を持つ record は
forced miss である。canonical VC handoff hash と context-identity hash は、すでに
upstream `mizar-vc` dependency-slice/proof-reuse key payload と `mizar-proof`
validation hash に含まれているため、この module は source runner や bridge payload を
発明せず owner-provided identity を記録・比較する。
...
10. reusable trusted class では accepted goal-polarity key が存在し、この cache schema
    で supported で、一致する;
11. `mizar-proof` が供給する場合、selected evidence hash、selected provenance hash、
    selection reason、tie-break key hash が一致する;
12. `mizar-proof` が export する場合、trusted used-axioms reference hash が一致する;
13. proof-reuse validation hash が一致する;
14. dependency artifact availability と記録された domain/digest check が成功する;
15. `uncacheable`、incomplete-footprint、unsupported-schema、unknown toolchain、
...
- reusable trusted class の accepted goal-polarity key 欠落、不一致、unsupported;
...
- 必要な各 component の欠落または mismatch が reuse を block すること:
  ...
  trusted used-axioms reference hash、reusable trusted class の accepted
  goal-polarity key、proof-reuse validation hash、policy、schema version、
  dependency artifact;
- kernel/proof status projection からの real `StatusReuseMetadata` conversion が
  supported accepted goal-polarity key を cache snapshot へ運ぶこと;
