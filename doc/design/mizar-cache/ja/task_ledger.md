...
| CACHESTORE-G003 | `external_dependency_gap` | `mizar-artifact` | Artifact committed publication-token integration が存在する。それまでは task 8 は local dependency artifact availability と記録された domain/digest だけを check する。 |
