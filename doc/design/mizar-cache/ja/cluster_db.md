...
aggregate index rebuild を実装する。task 14 は accepted aggregate row 上の
in-memory import スコープ view materialization と invalidation を実装する。
...
task 13 と task 14 は source-level write/origin-tracking と import-scoped-view の
data layer を公開する。
...
task 14 は `ClusterDbIndex::import_scoped_view` を in-memory cache-side projection として
公開する。request を canonicalize し、visible origin key を sort / deduplicate し、
欠落した visible origin、unsupported cluster-db / producer schema metadata、不明な
policy/toolchain/traversal compatibility、不明な producer schema compatibility を
reject し、visible origin を active verifier policy、schema compatibility、toolchain
compatibility と照合し、aggregate index を visible origin set で filter する。
scheduler hook、durable view file、`mizar-ir` adapter、proof status projection、
lookup-outcome policy、trace construction は追加しない。
...
新しい variant は unaccepted または hidden contribution を visible にするのではなく
fail closed しなければならない。

| Public enum | 前方互換性の決定 |
|---|---|
| `ClusterContributionVisibility` | `#[non_exhaustive]`; unknown visibility は importer-visible ではない。 |
| `ClusterContributionStatus` | `#[non_exhaustive]`; unknown status は accepted ではない。 |
| `ClusterContributionKind` | `#[non_exhaustive]`; unknown contribution kind は visible index に入れてはならない。 |
| `ClusterOriginFootprintCompleteness` | `#[non_exhaustive]`; unknown completeness state は origin insertion または view reuse を reject する。 |
| `ClusterIndexEntryKind` | `#[non_exhaustive]`; unknown index row は importer に visible ではない。 |
| `ClusterDbViewMiss` | `#[non_exhaustive]`; 新しい miss reason は diagnostic-only で fail closed する。 |
| `ClusterDbWriteRejection` | `#[non_exhaustive]`; 新しい rejection reason は visible index を mutate してはならない。 |
...
task 14 の実装はこれを in-memory の `ImportScopedView` として materialize する。
request は fail closed である。欠落した importer identity、unsupported
`cluster_db_schema_version`、欠落した producer schema version、不明または unsupported
な policy/schema/toolchain/traversal compatibility、欠落した visible origin、visible
origin の verifier-policy または producer compatibility metadata と request の不一致は
すべて `ClusterDbViewMiss` になる。
...
| `CLUSTERDB-G003` | `deferred` | task 14 は in-memory import-scoped view materialization と invalidation test を実装する。durable な `views/` file materialization は persistent cluster-db storage task が scheduled されるまで deferred のままにする。 |
...
- graph、subsumption-DAG、attribute、type、reduction index 全体で visible origin へ
  filter されること
- missing visible origin、unsupported cluster-db schema、不明な policy/schema/
  toolchain/traversal compatibility、mismatched verifier policy、missing producer
  schema metadata、mismatched producer compatibility metadata が fail-closed miss を
  強制すること
...
task 12-14 は scheduler integration、`mizar-ir` adapter integration、artifact
publication-token integration、proof status projection、proof winner selection、kernel
checking、`ResolutionTrace` construction を実装しない。task 14 は durable な `views/`
file も materialize しない。persistent storage は後続の cluster-db storage task に残る。
