...
Post-closeout task 21 status: complete。Task 21 は proof policy、selection、
status reuse metadata を、訂正済み kernel rejection taxonomy、保持された rejected
evidence check kind、accepted proof-obligation goal polarity に整合させる。元の
milestone の quality score は再評価せず、Step 3 の cache/artifact/ATP consumer が
必要とする focused follow-up を記録する。
...
- proof witness staging、manifest-gated publication reference、trusted selection
  に対する accepted proof-obligation goal polarity を含む、validation predicate としてだけ
  export される stable proof-reuse metadata を所有する。
...
| Proof-reuse metadata | Status metadata は policy fingerprint compatibility、obligation/context/dependency fingerprint、selected witness または deterministic discharge hash、trusted selection に対する accepted proof-obligation goal polarity、evidence identity、dependency artifact/schema compatibility、selected provenance、stable selection reason、validation hash を export する。この metadata は cache validation predicate であり、proof authority ではない。 |

## Post-Closeout Task 21 Alignment

task 21 は kernel soundness audit に対する focused Step 3 follow-up である。
現在の task-21 change は次を行う:

- `AcceptedGoalPolarity::AssertFalseForRefutation` を accepted、untainted、
  proof-obligation kernel input に対してだけ公開する;
- rejected kernel-evidence result で明示的な `KernelEvidenceCheckKind` を保持し、
  proof policy が proof-obligation goal-polarity failure と consistency-check
  diagnostic を区別できるようにする;
- その polarity を trusted selection metadata、status reuse metadata、
  proof-evidence identity、proof-reuse validation hash へ伝播する;
- source-backed corrected terminal kernel rejection が policy-open status に
  fallback しないようにする;
- goal-polarity `ContextMismatch` handling を structured `final_goal.polarity`
  rejection location に絞り、target-binding `ContextMismatch` は ordinary rejected
  diagnostic のままにする;
- trusted accepted、policy-permitted external、policy-assumed winner が rejected
  diagnostic より上位である規則を保つ;
- legacy `unsupported_certificate_format` gate の proof-side real-payload coverage は、
  public kernel legacy-certificate fixture または source runner が存在するまで
  `external_dependency_gap` と分類する。

この follow-up は cache lookup、artifact witness publication、kernel acceptance、
SAT solving、backend proof trust、proof search、fake proof payload、placeholder runner、
expectation rebaseline を追加しない。

task 21 review は focused fixes 後に findings なしで完了した。spec/docs review、
test-sufficiency review、full-implementation review、source/documentation consistency
review はすべて clean に完了した。最終 task-21 verification は `cargo fmt --check`、
`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test`、focused
proof/kernel tests、次に順序付けられた Step 3 consumer boundary check としての
`cargo test -p mizar-cache` で passed。
...
| Post-closeout task 21 reviews | task 21 の初期 review は fabricated legacy coverage、広すぎる `ContextMismatch` selection、non-open boundary coverage 欠落、check-kind preservation risk を指摘した。focused fixes で解消し、final spec/docs、test-sufficiency、full-implementation、source/doc consistency reviews は findings なし。 |
...
| PROOF-CLOSEOUT-G010 | `external_dependency_gap` | task 21 は legacy certificate payload を fabricate せずに legacy `unsupported_certificate_format` の proof-side real-payload coverage を追加できない。 | selection は real kernel record に対して defensive に保ち、proof-side real-payload coverage は public kernel fixture または source-derived runner を待つ。 |
...
| `cargo test -p mizar-cache` | post-closeout task-21 downstream boundary check として passed |
...
- original closeout では `mizar-cache` がまだ workspace crate ではなかったため、
  `cargo test -p mizar-cache` は実行していない。post-closeout task 21 の downstream
  boundary check では実行し、passed。将来の proof/cache integration task でも専用
  cache check を実行する。
