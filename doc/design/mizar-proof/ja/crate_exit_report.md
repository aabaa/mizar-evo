...
|---|---|---|---|
| PROOF-CLOSEOUT-G001 | `external_dependency_gap` | `mizar-cache` は workspace crate ではないため、proof-reuse validation には cache lookup / consumer contract がない。 | `mizar-cache` を独自 spec で構築する。`mizar-proof` metadata は validation input としてだけ消費する。 |
...
- `mizar-cache` は workspace crate ではないため、`cargo test -p mizar-cache` は
  実行していない。
...
without moving policy or winner selection into ATP. Do not add placeholder cache
behavior、artifact witness publication、kernel acceptance、SAT solving、backend
proof trust、proof search。Run the AGENTS.md review phases and full workspace
