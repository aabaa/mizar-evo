...
範囲外の `repo_metadata_conflict` が 1 件 `mizar-atp` にある。`mizar-atp` の
task-28 closeout guard は、現在の `mizar-proof` workflow が formal scaffold を
明示的に許可し完了しているにもかかわらず、まだ `crates/mizar-proof` を forbidden
placeholder crate と見なす。この audit はその conflict を記録するだけで、
`mizar-atp` は修復しない。
...
|---|---|---|---|
| `PROOF18-G001` | `external_dependency_gap` | `mizar-cache` はまだ design-only である。`doc/design/mizar-cache/en/todo.md` は存在するが、cache crate や proof-reuse validator は存在しない。 | `mizar-proof` は validation metadata だけを export する。将来の `mizar-cache` が lookup、cache hit/miss decision、policy compatibility check を所有する。 |
| `PROOF18-G002` | `external_dependency_gap` | `doc/design/internal/en/04.atp_portfolio_and_kernel_check_integration.md` は将来の `doc/design/mizar-cache/proof_reuse.md` を参照するが、詳細な cache proof-reuse document はまだ存在しない。 | metadata API を stable に保つ。`mizar-proof` では placeholder cache doc/API を作らない。 |
| `PROOF18-G003` | `external_dependency_gap` | 現在の artifact witness schema はまだ `DischargedBuiltin` witness ref を publish できない。 | accepted kernel evidence 後の internal trusted class として `DischargedBuiltin` を保ち、deterministic discharge hash を export し、artifact witness publication は defer する。 |
...
|---|---|---|---|
| `PROOF18-RM001` | `repo_metadata_conflict` | `cargo test -p mizar-atp` は `atp_task_twenty_eight_crate_exit_report_is_documented` で失敗する。`mizar-atp` closeout guard が workspace member `crates/mizar-proof` と `crates/mizar-proof` directory を task-28 placeholder としてまだ reject するためである。 | report のみ。本 `mizar-proof` audit では `mizar-atp` を編集しない。 |
...
kernel evidence だけに結びつけている。残作業は downstream integration または記録済み
branch-coverage follow-up であり、cache、external、diagnostic、witness metadata を
trusted status へ promote する許可ではない。
