...
訂正済み terminal kernel rejection には狭い ordering override がある。
otherwise-selectable result の最良が `PolicyOpen` である場合、
`invalid_sat_refutation`、proof-obligation goal-polarity の
`context_mismatch`、`missing_provenance`、または real legacy
unsupported-certificate gate record を持つ `KernelRejected` candidate を primary
failed proof diagnostic として代わりに選ぶ。`context_mismatch` branch は structured
`final_goal.polarity` rejection location と、保持された明示 `ProofObligation` kernel
evidence check kind に限られる。consistency-check polarity mismatch と target-binding
context mismatch は ordinary rejected diagnostic のままであり、policy-open output を
置き換えない。この override は、active policy の下で eligible な `KernelVerified`、
`DischargedBuiltin`、`PolicyPermittedExternal`、`PolicyAssumed` candidate を
上回らない。
...
| `accepted_goal_polarity` | trusted kernel selection に対する kernel-validated accepted proof-obligation goal polarity。現在は `assert_false_for_refutation` であり、non-trusted、rejected、open、policy-tainted、consistency-check outcome では absent。 |
...
accepted goal polarity、witness/discharge/evidence hash のいずれかが変われば、
downstream proof-reuse validation identity も変わらなければならない。
...
proof-obligation kernel evidence hash に bind され、accepted proof-obligation goal
polarity を持ち、selected candidate の evidence hash と一致しなければならない。
Accepted consistency check は non-selectable diagnostic であり、trusted marker を作らない。
