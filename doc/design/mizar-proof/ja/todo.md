...
21. **訂正済み kernel 拒否分類への policy 整合(kernel F1、F2)。** [x]
    - proof-policy 評価と status projection を監査後の kernel 契約に整合
      ...
      policy-open へ再分類したりしてはならない。selection/status reuse metadata は
      accepted proof-obligation evidence の goal polarity を記録し、reuse consumer が
      検証できるようにする。evidence class の upgrade は行わない
      (IV-005 を維持)。
    - 受け入れ条件: public kernel evidence API から利用可能な source-backed
      corrected rejection reason（`invalid_sat_refutation`、goal-polarity
      `context_mismatch`、`missing_provenance`）が policy 評価を通って failed
      status に至る Rust regression を持つ。legacy unsupported-certificate gate は
      real kernel record が供給されたとき terminal のままでなければならないが、
      proof-side real-payload regression coverage は public kernel legacy-certificate
      fixture または source runner が利用可能になるまで `external_dependency_gap` である。
      polarity を運ぶ selection/status reuse metadata fixture が task-17 の reuse
      export を round-trip する。
    ...
    - 状態: complete。`KernelPolicyInput` は accepted proof-obligation kernel result
      に対してだけ accepted goal polarity を公開する。policy decision は rejected
      kernel-evidence check kind を保持し、selection は trusted winner に accepted
      polarity を記録する。source-backed corrected terminal kernel rejection は
      otherwise-open fallback を置き換えられるが、trusted accepted、
      policy-permitted external、policy-assumed evidence を上回らない。
      target-binding `context_mismatch` と consistency-check goal-polarity
      mismatch は ordinary rejected diagnostic のままで、policy-open fallback を
      置き換えない。Legacy unsupported-certificate real-payload coverage は
      `external_dependency_gap` として分類する。focused fixes 後の review は
