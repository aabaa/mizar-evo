...
Rationale: task 7 は portfolio / built-in-discharge merge boundary をまたぎ、後続の
status projection のため trusted/non-trusted status distinction を保たなければならない。
`xhigh` を維持する。typo-only cleanup だけなら lower reasoning でよい。downstream の
artifact または VC API が新しい external dependency gap を露出する場合だけ上げる。
