...
|---|---|---|---|
| `PROOF18-G001` | `external_dependency_gap` | `mizar-cache` is still design-only; `doc/design/mizar-cache/en/todo.md` exists, but no cache crate or proof-reuse validator exists. | `mizar-proof` exports validation metadata only. Future `mizar-cache` owns lookup, cache-hit/miss decisions, and policy compatibility checks. |
| `PROOF18-G002` | `external_dependency_gap` | `doc/design/internal/en/04.atp_portfolio_and_kernel_check_integration.md` references future `doc/design/mizar-cache/proof_reuse.md`, but the detailed cache proof-reuse document does not exist yet. | Keep the metadata API stable; do not create placeholder cache docs or APIs in `mizar-proof`. |
