...
状態: 2026-06-13 時点で checked-in fixture manifest として完了。
...
いない。そのため Task 8 では、default-discovered な `.miz` corpus file ではなく、
この checked-in manifest を着地させる。

parse-only corpus runner と所有する grammar production が利用可能になった時点で、
下の各行は、case ID、source shape、parse-only expectation、Appendix A traceability
を変更せずに有効化できる。
...
- 有効化とは、source を `.miz` file として `tests/miz/{pass,fail}/parser/` に置き、
  ...
  `tests/coverage/spec_trace.toml` から link することを指す。
