...
| G-AUD-003 | token list drift があった。`step` は range loop 文法で使うが未予約、第 2 章は import の `..` を欠き、単独 `@` は扱いを明確化する必要があった。 | Appendix A、第 2 章、lexer 実装トラックで修正済みであり、dot-compound priority list にも反映した。`@` は annotation marker であり、単独の予約記号 token ではない。`step` と `..` に対する `mizar-lexer` / `mizar-session` table、lexical fixture、traceability entry は同期済みであり、該当する lexical table と dot-disambiguation requirement は `tests/coverage/spec_trace.toml` で covered になった。 |
...
| G-AUD-008 | dot role は selector access/update、namespace separation、active user symbol にまたがって同じ surface token を共有しうる。 | parser/syntax surface shape は parser task 10 で解決済み。qualified-name head は qualified surface として保持し、すでに parse 済みの term 後の `.` は selector/update postfix syntax になる。scope-dependent な selector-versus-namespace classification は resolver-owned のまま残す。dot-chain fixture は import、reference、qualified symbol、selector access、lvalue、active `.` user-symbol を owning parser task の着地に合わせて含める。 |
...
| G-AUD-010 | `compact_statement` と zero-step `iterative_equality` は、`x = y by A;` のような正当化付き等式で重なる。justified formula としても、`.=` continuation を持たない iterative equality としても読めるためである。 | parser task 19 / mizar-syntax S-013 で修正済み。parser は最初の equality の後に top-level `.=` continuation が続かない限り `CompactStatement` に dispatch し、続く場合だけ `IterativeEqualityStatement` を構築する。Task 19 fixture は `x = y by A;`、`x = y by A .= z by B;`、label / `then` variant を覆う。 |
| G-AUD-011 | Chapter 15 の例と prose は `per cases;` を含む一方、complete EBNF summary は `per cases` 後に bracket なしの `simple_justification` を表示する。既存 active parse-only coverage も、task 19 で deferred fragment として `then per cases;` を持っていた。 | parser task 20 の surface としては解決済み。parser は `per cases;` と `per cases by A;` の両方を受け入れ、明示 `by` がある場合だけ `JustificationClause` を記録し、両方を task-20 fixture で覆う。より厳密な proof / exhaustiveness requirement は後続 proof semantics または human-owned spec cleanup に残す。 |
...
- compact statement と iterative equality の境界 fixture は parser task 19 /
  mizar-syntax S-013 が所有し、`x = y by A;` の zero-`.=` overlap、
  `x = y by A .= z by B;` の continuation form、label / `then` variant を覆う。
...
- lexer token drift を defer したが、具体的な owner が弱かった。`mizar-lexer`
  TODO が `step` / `..` table と fixture 同期を所有して完了し、該当する
  lexical table coverage entry は covered になった。
...
- `compact_statement` / zero-step `iterative_equality` の overlap は G-AUD-010 の
  task-19 dispatch rule と fixture input で解決する。
- 第 2 章の dot-compound priority list に `..` を追加した。lexer fixture と
  dot-disambiguation coverage は lexer 実装トラックで追いついた。
...
## Parser Task 48 の follow-through

Parser Task 48 は `SPEC-07-PI-PLACEMENT` で特定された parser / syntax の placement
gap を閉じる。property implementation は top-level `PropertyImplementation` node
として表現し、その特殊化された parameter は source-shaped な
`DefinitionParameter -> TypeHead -> QualifiedSymbol + optional TypeArguments`
という経路を保つ。active な pass / fail corpus は、正しい property
implementation と不正形に対する bounded recovery を覆う。

これは syntax-only の credit である。parser と syntax tree は placement、ownership、
source shape を保持するが、参照された type、property、proof、correctness condition
が意味的に妥当かは判断しない。その作業は semantic Task 39 に deferred のまま残す。
この post-exit follow-through は新しい `mizar-syntax` task ID を作らず、historical
grammar gate を再開しない。
