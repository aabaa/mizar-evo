...
状態: task 4 の trivia model は実装済み。syntax-level の item attachment fixture は、
最初の task-S-009 item-node 増分で着地した。
...
S-018 audit では、`SurfaceTrivia` を rowan identity surface ではなく、range-attached な
side table として扱う。後続 query layer は、token stream が変わらない場合に
`SurfaceAst` の green tree を再利用し、frontend 所有の preprocessing output から
trivia を再構築または再 attach してよい。ただし `SurfaceAst::with_trivia` が、
すべての node / token target が記録された range で存在することを検証できる場合に
限る。そのため trivia target key、snapshot rendering、sorting rule は raw rowan
identity、file path、`SourceId` debug output を避ける。局所編集で token stream が
変わる場合、trivia owner は `SurfaceNodeId` だけで持ち越さず、新しい syntax tree に
対して再計算しなければならない。
...
最初の item-node fixture は、leading doc comment を task 5 の `PlaceholderItem` に
attach する。frontend が生成する attachment hint との統合は後続 step に残す。
