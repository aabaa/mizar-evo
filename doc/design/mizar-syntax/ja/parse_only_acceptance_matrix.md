...
| Declarations | `PO-DECL-N03` | negative | `definition redefine pred L: x <> y means thesis; coherence by A; end;` | `reject`: built-in の `<>` は `redefine_pred` の predicate symbol として使えない。 | `redefine_pred`, `pred_pattern`, `def_predicate_symbol` | `mizar-parser` | A.2, A.9, A.12 |
...
| Algorithms | `PO-ALG-P01` | positive | `definition algorithm f(x) -> set do var y := x; return y; end; end;` | `accept`: verification clause を伴わない parser task 32 の basic algorithm body shell。 | `definition_block`, `algorithm_def`, `algorithm_body`, `var_decl`, `return_stmt` | `mizar-test` | A.12, A.20 |
| Algorithms | `PO-ALG-P02` | positive | `definition algorithm f() do if thesis do break; else continue; end; while thesis do break; end; for i = a to b step s do continue; end; for j = b downto a do break; end; for x in S processed V do continue; end; for y in S do break; end; match t do case p do break; end; case q do continue; end; otherwise break; end; end; match u do case r do continue; end; exhaustive; end; end; end;` | `accept`: parser task 33 は control-flow statement を覆う。concrete verification clause は `PO-ALG-P03` で覆う。 | `if_stmt`, `while_stmt`, `for_range_stmt`, `for_collection_stmt`, `match_stmt`, `match_case`, `match_ending`, `break_stmt`, `continue_stmt` | `mizar-test` | A.20 |
| Algorithms | `PO-ALG-P03` | positive | `definition terminating algorithm f(n) requires thesis ensures thesis decreasing n do while thesis do invariant thesis; decreasing n; assert thesis; break; end; for i = a to b do invariant thesis; continue; end; for x in S do invariant thesis; break; end; end; end;` | `accept`: parser task 34 は terminating algorithm、ordered header verification clause、leading loop clause、`assert` を覆う。 | `algorithm_def`, `algorithm_termination_clause`, `algorithm_requires_clause`, `algorithm_ensures_clause`, `algorithm_decreasing_clause`, `loop_invariant_clause`, `loop_decreasing_clause`, `assert_stmt`, `term_list` | `mizar-test` | A.20 |
...
| Algorithms | `PO-ALG-R03` | recovery-required | `definition algorithm f() ensures thesis requires thesis decreasing do for i = a to b do decreasing i; invariant; continue; end; assert; end; end;` | `recover`: task 34 verification recovery は、順序違反の header clause、空の `TermList`、禁止される `for decreasing`、loop invariant formula 欠落、malformed `assert` formula を診断し、周囲の algorithm tree を保持する。 | `algorithm_def`, `algorithm_requires_clause`, `algorithm_decreasing_clause`, `for_range_stmt`, `loop_invariant_clause`, `assert_stmt`, `term_list`, `missing_formula`, `missing_term` | `mizar-parser` | A.20 |
...
| A.20 Algorithms and Computation | Algorithm definition、control statement、verification clause、claim placement |
