...
parser producer coverage は段階的だが、実装済み grammar task は現在、lexer error
token の recovered token node と、block closer、separator、literal、type / term /
formula operand、proof step、statement、skipped token、unmatched delimiter、task 33
algorithm-control recovery、および task 34 algorithm-verification recovery の明示的な
recovered node を生成する。下の表が現在の
producer inventory である。producer が載っていない recovery kind は、将来の parser
grammar task が producer を追加するときに syntax snapshot 語彙を変更しなくてよいよう、
`mizar-syntax` で構築可能にしておく。
...
| `MalformedAnnotation` | parser task 35 が valid annotation として parse できない annotation marker、argument list、proof-hint option、または body を認識する | source text を囲む marker | 不正な annotation marker / body span を覆う range。利用できる場合、任意の annotation owner child を付けてよい | `SyntaxDiagnosticCode::MalformedAnnotation`。不正 source が annotation node で表現されず skip される場合は `SurfaceTrivia::skipped_token_ranges` に `SkippedTokenReason::MalformedAnnotation` で記録する | `MalformedAnnotation` |
...
variant を約束している。S-017 の最終 public-enum audit では、これらを
下流 crate 向けに `#[non_exhaustive]` のままとし、lint-policy gate が、後続 task が
...
含めなければならない。今後この module に public enum を追加する場合は、着地前に
lint-policy classification のどちらか一方へ追加しなければならない。
