...
| `MissingEnd` | parser task 20 が `now`、`hereby`、`case`、`suppose` block に欠落 `end` を挿入する。parser task 22 は `ProofBlock` に挿入する。parser task 23 は `DefinitionBlockItem` に挿入する。recovery prepass も legacy / deferred block placeholder に対して挿入してよい | 挿入 placeholder の `SurfaceNodeKind::ErrorRecovery(MissingEnd)` | insertion point の zero-width range。block opener / context child を insertion range の外に保持してよい | `SyntaxDiagnosticCode::MissingEnd`。同じ recovery が source text も skip する場合を除き skipped range はない | `MissingEnd` |
