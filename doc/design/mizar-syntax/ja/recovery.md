...
| `MissingTerm` | parser task 9 が term list、parenthesized term、application argument、set enumeration、constructor field value 内で primary term を期待する。parser task 10 が structure-update field value を期待する | 挿入 placeholder | insertion point の zero-width range。任意の call / delimiter / context child を持て、range 外でもよい | `SyntaxDiagnosticCode::MalformedTermExpression`。純粋な挿入では skipped range はない | `MissingTerm` |
