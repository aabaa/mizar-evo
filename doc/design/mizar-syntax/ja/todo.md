...
12. **論理式のノード。** [x] — `mizar-parser` task 13〜14 と対。
   ...
   - 結果: parser task 13〜14 は実装済みである。atomic formula node、
     ...
     missing-formula recovery、theorem/lemma placeholder formula host、syntax
     typed accessor、parser unit test、active parse-only pass/fail corpus
     coverage を追加した。template predicate argument は task 31 / S-016 に
     deferred のままであり、formula を埋め込む Fraenkel / set-builder term
     は parser task 15 / S-011 で実装済みである。
...
13. **文のノード。** [ ] — `mizar-parser` task 16 と 18〜21 と対。
    ...
    - 進捗: parser tasks 16、18、19 は実装済みである。
      ...
      active parse-only pass/fail corpus coverage を覆う。S-013 を完了状態にするには
      parser tasks 20〜21 がまだ残る。
...
14. **定理・証明・正当化のノード。** [ ] — `mizar-parser` task 17 と 22 と対。
    - 正当化句（`by`）、`.{ … }` と `.*` を含む引用形、`let ... by references`、
      ...
      parser task 17 の citation/computation subset は完了済みであり、この bucket を
      done にするには parser task 22 がまだ残る。
    - 仕様: [15.statements.md](../../../spec/ja/15.statements.md)、
