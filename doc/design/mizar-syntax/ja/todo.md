...
11. **項のノード。** [ ] — `mizar-parser` task 9〜12、15 と対。
   ...
   - 進捗: parser task 9 により primary-term 増分が着地した:
     ...
     追加した。S-011 は parser task 15 のため open のままである。
...
12. **論理式のノード。** [ ] — `mizar-parser` task 13〜14 と対。
   ...
   - 進捗: parser task 13 の atomic formula node は、theorem/lemma
     placeholder formula host、term/type recovery、active parse-only pass/fail
     corpus coverage とともに実装済みである。S-012 は parser task 14 の
     connective、quantifier、parenthesized formula、`thesis`、`contradiction`
     のため open のままである。
