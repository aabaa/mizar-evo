...
9. **モジュール、item、shared path のノード。** [~] — `mizar-parser`
   task 4〜7 と対。
   ...
   - 進捗: parser task 4 により shared path-node 増分が着地した:
     ...
     `RelativePrefix`、および parser helper unit coverage。bucket は parser tasks
     5〜7 の module skeleton、import/export item、visibility、corpus case、
     traceability entry、doc-comment item attachment fixture が残っているため open。
...
11. **項のノード。** [ ] — `mizar-parser` task 9〜12、15 と対。
   - task 9 で導入した shared path vocabulary を消費する。続いて一次項（parser
     task 9）、未解決ドット連鎖と selector access / update（parser task 10、
     ドットの役割の surface 形状の決定を含む）、functional structure update、
