...
13. **文のノード。** [x] — `mizar-parser` task 16 と 18〜21 と対。
    ...
    - 結果: parser tasks 16、18、19、20、21 は実装済みである。
      ...
      shared-type `consider` variable、condition list、task-18 の
      simple-justification recovery、reconsider item list、target type、
      ...
      checker task 44 後は justification 省略 `reconsider` が semantic gate 付きの
      canonical syntax であり、まだ mandatory justification 欠落として扱う
      parser/syntax recovery fixture と、proof-block `reconsider` をまだ非対応
      と扱う parser slice の更新は parser task 47 が所有する。
...
18. **増分構文再利用の監査。** [x]
    ...
    - 結果: S-018 は `ast.md`、`trivia.md`、`recovery.md` に reuse contract を記録した。
      raw `SyntaxKind` 値はこの phase では append-only とする。`SurfaceNodeId` は
      決定的だが永続 identity ではない。trivia と recovery は range-attached surface
      として扱う。後続 query layer は frontend 所有の content key の背後で `SurfaceAst`
      を cache してよいが、incremental state を `mizar-syntax` に移さない。この監査で
      annotation `SurfaceNodeView` helper と rowan/raw-kind coverage の gap を見つけ、
      修正した。
...
19. **ソース／仕様の対応監査。** [x]
    - `mizar-frontend` task 16 の監査に倣う: [ast.md](./ast.md)、
      ...
      アップタスクとして記録する。
    - 結果: [source_spec_correspondence.md](./source_spec_correspondence.md) が
      parser task 35 までの public API、enum / diagnostic、task requirement、
      source、test の対応を記録する。監査では、新しい未実装の promised behavior、
      undocumented public behavior、残る欠落 targeted test、expectation drift、
      boundary violation、metadata conflict は見つからなかった。既存 follow-up は
      監査ノートで分類済みのまま残る。
...
20. **二言語ドキュメント同期監査。** [x]
    ...
    - 結果: [bilingual_documentation_synchronization.md](./bilingual_documentation_synchronization.md)
      が、README、crate plan、module spec、grammar-gate note、source/spec audit、
      TODO companion が S-020 までの status、public API 名、enum / diagnostic
      surface、task follow-up record、terminology、link、behavior promise について
      一致することを記録する。監査で見つかったのは documentation `design_drift`
      のみであり、source 変更なしに閉じた。
...
22. **predicate redefinition label の AST follow-through。** [x]
    - `mizar-parser` task 36 と同じ実装変更で pair する:
      `PredicateRedefinition` の builder / accessor / snapshot coverage を
      更新し、node が `PredicatePattern` の前に必須 label child を所有し、
      label 欠落時は missing-label recovery を使い、[ast.md](./ast.md) に
      記録した labeled target shape を render するようにする。この task は
      parser repair の syntax 側であり、parser surface 変更後の後追いでは
      ない。
    - テスト: typed accessor が label と pattern を source order で公開する。
      snapshot rendering が存在する label と missing-label recovery を区別する。
      downstream view は以前の label なし parser surface を安定 AST contract と
      して扱わない。
    - 結果: `mizar-parser` task 36 と pair 済み。`PredicateRedefinition` は
      `PredicatePattern` の前に label slot を所有し、欠落 label はその slot の
      `MissingTerm` recovery として表現する。syntax builder / unit coverage は
      label ありと missing label を確認し、active parser corpus は修正済みの
      labeled surface を使う。
    - 依存: `mizar-parser` task 36。仕様: [ast.md](./ast.md),
      [../../mizar-parser/ja/grammar.md](../../mizar-parser/ja/grammar.md)。

23. **predicate-label フォローアップ監査。** [x]
    - task 22 の syntax repair について、ソース/仕様対応監査と二言語
      ドキュメント同期監査を再実行する。残る AST、accessor、snapshot、
      parser/syntax contract gap をフォローアップタスクとして記録する。
      監査出力は [source_spec_correspondence.md](./source_spec_correspondence.md) と
      [bilingual_documentation_synchronization.md](./bilingual_documentation_synchronization.md)
      への S-023 更新である。paired parser repair に言及する status surface も
      確認する。parser README companion と top-level design roadmap docs を含む。
      historical crate-exit report も対象に含む。
      language spec 変更、実装変更、既存 `.miz` source 変更、expectation rebaseline
      は、監査で blocking gap を分類し独立 owner を記録する場合を除き対象外である。
    - 結果: S-023 は、task 22 / parser task 36 の修正が実装・テスト・文書化済みで
      あり、AST、accessor、snapshot、parser/syntax contract、source/spec、
      bilingual documentation、expectation、metadata gap が残っていないことを
      確認した。古い parser / top-level / syntax status text の documentation
      `design_drift` は、language behavior を変更せずに閉じた。
    - 依存: 22。仕様: [ast.md](./ast.md)、本 TODO、リポジトリの
      ドキュメント方針。

24. **AST module 境界のリファクタリング。** [x]
    - oversized な `crates/mizar-syntax/src/ast.rs` 実装を、公開
      `mizar_syntax` API、rowan-backed storage semantics、syntax-kind numbering、
      typed accessor の挙動、snapshot text、trivia attachment validation、
      parser-facing builder contract を変えずに private な責務別 module へ分割する。
      候補境界は syntax-kind definition、`SurfaceAst` storage/view、
      `SurfaceAstBuilder`、typed accessor、互換 node/token wrapper、snapshot
      rendering、green-tree construction、AST test。最終的な分割は現在のコード
      形状から選び、完了時に本 TODO へ記録する。
    - [ast.md](./ast.md) を正準 API 契約として保ち、crate-root re-export を
      安定させる。raw rowan traversal を公開せず、semantic field を導入せず、
      `SyntaxKind` を renumber せず、挙動 cleanup を移動と混ぜない。公開 API の
      変更は独立した spec task を要求する。
    - テスト: syntax unit test、parser test、frontend seam test、snapshot
      baseline を byte-stable に保つ。`cargo fmt --check` と syntax/parser Clippy
      を green に保つ。
    - 結果: `crates/mizar-syntax/src/ast.rs` は公開 `ast` module のままとし、
      `SyntaxKind`、公開 AST 型、builder、accessor、互換 wrapper は既存の
      `mizar_syntax::ast` と crate-root re-export path に残した。private な
      実装分割として、rowan green-tree construction を `src/ast/green.rs`、
      安定 snapshot rendering を `src/ast/snapshot.rs`、AST unit / snapshot
      coverage を `src/ast/tests.rs` へ移した。この分割では node kind を追加せず、
      `SyntaxKind` を renumber せず、snapshot text、trivia validation、公開 raw
      rowan traversal API を変更していない。
    - 依存: 22、23。仕様: [ast.md](./ast.md)、[trivia.md](./trivia.md)、
      [recovery.md](./recovery.md)、本 TODO。

25. **AST refactor フォローアップ監査。** [x]
    - task 24 の module split について、ソース/仕様対応監査と二言語
      ドキュメント同期監査を再実行する。API 一覧、source/test 対応、
      re-export path、snapshot stability、parser/syntax boundary promise の
      drift をフォローアップタスクとして記録する。
    - 結果: S-025 は、task 24 の private module split が実装・テスト・文書化済み
      であり、API list、source/test、re-export path、snapshot stability、
      parser/syntax boundary、source/spec、bilingual documentation、expectation、
      metadata、repository-status drift が残っていないことを確認した。audit /
      status surface は tasks 24 と 25 が完了済みで、S-021 が唯一の deferred
      `mizar-syntax` task であることに同期された。
    - 依存: 24。仕様: [ast.md](./ast.md)、
      [source_spec_correspondence.md](./source_spec_correspondence.md)、本 TODO、
      リポジトリのドキュメント方針。
