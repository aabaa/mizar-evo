...
task-30 `ReductionRegistration` まで、compatibility `Token` wrapper、および
...
を含む theorem payload は `PlaceholderItem` target のままである。duplicate visibility
...
および enclosing semicolon を所有させる。deferred な template predicate argument を含む
theorem payload は、その syntax が実装されるまで `PlaceholderItem` として残してよい。
