...
S-018 incremental reuse contract は実装済み。
...
実装分割は `crates/mizar-syntax/src/ast/` 配下に置く。`green.rs` は rowan
green-tree construction、`snapshot.rs` は安定 snapshot rendering、`tests.rs` は
AST unit / snapshot coverage を所有する。これらのファイルは source organization
の詳細に限られ、crate-root re-export、`mizar_syntax::ast` path、rowan storage
semantics、typed accessor、snapshot text、parser-facing builder contract を変更しない。
...
| `SurfaceNodeKind::AnnotationLabel` | なし | `SyntaxKind::AnnotationLabel` | parser task 35 の単一 library annotation label。identifier token と任意の `AnnotationArgumentList` を所有する |
...
Parser task 35 は現在の annotation 語彙を完成させる。`Annotation` は、固定の
annotation marker と任意の argument / option list、または `@[ ... ]` 用の nested
`LibraryAnnotation` を所有する。`LibraryAnnotation` は `AnnotationLabelList` を
所有し、その `AnnotationLabel` child はさらに `AnnotationArgumentList` を所有して
よい。`AnnotationArgument` は identifier、numeral、string-literal payload を
構文としてだけ保持する。`ProofHintOptionList` と `ProofHintOption` は、固定の
proof-hint option 名と value token を、ATP availability を検証せずに保持する。
`StandaloneDiagnosticAnnotation` は `@show_type` / `@eval` directive syntax を保持し、
後続 statement には attach しない。`AnnotatedStatement`、
`AnnotatedAlgorithmStatement`、`AnnotatedDefinitionContent`、
`AnnotatedRegistrationContent` は、1 個以上の leading `Annotation` node と所有する
concrete syntax node を包む。これらの node は annotation を評価せず、annotation
registry を検証せず、proof search に影響しない。`SurfaceNodeView` は
`as_annotation`、`as_library_annotation`、`as_annotation_label_list`、
`as_annotation_label`、`as_annotation_argument_list`、`as_annotation_argument`、
`as_proof_hint_option_list`、`as_proof_hint_option`、
`as_standalone_diagnostic_annotation`、`as_annotated_statement`、
`as_annotated_algorithm_statement`、`as_annotated_definition_content`、
`as_annotated_registration_content` を公開する。snapshot rendering は literal node
name を出力する。
...
S-018 audit は、後続の parser-query / LSP 作業に向けた現在の reuse boundary を
記録する。`mizar-syntax` は immutable な rowan-backed syntax output を提供する。
incremental query engine、`salsa` database、build-system state、cross-file cache
invalidation policy は所有しない。`mizar-frontend::SurfaceAstCacheKey` はすでに
token-stream content、parser version、parser input、edition によって parser output を
key 付けしている。後続 query layer は、その key の背後で `SurfaceAst` を cache してよいが、
parser arena や dense compatibility id を公開してはならない。

`SyntaxKind` の raw discriminant は rowan storage 語彙である。この phase では既存の
raw value は append-only とし、将来 reserved / reused range へ値を挿入する場合は、
同じ変更でこの表、rowan round-trip test、snapshot expectation を更新しなければならない。
未知の raw rowan value は引き続き `SyntaxKind::Unknown` に map され、認識できない
storage を読む infrastructure が安全に degrade できるようにする。

局所編集は `SurfaceNodeId` equality ではなく、content key、source range、安定した
snapshot text、rowan green-node equality を通じて観測する。局所編集では、影響を受けない
source-shaped region に等しい green subtree が生成されることがあるが、`SurfaceNodeId` は
compatibility vector とともに再構築され、前方の sibling が変わるとずれ得る。永続的な
semantic identity が必要な consumer は resolver/checker id を待つ。syntax consumer は
range を key にし、snapshot または green-node equality で検証してよい。

Trivia と recovery は reusable syntax boundary の一部だが、range-attached のままである。
token stream が変わらない comment-only edit では `SurfaceTrivia` を再構築または再 attach
でき、`SurfaceAst` は node/token trivia target が一致する range で存在することを検証する。
Recovery node は visible syntax node または recovered token leaf のままであり、skip された
source span は trivia に置く。これにより rowan token leaf は source-shaped に保たれ、LSP
client は diagnostic、formatting hint、code action を parser 内部に依存せず `SourceRange`
で対応付けられる。

今回の audit では、task 35 annotation 語彙に source/test drift が 1 件見つかった:
annotation node は `SurfaceNodeKind` / `SyntaxKind` に存在したが、typed
`SurfaceNodeView` helper と rowan/raw-kind unit coverage が欠けていた。S-018 は
`crates/mizar-syntax/src/ast.rs` でその drift を閉じる。追加の storage abstraction は
導入しない。

### 公開 enum の互換性

S-017 の最終 public-enum audit は、この module のすべての public enum を分類する。
将来の syntax vocabulary growth を約束する enum（`SyntaxKind`、`SurfaceNodeKind`、
`SurfaceTokenKind`）は、下流 crate 向けに `#[non_exhaustive]` とし、
`crates/mizar-syntax/tests/lint_policy.rs` がこれらの属性を固定する。
`MizarLanguage` は downstream の syntax category ではなく空の rowan marker enum
であるため、意図的に exhaustive のままとする。`SurfaceOperatorAssociativity` は
