...
| 27 | `TermPlaceholder` | task 11 まで残す temporary bracket `qua_arg` stub |
...
| `SurfaceNodeKind::TypeArguments` | なし | `SyntaxKind::TypeArguments` | parser task 8 の `of`、`over`、または bracket 構文の type argument wrapper。task 9 は `of`/`over` placeholder を `TermExpression` argument に置き換える |
| `SurfaceNodeKind::TermPlaceholder` | なし | `SyntaxKind::TermPlaceholder` | parser task 8 の一時的な syntax-only term-entry stub。parser task 11 が `qua` term を実装するまで bracket `qua_arg` 形だけに残す |
| `SurfaceNodeKind::TermExpression` | なし | `SyntaxKind::TermExpression` | parser task 9 の current term-expression wrapper。postfix、`qua`、operator task が拡張するまでは primary-term child を 1 個だけ所有する |
...
argument は nested `TypeExpression` として表す。`qua_arg` syntax の bracket argument は、
parser task 11 が `qua` term を実装するまで一時的な `TermPlaceholder` child のまま残す。
bracket close が欠ける場合は
`TypeArguments` node の下で
`MalformedTypeExpression` と `UnmatchedOpeningDelimiter` recovery を使う。
`TermPlaceholder` は type と `qua` template argument の相互再帰のための一時的な
syntactic stub として残り、未解決 bracket `qua_arg` 1 個の浅い source token を所有するだけで、
term classification、operator fact、name resolution を encode してはならない。
`SurfaceNodeView` は
...
`TermExpression` は現在、1 個の primary-term child を包む wrapper であり、後続 task が
postfix、`qua`、operator child を追加しても wrapper role は変わらない。
