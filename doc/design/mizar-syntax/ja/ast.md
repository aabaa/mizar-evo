...
| 44 | `FormulaExpression` | task-13 の現在の atomic formula increment 用 formula wrapper |
...
`SyntaxKind::is_node_kind` は `Root`、`Token`、`InfixExpression`、`PrefixExpression`、
`PostfixExpression`、`FormulaExpression`、`BuiltinPredicateApplication`、`IsAssertion`、
`AttributeTestChain`、`PredicateApplication`、`PredicateSegment`、`PredicateHead`、
`InlinePredicateApplication`、`ErrorRecovery`、上に列挙した task S-009 の共有 path node kind、
task 5 の module skeleton node kind / task 6 の import node kind / task 7 の export と
visibility node kind、および task S-010 の reserve / type node kind、さらに上に列挙した
task S-011 の term node kind に対して true であり、`is_token_kind` は token leaf kind
に対してのみ true である。
...
| `SurfaceNodeKind::FormulaExpression` | なし | `SyntaxKind::FormulaExpression` | parser task 13 の current formula wrapper。task 14 が connective / quantifier を追加するまで atomic-formula child を 1 つ所有する |
...
Parser task 13 は最初の formula node を追加する。`FormulaExpression` は現在、atomic formula
child を 1 つ包む wrapper であり、task 14 は wrapper role を変えずに connective、
quantifier、parenthesized、`thesis`、`contradiction` formula shape を置ける。最初に
frontend から到達できる host は theorem/lemma `PlaceholderItem` であり、`label: formula;`
payload だけを parse し、theorem/proof item 構造は task 22 に残す。
