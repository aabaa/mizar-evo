...
と上に列挙した task S-009 の共有 path node kind に対してのみ true であり、
`is_token_kind` は token leaf kind に対してのみ true である。将来の raw value は、
既存 snapshot と rowan test が raw 語彙変更時に明確に失敗するよう、末尾へ追加するか、
文書化された予約 range に割り当てるべきである。
