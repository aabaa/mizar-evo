...
| 27 | `TermPlaceholder` | task-8 type / attribute argument 内の一時 term-entry stub |
...
S-010 の reserve / type node kind に対してのみ true であり、`is_token_kind` は token
leaf kind に対してのみ true である。将来の raw value は、既存 snapshot と rowan test
が raw 語彙変更時に明確に失敗するよう、末尾へ追加するか、文書化された予約 range に
割り当てるべきである。
...
| `SurfaceNodeKind::AttributeRef` | なし | `SyntaxKind::AttributeRef` | parser task 8 の任意の `non` token、任意の `ParameterPrefix`、syntactic `QualifiedSymbol`、任意の parenthesized term-placeholder argument |
...
| `SurfaceNodeKind::TypeArguments` | なし | `SyntaxKind::TypeArguments` | parser task 8 の `of`、`over`、または bracket 構文の type argument wrapper。delimiter / keyword token と source-order の argument placeholder を所有する |
| `SurfaceNodeKind::TermPlaceholder` | なし | `SyntaxKind::TermPlaceholder` | parser task 8 の一時的な syntax-only term-entry stub。parser task 9 が concrete term node に置き換えるまで `TypeArguments` と attribute argument list 内で使う |
...
parenthesized `TermPlaceholder` argument を含む。struct-qualified attribute spelling は
...
`TypeArguments` は `of` / `over` token と comma-separated `TermPlaceholder`
arguments、または `[`、comma-separated type-template arguments、任意の `]` を所有する。
type expression として parse できる bracket argument は nested `TypeExpression` として表す。
`qua_arg` syntax を使う bracket argument は、一時的な `TermPlaceholder` child として表し、
identifier と任意の `qua` / radix-type tail token を所有させる。これは term / template
argument parsing が着地するまでの syntax-only placeholder である。bracket close が欠ける場合は
`TypeArguments` node の下で
`MalformedTypeExpression` と `UnmatchedOpeningDelimiter` recovery を使う。
`TermPlaceholder` は parenthesized attribute argument list にも再利用される。これは
type と term の相互再帰のための一時的な syntactic stub であり、1 個の term argument の浅い
source token を所有するだけで、term classification、operator fact、name resolution を
encode してはならない。`SurfaceNodeView` は
