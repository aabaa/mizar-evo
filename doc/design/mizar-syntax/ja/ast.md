...
task-32 `ClaimBlockItem` まで、compatibility `Token` wrapper、および
...
| `SurfaceNodeKind::TheoremItem` | なし | `SyntaxKind::TheoremItem` | parser task 22 の theorem declaration。任意の status token を構文的に保存し、`theorem`、label identifier または `MissingTerm`、`:`、`FormulaExpression` または `MissingFormula`、任意の `JustificationClause` または `ProofBlock`、任意の recovery、存在する場合の final semicolon を所有する |
...
| `SurfaceNodeKind::DefinitionBlockItem` | なし | `SyntaxKind::DefinitionBlockItem` | parser task 23 の `definition ... end;` block item。`definition`、source-order の concrete definition content または placeholder content、任意の `MissingEnd`、存在する場合の `end`、存在する場合の final semicolon を所有する |
...
| `SurfaceNodeKind::RegistrationBlockItem` | なし | `SyntaxKind::RegistrationBlockItem` | parser task 30 の `registration ... end;` block item。`registration`、source-order registration content、任意の recovery、`MissingEnd` または存在する場合の `end`、存在する場合の final semicolon を所有する |
...
| `SurfaceNodeKind::AlgorithmStatementList` | なし | `SyntaxKind::AlgorithmStatementList` | parser task 32 の body statement list。declaration、assignment、snapshot、return を source order で所有する。control flow、verification statement、annotation は parser tasks 33-35 に残す |
...
top-level visibility prefix を表す。child は source order で、annotation-prefix token
node があればそれら、1 個の `VisibilityMarker`、target item node である。表現済み
...
statement-level annotation は task 35 / S-016 に deferred されるため、`StatementItem` は
...
`DefinitionBlockItem` は `definition` opener、source-order content、任意の
recovery、存在する場合の block-closing `end` と semicolon を所有する。この増分で
...
`RegistrationBlockItem` は `registration` opener、source-order の registration
content、任意の recovery、存在する場合の block-closing `end` と semicolon を所有する。
