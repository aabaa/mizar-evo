...
| Design/source sync | Pass | S-019 は unimplemented promised public behavior や undocumented implementation-facing public behavior を見つけなかった。S-020 は英語・日本語 design docs を同期した。本 report は最後の crate-exit record を追加する。 |
...
|---|---|---|---|---|---|
| `tests/snapshots/mizar_syntax_surface_ast_current_vocabulary.snap` と `crates/mizar-syntax` Rust tests | rowan-backed syntax storage、deterministic snapshot、task 35 までの typed accessor、trivia attachment、recovery vocabulary、diagnostics、lint policy を guard する。 | Pass | Rust unit/lint/snapshot | 文書化済みの negative invariant test でだけ panic。 | [ast.md](./ast.md)、[trivia.md](./trivia.md)、[recovery.md](./recovery.md) |
