...
| `MalformedTypeExpression` | parser task 8 finds malformed type syntax that can continue at the current reserve or type-argument boundary | set when a reserve/type node remains represented; malformed tail tokens or missing delimiters are recovered syntactically |
| `MalformedTermExpression` | parser task 9 finds malformed primary term syntax that can continue at the current term-list or delimiter boundary | set when a term node remains represented; missing term arguments, malformed tails, or missing delimiters are recovered syntactically |
