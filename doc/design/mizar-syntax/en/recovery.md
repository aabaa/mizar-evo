...
| `MissingProofStep` | parser task 17 inserts a missing reference, grouped citation item, computation option, or computation option value while preserving a justification node; parser task 18 inserts a missing mandatory simple justification for `consider` / `reconsider`; parser task 19 inserts missing explicit citation references for conclusion and iterative-equality justifications; parser task 20 inserts missing citation references for an explicit `per cases by` tail; parser task 23 reuses general justification recovery for definition parameters and correctness conditions; future proof parsers may also use it for missing inference steps, case branches, or proof-closing steps | inserted placeholder | zero-width range at insertion point; optional proof/block/justification context child, allowed outside the insertion range | `SyntaxDiagnosticCode::MalformedJustification` for task-17 citation/computation-proof insertions, task-18 missing mandatory simple justifications, task-19 malformed explicit citation tails, task-20 malformed explicit `per cases by` tails, and task-23 malformed definition justifications; skipped source belongs to `SkippedTokenRange` when present | `MissingProofStep` |
