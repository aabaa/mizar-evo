...
| `MissingEnd` | parser task 20 inserts a missing `end` for `now`, `hereby`, `case`, or `suppose` blocks; parser task 22 inserts it for `ProofBlock`; parser task 23 inserts it for `DefinitionBlockItem`; the recovery prepass may also insert it for legacy/deferred block placeholders | `SurfaceNodeKind::ErrorRecovery(MissingEnd)` inserted placeholder | zero-width range at insertion point; may keep the block opener/context child outside the insertion range | `SyntaxDiagnosticCode::MissingEnd`; no skipped range unless the same recovery also skips source text | `MissingEnd` |
