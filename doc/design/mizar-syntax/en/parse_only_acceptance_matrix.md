...
| Declarations | `PO-DECL-N03` | negative | `definition redefine pred L: x <> y means thesis; coherence by A; end;` | `reject`: built-in `<>` cannot be used as the predicate symbol in `redefine_pred`. | `redefine_pred`, `pred_pattern`, `def_predicate_symbol` | `mizar-parser` | A.2, A.9, A.12 |
...
| Algorithms | `PO-ALG-R03` | recovery-required | `definition algorithm f() ensures thesis requires thesis decreasing do for i = a to b do decreasing i; invariant; continue; end; assert; end; end;` | `recover`: task-34 verification recovery diagnoses out-of-order header clauses, empty `TermList`, forbidden `for decreasing`, missing loop invariant formulas, and malformed `assert` formulas while preserving the surrounding algorithm tree. | `algorithm_def`, `algorithm_requires_clause`, `algorithm_decreasing_clause`, `for_range_stmt`, `loop_invariant_clause`, `assert_stmt`, `term_list`, `missing_formula`, `missing_term` | `mizar-parser` | A.20 |
...
| A.20 Algorithms and Computation | Algorithm definitions, control statements, verification clauses, claim placement |
