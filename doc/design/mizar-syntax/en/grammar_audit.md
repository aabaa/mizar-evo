...
| G-AUD-011 | Chapter 15 examples and prose include `per cases;`, while the complete EBNF summary prints an unbracketed `simple_justification` after `per cases`. Existing active parse-only coverage also had `then per cases;` as the deferred task-19 fragment. | Closed for the parser task-20 surface. The parser accepts both `per cases;` and `per cases by A;`, records an explicit `JustificationClause` only when `by` is present, and covers both forms in task-20 fixtures. Any stricter proof/exhaustiveness requirement remains later proof semantics or a human-owned spec cleanup. |
