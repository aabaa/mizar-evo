...
provenance record は source binding と payload fingerprint を持つ。backend log や trace
explanation ではない。
...
- `deferred`: task 5/6 は structured formula、declaration、soft-type payload を translator に渡す
  Rust projection input struct を定義する。opaque handoff byte からそれらの payload を復元する
  task は存在しない。
- `deferred`: task 5 が declaration と symbol-map translation を実装する。
...
task 5/6 implementation は Rust coverage を追加し、次を確認しなければならない:
