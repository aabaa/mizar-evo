...
task 22 は frontend task 25 の方針を `property_encoding` module に適用する。この module が
所有する public enum は downstream crate 向けに `#[non_exhaustive]` とする:
`AtpPropertyTargetKind`、`AtpPropertyEncodingStrategy`、`AtpPropertyEncodingError`。

Public enum inventory: `AtpPropertyTargetKind`, `AtpPropertyEncodingStrategy`, `AtpPropertyEncodingError`.

将来の property family、strategy、error variant は、source が使う前に仕様化しなければならない。
`mizar-atp` 内部では、generated formula、target declaration、provenance、backend-visible syntax、
proof status に影響する match は、paired spec が意図的 fallback を記録しない限り、明示的に保ち
