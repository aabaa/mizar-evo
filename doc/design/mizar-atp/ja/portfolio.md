...
proof-policy owner が必要である。`mizar-proof` は workspace crate ではないため、task 25 は
ここでは `external_dependency_gap` / `deferred` record と lint-policy guard として完了する。
...
Task 25 Re-evaluation: 要求された release-policy completion-order gate は、実際の
`mizar-proof` owner contract が存在するまでこの module の外に残る。この module は
adversarial mock completion order 下の no-early-stop candidate ordering をテストし続けてよいが、
early-stop oracle や local proof-policy winner selector を fabricate してはならない。
...
- `external_dependency_gap` / `deferred`: task 25 は release-policy portfolio
  completion-order gate を再評価する。`mizar-proof` は workspace crate ではなく policy
  task も利用できないため、later-kernel-verifiable-vs-earlier-externally-attested winner
  selection、policy tie-breaking、early-stop finality は、boundary violation なしに
  `mizar-atp` で実装できない。
...
downstream proof-policy owner が存在した後にだけ追加する。
