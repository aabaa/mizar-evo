...
| ATP-AUDIT-G005 | `external_dependency_gap` | Portfolio collection は early-stop proof finality、winner selection、kernel checking、proof policy を行わない。task 25 は downstream policy authority に依存する。 | `mizar-proof` / proof-policy owner と `mizar-atp` portfolio follow-up。 | Downstream proof policy が pending candidate が selected class を覆せないことを報告できる。 | portfolio completion-order independence gate と将来の early-stop integration。 |
