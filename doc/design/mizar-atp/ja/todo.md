...
deferred のままである。`mizar-proof` は現在 workspace crate だが、ATP/proof integration は
引き続き `external_dependency_gap` であり、ここで local proof-policy placeholder を追加する
理由にはならない。
...
25. **portfolio 完了順独立性 gate。** [x]
    ...
    - Status: deferred/external_dependency_gap completion。Task 25 は
      ...
      再評価した。`mizar-proof` は現在 proof-policy contract を所有するが、
      `mizar-atp` はその API を呼び出す integration task をまだ持たない。そのため、
      後から返った kernel-verifiable candidate と先に返った externally attested result の
      winner test、tie policy、early-stop finality oracle は、proof-policy 境界を越えずに
      `mizar-atp` 内へ実装できない。既存の task-18/task-21 coverage は、shuffled
      completion order 下の crate-local no-early-stop deterministic candidate handoff を
      既に guard している。この task は external_dependency_gap を記録し、paired docs を
      更新し、mock proof-policy oracle、placeholder `mizar-proof` adapter、accepted proof
...
26. **architecture-22 フォローアップ監査。** [x]
    ...
    - Status: audit-only follow-up として完了。source/spec audit と bilingual
      ...
      `ATP-AUDIT-G005` は、ATP/proof integration task が `mizar-proof` の release policy
      finality、winner selection、tie-breaking、candidate displacement を消費するまで、
      単一の policy-boundary / completion-order follow-up として残る。
...
28. **crate exit report と quality review。** [x]
    ...
    - `mizar-atp` が candidate evidence producer に留まることを再確認する:
      ...
      material、local proof-policy adapter、placeholder `mizar-cache` crate は導入しない。
...
32. **Counterexample search on open statements。** [ ]
    - 契約: [AIREV-OPEN-STATEMENT-COUNTEREXAMPLE](../../task_contracts/ja/AIREV-OPEN-STATEMENT-COUNTEREXAMPLE.md)。
