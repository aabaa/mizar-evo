...
pair を監査する。この task は Rust production source、public API、`.miz` fixture、
...
|---|---|---|
| `00.crate_plan.md` | Crate responsibility、authority order、design/source inventory、known gap、task 24 までの task decomposition、hard gate、verification expectation。 | 同期済み。 |
...
| `portfolio.md` | policy-neutral planning、no-early-stop collection、candidate / evidence-set ordering、fail-closed result matching、downstream proof-policy boundary、determinism suite、public enum inventory。 | 同期済み。 |
| `source_spec_audit.md` | public module export、public surface inventory、cross-module evidence、ATP-AUDIT gap register、`ProofWitnessRef` / `VerifiedArtifact` artifact-surface acknowledgement、source/spec drift なしの分類。 | 同期済み。 |
| `bilingual_sync_audit.md` | audit scope、method、pair inventory、classification、task-24 sync edits、remaining external/deferred work。 | この paired audit document により同期済み。 |
| `todo.md` | ordered task list、task 24 までの完了 task、deferred task 15/16 status、public enum task status、source/spec audit status、次 task 25 dependency wording、verification expectation。 | 同期済み。 |

## Classification

Task 24 は新しい `spec_gap`、`test_gap`、`design_drift`、`source_drift`、
...
- `external_dependency_gap`: portfolio early-stop finality と winner selection は
  downstream proof policy に依存する。raw backend completion order は proof identity の外に残る。
