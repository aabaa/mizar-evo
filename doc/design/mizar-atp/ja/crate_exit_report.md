...
Post-kernel-audit consumer correction: task 29 は post-audit kernel F1/F2/F6
契約に対する ATP の crate-owned consumer conformance を記録する。focused translator
regression と documentation だけを追加し、名指しされた kernel corpus fixture を
source-derived ATP-built candidate 経由で実行する部分は、real extraction が存在するまで
external/deferred として分類し続ける。

## Scope

Milestone scope:

- task 1 から task 29 までの `mizar-atp` workspace crate を構築する。
...
| Milestone-owned coverage | passed | crate-local test は problem validation、task-29 upstream F1 polarity rejection と ATP F2 local-source rejection を含む translation、property encoding、concrete encoder、backend runner behavior、portfolio collection、metadata projection、public enum policy、mock corpus coverage、determinism を cover する。 |
| Test expectation integrity | passed | 既存 `.miz` fixture または expectation sidecar を実装挙動に合わせて変更していない。active source-derived ATP execution と task-29 joint kernel-corpus-through-ATP-candidate path は deferred のままである。 |
| Design/source synchronization | passed | source/spec、bilingual、Architecture-22 follow-up、module-boundary、task-29 consumer-boundary audit は source layout と public module table に一致する。 |
...
| Verification | passed | historical task-28 の crate-local Rust command、broad workspace command、diff check は closeout commit 前に成功した。task-29 focused、crate-local、kernel、clippy、workspace、diff verification もこの update commit 前に成功した。 |
...
|---|---|---|---|
| ATP-CLOSEOUT-G001 | `external_dependency_gap` | concrete backend output を kernel-parseable formula/substitution candidate payload に map する paired real-output extraction spec/source module がなく、この環境で検証済みの supported real backend executable route もない。task 29 も、名指しされた kernel polarity/local-hypothesis corpus fixture を ATP-built candidate execution に入れる部分を、この route が存在するまで外に残す。 | task 15-16 または source-derived ATP candidate corpus execution を再開する前に、backend-specific EN/JA extraction spec、guarded fixture、backend proof material を除外する candidate mapping を追加する。 |
...
| ATP-CLOSEOUT-G005 | `deferred` | active `.miz` advanced-semantics execution と source-derived ATP extraction は利用できない。task 20 は metadata-only corpus anchor を使い、task 29 は同じ理由で joint kernel-corpus-through-ATP-candidate execution を deferred に保つ。 | staged runner と source extraction contract を追加してから、metadata-only coverage を active corpus coverage に置き換える。 |
...
external runner と extraction gap により blocked のままである。task-29 joint
kernel-corpus-through-ATP-candidate route も同じ gap により blocked のままである。
...
- `cargo test` passed。
- `git diff --check` は final staging 前に passed。
...
- task 29 では、real source-derived ATP extraction route または real-output candidate
  extractor が存在しないため、名指しされた kernel corpus fixture を ATP-built
  candidate 経由では実行しない。
...
```text
mizar-atp task 29 commit が存在する状態から evidence-pipeline correction を継続して
ください。まず `git status --short` が clean であること、task 29 が focused
translator regression と paired documentation だけを追加したことを確認してください。
Step 3 は完了済みです。`doc/design/todo.md` の Step 4 に入り、`mizar-checker task 45`
から再開してください。正式な `mizar-cache` と `mizar-proof` API は owner-approved
