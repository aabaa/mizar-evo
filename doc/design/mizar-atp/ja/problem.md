...
task 3 が具体的な Rust name と visibility を選ぶが、module は次の conceptual shape を
表現できなければならない:
...
| Field | 要件 |
|---|---|
| `problem_id` | problem content と選択された backend-neutral profile の deterministic identity。source edit をまたぐ proof-reuse identity ではなく、kernel evidence でもない。 |
| `vc_id` | snapshot-local な VC ordering と collation id。これ単体を stable target binding として使ってはならない。 |
| `target_binding` | validated `VcIr` / VC handoff input から導出される stable target fingerprint と producer binding。target binding の不足または不一致は fail closed する。 |
...
| `declarations` | formula layer に必要な symbol、sort、function、predicate、generated-binder declaration。backend-visible declaration は provenance を持たなければならない。`symbol_map` binding は encoding と diagnostics を支えるが、provenance の代替ではない。 |
...
必須の target binding、formula payload、provenance、symbol mapping、type context、
または `logic_profile` field 自体が不足または invalid な場合、construction は fail
closed する。valid な Mizar-side input であっても selected profile が扱えない formula
feature は、その profile では unsupported/open-status outcome として分類する。ATP
unavailability、unsupported profile、timeout、unknown、crash、backend error は VC を
open のままにするか diagnostic を生成するだけで、accepted proof status を作らない。
...
- `deferred`: task 3 がこの spec に従って Rust data shape、deterministic debug
  rendering、construction test を実装する。
...
task 3 は Rust coverage を追加し、次を確認しなければならない:
...
- target binding、formula payload、provenance、symbol-map row、type-context binding の
  不足、および duplicate id に対する fail-closed construction rejection;
