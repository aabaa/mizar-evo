...
source/spec correspondence audit. It changes no Rust production source,
public API, `.miz` fixture, expectation, language specification, backend route,
kernel check, proof policy, artifact witness, cache behavior, or downstream
integration.
...
|---|---|---|
| `00.crate_plan.md` | Crate responsibility, authority order, design/source inventory, known gaps, task decomposition through task 24, hard gates, and verification expectations. | Synchronized. |
...
| `portfolio.md` | Policy-neutral planning, no-early-stop collection, candidate/evidence-set ordering, fail-closed result matching, downstream proof-policy boundary, determinism suite, and public enum inventory. | Synchronized. |
| `source_spec_audit.md` | Public module exports, public surface inventory, cross-module evidence, ATP-AUDIT gap register, `ProofWitnessRef` / `VerifiedArtifact` artifact-surface acknowledgement, and no source/spec drift classification. | Synchronized. |
| `bilingual_sync_audit.md` | Audit scope, method, pair inventory, classification, task-24 sync edits, and remaining external/deferred work. | Synchronized by this paired audit document. |
| `todo.md` | Ordered task list, completed tasks through task 24, deferred task 15/16 status, public enum task status, source/spec audit status, next task 25 dependency wording, and verification expectations. | Synchronized. |
...
- `external_dependency_gap`: portfolio early-stop finality and winner
  selection depend on downstream proof policy; raw backend completion order
  remains outside proof identity.
