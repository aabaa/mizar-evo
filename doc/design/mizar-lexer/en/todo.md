...
These tasks are derived from the audit notes below. They are grouped by the layer that should own the work.
...
5. Document source-text normalization policy.
...
6. Add property-style final-token span tests.
   - Include the existing priority-task coverage plus malformed recovery cases from the text audit.
   - Assert every final token span still slices back to its lexeme after `LexemeRun` splitting and string-context disambiguation.
...
12. Add fuzz coverage for valid UTF-8 strings.
    - Include arbitrary valid UTF-8 input for `preprocess_source_for_lexing` and `scan_raw`.
    - Promote stable minimized failures into `tests/lexical`.

13. Add large-input and long-line benchmarks.
    - Measure raw scanning, preprocessing, and `SourceLineIndex` construction separately.
    - Include large `.miz`-like input, very long lines, many comments, and many multi-byte characters in comments.
