...
1. comment-removal edge tests を拡充する。
...
2. nested multi-line comment policy を決める。
...
4. public API stability policy を決める。
...
7. performance benchmarking を追加する。
...
8. source-loading boundary における UTF-8 BOM handling policy を決める。
...
9. UTF-8 file loading を仕様化して test する。
...
10. newline normalization を仕様化して test する。
...
11. preprocess source-map tests を実装する。
