...
| 増分 proof reuse identity と semantic dependency slice | VC/cache/semantic phase | 字句解析器は所有しません。lexer fingerprint は tokenization と parser-facing classification input だけを覆い、proof dependency や `ObligationAnchor` ではありません。 |
