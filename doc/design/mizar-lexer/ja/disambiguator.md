...
- active user-symbol kind の subset に制限された symbol/operator 位置;
...
`ParserLexContext` は `UserSymbolKindSet` も保持できます。General、symbolic、recovery modes は、その spelling の active candidate のうち少なくとも一つが admitted kind を持つ場合にだけ user symbol を許可します。これにより parser integration は active lexical environment を再構築せずに predicate-only、functor-only、mode-only などの category-specific lexical view を要求できます。Final `UserSymbol` token は引き続き spelling と span だけを保持します。kind、arity、overload、provenance details が必要な downstream phases は active lexical environment から candidate list を復元します。
...
- active user-symbol kind の subset だけを許す parser context;
