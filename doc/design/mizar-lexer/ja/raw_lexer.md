...
Import pre-scan は restricted syntax mode で raw lexer output を読みます。module path spellings と source spans を抽出するために必要な top-level import structure だけを認識します。package/module existence、visibility、re-export legality、imported symbol identity を resolve してはいけません。
...
- literal scanning 導入後の unterminated literal-like spans;
