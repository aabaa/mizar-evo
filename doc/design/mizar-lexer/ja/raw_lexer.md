...
|---|---|
| エントリポイント | `load_source_text_from_bytes`, `preprocess_source_for_lexing`, 厳密な `scan_raw`, 回復可能な `scan_raw_recoverable`, `scan_import_prelude`, `build_lexical_environment`, `build_scope_skeleton`, `lex`, `disambiguate` は、実行可能な字句解析器の受け渡し経路として維持します。 |
...
この層では、コード領域の識別子・数字・予約語・予約記号・ユーザーシンボルの綴りは ASCII のみです。認識された parser-assisted string argument span は狭い例外です。前処理はコメント除去と ASCII 診断の前に、単一行の引用符付き span をそのまま保持するため、annotation-style argument 内の Unicode と comment marker は後続の plan-sensitive tokenization に渡されるソーステキストとして残ります。認識には、その時点までに蓄積した lexical text を使うため、除去されたコメント本文が string-argument context を作ることはありません。コメント、ドキュメンテーションコメント、および認識された単一行 string argument span の外でコード領域に届いた非 ASCII テキストは、字句境界における不正な入力です。前処理はそれを `NonAsciiCode` として報告し、直接の生スキャンは未対応の文字を拒否します。ASCII の綴りへ変換して受け入れることはありません。
...
1. `preprocess_source_for_lexing` は入力を先頭から順に走査します。コメント除去の前に、認識された単一行 parser-assisted string argument span をそのまま保持します。コメントは字句テキストから取り除きますが、コメント内の改行は残して行位置が崩れないようにし、除去によって隣接するトークンの形をしたテキストが連結してしまう場合は合成レイアウトを挿入します。コメント本体はソーススパン付きのトリビアとして保持し、字句範囲から読み込み済みソース範囲へ戻す軽量な前処理マップも記録します。`\r`、認識された string argument span の外にあるコード領域内の非 ASCII 文字、閉じていない複数行コメントは前処理の診断として報告します。複数行コメントは入れ子になりません。`::=` の開始記号の後に最初に現れる `=::` がコメントを閉じ、内部の `::=` の綴りは通常のコメントテキストとして扱います。このヘルパーは、ファイルの読み込みや OS ごとの改行正規化は担当しません。
2. `scan_raw` は、LF のみの字句テキストを `char_indices` カーソルで読みます。連続するレイアウトを 1 個の `Layout` にまとめ、`@` から始まる注釈マーカー(annotation marker)を認識し、`@` 以外の ASCII 図形文字を連続したランとしてまとめます。そのランがすべて数字なら `NumeralLike`、そうでなければ `LexemeRun` です。対応していない文字は `LexError` になります。`scan_raw_recoverable` は同じトークン化規則を使いつつ、問題箇所の精密なスパンを持つ `RawScanDiagnostic` を記録し、不正な綴りに対応する `RawTokenKind::Error` sentinel を送出して、回復境界をまたいでテキストを連結せずに利用可能な部分的生トークンを保持するために再開します。
...
厳密な生スキャナーの重要な不変条件は、スパンの連続性です。出力された生トークンは必ず元入力の正確なバイト列を指し、生トークンの字句単位(lexeme)を連結するとスキャナーの入力が復元できます。回復可能スキャナーは、診断スパンに `RawTokenKind::Error` sentinel を挿入してこの不変条件を保ちます。利用可能な raw unit だけが必要な呼び出し側は sentinel を除外できますが、回復境界をまたいだテキスト連結を避けるために sentinel を利用できます。
...
`scan_raw` は、未対応の生入力に対して `RawTokenKind::Error` を出力するのではなく、`LexError` を返します。`scan_raw_recoverable` は次を返します。
...
これは妥当なスパンの生トークンを出力し、不正な生入力を診断として記録し、各問題スパンに `Error` 生トークンを出力して、次の文字境界から再開します。不正な注釈マーカーでは問題となる `@` だけを消費するため、`@ name` のような後続テキストは後続の生トークンを生成できます。
...
1. ソース位置でアクティブな字句エントリ。インポート由来のエントリと、既に完了した
   現在のモジュールの宣言を含む
2. 予約複合記号
3. 予約語
4. 識別子構文
...
字句環境は、同じ綴りのローカル / インポートのオーバーロード候補を、種別とアリティの
メタデータ付きで後続の意味論的な解決のために保持します。これらは、字句解析器が選ぶ
トークンの綴りを変えません。互換性のない同綴りのインポートは、インポート順で選ばず、
インポート / リンクの診断として扱います。
...
ソース前処理の診断は、安定したコードとバイトスパンに加えて、構造化ペイロードを持ちます。`CarriageReturn` は改行正規化の回復ヒントを、`NonAsciiCode` は拒否された文字と UTF-8 長を、`UnterminatedMultiLineComment` は開始記号のスパンとコメント回復ヒントを保持します。
