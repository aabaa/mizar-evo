...
| ELAB-G007 | `source_drift` | 現在の source に `elaborator` module はない。 | task 8 がこの spec の後で実装を導入する。 |
...
- source-written `qua` と inserted view は、underlying term と view provenance、既に確立済みの
  type fact へ lower する。新しい proof step ではない。
