...
本書は監査成果物である。同一変更で実施した仕様修正は各所見の **処置** に
記載し、それ以外は後続タスク向けの記録とする(`mizar-core` エラボレータの
todo.md 改訂は意図的に後続へ委ねる)。
...
## mizar-core(エラボレータ)への影響 — todo.md 改訂タスクへの入力

所見から次の実装義務が従う。**todo.md 自体は本変更では意図的に改訂しな
い。**

1. **レダクト/ビュー低下(F1、F3)。** Core IR は改名/複数経路 inherit
   辺上の `qua` と有界型パラメータのインスタンス化にビュー項を必要とし、
   属性アトムとフィールド選択はビュー項に対して放出されなければならな
   い。外延性の放出は厳密インスタンスガードへ切り替える。これは builtin
   type bridge と直近で入った typed-AST エラボレーション
   (`feat: bridge type elaboration to resolved typed ast`)に触れる。
...
soundness-argument 文書の仮定は、F1 のエンコードが実装された時点で
再訪すべきである。
