...
| 46 | `requires`/`ensures`/`assert`、invariant/decreasing、call target/actual/result binder/substitution-request metadata、recursive group、declared terminating intent、measure availability、契約の自己参照の拒否 (spec 20.4.1: `requires` または `decreasing` による algorithm 自身や再帰グループの適用、`terminating` でない algorithm の `ensures` による適用)。 | Specs 20.4-20.5/20.7-20.8/20.13、Core 34-35/42-45、`MT10-CIR-ALG`。 | Request/metadata 搬送のみ。call/result substitution の生成・適用・検証は VC Task-30 descendant。contract axiom、VC、termination proof/promotion/recursive encoding は作らない。契約の自己参照の拒否は public code を持たない structured local diagnostic とする。 |
...
| 54 | `terminating` algorithm 本体の定義的断片の分類と、ガード付き定義式の構成: ループのない断片の本体の網羅性、局所ガード、経路条件、`const` の置換、boolean 条件、相互再帰グループの全か無か、抽象のままの template parameter。 | Spec 20.2.6、20.7.3、20.8.2、20.13.6、Core 42-43・46、`MT10-CIR-ALG`。 | 定義式は来歴付きの Core metadata であり、事実ではない。断片外の本体は分類だけを記録し、定義式を持たない。VC、受理、前提としての利用、MVM、extraction は扱わない。 |
...
owner が必要とする authenticated metadata だけを記録する。例外は §20.13.6 の
定義式であり、Core 54 が未受理の Core formula として構成する。

## Accepted Phase-10 ControlFlowIr Task Graph

| Core task | Bounded phase-10 family | Dependencies / prepared consumer | Exit boundary |
|---|---|---|---|
| 48 | Core 42-43 に対する deterministic basic block/edge、local、statement placement、program context、source map、fallthrough、structured exit、illegal break/continue、unsupported local declaration、`return` なしに本体の終わりへ到達する値を返す経路 (spec 20.2.6) のsource-derived structural diagnostic。 | Core 42-43、`MT10-CFG-PV`。最初の source-derived CFG snapshot baseline と同じ commit で `SnapshotKind::ControlFlowIr` と schema/guard を追加し、各structural diagnostic familyをreal-source negative caseでcoverする。 | 限定的な診断観測は snapshot baseline や本 task の完了を意味しない。Empty snapshot-infrastructure prerequisite、semantic contract attachment、VC/proof/artifact は禁止。 |
...
| 51 | Snapshot/claim flow state。snapshot program-point owner、captured-local identity/context、hidden-loop-value ref、claim-to-snapshot link。 | Nesting に応じ Core 47-50、`MT10-CFG-PV`。 | state theorem acceptance、old-state substitution、artifact/extraction schema、VC は扱わない。 void-claim profile は実際の空 interface・return CFG だけを検証し、snapshot capture や本 family の完了を主張しない。 |
