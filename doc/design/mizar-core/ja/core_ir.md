...
task 21 は public surface が存在した後で最終的な public enum compatibility decision を
記録する。それまでは、owning module spec が exhaustive exception を明示しない限り、
implementation task は public enum を `#[non_exhaustive]` にする。task-2 specification が
所有する exhaustive public enum exception はない。
