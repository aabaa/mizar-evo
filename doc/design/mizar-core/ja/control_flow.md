...
## obligation handoff site

Task 18 は明示的な `ControlFlowIr` metadata を、`core_ir.md` の handoff view 用の deferred な
flow-derived obligation seed へ変換する。この変換は `ControlFlowIr` に既に存在する CFG table と
core id だけを使う。source text を走査せず、checker-owned payload を仮造しない。

flow-derived handoff site は次を含む。

- entry `requires` site。caller-side precondition seed として扱う。
- return `ensures` site。postcondition seed として扱う。
- algorithm-level / statement-level assertion site。
- algorithm-level / loop-level invariant site。
- algorithm / loop decreasing measure site と partial termination site。
- ghost-only pick local と ghost assignment effect。ghost-erasure safety seed として扱う。

task 18 では、これらの seed は `Deferred` である。正確な VC context、call/result substitution、
well-foundedness goal schema、ghost-erasure proof shape、`VcId` 割り当て、編集をまたぐ
`ObligationAnchor` construction は `mizar-vc` の責務である。core handoff は、後続 phase が
raw source spelling を必要としないように、source ref、core ref、CFG-local site ref、normalized
semantic origin、anchor-ready な program path を記録する。
...
task 21 は `control_flow` の public enum をすべて downstream forward-compatible API
surface として分類する。将来の CFG、local-state、contract-site、termination、handoff、
diagnostic category を下流 crate の exhaustive match を壊さずに追加できるよう、各 enum
は `#[non_exhaustive]` を維持しなければならない。
...
task 18 の test は次を覆う。

- 既存 core の theorem、definition correctness、checker-initial seed が、label、path、source ref、
  provenance、元 seed への backreference とともに handoff に保存されること。
- flow-derived contract、termination、ghost-erasure site が、利用可能な formula / term ref、
  CFG-site backreference、deterministic handoff ordering を持つ deferred seed を生成すること。
- `VcId`、`ObligationAnchor`、source-derived call / pattern / snapshot / claim payload、
  concrete termination goal schema を仮造しないこと。
