...
| 46 | `requires`/`ensures`/`assert`, loop invariants/decreasing terms, call target/actual/result-binder and substitution-request metadata, recursive-group membership, declared terminating intent, termination-measure availability, and contract self-reference rejection (spec 20.4.1: `requires` or `decreasing` applying the algorithm or its recursive group, or `ensures` doing so in an algorithm that is not `terminating`). | Specs 20.4-20.5, 20.7-20.8, and 20.13; Core 34-35 and 42-45; `MT10-CIR-ALG`. | Transport requests and metadata only. Actual call/result substitution generation, application, and validation belong to VC Task-30 descendants. No contract axiom, VC, termination proof, promotion, or recursive encoding is emitted. Contract self-reference rejection is a structured local diagnostic with no public code. |
| 47 | Snapshot and claim shells, snapshot identity, captured algorithm context request, visible runtime/ghost locals, hidden-loop-value references, claim body/order, and missing/unsupported state. | Spec 20.6; Core 37 theorem shells and Core 42-46; `MT10-CIR-ALG`. | Claim links only to already-lowered Core-37 theorem/statement shells. No source-text reconstruction, context fact invention, old-state substitution, claim proof, CFG capture, VC, or acceptance. The [bounded void claim](elaborator.md) retains an earlier empty-interface algorithm dependency and its real theorem/proof sources; captured-state claims remain deferred. The implemented [flat snapshot shell](core_ir.md#flat-snapshot-statement) retains actual names and visible declaration captures; state claims remain deferred. |
| 54 | Definitional-fragment classification of `terminating` algorithm bodies and construction of their guarded defining equations: exhaustiveness of the loop-free fragment body, local guards, path conditions, `const` substitution, boolean conditions, all-or-none mutually recursive groups, and abstract template parameters. | Specs 20.2.6, 20.7.3, 20.8.2, and 20.13.6; Core 42-43 and 46; `MT10-CIR-ALG`. | Equations are Core metadata with provenance, never facts. A body outside the fragment records its classification and no equation. No VC, acceptance, premise availability, MVM, or extraction. |
...
The exception is the §20.13.6 defining equations, which Core 54 constructs as
unaccepted Core formulas.
...
|---|---|---|---|
| 48 | Basic CFGs: deterministic blocks/edges, locals, statement placements, program contexts, source maps, fallthrough, structured exits, and source-derived structural diagnostics for illegal break/continue, unsupported local declarations, and a value-returning path that reaches the end of the body without `return` (spec 20.2.6). | Core 42-43; `MT10-CFG-PV`. The task that first lands a source-derived CFG snapshot baseline adds `SnapshotKind::ControlFlowIr` and its schema/guard in the same commit; real-source negative cases cover every structural diagnostic family. | Bounded diagnostic observations do not claim the snapshot baseline or complete this task. No empty snapshot-infrastructure prerequisite, semantic contract attachment, VC, proof status, or artifact. |
...
| 51 | Snapshot/claim flow state: snapshot program-point ownership, captured-local identity/context, hidden-loop-value references, and claim-to-snapshot links. | Core 47-50 as nesting requires; `MT10-CFG-PV`. | No state theorem acceptance, old-state substitution, artifact/extraction schema, or VC. The void-claim profile validates the actual empty-interface return CFG only; it does not complete this family. The implemented [flat snapshot flow](control_flow.md#flat-snapshot-flow) captures exact visible locals and pre-statement context; nested state and claim links remain deferred. |
