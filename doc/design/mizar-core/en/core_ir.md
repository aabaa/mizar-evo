...
- Template type-parameter inhabitation is represented as an explicit
  `Exists` formula over a checker-supplied witness binder. Template type actual
  gate failures remain diagnostics/backrefs; Core IR does not add an
  actual-side existential axiom for rejected actuals.
