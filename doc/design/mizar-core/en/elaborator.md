...
## Bounded return Pick lowering

The sealed `return the set` lowers to one existing runtime `Pick` and a `Return` of its fresh hidden binder, with the actual set predicate. Pick source is the choice occurrence contained within the owning return range; this authenticated containment replaces flat disjoint-range checking only for this pair. A separate fresh logical binder forms `exists q. is_set(q)` in an Active `GeneratedNonEmptiness` seed linked by existing Core references to this Pick and algorithm, with complete source maps. Reuse existing terms/formulas/seeds and expose no hidden binder to source contracts; emit no StableChoice origin, witness, accepted inhabitation proof or termination promotion.

## Bounded equals-infix functor theorem

The existing sealed route stores the actual formal-reference RHS as an Opaque term definition with both set formals. Under §16.6.2, one Active DefinitionCorrectness seed encodes `forall x,y ((set(x) & set(y)) -> set(t))` using the actual RHS t and declared result type, with clause/source provenance and complete body/goal backrefs. Keep the theorem `forall q1,q2:set F(q1,q2)=q1` and the two-local IntroduceBinder chain ending at `F(l1,l2)=l1` under only their set guards; one Active TheoremProof seed belongs to the PendingAutomaticProof theorem. Preserve operator/argument order and six distinct binder identities. No pending defining equation, coherence goal or static result type is an accepted premise; no unfolding, registration or new Core representation follows.
