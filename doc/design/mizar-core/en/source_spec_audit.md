...
| Step 2 lowers soft type and fact data into explicit predicates, assumptions, view provenance, optional reduct-view metadata, template type-parameter inhabitation assumptions, template type-actual gate records, scheme-actual validation records, carried obligations, diagnostics, or deferred seeds without registration activation. | Type/fact seed/output types, `TemplateTypeParameterInhabitationSeed`, `TemplateTypeActualGateSeed`, `TemplateSchemeActualSeed`, `TemplateSchemeActual`, and `lower_type_and_fact_inputs`. | Task 9 tests for declared binders, attributes, modes, cluster facts, `qua`, reconsidering, carried/missing evidence; task 27 rejects empty reduct-view metadata; task 28 tests schema inhabitation assumptions, accepted actual-gate evidence preservation, missing-existential diagnostics, and invalid gate payload rejection; task 29 tests directional scheme-actual evidence, skipped guard seeds, predicate/functor/algorithm actual forms, enclosing-parameter substitution metadata, diagnostic-only rejections, and fail-closed malformed payloads. | Implemented for explicit checker-owned payloads; Step 2 does not create view terms, actual-side existential axioms for rejected template type actuals, instantiated functor guard axioms, active guard VCs, fresh scheme symbols, or source-derived closure expansions. |
