...
const STEP5C5_NARROWER_ID: &str = "pass_type_elaboration_pred_redefine_narrower_loci_001";
...
pub(in crate::runner) fn step5c5_argument_admitted(root: Option<&Path>, case: &TestCase) -> bool {
    ...
    let narrower = case.id.0 == STEP5C5_NARROWER_ID;
    let property = case.id.0 == STEP5C5_PROPERTY_ID || dependent || narrower;
    if !property && case.id.0 != STEP5C5_ARGUMENT_ID {
        return false;
    }
    let directory = if narrower {
        "pass/predicates"
    } else if property {
        "pass/functors"
    } else {
    ...
    case.expectation.schema_version == 1
        ...
        && case.expectation.spec_refs[0].0
            == if narrower {
                "spec.en.09.predicates.redefine.coherence"
            } else if dependent {
...
pub(in crate::runner) fn is_step5c3_argument_candidate(case: &TestCase) -> bool {
    [STEP5C3_ARGUMENT_ID, STEP5C3_WIDENING_ID].iter().any(|id| {
        case.id.0 == *id
            || case.expectation.id.0 == *id
            || case.source_path.file_stem().is_some_and(|stem| stem == *id)
            || case
...
pub(in crate::runner) fn step5c3_argument_admitted(root: Option<&Path>, case: &TestCase) -> bool {
    let widening = case.id.0 == STEP5C3_WIDENING_ID;
    let (id, directory, kind, outcome, category, detail, domain) = if widening {
        (
            STEP5C3_WIDENING_ID,
            "pass",
            crate::expectation::TestKind::Pass,
    ...
    let path = format!("tests/miz/{directory}/types/{id}.miz");
    let exact_path = |actual: &Path, expected: &Path| match root {
        Some(root) => {
            workspace_relative_source(root, actual).is_some_and(|path| Path::new(&path) == expected)
        }
...
const STEP5C5_CASES: [(&str, &str, PipelinePhase, ExpectedOutcome); 12] = [
    (
    ...
    (
        STEP5C5_NARROWER_ID,
        "tests/miz/pass/predicates/pass_type_elaboration_pred_redefine_narrower_loci_001.miz",
        PipelinePhase::TypeCheck,
        ExpectedOutcome::Pass,
    ),
];

const STEP5C5_BLOCKED_CASE_IDS: [&str; 5] = [
...
const STEP5C3_ATTRIBUTE_CASES: [(&str, &str, PipelinePhase, ExpectedOutcome); 7] = [
    (
    ...
    (
        STEP5C3_WIDENING_ID,
        "tests/miz/pass/types/pass_type_elaboration_argument_attribute_widening_001.miz",
        PipelinePhase::TypeCheck,
        ExpectedOutcome::Pass,
    ),
];
...
fn step5c3_case(
    ...
    if is_step5c3_argument_candidate(case) && !step5c3_argument_admitted(None, case) {
        return None;
    }
    STEP5C3_ATTRIBUTE_CASES
        .iter()
...
mod tests {
    ...
    use super::{
        ...
        STEP5C3_ATTRIBUTE_CASES, STEP5C4_MODE_CASES, STEP5C5_BLOCKED_CASE_IDS, STEP5C5_CASES,
        STEP5C7_TERM_CASES, is_active_type_elaboration, validate_active_type_elaboration_tags,
    };
    ...
    #[test]
    fn step5c3_inventory_has_seven_unique_id_source_pairs() {
        assert_eq!(STEP5C3_ATTRIBUTE_CASES.len(), 7);
        assert_eq!(
            STEP5C3_ATTRIBUTE_CASES
                .iter()
                .map(|(id, source, _, _)| (*id, *source))
    ...
    fn step5c5_inventory_admission_and_blocked_rows_are_exact() {
        assert_eq!(STEP5C5_CASES.len(), 12);
        assert_eq!(
            STEP5C5_CASES
                .iter()
                .map(|(id, source, _, _)| (*id, *source))
    ...
    fn step5c3_argument_admission_execution_and_inventory_are_exact() {
        ...
        let root = workspace_root();
        let plan = build_test_plan(&config()).unwrap();
        for id in [super::STEP5C3_ARGUMENT_ID, super::STEP5C3_WIDENING_ID] {
            let original = plan.cases.iter().find(|case| case.id.0 == id).unwrap();
            ...
            assert_eq!(
                result.actual_detail_keys,
                if id == super::STEP5C3_ARGUMENT_ID {
                    vec!["types.application.argument_type_mismatch".to_owned()]
                } else {
                    Vec::new()
                }
            );
            ...
            for mutate in [
                ...
                |case| case.expectation.schema_version = 2,
                |case| {
                    case.expectation.kind = match case.expectation.kind {
                        TestKind::Pass => TestKind::Fail,
                        _ => TestKind::Pass,
                    }
                },
                |case| case.expectation.expected_phase = Some(PipelinePhase::Resolve),
                |case| {
                    case.expectation.expected_outcome = match case.expectation.expected_outcome {
                        ExpectedOutcome::Pass => ExpectedOutcome::Fail,
                        _ => ExpectedOutcome::Pass,
                    }
                },
                |case| {
                    case.expectation.failure_category =
            ...
            let mut legacy = plan
                .cases
                .iter()
                .find(|case| case.id.0 == "fail_type_elaboration_attr_duplicate_same_subject_001")
                .unwrap()
                .clone();
            legacy.expectation.id = original.expectation.id.clone();
            rejected(&legacy);
