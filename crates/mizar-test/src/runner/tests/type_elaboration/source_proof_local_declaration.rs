...
fn task269a_resolver_local_row_and_lower_mutations_fail_at_the_owner() {
    let (ast, module, _shells, symbols) =
        task253_ast_from_source_text(TASK269A_SOURCE_TEXT, 269_100);
    for mutation in [
        ...
        assert!(
            ...
            .expect("Task269A exact mutation selector")
            .is_err(),
            "mutation {mutation:?} unexpectedly succeeded"
        );
    }
}
...
fn task269a_near_misses_and_public_route_remain_isolated() {
    ...
    let recovered_ast = task269a_ast_with_recovered_name_token(&ast);
    assert!(
...
fn task269a_ast_with_recovered_name_token(ast: &mizar_syntax::SurfaceAst) -> mizar_syntax::SurfaceAst {
