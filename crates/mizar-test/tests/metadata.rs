...
fn active_runner_reports_are_byte_stable_across_repeated_runs() {
    ...
    assert_eq!(active_proof_verification_cases(&plan).count(), 23);
...
fn repository_corpus_plan_succeeds() {
    ...
    for (snapshot_ref, id, directory) in source_snapshot_refs {
        let requirement = plan
            ...
            .requirements
            .iter()
            .find(|row| row.id.0 == snapshot_ref)
            .unwrap();
        assert_eq!(requirement.stage, Stage::ProofVerification);
        assert_eq!(requirement.status, RequirementStatus::Covered);
        assert_eq!(requirement.coverage, CoverageShape::Snapshot);
        ...
        if matches!(
            ...
            assert_eq!(
                ...
                } else if snapshot_ref == "spec.en.mizar_vc.vc_ir.equals_functor_snapshot" {
        ...
        let coverage = plan
            ...
            .unwrap();
        assert_eq!(coverage.computed_status, RequirementStatus::Covered);
        assert_eq!(coverage.evidence.snapshot, 1);
        assert!(coverage.missing_shapes.is_empty());
    }
...
fn repository_type_elaboration_runner_executes_active_source_derived_seeds() {
    ...
    assert_eq!(report.error_count(), 0, "{:#?}", report.diagnostics);
    assert_eq!(report.results.len(), 266);
    assert_eq!(report.passed_count(), 266);
    assert_eq!(report.failed_count(), 0);
...
fn type_elaboration_cli_reports_active_runner_summary() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("type-elaboration cases: 266"));
    assert!(stdout.contains("passed: 266"));
    assert!(stdout.contains("failed: 0"));
}
...
fn proof_verification_cli_reports_task180_and_step5c2_summary() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("proof-verification cases: 23"));
    assert!(stdout.contains("passed: 23"));
    assert!(stdout.contains("failed: 0"));
}
...
fn step5c11_advanced_semantics_cli_executes_pending_registration_intake() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("advanced-semantics cases: 6"), "{stdout}");
    assert!(stdout.contains("passed: 6"), "{stdout}");
