...
const PROPERTY_IMPLEMENTATIONS_REQUIREMENT_ID: &str =
...
fn active_runner_reports_are_byte_stable_across_repeated_runs() {
    ...
    assert_eq!(active_proof_verification_cases(&plan).count(), 11);
...
fn repository_corpus_plan_succeeds() {
    ...
    let return_snapshot_ref = "spec.en.mizar_vc.vc_ir.algorithm_ensures_return_snapshot";
    let return_requirement = plan
        .manifest
        .requirements
        .iter()
        .find(|row| row.id.0 == return_snapshot_ref)
        .unwrap();
    assert_eq!(return_requirement.stage, Stage::ProofVerification);
    assert_eq!(return_requirement.status, RequirementStatus::Covered);
    assert_eq!(return_requirement.coverage, CoverageShape::Snapshot);
    assert!(return_requirement.required);
    assert_eq!(
        return_requirement.tests,
        [PathBuf::from(
            "tests/miz/pass/algorithms/pass_proof_verification_algorithm_ensures_return_001.expect.toml"
        )]
    );
    let return_coverage = plan
        .coverage_report
        .requirements
        .iter()
        .find(|row| row.id.0 == return_snapshot_ref)
        .unwrap();
    assert_eq!(return_coverage.computed_status, RequirementStatus::Covered);
    assert_eq!(return_coverage.evidence.snapshot, 1);
    assert!(return_coverage.missing_shapes.is_empty());
    let deferred_proof_rows =
        plan.manifest
            ...
            .filter(|requirement| {
                ...
                // exact baseline guards above apply separately to the legacy rows.
                let audit1_row =
                ...
                requirement.stage == Stage::ProofVerification
                    ...
                    && requirement.id.0 != return_snapshot_ref
                    && !audit1_row
            })
            .collect::<Vec<_>>();
...
fn repository_type_elaboration_runner_executes_active_source_derived_seeds() {
    ...
    let parenthesized_reserved_variable_inequality_case = active_type_elaboration_cases(&plan)
        .find(|case| {
            case.id.0 == "pass_type_elaboration_parenthesized_reserved_variable_inequality_001"
        })
        .expect("Task241 parenthesized reserved-variable inequality bridge should be active");
    assert_eq!(
        parenthesized_reserved_variable_inequality_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    ...
    let parenthesized_reserved_variable_membership_case = active_type_elaboration_cases(&plan)
        .find(|case| {
            case.id.0 == "pass_type_elaboration_parenthesized_reserved_variable_membership_001"
        })
        .expect("Task243 parenthesized reserved-variable membership bridge should be active");
    assert_eq!(
        parenthesized_reserved_variable_membership_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    ...
    let right_parenthesized_reserved_variable_membership_case =
        active_type_elaboration_cases(&plan)
            .find(|case| {
                case.id.0
                    == "pass_type_elaboration_right_parenthesized_reserved_variable_membership_001"
            })
            .expect("Task245 right-parenthesized membership bridge should be active");
    assert_eq!(
        right_parenthesized_reserved_variable_membership_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    let parenthesized_two_edge_local_mode_reserved_variable_equality_case =
        active_type_elaboration_cases(&plan)
            .find(|case| {
                case.id.0
                    == "pass_type_elaboration_parenthesized_two_edge_local_mode_reserved_variable_equality_001"
            })
            .expect("Task246 parenthesized two-edge local-mode equality bridge should be active");
    assert_eq!(
        parenthesized_two_edge_local_mode_reserved_variable_equality_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    ...
    let local_object_mode_long_chain_two_hop_asserted_head_case = active_type_elaboration_cases(
        &plan,
    )
    .find(|case| {
    ...
    .expect(
        "Task225 seven-expansion object-terminal two-hop asserted-head bridge should be active",
    );
    assert_eq!(
        local_object_mode_long_chain_two_hop_asserted_head_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    let local_mode_long_chain_three_hop_asserted_head_case = active_type_elaboration_cases(&plan)
        .find(|case| {
            case.id.0 == "pass_type_elaboration_local_mode_long_chain_three_hop_asserted_head_001"
        })
        .expect(
            "Task226 seven-expansion set-terminal three-hop asserted-head bridge should be active",
        );
    assert_eq!(
        local_mode_long_chain_three_hop_asserted_head_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    let local_object_mode_long_chain_three_hop_asserted_head_case = active_type_elaboration_cases(
        &plan,
    )
    .find(|case| {
        case.id.0
    ...
    .expect(
        "Task227 seven-expansion object-terminal three-hop asserted-head bridge should be active",
    );
    assert_eq!(
        local_object_mode_long_chain_three_hop_asserted_head_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    let local_mode_long_chain_four_hop_asserted_head_case = active_type_elaboration_cases(&plan)
        .find(|case| {
            case.id.0 == "pass_type_elaboration_local_mode_long_chain_four_hop_asserted_head_001"
        })
        .expect(
            "Task228 seven-expansion set-terminal four-hop asserted-head bridge should be active",
        );
    assert_eq!(
        local_mode_long_chain_four_hop_asserted_head_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    let local_object_mode_long_chain_four_hop_asserted_head_case = active_type_elaboration_cases(
        &plan,
    )
    .find(|case| {
        case.id.0 == "pass_type_elaboration_local_object_mode_long_chain_four_hop_asserted_head_001"
    })
    .expect(
        "Task229 seven-expansion object-terminal four-hop asserted-head bridge should be active",
    );
    assert_eq!(
        local_object_mode_long_chain_four_hop_asserted_head_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    let local_mode_long_chain_five_hop_asserted_head_case = active_type_elaboration_cases(&plan)
        .find(|case| {
            case.id.0 == "pass_type_elaboration_local_mode_long_chain_five_hop_asserted_head_001"
        })
        .expect(
            "Task230 seven-expansion set-terminal five-hop asserted-head bridge should be active",
        );
    assert_eq!(
        local_mode_long_chain_five_hop_asserted_head_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    let local_object_mode_long_chain_five_hop_asserted_head_case = active_type_elaboration_cases(
        &plan,
    )
    .find(|case| {
        case.id.0 == "pass_type_elaboration_local_object_mode_long_chain_five_hop_asserted_head_001"
    })
    .expect(
        "Task231 seven-expansion object-terminal five-hop asserted-head bridge should be active",
    );
    assert_eq!(
        local_object_mode_long_chain_five_hop_asserted_head_case
            .expectation
            .expected_outcome,
        ExpectedOutcome::Pass
    );
    ...
    assert_eq!(report.error_count(), 0, "{:#?}", report.diagnostics);
    assert_eq!(report.results.len(), 258);
    assert_eq!(report.passed_count(), 258);
    assert_eq!(report.failed_count(), 0);
...
fn repository_parse_only_cases_separate_active_runner_seeds_from_future_metadata() {
    ...
    let property_implementations_requirement = plan
        .manifest
        .requirements
        .iter()
        .find(|requirement| requirement.id.0 == PROPERTY_IMPLEMENTATIONS_REQUIREMENT_ID)
        .expect("property-implementation parse-only requirement should exist");
    assert_eq!(property_implementations_requirement.stage, Stage::ParseOnly);
    assert_eq!(
        property_implementations_requirement.tests,
        vec![
            PathBuf::from(
...
fn repository_parse_only_runner_executes_active_minimal_parser_seeds() {
    ...
    assert!(report.results.iter().any(|result| {
        result.id.0 == "fail_parser_property_implementations_recovery_001"
            && result.actual_diagnostic_codes
                == vec![
                    "malformed_term_expression".to_owned(),
                    "malformed_formula_expression".to_owned(),
...
fn declaration_symbol_cli_reports_active_runner_summary() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("declaration-symbol cases: 11"));
    assert!(stdout.contains("passed: 11"));
    assert!(stdout.contains("failed: 0"));
}
...
fn type_elaboration_cli_reports_active_runner_summary() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("type-elaboration cases: 258"));
    assert!(stdout.contains("passed: 258"));
    assert!(stdout.contains("failed: 0"));
}
...
fn proof_verification_cli_reports_task180_and_step5c2_summary() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("proof-verification cases: 11"));
    assert!(stdout.contains("passed: 11"));
    assert!(stdout.contains("failed: 0"));
}
