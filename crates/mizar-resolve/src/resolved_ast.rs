...
impl ResolvedAst {
    ...
    pub fn snapshot_text(&self) -> String {
        ...
        output.push('\n');
        let _ = writeln!(output, "root: node#{}", self.nodes.root().index());
        ...
        if self.name_refs.is_empty() {
            output.push_str("  <none>\n");
        } else {
            for (id, entry) in self.name_refs.iter() {
                write_name_ref_snapshot(&mut output, id, entry);
            }
        }
        output.push_str("label_refs:\n");
        if self.label_refs.is_empty() {
            output.push_str("  <none>\n");
        } else {
            for (id, entry) in self.label_refs.iter() {
                write_label_ref_snapshot(&mut output, id, entry);
            }
        }
        output.push_str("imports:\n");
        let mut import_count = 0;
        for (id, import) in self.imports.imports() {
            import_count += 1;
        ...
        for (id, export) in self.imports.exports() {
            export_count += 1;
        ...
        let canonical_modules = self.imports.canonical_import_modules();
...
mod tests {
    ...
    use mizar_session::{
        ...
        InMemorySessionIdAllocator, SessionIdAllocator, SourceAnchor,
    };
