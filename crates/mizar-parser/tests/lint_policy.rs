...
#[test]
fn parser_manifest_opts_into_workspace_lints() {
...
#[test]
fn parser_allow_exceptions_are_documented_inline() {
    let root = crate_root();
    let mut violations = Vec::new();

    for path in parser_rust_target_files(&root) {
...
fn public_forward_compatible_enums_are_marked_non_exhaustive() {
    let root = crate_root();
    let expected = [
        ("src/lib.rs", "ParserTokenKind"),
        ("src/lib.rs", "StringRequiredContext"),
    ];
    ...
    assert!(
        violations.is_empty(),
        "pre-resolver public enum forward-compatibility policy requires \
         #[non_exhaustive] on:\n{}",
        violations.join("\n")
    );
}
...
fn every_public_enum_has_a_forward_compatibility_decision() {
    ...
    let source = read_to_string(&root.join("src/lib.rs"));
    let mut discovered = public_enum_names(&source);
    discovered.sort_unstable();

    assert_eq!(
        discovered, classified,
        "each public mizar-parser enum must be classified as #[non_exhaustive] \
...
"#;
    ...
    assert_eq!(
        ...
        vec![
            ...
            PathBuf::from("crate/tools/parser_tool.rs"),
...
fn default_cargo_target_directories_are_scanned() {
    let root =
        std::env::temp_dir().join(format!("mizar_parser_lint_policy_{}", std::process::id()));
    ...
    write_test_file(&root.join("benches/parser.rs"), "fn bench() {}\n");
    write_test_file(&root.join("examples/parser.rs"), "fn example() {}\n");

    let files = parser_rust_target_files(&root);

    assert!(files.contains(&root.join("src/lib.rs")));
    assert!(files.contains(&root.join("build.rs")));
    assert!(files.contains(&root.join("benches/parser.rs")));
    assert!(files.contains(&root.join("examples/parser.rs")));
    remove_dir_if_exists(&root);
}
...
#[test]
fn public_api_is_reachable_from_external_crate_path() {
    let source_id = source_id();
    let output = mizar_parser::parse(mizar_parser::ParseRequest::new(
        source_id,
        mizar_session::Edition::new("2026"),
        vec![mizar_parser::ParserToken::new(
            mizar_parser::ParserTokenKind::Identifier,
            "alpha",
        ...
        vec![mizar_parser::OperatorFixityEntry::new(
            "++",
            10,
            mizar_parser::OperatorAssociativity::Left,
        )],
    ));
    ...
    assert_eq!(
        output
            .ast
            .expect("parser API should produce AST")
            .token_texts(),
        vec!["alpha"]
    );
    let _ = mizar_parser::ParseOutput::new(None, Vec::new());
    let _ = mizar_parser::StringRequiredContext::None;
}
...
fn parser_rust_target_files(root: &Path) -> Vec<PathBuf> {
...
fn collect_rust_files(dir: &Path, files: &mut Vec<PathBuf>) {
    if !dir.exists() || dir.file_name().and_then(|name| name.to_str()) == Some("target") {
        return;
    }
...
fn source_id() -> mizar_session::SourceId {
    use mizar_session::{BuildSnapshotId, Hash, InMemorySessionIdAllocator, SessionIdAllocator};

    let hex = "09".repeat(Hash::BYTE_LEN);
...
fn workspace_root() -> PathBuf {
    crate_root()
        ...
        .expect("crate is under workspace/crates/mizar-parser")
        .to_path_buf()
}
