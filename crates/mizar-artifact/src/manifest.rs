...
mod tests {
    ...
    use crate::{
        ...
        registration_summary::{
            ...
            current_schema_version as registration_summary_schema_version,
            registration_summary_json,
        },
        ...
        verified_artifact::{
            ...
            VerifiedExport, current_schema_version as verified_schema_version,
    ...
    fn manifest_commit_publishes_old_or_new_and_ignores_orphans() {
        ...
        let module_b = identity("pkg", "articles/b", "2026");
        let artifact_a =
            sample_verified_artifact(module_a.clone(), "articles/a.miz", hash(1), false);
        let artifact_b =
            sample_verified_artifact(module_b.clone(), "articles/b.miz", hash(2), false);
    ...
    fn replayed_identical_commit_is_idempotent() {
        ...
        let first_commit = first
            .commit(ManifestCommitOptions::default())
            .expect("first commit");
        let first_text = fs::read_to_string(root.path().join(ARTIFACT_MANIFEST_PATH))
        ...
        let replay_text = fs::read_to_string(root.path().join(ARTIFACT_MANIFEST_PATH))
            .expect("replay manifest text");
    ...
    fn manifest_validates_module_and_registration_summary_sidecars() {
        ...
        let artifact =
            sample_verified_artifact(module.clone(), "articles/a.miz", source_hash, false);
        let mut entry = publish_verified_artifact(root.path(), "artifacts/a.json", &artifact);
    ...
    fn module_summary_artifact_hash(
        summary: &ModuleSummary,
    ...
    fn publish_registration_summary(
        ...
        let domain =
            artifact_hash_domain(REGISTRATION_SUMMARY_SCHEMA_FAMILY, summary.schema_version);
