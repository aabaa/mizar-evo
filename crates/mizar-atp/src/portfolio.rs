...
mod tests {
    ...
    use crate::{
        backend::{
            ...
            BackendKind, BackendObservation, BackendProfile, BackendProfileId,
            BackendResourceLimits, EncodedBackendProblem, EncodedBackendProblemParts,
            classify_backend_observation, synthetic_backend_result,
            synthetic_backend_result_with_diagnostics,
        },
        problem::{
            AtpAtom, AtpDeclaration, AtpDeclarationId, AtpDeclarationKind, AtpFingerprint,
            AtpFormula, AtpFormulaId, AtpFormulaTree, AtpProblemParts, AtpProvenance,
            AtpProvenanceId, AtpSourceBinding, AtpSourceRef, AtpSymbolMapEntry, AtpSymbolSource,
    ...
    fn shuffled_completion_produces_identical_candidate_sets_and_order() {
        ...
        let plan_a =
            plan_portfolio(fixture.input(vec![run_a.clone(), run_b.clone()])).expect("plan a");
        let plan_b =
            plan_portfolio(fixture.input(vec![run_b.clone(), run_a.clone()])).expect("plan b");
        let result_a = fixture.proved_result(run_a);
        let result_b = fixture.proved_result(run_b);

        let evidence_a =
            collect_portfolio_results(plan_a, vec![result_a.clone(), result_b.clone()])
                .expect("evidence a");
        let evidence_b =
    ...
    fn result_metadata_mismatch_is_rejected() {
        ...
        let error =
            collect_portfolio_results(plan, vec![fixture.proved_result(mismatched_same_id)]);
        ...
        let error =
            collect_portfolio_results(plan, vec![fixture.proved_result(mismatched_command)]);
    ...
    fn no_schedulable_or_budget_exhausted_plan_has_no_candidates() {
        ...
        let empty_evidence =
            collect_portfolio_results(empty_plan, Vec::new()).expect("empty evidence");
        assert_eq!(
    ...
    fn stopped_plans_still_reject_unexpected_results() {
        ...
        let error =
            collect_portfolio_results(cancelled_plan, vec![fixture.proved_result(unexpected)]);
    ...
    fn target_binding(vc_index: u32) -> AtpTargetBinding {
        AtpTargetBinding::new(
            AtpFingerprint::new(18, format!("target-vc-{vc_index}").into_bytes())
                .expect("fingerprint"),
