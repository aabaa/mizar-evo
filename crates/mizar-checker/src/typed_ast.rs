...
impl TypedAst {
    pub fn try_new(parts: TypedAstParts) -> Result<Self, TypedAstError> {
        ...
        Ok(Self {
            ...
            source_property_implementation: None,
            ...
            source_proof_local_given_use_type: None,
            ...
            source_proof_local_given_descendant_binding: None,
            source_proof_local_given_descendant_type: None,
            source_proof_local_given_descendant_use_term: None,
            nodes: parts.nodes,
    ...
    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_let_binding_for_test(
        &mut self,
        handoff: SourceProofLocalLetBindingHandoff,
    ) {
        self.source_proof_local_let_binding = Some(Box::new(handoff));
    }

    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_binding_for_test(
        &mut self,
        handoff: SourceProofLocalGivenBindingHandoff,
    ) {
        self.source_proof_local_given_binding = Some(Box::new(handoff));
    }

    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_let_type_for_test(
        &mut self,
        handoff: SourceProofLocalLetTypeHandoff,
    ) {
        self.source_proof_local_let_type = Some(Box::new(handoff));
    }

    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_type_for_test(
        &mut self,
    ...
    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_use_type_for_test(
        &mut self,
        handoff: SourceProofLocalGivenUseTypeHandoff,
    ) {
        self.source_proof_local_given_use_type = Some(Box::new(handoff));
    }

    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_use_term_for_test(
        &mut self,
        handoff: SourceProofLocalGivenUseTermHandoff,
    ) {
        self.source_proof_local_given_use_term = Some(Box::new(handoff));
    }

    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_condition_binding_for_test(
        &mut self,
        handoff: SourceProofLocalGivenConditionBindingHandoff,
    ) {
        self.source_proof_local_given_condition_binding = Some(Box::new(handoff));
    }

    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_condition_type_for_test(
        &mut self,
        handoff: SourceProofLocalGivenConditionTypeHandoff,
    ) {
        self.source_proof_local_given_condition_type = Some(Box::new(handoff));
    }
    ...
    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_descendant_binding_for_test(
        &mut self,
        handoff: SourceProofLocalGivenDescendantBindingHandoff,
    ) {
        self.source_proof_local_given_descendant_binding = Some(Box::new(handoff));
    }

    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_descendant_type_for_test(
        &mut self,
        handoff: SourceProofLocalGivenDescendantTypeHandoff,
    ) {
        self.source_proof_local_given_descendant_type = Some(Box::new(handoff));
    }

    #[cfg(test)]
    pub(crate) fn inject_source_proof_local_given_descendant_use_term_for_test(
        &mut self,
        handoff: SourceProofLocalGivenDescendantUseTermHandoff,
    ) {
        self.source_proof_local_given_descendant_use_term = Some(Box::new(handoff));
    }
    ...
    pub(crate) fn replace_source_set_term_and_atomic_formula_for_test(
        ...
        atomic_formula: SourceAtomicFormulaHandoff,
    ) {
        self.source_set_term = Some(set_term);
        self.source_atomic_formula = Some(atomic_formula);
    }
    ...
    pub fn with_source_evidence(
        ...
        if (self.source_proof_local_let_binding.is_some()
            || (self.source_proof_local_given_binding.is_some()
                ...
                || self.source_proof_local_given_descendant_binding.is_some()
                || self.source_proof_local_given_descendant_type.is_some()
                || self.source_proof_local_given_descendant_use_term.is_some()))
    ...
    pub fn with_source_property_implementation(
        ...
        let source_context = self
            .source_context
            .as_ref()
            .ok_or(TypedAstError::InvalidSourcePropertyImplementation)?;
        let source_type = self
            .source_type
            .as_ref()
            .ok_or(TypedAstError::InvalidSourcePropertyImplementation)?;
        ...
        handoff
            ...
            .map_err(|_| TypedAstError::InvalidSourcePropertyImplementation)?;
        self.source_property_implementation = Some(handoff);
        self.initial_obligations = initial_obligations;
        Ok(self)
    }
    ...
    pub fn with_source_condition_formula_composition(
        ...
        let source_term = self
            .source_term
            .as_ref()
            .ok_or(TypedAstError::InvalidSourceConditionFormulaComposition)?;
        let source_application = self
            .source_application
            .as_ref()
            .ok_or(TypedAstError::InvalidSourceConditionFormulaComposition)?;
        let source_set_term = self
            .source_set_term
            .as_ref()
            .ok_or(TypedAstError::InvalidSourceConditionFormulaComposition)?;
        let source_atomic_formula = self
    ...
    pub fn with_source_statement_references(
        ...
        if !statements.is_task_258b1_profile()
            && !statements.is_task_258b5a_profile()
            && !statements.is_task_258b5b_profile()
        {
            return Err(TypedAstError::InvalidSourceStatement);
        }
        references
    ...
    pub fn with_source_proof_local_let_binding(
        ...
        let installation_available = self.source_proof_local_let_binding.is_none()
            ...
            && self.diagnostics.is_empty();
        handoff
            .validate_complete_installation(self.source_id, &self.module_id, installation_available)
            .map_err(|_| TypedAstError::InvalidSourceProofLocalLetBinding)?;
        self.source_proof_local_let_binding = Some(Box::new(handoff));
        Ok(self)
    }
    ...
    pub fn with_source_proof_local_let_type(
        ...
        handoff
            ...
            .map_err(|_| TypedAstError::InvalidSourceProofLocalLetType)?;
        self.source_proof_local_let_type = Some(Box::new(handoff));
        Ok(self)
    }
    ...
    pub fn with_source_proof_local_given_binding(
        ...
        handoff
            .validate_complete_installation(self.source_id, &self.module_id, installation_available)
            .map_err(|_| TypedAstError::InvalidSourceProofLocalGivenBinding)?;
        self.source_proof_local_given_binding = Some(Box::new(handoff));
        Ok(self)
    }
    ...
    pub fn with_source_proof_local_given_use_type(
        ...
        handoff
            ...
            .map_err(|_| TypedAstError::InvalidSourceProofLocalGivenUseType)?;
        self.source_proof_local_given_use_type = Some(Box::new(handoff));
        Ok(self)
    }

    pub fn with_source_proof_local_given_use_term(
        mut self,
        handoff: SourceProofLocalGivenUseTermHandoff,
    ) -> Result<Self, TypedAstError> {
        ...
        handoff
            ...
            .map_err(|_| TypedAstError::InvalidSourceProofLocalGivenUseTerm)?;
        self.source_proof_local_given_use_term = Some(Box::new(handoff));
        Ok(self)
    }
    ...
    pub fn with_source_proof_local_given_condition_binding(
        ...
        handoff
            .validate_complete_installation(self.source_id, &self.module_id, installation_available)
            .map_err(|_| TypedAstError::InvalidSourceProofLocalGivenConditionBinding)?;
        self.source_proof_local_given_condition_binding = Some(Box::new(handoff));
        Ok(self)
    }

    pub fn with_source_proof_local_given_condition_type(
        mut self,
        handoff: SourceProofLocalGivenConditionTypeHandoff,
    ) -> Result<Self, TypedAstError> {
        ...
        handoff
            ...
            .map_err(|_| TypedAstError::InvalidSourceProofLocalGivenConditionType)?;
        self.source_proof_local_given_condition_type = Some(Box::new(handoff));
        Ok(self)
    }
    ...
    pub fn with_source_proof_local_given_descendant_binding(
        ...
        handoff
            .validate_complete_installation(self.source_id, &self.module_id, installation_available)
            .map_err(|_| TypedAstError::InvalidSourceProofLocalGivenDescendantBinding)?;
        self.source_proof_local_given_descendant_binding = Some(Box::new(handoff));
        Ok(self)
    }

    pub fn with_source_proof_local_given_descendant_type(
        mut self,
        handoff: SourceProofLocalGivenDescendantTypeHandoff,
    ) -> Result<Self, TypedAstError> {
        ...
        handoff
            ...
            .map_err(|_| TypedAstError::InvalidSourceProofLocalGivenDescendantType)?;
        self.source_proof_local_given_descendant_type = Some(Box::new(handoff));
        Ok(self)
    }

    pub fn with_source_proof_local_given_descendant_use_term(
        mut self,
        handoff: SourceProofLocalGivenDescendantUseTermHandoff,
    ) -> Result<Self, TypedAstError> {
        ...
        handoff
            ...
            .map_err(|_| TypedAstError::InvalidSourceProofLocalGivenDescendantUseTerm)?;
        self.source_proof_local_given_descendant_use_term = Some(Box::new(handoff));
        Ok(self)
    }
    ...
    pub fn with_source_application_statement_witnesses(
        ...
        statements
            ...
            .map_err(|_| TypedAstError::InvalidSourceStatement)?;
        if !statements.is_task_258b3m2b2b1a_profile() && !statements.is_task_258b3m2b2b1b1_profile()
        {
            return Err(TypedAstError::InvalidSourceStatement);
        }
        witnesses
            ...
            .map_err(|_| TypedAstError::InvalidSourceStatement)?;
        self.source_application = Some(application);
        self.source_statement = Some(statements);
        self.source_statement_witnesses = Some(witnesses);
        Ok(self)
    }
    ...
    pub fn with_source_structure_statement_witnesses(
        ...
        if !statements.is_task_258b3m2b2b2a_profile()
            && !statements.is_task_258b3m2b2b2b_profile()
            && !statements.is_task_258b3m2b2b2c_profile()
        {
            return Err(TypedAstError::InvalidSourceStatement);
        }
        witnesses
    ...
    pub fn with_source_set_term_statement_witnesses(
        ...
        witnesses
            ...
            .map_err(|_| TypedAstError::InvalidSourceStatement)?;
        self.source_set_term = Some(set_terms);
        self.source_statement = Some(statements);
        self.source_statement_witnesses = Some(witnesses);
        Ok(self)
    }
...
pub enum InitialObligationKind {
    ...
    PropertyImplementationUniqueness,
}
...
impl fmt::Display for TypedAstError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidSourcePropertyImplementation => formatter
                .write_str("typed AST source property-implementation handoff is inconsistent"),
            ...
            Self::InvalidSourceProofLocalLetBinding => formatter
                .write_str("typed AST source proof-local let-binding handoff is inconsistent"),
            Self::InvalidSourceProofLocalLetType => {
                formatter.write_str("source proof-local let type handoff is invalid")
            }
            ...
            Self::InvalidSourceProofLocalGivenUseType => {
                formatter.write_str("source proof-local given-use type handoff is invalid")
            }
            Self::InvalidSourceProofLocalGivenUseTerm => {
                formatter.write_str("source proof-local given-use term handoff is invalid")
            }
            ...
            Self::InvalidSourceProofLocalGivenDescendantType => {
                formatter.write_str("source proof-local given-descendant type handoff is invalid")
            }
            Self::InvalidSourceProofLocalGivenDescendantUseTerm => formatter
                .write_str("source proof-local given-descendant-use term handoff is invalid"),
