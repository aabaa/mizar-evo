...
use crate::{
    ...
    source_proof_local_declaration::{
        ...
        SourceProofLocalGivenDescendantBindingHandoff, SourceProofLocalLetBindingHandoff,
    },
    ...
    typed_ast::{
        InitialObligationKind, InitialObligationTable, LocalTypeContextId, NodeRecoveryState,
        NormalizedTypeId, TypeDiagnosticId, TypeDiagnosticSeverity, TypeDiagnosticTable,
        TypeEntryActual, TypeFactId, TypedAst, TypedNodeId, TypedSiteRef, TypingState,
    },
};
...
pub struct ResolvedTypedAst {
    ...
    source_proof_local_given_use_term: Option<Box<SourceProofLocalGivenUseTermHandoff>>,
    ...
    source_proof_local_given_descendant_use_term:
        Option<Box<SourceProofLocalGivenDescendantUseTermHandoff>>,
...
impl ResolvedTypedAst {
    ...
    pub const fn source_proof_local_let_type(&self) -> Option<&SourceProofLocalLetTypeHandoff> {
        match self.source_proof_local_let_type.as_ref() {
            Some(handoff) => Some(handoff),
            None => None,
        }
    }
    ...
    pub const fn source_proof_local_given_use_term(
        &self,
    ) -> Option<&SourceProofLocalGivenUseTermHandoff> {
        match self.source_proof_local_given_use_term.as_ref() {
            Some(handoff) => Some(handoff),
            None => None,
        }
    }
    ...
    pub const fn source_proof_local_given_condition_binding(
        ...
        match self.source_proof_local_given_condition_binding.as_ref() {
            Some(handoff) => Some(handoff),
            None => None,
        }
    }

    pub const fn source_proof_local_given_condition_type(
        &self,
    ...
    pub const fn source_proof_local_given_descendant_use_term(
        &self,
    ) -> Option<&SourceProofLocalGivenDescendantUseTermHandoff> {
        match self.source_proof_local_given_descendant_use_term.as_ref() {
            Some(handoff) => Some(handoff),
            None => None,
        }
    }
    ...
    pub fn debug_text(&self) -> String {
        ...
        if let Some(source_proof_local_let_type) = &self.source_proof_local_let_type {
            output.push_str(&source_proof_local_let_type.debug_text());
        }
        ...
        if let Some(source_proof_local_given_use_term) = &self.source_proof_local_given_use_term {
            output.push_str(&source_proof_local_given_use_term.debug_text());
        }
        if let Some(source_proof_local_given_condition_binding) =
            &self.source_proof_local_given_condition_binding
        {
            output.push_str(&source_proof_local_given_condition_binding.debug_text());
        }
        if let Some(source_proof_local_given_condition_type) =
        ...
        if let Some(source_proof_local_given_descendant_binding) =
        ...
        if let Some(source_proof_local_given_descendant_use_term) =
            &self.source_proof_local_given_descendant_use_term
        {
            output.push_str(&source_proof_local_given_descendant_use_term.debug_text());
        }
...
pub enum ResolvedTypedAstError {
    ...
    InvalidSourceProofLocalGivenDescendantUseTerm,
...
impl fmt::Display for ResolvedTypedAstError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidSourceProofLocalLetType => formatter
                .write_str("resolved typed AST source proof-local let type handoff is invalid"),
            Self::InvalidSourceProofLocalGivenBinding => formatter.write_str(
            ...
            Self::InvalidSourceProofLocalGivenUseTerm => formatter.write_str(
                "resolved typed AST source proof-local given-use term handoff is invalid",
            ),
            Self::InvalidSourceProofLocalGivenConditionBinding => formatter.write_str(
            ...
            Self::InvalidSourceProofLocalGivenDescendantUseTerm => formatter.write_str(
                "resolved typed AST source proof-local given-descendant-use term handoff is invalid",
            ),
...
impl<'a> ResolvedTypedAstAssembler<'a> {
    ...
    fn assemble(self) -> Result<ResolvedTypedAst, ResolvedTypedAstError> {
        ...
        let source_proof_local_let_type = self
            ...
            .cloned()
            .map(Box::new);
        if let Some(handoff) = &source_proof_local_let_type {
            let installation_available =
                source_proof_local_let_type_typed_profile_is_exact(self.inputs.typed_ast)
                    && source_proof_local_let_type_inputs_are_syntax_only(&self.inputs);
            handoff
                ...
                .map_err(|_| ResolvedTypedAstError::InvalidSourceProofLocalLetType)?;
        }
        ...
        let source_proof_local_given_binding = self
            .inputs
            .typed_ast
            .source_proof_local_given_binding()
            .cloned()
            .map(Box::new);
        if let Some(handoff) = &source_proof_local_given_binding {
            let installation_available =
                source_proof_local_given_binding_typed_profile_is_empty(self.inputs.typed_ast)
                    && source_statement_inputs_are_syntax_only(&self.inputs);
            handoff
                .validate_complete_installation(source_id, &module_id, installation_available)
                .map_err(|_| ResolvedTypedAstError::InvalidSourceProofLocalGivenBinding)?;
        }
        let source_proof_local_given_type = self
            .inputs
            .typed_ast
            .source_proof_local_given_type()
            .cloned()
            .map(Box::new);
        if let Some(handoff) = &source_proof_local_given_type {
            let installation_available =
                source_proof_local_given_type_typed_profile_is_exact(self.inputs.typed_ast)
                    && source_proof_local_given_type_inputs_are_syntax_only(&self.inputs);
            handoff
                .validate_complete_installation(
                    source_id,
                    &module_id,
        ...
        let source_proof_local_given_use_term = self
            .inputs
            .typed_ast
            .source_proof_local_given_use_term()
            .cloned()
            .map(Box::new);
        if let Some(handoff) = &source_proof_local_given_use_term {
            let installation_available =
                source_proof_local_given_use_term_typed_profile_is_exact(self.inputs.typed_ast)
                    && source_proof_local_given_use_term_inputs_are_syntax_only(&self.inputs);
            handoff
                .validate_complete_installation(
                    source_id,
                    &module_id,
                ...
                .map_err(|_| ResolvedTypedAstError::InvalidSourceProofLocalGivenUseTerm)?;
        }
        let source_proof_local_given_condition_binding = self
            .inputs
            .typed_ast
            .source_proof_local_given_condition_binding()
            .cloned()
            .map(Box::new);
        if let Some(handoff) = &source_proof_local_given_condition_binding {
            let installation_available =
                source_proof_local_given_condition_binding_typed_profile_is_empty(
                    self.inputs.typed_ast,
                ) && source_proof_local_given_condition_binding_inputs_are_syntax_only(
                    &self.inputs,
                );
            handoff
                .validate_complete_installation(source_id, &module_id, installation_available)
                .map_err(|_| ResolvedTypedAstError::InvalidSourceProofLocalGivenConditionBinding)?;
        }
        let source_proof_local_given_condition_type = self
            .inputs
            .typed_ast
            .source_proof_local_given_condition_type()
            .cloned()
            .map(Box::new);
        if let Some(handoff) = &source_proof_local_given_condition_type {
            let installation_available =
                source_proof_local_given_condition_type_typed_profile_is_exact(
                    self.inputs.typed_ast,
                ) && source_proof_local_given_condition_type_inputs_are_syntax_only(&self.inputs);
            handoff
                .validate_complete_installation(
                    source_id,
                    &module_id,
        ...
        let source_proof_local_given_descendant_binding = self
            .inputs
            .typed_ast
            .source_proof_local_given_descendant_binding()
            .cloned()
            .map(Box::new);
        if let Some(handoff) = &source_proof_local_given_descendant_binding {
            let installation_available =
                source_proof_local_given_descendant_binding_typed_profile_is_empty(
                    self.inputs.typed_ast,
                ) && source_proof_local_given_descendant_binding_inputs_are_syntax_only(
                    &self.inputs,
                );
            handoff
                .validate_complete_installation(source_id, &module_id, installation_available)
        ...
        let source_proof_local_given_descendant_use_term = self
            .inputs
            .typed_ast
            .source_proof_local_given_descendant_use_term()
            .cloned()
            .map(Box::new);
        if let Some(handoff) = &source_proof_local_given_descendant_use_term {
            let installation_available =
                source_proof_local_given_descendant_use_term_typed_profile_is_exact(
                    self.inputs.typed_ast,
                ) && source_proof_local_given_descendant_use_term_inputs_are_syntax_only(
                    &self.inputs,
                );
            handoff
                .validate_complete_installation(
                ...
                .map_err(|_| {
                    ResolvedTypedAstError::InvalidSourceProofLocalGivenDescendantUseTerm
                })?;
        }
        if self
            .inputs
        ...
        let source_structure = self.inputs.typed_ast.source_structure().cloned();
        if let Some(source_structure) = &source_structure {
            let source_term = self
                .inputs
                .typed_ast
                .source_term()
                .ok_or(ResolvedTypedAstError::InvalidSourceStructure)?;
            source_structure
        ...
        if let Some(source_predicate_definition) = &source_predicate_definition {
            let source_context = self
                ...
                .typed_ast
                .source_context()
                .ok_or(ResolvedTypedAstError::InvalidSourcePredicateDefinition)?;
            let source_type = self
                .inputs
                .typed_ast
                .source_type()
            ...
            source_predicate_definition
                ...
                .map_err(|_| ResolvedTypedAstError::InvalidSourcePredicateDefinition)?;
        } else if initial_obligations
            .iter()
        ...
        let source_condition_formula_composition = self
            .inputs
            .typed_ast
            .source_condition_formula_composition()
            .cloned();
        if let Some(source_condition_formula_composition) = &source_condition_formula_composition {
            if source_composite_formula.is_some() || source_formula_composition.is_some() {
                return Err(ResolvedTypedAstError::InvalidSourceConditionFormulaComposition);
            }
            let source_term = self
                .inputs
                .typed_ast
                .source_term()
                .ok_or(ResolvedTypedAstError::InvalidSourceConditionFormulaComposition)?;
            let source_application = source_application
                .as_ref()
                .ok_or(ResolvedTypedAstError::InvalidSourceConditionFormulaComposition)?;
            let source_set_term = source_set_term
                .as_ref()
                .ok_or(ResolvedTypedAstError::InvalidSourceConditionFormulaComposition)?;
            let source_atomic_formula = source_atomic_formula
                .as_ref()
        ...
        if let Some(source_statement) = &source_statement {
            ...
            match (&source_statement_references, &source_statement_witnesses) {
                ...
                {
                    witnesses
                        .validate_installation_with_set_term(
                            source_id,
                            &module_id,
                            source_statement,
                            source_term,
...
fn source_proof_local_given_binding_typed_profile_is_empty(typed_ast: &TypedAst) -> bool {
    typed_ast.resolved_root().is_none()
        && typed_ast.source_context().is_none()
        && typed_ast.source_type().is_none()
        && typed_ast.source_attribute().is_none()
...
fn source_proof_local_let_type_typed_profile_is_exact(typed_ast: &TypedAst) -> bool {
    typed_ast.resolved_root().is_none()
        && typed_ast.source_context().is_none()
        && typed_ast.source_type().is_none()
        && typed_ast.source_attribute().is_none()
...
fn source_proof_local_given_type_typed_profile_is_exact(typed_ast: &TypedAst) -> bool {
    typed_ast.resolved_root().is_none()
        && typed_ast.source_context().is_none()
        && typed_ast.source_type().is_none()
        && typed_ast.source_attribute().is_none()
...
fn source_proof_local_given_use_term_typed_profile_is_exact(typed_ast: &TypedAst) -> bool {
    typed_ast.resolved_root().is_none()
        && typed_ast.source_context().is_none()
        && typed_ast.source_type().is_none()
        && typed_ast.source_attribute().is_none()
...
fn source_proof_local_given_condition_binding_typed_profile_is_empty(typed_ast: &TypedAst) -> bool {
    typed_ast.resolved_root().is_none()
        && typed_ast.source_context().is_none()
        && typed_ast.source_type().is_none()
        && typed_ast.source_attribute().is_none()
...
fn source_proof_local_given_condition_type_typed_profile_is_exact(typed_ast: &TypedAst) -> bool {
    typed_ast.resolved_root().is_none()
        && typed_ast.source_context().is_none()
        && typed_ast.source_type().is_none()
        && typed_ast.source_attribute().is_none()
...
fn source_proof_local_given_descendant_binding_typed_profile_is_empty(
    typed_ast: &TypedAst,
) -> bool {
    typed_ast.resolved_root().is_none()
        && typed_ast.source_context().is_none()
...
fn source_proof_local_given_descendant_use_term_typed_profile_is_exact(
    typed_ast: &TypedAst,
) -> bool {
    typed_ast.resolved_root().is_none()
        && typed_ast.source_context().is_none()
...
fn source_proof_local_let_type_inputs_are_syntax_only(inputs: &ResolvedTypedAstInputs<'_>) -> bool {
...
fn source_proof_local_given_use_term_inputs_are_syntax_only(
    inputs: &ResolvedTypedAstInputs<'_>,
) -> bool {
    source_statement_inputs_are_syntax_only(inputs) && inputs.node_hints.is_empty()
}

fn source_proof_local_given_condition_binding_inputs_are_syntax_only(
    inputs: &ResolvedTypedAstInputs<'_>,
) -> bool {
    source_statement_inputs_are_syntax_only(inputs) && inputs.node_hints.is_empty()
}
...
fn source_proof_local_given_descendant_use_term_inputs_are_syntax_only(
    inputs: &ResolvedTypedAstInputs<'_>,
) -> bool {
    source_statement_inputs_are_syntax_only(inputs) && inputs.node_hints.is_empty()
}
...
fn build_resolved_nodes(
    ...
    let source_proof_local_role = if inputs.typed_ast.source_proof_local_let_type().is_some() {
        Some("source.proof-local.let.type")
    } else if inputs.typed_ast.source_proof_local_given_type().is_some() {
        Some("source.proof-local.given.type")
    } else if inputs
        .typed_ast
        .source_proof_local_given_use_type()
        .is_some()
    {
        Some("source.proof-local.given-use.type")
    } else if inputs
        .typed_ast
        .source_proof_local_given_use_term()
        .is_some()
    {
        Some("source.proof-local.given-use.term")
    } else if inputs
        .typed_ast
        .source_proof_local_given_condition_type()
        .is_some()
...
fn resolved_node_kind(
    ...
    source_proof_local_role: Option<&'static str>,
) -> ResolvedTypedNodeKind {
    if let Some(role) = source_proof_local_role {
