...
fn exact_binding_profile(
    ...
    if profile == StatementProfile::Task258B1 {
    ...
    for (formula_index, expected_range) in expected_ranges.iter().copied().enumerate() {
        ...
        let expected_edges = [
            (
                ...
                SourceAtomicTermTarget::Primary(SourcePrimaryTermId::new(formula_index * 2)),
            ),
            (
                SourceAtomicEdgeRole::BuiltinRightOperand,
                SourceAtomicTermTarget::Primary(SourcePrimaryTermId::new(formula_index * 2 + 1)),
            ),
        ];
        ...
        let Some(left) = primary_terms
            .terms()
            .get(SourcePrimaryTermId::new(formula_index * 2))
        else {
            return false;
        };
        let Some(right) = primary_terms
            .terms()
            .get(SourcePrimaryTermId::new(formula_index * 2 + 1))
        else {
            return false;
        };
        match (
...
fn validate_statement_rows(
        ...
        expected.iter().copied().enumerate()
    {
        ...
        if statement.owner != SourceTheoremOwnerId::new(0)
            ...
            || statement.source_ordinal != index
    ...
    let formulas = (0..4)
        ...
        .collect::<Vec<_>>();
    let statement_descendants = [
        [false, true, true, true],
        [false, false, true, false],
        [false, false, false, false],
        [false, false, false, false],
    ];
    let formula_descendants = [
        [true, true, true, true],
...
fn validate_input_fact_rows(
    ...
    for index in 0..rows {
        ...
        for (term_index, use_id) in expected_uses.into_iter().enumerate() {
            ...
            if reference.term() != SourcePrimaryTermId::new(index * 2 + term_index)
