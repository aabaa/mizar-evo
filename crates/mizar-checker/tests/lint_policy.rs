...
#[test]
fn checker_manifest_dependency_boundary_is_step5c11_scoped() {
    let manifest_path = crate_root().join("Cargo.toml");
    let manifest = read_to_string(&manifest_path);
    ...
    assert_eq!(
        ...
        "{} must keep the Step 5C.11 authenticated registration intake dependency boundary",
        manifest_path.display()
    );
}

#[test]
fn checker_task258b1_syntax_alias_remains_test_only() {
    let root = crate_root();
...
#[test]
fn checker_source_syntax_import_is_limited_to_registration_intake() {
    let root = crate_root();
